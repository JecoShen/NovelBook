...
- 任务必须在 `.worktree/<slug>/` 独立 worktree 进行，**不要**在主工作区直接改动；新建 worktree 后先 `bun install`。
- 远端 `master` 更新后立即 `git fetch && git merge --ff-only origin/master`；**禁止** force push `master`。
