...
- **Mobile tool panel (< 768 px) — overlay drawer, not a squeezed column.** Below `md` the left tool panel stops resizing the editor and becomes an absolute drawer: width clamped to `calc(100vw − 80px)` so the scrim keeps a ≥ 32 px tappable strip at the right edge (a drawer that covers its own scrim has no return path), `z-index 40` over a `z-index 30` scrim (the same `bg-black/50` as the Dialog overlay). Three return paths: tap the scrim strip, tap the panel's close button (`panel-left-close` icon), or open a file (auto-close on file-open intent, repeat taps on the same file included; browsing a directory row does not close the drawer — expansion has its own chevron). Desktop `min-width` guards on the workbench (640 / 260 px) and the panel resize handle apply to `md` and up only — a min-width guard on mobile is a right-overflow bug, and a 2-px drag handle is not a touch affordance.
- **Command palette — top-docked, z-index 9010 (above Dialog's 9000).** Triggered by `Ctrl+K` / `Ctrl+P` or the search button atop the Activity Bar. Top-docked at `14vh`, never centered: every palette the user has met (VS Code, GitHub, Linear) docks top. It is a combobox, not a focus-trapped dialog — DOM focus never leaves the input; arrow keys move `aria-activedescendant`, Enter picks, Esc and scrim click close (palette key events stop at the input so they never reach a Dialog's document-level Esc listener underneath). Three item kinds in fixed group order: actions → settings jumps → files, searched by the same workspace reference search (path / pinyin / pinyin initials / typo tolerance) as the editor's `@` mentions. Activity Bar slot shortcuts are `Alt+1..9` in rail order (书架 … 设置), shown in the slot tooltips as `· Alt+N` — `Ctrl+1..8` is reserved by browser tab switching and cannot be intercepted by a page, so Alt is the only viable modifier. One-shot chapter creation ("当前卷 +1 章") creates `manuscript/<latest-volume>/<NNN+1>-chapter/index.md` with title `第 N 章` and opens it immediately, no dialog; it lives in the palette and the file tree's root context menu, novel workspaces only.
...
- **Bookshelf cover** (ProjectPickerScreen): a deliberate skeuomorphic book — spine edge `0 2px 0 color-mix(in srgb, var(--border-strong) 72%, transparent)` over ambient `0 12px 24px color-mix(in srgb, var(--shadow-color) 13%, transparent)`, a 1 px cover border, and 1 px fallback texture lines at 24 % / 76 %. The spine layer reads `--border-strong` rather than `--shadow-color` because it is the book's edge, not a cast shadow; the shelf board adds its own `0 5px 8px @ 10 %`. The bookshelf is the one surface where literal object metaphor wins over the ambient-only rule — a recorded exception, not a precedent for other cards.
...
- **The Single-Primary Toolbar Rule.** A dense workbench toolbar shows at most ONE primary text button (the creation action, e.g. 新建切片 with an English data-model tooltip per the Tooltip-Gloss rule). Selection-dependent and secondary actions (edit/delete selected, preview) live in the `⋯` overflow menu, not as permanent text buttons; utility toggles (refresh, panel visibility, close) are icon-only with `aria-label` + tooltip.
