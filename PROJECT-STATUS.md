...
- **SDD 调研报告 6/6 完整闭环 (2026-08-20)**：2026-08-18 调研报告 P0-P2 6 条建议全部落位 — P0-1 beat (i128 80/80 章 Save the Cat 16 节拍值域) / P0-2 chapter-hook (i129 v0.3 5 模式 detector, reversal TP 100% FP 0% / short-drop TP 100% FP 0%) / P1-3 lore-resolver (M-1..M-11 minor 11/11 APPLIED + I-1 production wiring, JSONL carryOverPaths 3-章 sliding window) / P1-4 scene-six-questions (level=low 软提示 + CJK-safe baseline, V1+V2 0/80 baseline) / P2-5 scene-master-list (8 列半自动工具, 80/80 V1+V2 baseline 6 自动列 100% 命中) / P2-6 templates (8 entity README + 9th relationship cross-entity 模板)。累计 6 批次 archive 模式（worktree + cp 主工作区 untracked + 0 push / 0 merge / 0 force-push），main HEAD `05cb9e0c` 0 改动,各 worktree 物理释放,branch 永久可查（reflog 30 天窗口）。V3+ 写作期使用 scene-master-list + content-node templates 工具护栏,产物见 [reference/scene-master-list.md](reference/scene-master-list.md) 与 [assets/workspace/.nbook/templates/README.md](assets/workspace/.nbook/templates/README.md)。
