...
- 2026-08-20：SDD 调研报告 4/6 (67%) 完整闭环 — 5 批次链沿用 archive 模式 (worktree + cp + 0 push / 0 merge / 0 force-push) 收口。(1) P1-3 lore-resolver MVP + M-1..M-11 minor 11/11 APPLIED + I-1 production wiring 5 commits 合并入 main (49e62466 + 8e293aae 等 14 commits)，spec v4.5 → v4.7，32/32 lore + 16/16 writer profile tests pass，tsc 0 errors，i134 perf 0.00/0.31/3.49ms 远低 100/20/50ms 阈值 (余量 10000x/64x/14x)，7/7 验收；(2) P1-4 scene-six-questions 9 commits on `feat-p1-4-scene-six-questions` (已 worktree 释放, branch 保留)，Story Grid 6 问模板 + level=low 软提示 ruleset + CJK-safe baseline 脚本，V1+V2 0/80 (0%) baseline 合理，8/8 验收 + 5 corrections 落位 (level=info→low 全局 sweep / chapter count 60→80 / 11→10 字段 / 5 处 residual drift / CJK `\b`→lookahead)；(3) A1 minor nits cleanup commit `614d1a08`，magic numbers 提 named const + 6 test files DRY (新 `lore-test-helpers.ts` 12 行), 8 files +24/-46 净 -22, 32/32 tests pass；(4) A2-a + P1-4 worktree 处置释放 1.4GB+1GB，`feat-p1-3-minor-nits` + `feat-p1-3-lore-resolver` 双 branch 删 (内容 main 已有), P1-4 worktree 释放用户选 A1, main HEAD `39359ec5` 0 改动, 9 commits 永久可查 reflog 30 天窗口；(5) main ↔ origin 仍 ahead 4 (archive 0 push 守住)。调研报告 6/6 (100%) 完整闭环（2026-08-20 第 6 批次 P2-5 + P2-6 沿用 archive 模式落位，详见下文「SDD 调研报告」段）。
- 2026-08-20：I-1 audit batch — 沿用 archive 模式 (worktree + cp + 0 push / 0 merge / 0 force-push) 落位 Cat 3/4 修正。`audit-i-1-production-wiring` branch 1 commit (bc4c59d5, docs(spec,plan): I-1 audit — Cat 3 假引用 + Cat 4 main HEAD 校准; Cat 7 High 留 user 决策), 0 push / 0 merge 守住。修正: spec L332 "PM2 lease 操作手册: CLAUDE.md 中段"→"仓 CLAUDE.md + `novel-frontend-display-fix-2026-08-17` memory" / plan L21 "per CLAUDE.md" 假引用 + 0 改 master 不变式违反 flag (Cat 7 High 2026-08-19 实际 `07c182f3` force-add 6 archive 文件到 main, 留 user 决策, 不在本次 spec/plan 文字修范围) / plan L47 main HEAD `f92ce113`→`891ffef5` 校准。Phase 3/5 闭环, 累计 9 批次 archive-worktree 闭包。
- 2026-08-20：Cat 7 D 决策文档化 (5 批次 audit 闭环收口) — 用户选 D 接受现状 + 文档化 (不 revert force-add)。`audit-cat7-d-docs` branch 1 commit (ead556f0, docs(spec): I-1 §13 Cat 7 D 决策 — 4 protected assets 改为 shared mutable on main, 5 批次首例外), main HEAD 推进 1 commit (下方), 0 push / 0 merge 守住。校正: (1) I-1 spec 加 §13 段说明 4 protected assets (writer.profile.tsx / neuro-agent-harness.ts / server/agent/lore/* / llmlint) **shared mutable on main** for I-1 era, 本 batch 是 5 批次首例外, 后续 4 批次沿用 0 改 master; (2) 5 批次 memory (p1-3-archive-closed / i-1-production-wiring-landing / p1-3-8-minor-fixes / p1-3-3-remaining-minor / m-11-carryover-paths) 同步加 Cat 7 D 决策段; (3) PROJECT-STATUS L43 entry (本行)。**Why D**: 49e62466 merge 是 user 拍板 (PROJECT-STATUS 提"14 commits 合并入 main"), branch `feat-i-1-production-wiring` 已删, revert 实施代码会丢工作, writer.profile.tsx/neuro-agent-harness.ts 都在生产环境跑, revert 破坏服务, llmlint i133 chapter-hook 已 tracked 依赖 V1+V2 80/80 baseline。**累计 10 批次 archive-worktree 闭包**, 5 批次 audit Phase 1-3 + Cat 7 D 文档化, Phase 4-5 (3 minor batches) 仍 deferred (cost 预算超, 优先级低)。
- 2026-08-20：P1-3 lore audit batch — 沿用 archive 模式 (worktree + cp 主工作区 untracked + 0 push / 0 merge / 0 force-push) 落位 4 spec 假引用/计数修正 + 5 plan main HEAD/批次校准 + i134 perf patch report 物理补生成 (`workspace/.../i134-p1-3-lore-perf-benchmark.md` 2.9K 修 4 批次 root cause)。`audit-p1-3-lore-resolver` branch 1 commit (5cbda2d4, docs(spec,plan): P1-3 lore audit — 4 spec 假引用/计数 修正 + 5 plan main HEAD 校准/批次口径统一), main HEAD 推进 1 commit (下方 status commit, PROJECT-STATUS.md L43 entry), 0 push / 0 merge 守住。修正: spec L426 "per CLAUDE.md"→"per `vol2-v45-archive-mode` memory" / L432 "新增 6 文件"→"7 文件" / L451 "~1080 行"→"~980 行 (120+80+150+180+150+200+100)" / L511-512 "CLAUDE.md 顶部/中段"→"memory" / plan L43/L52/L1414/L1525/L1532 main HEAD 831270bf 加 "当前 main HEAD ca91cb36 2026-08-20 校准" 注释 / L1584 "archive 5 批次"→"7 批次 (P1-3/I-1/P1-4/A1 nits/P1-3 minor/P2-5+P2-6/P1-4 audit)"。Phase 2/5 闭环, 累计 8 批次 archive-worktree 闭包, I-1 High 留 Phase 3。
- 2026-08-20：P1-4 audit batch — 沿用 P2-5+P2-6 audit archive 模式 (worktree + cp + 0 push / 0 merge / 0 force-push) 落位 9 处 P1-4 spec/plan 错误修正。`audit-p1-4-scene-six-questions` branch 1 commit (9f890fa9, docs(spec,plan): audit P1-4 — 9 处错误修正), main HEAD 仍 `1ddb3481` 0 改动 (audit 不动 main, branch 永久可查 reflog 30 天窗口)。修正 9 处: spec §3 L118 '60 章'→'80 章' / L119 'V2 30'→'50' / L173 main HEAD 11c022ed→1ddb3481 / L226 + L253 '5 批次'→'7 批次' / plan L50-51 + L70 main HEAD / L542 '0/60'→'0/80' / L640 '60 章 beat'→'80' / L711 '60/60'→'80/80' / L719 V2 'ch-031~ch-060 0/30'→'ch-031~ch-080 0/50'。memory 修订 (主工作区, 不入 git) 6 处: p1-4-scene-six-questions-archive-closed.md (4 处: commits 9→10 / main HEAD / 调研报告 4/6→6/6 / archive 批次 5→7) + p1-4-worktree-disposed-2026-08-20.md (2 处: main HEAD / 调研报告 4/6→6/6)。Phase 1/5 完整闭环, 4 批次 (P1-3 lore / I-1 / 8 minor / 3 minor) 审计留 Phase 2-5。
...
- **SDD 调研报告 6/6 完整闭环 (2026-08-20)**：2026-08-18 调研报告 P0-P2 6 条建议全部落位 — P0-1 beat (i128 80/80 章 Save the Cat 16 节拍值域) / P0-2 chapter-hook (i129 v0.3 5 模式 detector, reversal TP 100% FP 0% / short-drop TP 100% FP 0%) / P1-3 lore-resolver (M-1..M-11 minor 11/11 APPLIED + I-1 production wiring, JSONL carryOverPaths 3-章 sliding window) / P1-4 scene-six-questions (level=low 软提示 + CJK-safe baseline, V1+V2 0/80 baseline) / P2-5 scene-master-list (8 列半自动工具, 80/80 V1+V2 baseline 6 自动列 100% 命中) / P2-6 templates (8 entity README + 9th relationship cross-entity 模板)。累计 10 批次 archive-worktree 闭包 (worktree + cp 主工作区 untracked + 0 push / 0 merge / 0 force-push, **2026-08-20 已 push origin** main HEAD `b25456a7`) — P1-3 lore-resolver / I-1 production wiring / P1-4 scene-six-questions / A1 nits / P1-3 minor 残留 (8+3+M-11) / P2-5+P2-6 / P1-4 audit / **P1-3 lore audit** (i134 patch report 物理补生成) / **I-1 audit** (Cat 3/4 修) / **Cat 7 D 决策文档化** (4 protected assets shared mutable on main, 5 批次首例外)。V3+ 写作期使用 scene-master-list + content-node templates 工具护栏,产物见 [reference/scene-master-list.md](reference/scene-master-list.md) 与 [assets/workspace/.nbook/templates/README.md](assets/workspace/.nbook/templates/README.md)。
