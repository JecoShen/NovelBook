    ...
    <Transition name="fade">
      ...
      >
        ...
        >
          <div
            v-if="item.separator"
            class="mx-1 my-1 h-px bg-[var(--border-color)]/50"
            @mouseenter="activeSubmenuIndex = null"
          />
          <button
            v-else
          ...
          >
            ...
            <span
              v-if="item.shortcut"
              class="ml-4 shrink-0 text-[11px] text-[var(--text-muted)]"
            >{{ item.shortcut }}</span>
            <span
              v-if="item.children?.length"
              class="i-lucide-chevron-right ml-2 h-3.5 w-3.5 shrink-0 text-[var(--text-muted)]"
            />
          </button>
          <div
