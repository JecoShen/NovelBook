...
<template>
  ...
  <Teleport
    v-if="isMounted"
    :to="typeof teleportTarget === 'string' ? teleportTarget : 'body'"
    :disabled="teleportTarget === false"
  >
    <Transition name="nb-dialog">
      <div
        v-if="modelValue"
        class="fixed inset-0 z-[9000] flex items-center justify-center transition-colors duration-200"
        :class="[
        ...
        @contextmenu.self="handleOverlayContextMenu"
      >
        <!-- 对话框主体 -->
        <div
        ...
        >
          ...
          <div
            ...
            class="flex items-center justify-between px-5 py-2 border-b border-[var(--border-color)]"
          ...
          >
            <slot name="header">
              <div class="flex min-w-0 flex-1 items-center gap-3">
                ...
                <button
                  v-if="props.closable"
                  class="flex items-center justify-center w-7 h-7 rounded-md text-[var(--text-muted)] bg-transparent border-none cursor-pointer transition-colors duration-200 hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)] active:scale-95 disabled:cursor-not-allowed disabled:opacity-50"
                  :disabled="props.busy"
                  @click="requestClose('close-button')"
                >
                  <span class="i-lucide-x w-4.5 h-4.5" />
                </button>
              </div>
            </slot>
          </div>
          ...
          <!-- body 区域 -->
          <div
            class="flex-1 flex flex-col gap-4 overflow-y-auto px-5 py-4 text-[14px] leading-relaxed text-[var(--text-secondary)]"
          ...
          <div
            ...
            class="flex items-center justify-end gap-2.5 px-5 py-2 border-t border-[var(--border-color)] bg-transparent"
          >
            <slot
              name="footer"
              :confirm="handleConfirm"
              :cancel="() => requestClose('cancel')"
            >
              <button
              ...
              <button
                class="inline-flex items-center justify-center h-8 px-4 rounded-md text-[13px] font-medium cursor-pointer border border-transparent bg-[var(--accent-main)] text-[var(--text-inverse)] transition-all duration-200 hover:opacity-90 hover:shadow-md active:scale-95 disabled:cursor-not-allowed disabled:opacity-50"
                :disabled="props.busy"
                @click="handleConfirm"
              >
                {{ t("common.confirm") }}
