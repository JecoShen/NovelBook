  ...
  >
    <Transition name="nb-dialog-window">
      <div
        v-if="modelValue"
        ref="windowRef"
        class="nb-dialog-window fixed z-[8990] flex flex-col overflow-hidden rounded-lg border border-[var(--border-color)] text-[var(--text-main)]"
        data-dialog-window
        data-dialog-surface
        :style="windowStyle"
      >
        ...
        <div class="flex shrink-0 items-center gap-2 border-b border-[var(--border-color)] pr-2">
          <div
            ref="dragHandleRef"
            class="flex min-w-0 flex-1 cursor-move touch-none select-none items-center gap-2 py-2 pl-4"
          >
            <span class="i-lucide-grip-vertical h-4 w-4 shrink-0 text-[var(--text-muted)]" />
            <slot name="header">
          ...
          <button
            v-if="closable"
            type="button"
            class="flex h-7 w-7 shrink-0 items-center justify-center rounded-md text-[var(--text-muted)] transition-colors hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)] disabled:cursor-not-allowed disabled:opacity-50"
            :disabled="busy"
        ...
        <!-- body 区域 -->
        <div
          class="flex min-h-0 flex-1 flex-col text-[14px] leading-relaxed text-[var(--text-secondary)]"
          :class="bodyClass"
        >
          <slot />
        </div>
        ...
        <div
          v-if="$slots.footer"
          class="flex shrink-0 items-center justify-end gap-2.5 border-t border-[var(--border-color)] px-4 py-2"
        >
          <slot name="footer" />
        </div>
      </div>
