...
<template>
  ...
  >
    ...
    <div
      ref="resizeHandleRef"
      class="group absolute -top-1 left-0 right-0 z-30 h-2 cursor-row-resize"
    >
      <div
    ...
    <div class="sticky top-0 z-20 flex h-[44px] shrink-0 items-center justify-between border-b border-[var(--border-color)] bg-[var(--bg-sidebar)] px-3 py-2">
      <div
        class="flex min-w-0 flex-1 cursor-pointer select-none items-center gap-2 overflow-hidden"
        @click="toggleHeight"
      >
        <slot name="header" />
      </div>
      ...
      <div class="ml-3 flex shrink-0 items-center gap-1.5">
        ...
        >
          <span
            :class="isCollapsed ? 'i-lucide-chevron-up' : 'i-lucide-chevron-down'"
            class="h-4 w-4"
          />
        </button>
        <button
