...
<template>
  ...
  <div
    ref="fieldRef"
    class="form-color-field block"
  >
    <span
      v-if="label || variableName"
      class="mb-1.5 flex min-w-0 items-center justify-between gap-3"
    >
      <span
        v-if="label"
        class="truncate text-xs font-medium text-[var(--text-secondary)]"
      >{{ label }}</span>
      <code
        v-if="variableName"
    ...
    <span
      class="flex h-9 items-center gap-2 rounded-md border bg-[var(--bg-input)] px-1.5 transition-colors"
      :class="valid ? 'border-[var(--border-color)] focus-within:border-[var(--accent-main)]' : 'border-[var(--status-danger-border)]'"
    >
      <!-- 色块按钮：打开调色盘弹层 -->
      <button
        ref="swatchRef"
        type="button"
        class="relative h-6 w-6 shrink-0 overflow-hidden rounded border border-[var(--border-color)] shadow-sm transition-transform hover:scale-105 disabled:cursor-not-allowed disabled:opacity-60"
        :title="t('common.colorFieldOpenPicker')"
        :disabled="disabled"
        @click="togglePopover"
      >
        <span class="nb-color-checkerboard absolute inset-0" />
        <span
      ...
      <button
        v-if="eyeDropperCtor"
        type="button"
        class="flex h-6 w-6 shrink-0 items-center justify-center rounded text-[var(--text-muted)] transition-colors hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)] disabled:cursor-not-allowed disabled:opacity-60"
        :title="t('common.colorFieldEyedropper')"
        :disabled="disabled"
        @click="void pickFromScreen()"
      >
        <span class="i-lucide-pipette h-3.5 w-3.5" />
      </button>
    </span>

    <span
      v-if="draftValue && !valid"
      class="mt-1 block text-[11px] text-[var(--status-danger)]"
    >{{ t("common.colorFieldInvalid") }}</span>
    ...
    <Teleport :to="popoverTeleportTarget">
      <div
        v-if="popoverOpen"
        ref="popoverRef"
        class="nb-color-popover fixed z-[9400] rounded-lg border border-[var(--border-color)] bg-[var(--bg-panel)] p-2"
        :style="popoverStyle"
      >
        <ClientOnly>
          <ColorPicker
