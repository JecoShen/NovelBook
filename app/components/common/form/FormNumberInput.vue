...
<template>
  ...
  >
    ...
    <span class="ml-1 flex h-5 w-4 shrink-0 flex-col overflow-hidden rounded-sm border border-[var(--border-color)] bg-[var(--bg-panel)]">
      <button
        type="button"
        class="flex h-1/2 items-center justify-center text-[var(--text-muted)] transition-colors hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)] disabled:opacity-40"
        :disabled="props.disabled || props.readonly"
        title="增加"
        @click="stepValueBy('up')"
      >
        <span class="i-lucide-chevron-up h-2.5 w-2.5" />
      </button>
      <button
        type="button"
        class="flex h-1/2 items-center justify-center border-t border-[var(--border-color)] text-[var(--text-muted)] transition-colors hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)] disabled:opacity-40"
        :disabled="props.disabled || props.readonly"
        title="减少"
