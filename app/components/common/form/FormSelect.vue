      ...
      <span class="flex items-center gap-1.5 min-w-0 pr-2">
        ...
        >
          <template v-if="opt.value === modelValue">
            <span
              v-if="opt.indicatorClass"
              class="h-1.5 w-1.5 rounded-full shrink-0 shadow-sm"
              :class="opt.indicatorClass"
            />
            <span
              v-else-if="opt.iconClass"
              class="shrink-0 h-3 w-3 text-[var(--text-muted)]"
              :class="opt.iconClass"
            />
        ...
        <span
          v-if="!options.some(o => o.value === modelValue)"
          class="text-[var(--text-muted)] opacity-80"
        >{{ placeholder || t("common.selectOption") }}</span>
      </span>
      <span
        class="i-lucide-chevron-down h-3.5 w-3.5 text-[var(--text-muted)] shrink-0 transition-transform duration-200"
    ...
    <transition :name="resolvedDirection === 'up' ? 'dropdown-up' : 'dropdown-down'">
      ...
      >
        ...
        >
          ...
          <span
            v-else-if="opt.iconClass"
            class="shrink-0 h-3 w-3 opacity-70"
            :class="opt.iconClass"
          />
          <span class="min-w-0 flex-1">
            ...
            <span
              ...
              class="mt-0.5 block truncate text-[10px] font-normal text-[var(--text-muted)]"
            >{{ opt.description }}</span>
          </span>
          <span
            v-if="!props.hideCheckmark && opt.value === modelValue"
            class="i-lucide-check h-3 w-3 text-[var(--accent-main)] ml-auto shrink-0"
          />
        </div>
      </div>
    </transition>
