...
<template>
  <Teleport
    :disabled="!props.anchorRect || !props.teleportTarget"
    :to="props.teleportTarget ?? 'body'"
  >
    <div
      ref="panelRef"
    ...
    >
      <div
        class="flex items-center gap-2 border-b border-[var(--border-color)] bg-[var(--bg-sidebar)]"
        :class="props.density === 'compact' ? 'px-2.5 py-1.5' : 'px-3 py-2.5'"
      >
        <span
          class="font-mono font-bold text-[var(--accent-text)]"
          :class="props.density === 'compact' ? 'text-[11px]' : 'text-xs'"
        >{{ props.prefix }}</span>
        <span
      ...
      <div
        ...
        :class="props.density === 'compact' ? 'p-1.5' : 'p-2'"
      >
        <template
        ...
        >
          <div
            v-if="section.title"
            class="font-semibold text-[var(--text-muted)]"
            :class="props.density === 'compact' ? 'px-2 pb-1 pt-2 text-[10px]' : 'px-2 py-1 text-[10px] uppercase tracking-[0.18em]'"
          >
            {{ section.title }}
          </div>
          ...
          >
            <span
              ...
              :class="[props.density === 'compact' ? 'h-3.5 w-3.5' : 'h-4 w-4', item.iconClass]"
            />
            <span class="min-w-0 flex-1">
              <span class="flex items-center gap-2">
                <span
                  class="truncate font-medium"
                  :class="props.density === 'compact' ? 'text-xs' : 'text-sm'"
                >{{ item.label }}</span>
                <span
                  v-if="item.hint"
                  class="shrink-0 rounded-full border border-[var(--border-color)] bg-[var(--bg-input)] px-1.5 py-0.5 font-mono text-[10px] text-[var(--text-muted)]"
                >
                  {{ item.hint }}
                </span>
              </span>
              <span
                class="mt-0.5 block text-[var(--text-muted)]"
                :class="props.density === 'compact' ? 'text-[10px] leading-4' : 'text-[11px] leading-5'"
              >
                {{ item.description }}
              </span>
            </span>
