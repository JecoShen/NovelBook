...
<template>
  <div class="grid gap-4">
    ...
    >
      <template #actions>
        <div
          v-if="props.scope === 'project' && props.inheritanceMode === 'manual'"
          class="flex shrink-0 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] p-0.5"
        >
          <button
            type="button"
      ...
      <LowCodeComboboxField
        v-else-if="field.component === 'combobox'"
        :field="field"
        :model-value="fieldValue(field)"
        :disabled="fieldDisabled(field)"
        @update:model-value="updateField(field, $event)"
      ...
      <LowCodeResourcePresetField
        v-else-if="field.component === 'resource-preset'"
        :field="field"
        :model-value="fieldValue(field)"
        :scope="props.scope"
        :disabled="fieldDisabled(field)"
        :mutations="fieldResourceMutations(field)"
        @update:model-value="updateField(field, $event)"
        @update:mutations="updateFieldResourceMutations(field, $event)"
      />
    </LowCodeFieldShell>
  </div>
</template>
