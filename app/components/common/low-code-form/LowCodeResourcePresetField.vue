...
<template>
  <div class="grid gap-2">
    <div
      v-if="!resource"
      class="rounded border border-[var(--border-color)] bg-[var(--bg-input)] px-3 py-2 text-sm text-[var(--text-muted)]"
    >
      Resource preset is unavailable.
    </div>
    <div
      v-else-if="shouldShowDisabledEmpty"
      class="rounded-md border border-dashed border-[var(--border-color)] bg-[var(--bg-input)]/40 px-3 py-2 text-[11px] text-[var(--text-muted)]"
    >
      没有可用资源。
    </div>
    <template v-else>
      ...
      <div class="overflow-visible rounded-md border border-[var(--border-color)] bg-[var(--bg-input)]/25">
        <div class="flex min-w-0 items-center gap-2 px-2 py-2">
          ...
          />
          <button
            v-if="canManage || selectedKey"
            type="button"
            class="inline-flex h-7 shrink-0 items-center gap-1 rounded-md border border-[var(--border-color)] px-2 text-[11px] text-[var(--text-main)] hover:bg-[var(--bg-hover)] disabled:opacity-50"
            :disabled="!selectedKey && !canManage"
            :title="managerLabel"
            @click="managerOpen = !managerOpen"
          >
            <span
              :class="managerOpen ? 'i-lucide-chevron-up' : 'i-lucide-panel-bottom-open'"
              class="h-3.5 w-3.5"
            />
            {{ managerOpen ? "收起" : "管理" }}
          </button>
        ...
        >
          <div
            v-if="canManage"
            class="flex flex-wrap items-center gap-1.5 border-b border-[var(--border-color)] px-2 py-1.5"
          >
            <button
              v-if="canCreate"
              type="button"
              class="inline-flex h-7 items-center gap-1 rounded-md border border-[var(--border-color)] px-2 text-[11px] text-[var(--text-main)] hover:bg-[var(--bg-hover)]"
              title="新建资源"
            ...
            <button
              v-if="resource.capabilities.rename"
              type="button"
              class="inline-flex h-7 items-center gap-1 rounded-md border border-[var(--border-color)] px-2 text-[11px] text-[var(--text-main)] hover:bg-[var(--bg-hover)] disabled:opacity-50"
              :disabled="!canRename"
              title="重命名当前资源"
            ...
            <button
              v-if="canCopyGlobalToProject"
              type="button"
              class="inline-flex h-7 items-center gap-1 rounded-md border border-[var(--accent-main)]/40 bg-[var(--accent-bg)] px-2 text-[11px] text-[var(--accent-text)] hover:opacity-90"
              title="复制全局资源到项目并选中"
              @click="copyGlobalToProject"
            >
          ...
          <div class="grid min-h-[166px] items-stretch gap-0 md:grid-cols-[minmax(11rem,0.34fr)_minmax(0,1fr)]">
            ...
            <div class="h-full overflow-y-auto border-b border-[var(--border-color)] p-1.5 md:border-b-0 md:border-r md:border-[var(--border-color)]">
              ...
              >
                <span
                  ...
                  :class="option.key === selectedKey ? 'bg-[var(--accent-main)]' : 'bg-[var(--text-muted)]/45'"
                />
                <span class="min-w-0 flex-1">
                  <span class="block truncate text-xs">{{ option.label }}</span>
                  <span
                    v-if="option.description"
                    class="mt-0.5 block truncate text-[10px] text-[var(--text-muted)]"
                  >{{ option.description }}</span>
                </span>
                <span
                  v-if="option.origin"
                  class="rounded px-1.5 py-0.5 text-[10px]"
                  :class="option.origin === 'global' ? 'bg-[var(--bg-input)] text-[var(--text-muted)]' : 'bg-[var(--accent-bg)] text-[var(--accent-text)]'"
                >
                  {{ option.origin === "global" ? "全局" : "项目" }}
                </span>
                <span
                  v-if="option.key === selectedKey"
                  class="rounded bg-[var(--accent-bg)] px-1.5 py-0.5 text-[10px] text-[var(--accent-text)]"
                >当前</span>
                <button
            ...
            <div class="min-w-0">
              <div
                v-if="!selectedKey"
                class="px-2.5 py-2 text-[11px] text-[var(--text-muted)]"
              >
                请选择一个资源。
              </div>
              <div
              ...
              >
                <div
                  v-if="selectedIsGlobalReadonly"
                  class="flex items-center justify-between gap-2 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)]/50 px-2.5 py-2 text-[11px] text-[var(--text-secondary)]"
                >
                  <span class="min-w-0">这个资源来自全局库，在项目配置中只读。</span>
                  <button
                    v-if="canCopyGlobalToProject"
                    type="button"
                    class="inline-flex h-7 shrink-0 items-center gap-1 rounded-md border border-[var(--accent-main)]/40 bg-[var(--accent-bg)] px-2 text-[11px] text-[var(--accent-text)] hover:opacity-90"
                    @click="copyGlobalToProject"
                  >
                    <span class="i-lucide-copy-plus h-3.5 w-3.5" />
                    复制到项目
      ...
      >
        ...
        <template #footer="{ cancel }">
          <button
            type="button"
            class="inline-flex h-8 items-center justify-center rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-4 text-[13px] font-medium text-[var(--text-main)] hover:bg-[var(--bg-hover)]"
            @click="cancel"
          >
            取消
          </button>
          <button
            type="button"
            class="inline-flex h-8 items-center justify-center rounded-md border border-transparent bg-[var(--accent-main)] px-4 text-[13px] font-medium text-[var(--text-inverse)] hover:opacity-90 disabled:opacity-50"
      ...
      >
        ...
        <template #footer="{ cancel }">
          ...
          <button
            type="button"
            class="inline-flex h-8 items-center justify-center rounded-md border border-[var(--danger)] px-4 text-[13px] font-medium text-[var(--danger)] hover:bg-[var(--danger-soft)]"
