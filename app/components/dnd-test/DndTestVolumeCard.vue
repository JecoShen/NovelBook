...
<template>
  ...
  >
    ...
    >
      <button
        type="button"
        class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-slate-500 transition-colors hover:bg-white/80 hover:text-slate-900"
        :title="volume.isExpanded ? '收起篇内章节' : '展开篇内章节'"
        @click="emit('toggle', volume.id)"
      >
        <span
          :class="volume.isExpanded ? 'i-lucide-chevron-down' : 'i-lucide-chevron-right'"
          class="h-4 w-4"
        />
      </button>
      ...
      <button
        ...
        title="拖拽排序篇"
      >
        <span class="i-lucide-grip h-4.5 w-4.5" />
      </button>
      ...
      <div class="min-w-0 flex-1">
        <div class="flex items-center gap-2">
          ...
          <span
            class="rounded-full px-2 py-0.5 text-[10px] font-semibold tracking-[0.14em] text-white"
            :style="{ backgroundColor: volume.color }"
          >篇</span>
          <span
            v-if="selectedCount > 0"
            class="rounded-full bg-slate-900 px-2 py-0.5 text-[10px] font-semibold text-white"
          >{{ selectedCount }} 个已选</span>
        </div>
      ...
      <div class="shrink-0 text-right">
        ...
        <div class="mt-1 text-[10px] text-slate-400">
          {{ volume.chapters.reduce((sum, chapter) => sum + chapter.words, 0) }} 字
        </div>
      </div>
    </div>
    ...
    >
      <div
        ...
        class="max-h-[280px] space-y-2 overflow-y-auto pr-1"
      >
        <DndTestChapterRow
      ...
      <div
        v-else
        class="rounded-[18px] border border-dashed border-slate-300 bg-white/85 px-4 py-8 text-center text-xs leading-6 text-slate-500"
      >
