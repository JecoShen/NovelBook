  ...
  <aside class="comment-flow-panel flex h-full w-[360px] shrink-0 flex-col border-l border-[var(--border-color)] bg-[var(--editor-bg)]">
    <header class="flex h-11 shrink-0 items-center justify-between border-b border-[var(--border-color)] px-4">
      <div class="flex min-w-0 items-center gap-2">
        <span class="i-lucide-message-square-text h-4 w-4 text-[var(--accent-main)]" />
        <div class="truncate text-sm font-semibold text-[var(--text-main)]">
          {{ t("markdownStudio.comments.title") }}
        </div>
      ...
      <button
        ...
        :title="t('markdownStudio.comments.close')"
        @click="emit('close')"
      >
    ...
    <div class="min-h-0 flex-1 overflow-y-auto p-4 custom-scrollbar">
      <div
        ...
        class="flex h-full min-h-[240px] flex-col items-center justify-center rounded-lg border border-dashed border-[var(--border-color)] text-center text-xs text-[var(--text-muted)]"
      >
        <span class="i-lucide-message-square h-6 w-6" />
        <div class="mt-2">
          {{ t("markdownStudio.comments.empty") }}
        </div>
      </div>
      ...
      >
        ...
        >
          <div class="block w-full text-left">
            <div class="flex items-start justify-between gap-3">
              <div class="flex min-w-0 items-start gap-2">
                ...
                >{{ comment.index }}</span>
                <div class="min-w-0">
                  <div class="truncate text-xs font-medium text-[var(--text-main)]">
                    {{ t("markdownStudio.comments.original", { text: snippet(comment) }) }}
                  </div>
                </div>
              </div>
              <div class="flex shrink-0 items-center gap-1 opacity-70 transition-opacity group-hover:opacity-100">
                <button
                  type="button"
                  class="flex h-6 w-6 items-center justify-center rounded-md text-[var(--text-muted)] hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)]"
                  :title="t('markdownStudio.comments.edit')"
                ...
                <button
                  type="button"
                  class="flex h-6 w-6 items-center justify-center rounded-md text-[var(--text-muted)] hover:bg-[var(--status-danger-bg)] hover:text-[var(--status-danger)]"
                  :title="t('markdownStudio.comments.delete')"
          ...
          <div
            v-if="editingIndex === comment.index"
            class="mt-3"
          >
            <textarea
              v-model="editingBody"
            ...
            <div class="mt-2 flex items-center gap-2">
              <button
                type="button"
                class="inline-flex h-7 items-center gap-1 rounded-md bg-[var(--accent-main)] px-3 text-xs font-medium text-[var(--text-inverse)] hover:opacity-90"
              ...
              <button
                type="button"
                class="inline-flex h-7 items-center gap-1 rounded-md px-3 text-xs text-[var(--text-secondary)] hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)]"
          ...
          <div
            v-else
            class="mt-3 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-3 py-2 text-xs leading-5 text-[var(--text-secondary)]"
          >
