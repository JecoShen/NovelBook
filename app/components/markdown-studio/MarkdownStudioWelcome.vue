...
<template>
  <section class="studio-welcome-root min-h-0 flex-1 overflow-y-auto bg-[var(--editor-bg)] px-6 py-5 custom-scrollbar">
    <div
      v-if="readonlyNode"
      class="studio-welcome-container mx-auto flex w-full max-w-[720px] flex-col gap-5"
    >
      <div class="flex items-start gap-4 border-b border-[var(--border-color)] pb-5">
        <div class="flex h-11 w-11 shrink-0 items-center justify-center rounded-lg border border-[var(--border-color)] bg-[var(--bg-input)] text-[var(--accent-text)]">
          <span class="i-lucide-lock-keyhole h-5 w-5" />
        </div>
        <div class="min-w-0 flex-1">
          <h1 class="text-lg font-semibold text-[var(--text-main)]">
            {{ t("markdownStudio.welcome.uneditableTitle") }}
          </h1>
          <p class="mt-1 text-sm leading-6 text-[var(--text-secondary)]">
            {{ t("markdownStudio.welcome.uneditableDescription") }}
          </p>
        </div>
      </div>
      ...
      <div class="rounded-lg border border-[var(--border-color)] bg-[var(--bg-input)] px-4 py-3 text-xs text-[var(--text-secondary)]">
        <div
          class="truncate font-mono text-[var(--text-main)]"
      ...
      >
        <span class="i-lucide-folder-tree h-5 w-5 shrink-0 text-[var(--accent-text)]" />
        <span class="min-w-0">
    ...
    <div
      v-else
      class="studio-welcome-container mx-auto flex w-full flex-col"
    ...
    >
      <header class="studio-welcome-hero">
        <div class="min-w-0">
          <div class="welcome-eyebrow">
            <span
              :class="novelWorkspace ? 'i-lucide-pen-line' : 'i-lucide-folder-cog'"
              class="h-4 w-4"
            />
          ...
          <p class="mt-1.5 max-w-[680px] text-sm leading-6 text-[var(--text-secondary)]">
            {{ welcomeDescription }}
          </p>
        </div>
        <div class="welcome-hero-actions">
          ...
          <button
            ...
            @click="emit('open-files')"
          >
            <span class="i-lucide-folder-tree h-4 w-4" />
      ...
      <div class="welcome-action-grid">
        ...
        >
          <span class="i-lucide-panel-right-open h-4 w-4 shrink-0 text-[var(--accent-text)]" />
          <span class="min-w-0">
      ...
      <section class="welcome-recent-section">
        <div class="welcome-section-heading">
          <h2 class="welcome-section-title">
            {{ t("markdownStudio.welcome.continueSection") }}
          </h2>
          <span
            v-if="visibleTabs.length > 0"
            class="welcome-section-hint"
          >{{ visibleTabs.length }} / 5</span>
        </div>
        <div
          v-if="visibleTabs.length > 0"
          class="welcome-tab-list"
        >
          <button
            v-for="tab in visibleTabs.slice(0, 5)"
            :key="tab.path"
            type="button"
            class="welcome-tab-row"
            :title="tab.path"
          ...
          >
            ...
            <span class="min-w-0 flex-1">
              <span
                class="block truncate text-sm font-medium text-[var(--text-main)]"
        ...
        <button
          v-else
          type="button"
          class="welcome-empty-row"
          @click="novelWorkspace ? emit('open-path', 'manuscript/') : emit('open-files')"
        >
          <span class="i-lucide-arrow-right h-4 w-4 shrink-0 text-[var(--accent-text)]" />
