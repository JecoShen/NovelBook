...
<template>
  <aside
    ref="activityBarRef"
    class="workbench-activity-bar flex w-12 shrink-0 flex-col items-center border-r border-[var(--border-color)] bg-[var(--bg-sidebar)] py-2"
  ...
  >
    <div class="flex min-h-0 w-full flex-1 flex-col items-center">
      ...
      >
        ...
        >
          ...
          >
            <span
              v-if="active(item)"
              class="absolute inset-y-1 left-0 w-0.5 rounded-r bg-[var(--accent-main)]"
            />
            <span
              :class="iconClasses[item.id]"
              class="h-[18px] w-[18px]"
            />
          </button>
        </Tooltip>

        <div class="my-1 h-px w-7 bg-[var(--border-color)]" />
      </div>
      ...
      <div
        v-if="secondaryItems.overflow.length > 0"
        ref="moreRootRef"
        class="relative mb-1 h-10 w-10 shrink-0"
      >
        <Tooltip
          :text="moreOpen ? '' : t('ide.activityBar.more')"
          placement="right"
        >
          <button
            ref="moreButtonRef"
          ...
          >
            <span class="i-lucide-ellipsis h-[18px] w-[18px]" />
          </button>
        </Tooltip>
        <div
          v-if="moreOpen"
    ...
    <div
      ref="footerRef"
      class="mt-auto flex w-full shrink-0 flex-col items-center gap-1"
    >
