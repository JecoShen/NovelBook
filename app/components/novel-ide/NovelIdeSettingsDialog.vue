...
<template>
      ...
      <section class="shrink-0 rounded-2xl border border-[var(--border-color)] border-opacity-70 bg-[var(--bg-input)] bg-opacity-20 px-4 py-3 shadow-sm">
        <div class="flex flex-wrap items-start justify-between gap-3">
          ...
          <div class="flex shrink-0 flex-wrap items-center justify-end gap-2">
            <div
              v-if="activeScope === 'project'"
              class="flex min-w-[300px] items-center gap-2 rounded-lg border border-[var(--border-color)] border-opacity-60 bg-[var(--bg-panel)] bg-opacity-35 px-3 py-2"
            >
              <span class="shrink-0 text-[11px] font-semibold text-[var(--text-muted)]">Project</span>
              <span
                class="min-w-0 flex-1 truncate text-xs font-medium text-[var(--text-main)]"
                :title="targetLabel"
              >{{ targetLabel }}</span>
            </div>
            ...
            >
              <span
                v-if="activeSaveDirty && !activeSaveLoading"
              ...
              <span class="relative flex items-center gap-1.5">
                <span
                  v-if="activeSaveLoading || activeSaveSaving"
                  class="i-lucide-loader-2 h-3.5 w-3.5 animate-spin"
                />
                <span
                  v-else
                  class="i-lucide-save h-3.5 w-3.5"
                />
      ...
      <div class="flex min-h-0 flex-1 flex-col gap-4 md:flex-row md:gap-6">
        ...
        <aside class="flex w-full min-w-0 flex-col pb-2 md:w-[220px] md:shrink-0">
          <div class="mb-3 mt-1 hidden px-3 md:block">
            <div class="text-[11px] font-bold uppercase tracking-wider text-[var(--text-muted)]">
              {{ activeScope === "browser" ? t("settings.scope.browserState") : t("settings.scope.configFile") }}
            </div>
          </div>
          ...
          <div class="flex min-w-0 gap-1.5 overflow-x-auto pb-1 md:flex-col md:overflow-visible md:pb-0">
            ...
            >
              <!-- 图标 -->
              <div
                class="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg transition-colors duration-300"
                :class="activeSection === item.value ? 'bg-[var(--accent-bg)] text-[var(--accent-text)] shadow-sm font-semibold' : 'bg-transparent text-[var(--text-muted)] group-hover:text-[var(--text-main)]'"
              >
                <span
          ...
          <div class="mt-auto hidden pt-4 md:block">
            <div class="flex items-center justify-between gap-3 rounded-xl border border-[var(--border-color)] border-opacity-60 bg-[var(--bg-input)] bg-opacity-15 px-3.5 py-3 shadow-sm">
              <div class="min-w-0">
                <div class="truncate text-[11px] font-medium leading-relaxed text-[var(--text-secondary)]">
                  {{ versionLabel }}
                </div>
                <div class="mt-0.5 text-[10px] leading-relaxed text-[var(--text-muted)]">
                  Neuro Book
                </div>
              </div>
              <a
        ...
        <section class="min-w-0 flex-1 relative flex flex-col overflow-hidden">
          ...
          <div class="flex-1 overflow-y-auto custom-scrollbar pr-2 pb-6">
            ...
            >
              ...
              >
                <div class="rounded-2xl border border-[var(--status-info-border)] bg-[var(--status-info-bg)] px-5 py-4 text-[var(--text-main)]">
                  <div class="flex items-start gap-3">
                    <span class="i-lucide-shield-check mt-0.5 h-5 w-5 shrink-0 text-[var(--status-info)]" />
                    <div>
                ...
                <div class="rounded-2xl border border-[var(--border-color)] bg-[var(--bg-panel)] px-5 py-4 shadow-sm">
                  <div class="flex flex-wrap items-center justify-between gap-3">
                    <div>
                      <div class="text-sm font-medium text-[var(--text-main)]">
                        auth.enabled
                      </div>
                      <div class="mt-1 text-xs text-[var(--text-secondary)]">
                        {{ t("settings.security.runtimeStatusDescription") }}
                      </div>
                    </div>
                    <div
                      class="rounded-full border px-3 py-1 text-xs font-medium"
                      :class="bootAuthEnabled === null ? 'border-[var(--border-color)] bg-[var(--bg-input)] text-[var(--text-muted)]' : bootAuthEnabled ? 'border-[var(--status-success-border)] bg-[var(--status-success-bg)] text-[var(--status-success)]' : 'border-[var(--status-warning-border)] bg-[var(--status-warning-bg)] text-[var(--status-warning)]'"
                    >
                  ...
                  <div class="mt-4 text-xs font-medium text-[var(--text-secondary)]">
                    {{ t("settings.security.exampleTitle") }}
                  </div>
                  ...
                  <p class="mt-3 text-xs leading-5 text-[var(--status-warning)]">
              ...
              <!-- 前端设定 -->
              <div
                v-else-if="activeSection === 'frontend'"
                key="frontend"
                class="space-y-4 pt-1"
              >
                <div class="grid gap-3">
                  <div class="group flex items-center gap-4 rounded-2xl border border-[var(--border-color)] bg-[var(--bg-panel)] px-5 py-4 shadow-sm transition-all duration-300 hover:shadow-md">
                    <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[var(--bg-input)] text-[var(--text-secondary)] transition-colors group-hover:bg-[var(--accent-bg)] group-hover:text-[var(--accent-main)]">
                      <span class="i-lucide-languages h-5 w-5" />
                    </div>
                    <div class="min-w-0 flex-1">
                      <div class="text-sm font-medium text-[var(--text-main)]">
                      ...
                      <div class="mt-0.5 text-xs text-[var(--text-secondary)]">
                        {{ t("settings.frontend.languageDescription") }}
                      </div>
                    </div>
                  ...
                  <div class="rounded-2xl border border-[var(--border-color)] bg-[var(--bg-panel)] px-5 py-4 shadow-sm">
                    ...
                    <div class="flex flex-wrap items-start gap-4">
                      <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[var(--bg-input)] text-[var(--text-secondary)]">
                        <span class="i-lucide-palette h-5 w-5" />
                      </div>
                      <div class="min-w-0 flex-1">
                        <div class="text-sm font-medium text-[var(--text-main)]">
                        ...
                        <div class="mt-0.5 text-xs text-[var(--text-secondary)]">
                          {{ t("settings.frontend.themeDescription") }}
                        </div>
                        <div class="mt-2 text-xs text-[var(--text-muted)]">
                          {{ activeResolvedTheme.label }} · {{ activeThemeIsBuiltIn ? t("settings.frontend.themeBuiltInPreset") : t("settings.frontend.themeCustomPreset") }}
                        </div>
                      </div>
                      <div class="flex shrink-0 items-center gap-2">
                        <button
                          type="button"
                          class="inline-flex h-8 items-center gap-1.5 rounded-md border border-transparent bg-[var(--accent-main)] px-3 text-xs font-medium text-[var(--text-inverse)] transition-opacity hover:opacity-90"
                        ...
                        <button
                          type="button"
                          class="inline-flex h-8 items-center gap-1.5 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-3 text-xs font-medium text-[var(--text-main)] transition-colors hover:bg-[var(--bg-hover)]"
                        ...
                        <input
                          ...
                          @change="void importThemeFile($event)"
                        >
                      </div>
                    </div>
                    ...
                    <!-- 内置主题卡片网格 -->
                    <div class="mt-4">
                      <div class="mb-2 text-[11px] font-medium uppercase tracking-wide text-[var(--text-muted)]">
                        {{ t("settings.frontend.themeBuiltInGroup") }}
                      </div>
                      <div class="grid gap-2.5 [grid-template-columns:repeat(auto-fill,minmax(150px,1fr))]">
                        <div
                          v-for="card in builtInThemeCards"
                          :key="card.id"
                          class="group relative cursor-pointer overflow-hidden rounded-lg border transition-all"
                          :class="activeResolvedTheme.id === card.id ? 'border-[var(--accent-main)] shadow-[0_0_0_1px_var(--accent-main)]' : 'border-[var(--border-color)] hover:border-[var(--border-strong)] hover:shadow-sm'"
                          @click="updateTheme(card.id)"
                        >
                          <!-- 迷你预览：使用该主题自己的变量绘制 -->
                          <div
                            class="relative h-16"
                            :style="{ background: card.vars['--bg-main'] }"
                          >
                            <div
                              class="absolute inset-y-1.5 left-1.5 w-6 rounded-sm"
                              :style="{ background: card.vars['--bg-sidebar'] }"
                            />
                            <div
                              class="absolute bottom-1.5 left-9 right-1.5 top-1.5 rounded-sm border px-1.5 py-1"
                              :style="{ background: card.vars['--bg-panel'], borderColor: card.vars['--border-color'] }"
                            >
                              <div
                                class="text-[11px] font-semibold leading-none"
                                :style="{ color: card.vars['--text-main'] }"
                              >
                                Aa
                              </div>
                              <div
                                class="mt-1 h-1 w-9 rounded-full"
                                :style="{ background: card.vars['--text-muted'] }"
                              />
                              <div class="absolute bottom-1 left-1.5 flex items-center gap-1">
                                <span
                                  class="h-1.5 w-3 rounded-full"
                                  :style="{ background: card.vars['--accent-main'] }"
                                />
                                <span
                                  class="h-1.5 w-1.5 rounded-full"
                                  :style="{ background: card.vars['--status-info'] }"
                                />
                                <span
                                  class="h-1.5 w-1.5 rounded-full"
                                  :style="{ background: card.vars['--status-success'] }"
                                />
                                <span
                                  class="h-1.5 w-1.5 rounded-full"
                                  :style="{ background: card.vars['--status-warning'] }"
                                />
                                <span
                                  class="h-1.5 w-1.5 rounded-full"
                                  :style="{ background: card.vars['--status-danger'] }"
                                />
                              </div>
                            </div>
                            <!-- 悬停操作：复制为自定义 / 导出 -->
                            <div class="absolute right-1 top-1 flex items-center gap-0.5 rounded-md border border-[var(--border-color)] bg-[var(--bg-panel)] p-0.5 opacity-0 shadow-sm transition-opacity group-hover:opacity-100">
                              <button
                                type="button"
                                class="flex h-6 w-6 items-center justify-center rounded text-[var(--text-secondary)] transition-colors hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)]"
                                :title="t('settings.frontend.themeCopy')"
                                @click.stop="openThemeCopier(card.id)"
                              >
                              ...
                              <button
                                type="button"
                                class="flex h-6 w-6 items-center justify-center rounded text-[var(--text-secondary)] transition-colors hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)]"
                                :title="t('settings.frontend.themeExport')"
                                @click.stop="exportTheme(card.id)"
                              >
                          ...
                          <div class="flex items-center gap-1.5 border-t border-[var(--border-color)] bg-[var(--bg-input)] px-2 py-1.5">
                            <span
                              ...
                              :class="card.appearance === 'dark' ? 'i-lucide-moon' : 'i-lucide-sun'"
                            />
                            <span class="min-w-0 flex-1 truncate text-xs text-[var(--text-main)]">{{ card.name }}</span>
                            <span
                              v-if="activeResolvedTheme.id === card.id"
                              class="i-lucide-check h-3.5 w-3.5 shrink-0 text-[var(--accent-main)]"
                            />
                          </div>
                        </div>
                      </div>
                    ...
                    <div class="mt-4">
                      <div class="mb-2 text-[11px] font-medium uppercase tracking-wide text-[var(--text-muted)]">
                        {{ t("settings.frontend.themeCustomGroup") }}
                      </div>
                      <div
                        v-if="customThemeCards.length"
                        class="grid gap-2.5 [grid-template-columns:repeat(auto-fill,minmax(150px,1fr))]"
                      >
                        <div
                          v-for="card in customThemeCards"
                          :key="card.id"
                          class="group relative cursor-pointer overflow-hidden rounded-lg border transition-all"
                          :class="activeThemeId === card.id ? 'border-[var(--accent-main)] shadow-[0_0_0_1px_var(--accent-main)]' : 'border-[var(--border-color)] hover:border-[var(--border-strong)] hover:shadow-sm'"
                          @click="updateTheme(card.id)"
                        >
                          <!-- 迷你预览：使用该主题自己的变量绘制 -->
                          <div
                          ...
                          >
                            ...
                            <div class="absolute right-1 top-1 flex items-center gap-0.5 rounded-md border border-[var(--border-color)] bg-[var(--bg-panel)] p-0.5 opacity-0 shadow-sm transition-opacity group-hover:opacity-100">
                              <button
                                ...
                                type="button"
                                class="flex h-6 w-6 items-center justify-center rounded text-[var(--text-secondary)] transition-colors hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)]"
                                :title="t('settings.frontend.themeEdit')"
                                @click.stop="openThemeEditor(card.custom)"
                              >
                              ...
                              <button
                                v-if="card.custom"
                                type="button"
                                class="flex h-6 w-6 items-center justify-center rounded text-[var(--status-danger)] transition-colors hover:bg-[var(--status-danger-bg)]"
                                :title="t('settings.frontend.themeDelete')"
                                @click.stop="requestDeleteTheme(card.custom)"
                              >
                      ...
                      <div
                        v-else
                        class="rounded-md bg-[var(--bg-input)] px-3 py-2 text-xs text-[var(--text-muted)]"
                      >
                        {{ t("settings.frontend.themeNoCustom") }}
                      </div>
                    </div>
                  </div>
                  ...
                  <div class="group flex items-center gap-4 rounded-2xl border border-[var(--border-color)] bg-[var(--bg-panel)] px-5 py-4 shadow-sm transition-all duration-300 hover:shadow-md">
                    <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[var(--bg-input)] text-[var(--text-secondary)] transition-colors group-hover:bg-[var(--accent-bg)] group-hover:text-[var(--accent-main)]">
                      <span class="i-lucide-brain-circuit h-5 w-5" />
                    </div>
                    <div class="min-w-0 flex-1">
                      <div class="text-sm font-medium text-[var(--text-main)]">
                      ...
                      <div class="mt-0.5 text-xs text-[var(--text-secondary)]">
                        {{ t("settings.frontend.reasoningDescription") }}
                      </div>
                    </div>
                    <div class="w-40 shrink-0">
                      <FormSelect
                        :model-value="selectedReasoning"
                        :options="novelIdeStore.reasoningOptions.map((item) => ({ value: item, label: item }))"
                        @update:model-value="selectedReasoning = $event"
                      />
                    </div>
                  </div>
                  ...
                  <div class="group flex items-center gap-4 rounded-2xl border border-[var(--border-color)] bg-[var(--bg-panel)] px-5 py-4 shadow-sm transition-all duration-300 hover:shadow-md">
                    <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[var(--bg-input)] text-[var(--text-secondary)] transition-colors group-hover:bg-[var(--accent-bg)] group-hover:text-[var(--accent-main)]">
                      <span class="i-lucide-layout h-5 w-5" />
                    </div>
                    <div class="min-w-0 flex-1">
                      <div class="text-sm font-medium text-[var(--text-main)]">
                      ...
                      <div class="mt-0.5 text-xs text-[var(--text-secondary)]">
                        {{ t("settings.frontend.viewModeDescription") }}
                      </div>
                    </div>
                    <div class="w-40 shrink-0">
                      <FormSelect
              ...
              >
                <div class="flex flex-wrap items-center justify-between gap-4">
                  <div class="max-w-xl">
                    <h3 class="text-base font-semibold text-[var(--text-main)]">
                      {{ t("settings.editor.markdownTitle") }}
                    </h3>
                    <p class="mt-1 text-xs text-[var(--text-secondary)]">
                      {{ t("settings.editor.markdownDescription") }}
                    </p>
                  </div>
                  <button
                    type="button"
                    class="inline-flex h-8 items-center gap-1.5 rounded-lg border border-[var(--border-color)] bg-[var(--bg-panel)] px-3 text-xs text-[var(--text-secondary)] transition-colors hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)]"
                    @click="resetEditorPreferences"
                  >
                    <span class="i-lucide-rotate-ccw h-3.5 w-3.5" />
                ...
                <div class="grid gap-3">
                  <div class="group flex items-center gap-4 rounded-2xl border border-[var(--border-color)] bg-[var(--bg-panel)] px-5 py-4 shadow-sm transition-all duration-300 hover:shadow-md">
                    <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[var(--bg-input)] text-[var(--text-secondary)] transition-colors group-hover:bg-[var(--accent-bg)] group-hover:text-[var(--accent-main)]">
                      <span class="i-lucide-type h-5 w-5" />
                    </div>
                    <div class="min-w-0 flex-1">
                      <div class="text-sm font-medium text-[var(--text-main)]">
                        {{ t("settings.editor.bodyFontTitle") }}
                      </div>
                      <div class="mt-0.5 text-xs text-[var(--text-secondary)]">
                        {{ t("settings.editor.bodyFontDescription") }}
                      </div>
                    </div>
                    <div class="w-72 shrink-0">
                      ...
                      <datalist id="markdown-editor-font-options">
                        <option
                          v-for="option in editorFontOptions"
                          :key="option.value"
                          :value="option.value"
                        >
                          {{ option.label }}
                  ...
                  <div class="grid gap-3 md:grid-cols-2">
                    <label class="rounded-2xl border border-[var(--border-color)] bg-[var(--bg-panel)] px-5 py-4 shadow-sm">
                      ...
                      <input
                        type="number"
                        class="mt-3 h-8 w-full rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-2.5 text-sm text-[var(--text-main)] outline-none focus:border-[var(--accent-main)] focus:ring-1 focus:ring-[var(--accent-main)] focus:ring-opacity-20"
                        :value="markdownEditorPreferences.fontSize"
                        min="12"
                        max="28"
                        step="1"
                        @input="updateEditorNumber('fontSize', ($event.target as HTMLInputElement).value, 12, 28)"
                      >
                    </label>
                    ...
                    <label class="rounded-2xl border border-[var(--border-color)] bg-[var(--bg-panel)] px-5 py-4 shadow-sm">
                      ...
                      <input
                        type="number"
                        class="mt-3 h-8 w-full rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-2.5 text-sm text-[var(--text-main)] outline-none focus:border-[var(--accent-main)] focus:ring-1 focus:ring-[var(--accent-main)] focus:ring-opacity-20"
                        :value="markdownEditorPreferences.lineHeight"
                        min="1.2"
                        max="2.6"
                        step="0.05"
                        @input="updateEditorNumber('lineHeight', ($event.target as HTMLInputElement).value, 1.2, 2.6)"
                      >
                    </label>
                    ...
                    <label class="rounded-2xl border border-[var(--border-color)] bg-[var(--bg-panel)] px-5 py-4 shadow-sm">
                      ...
                      <input
                        type="number"
                        class="mt-3 h-8 w-full rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-2.5 text-sm text-[var(--text-main)] outline-none focus:border-[var(--accent-main)] focus:ring-1 focus:ring-[var(--accent-main)] focus:ring-opacity-20"
                        :value="markdownEditorPreferences.contentWidth"
                        min="520"
                        max="1280"
                        step="20"
                        @input="updateEditorNumber('contentWidth', ($event.target as HTMLInputElement).value, 520, 1280)"
                      >
                    </label>
                    ...
                    <div class="rounded-2xl border border-[var(--border-color)] bg-[var(--bg-panel)] px-5 py-4 shadow-sm">
                      <div class="flex items-center justify-between gap-4">
                        <div class="min-w-0">
                          <div class="text-sm font-medium text-[var(--text-main)]">
                            {{ t("settings.editor.paragraphIndentTitle") }}
                          </div>
                          <div class="mt-0.5 text-xs text-[var(--text-secondary)]">
                            {{ t("settings.editor.paragraphIndentDescription") }}
                          </div>
                        </div>
                        <button
                          type="button"
                          class="relative h-6 w-11 rounded-full border transition-colors"
                          :class="markdownEditorPreferences.paragraphIndentEnabled ? 'border-[var(--accent-main)] bg-[var(--accent-main)]' : 'border-[var(--border-color)] bg-[var(--bg-input)]'"
                          @click="updateEditorPreferences({ paragraphIndentEnabled: !markdownEditorPreferences.paragraphIndentEnabled })"
                        >
                          <span
                            class="absolute top-0.5 h-[18px] w-[18px] rounded-full bg-white shadow transition-transform"
                            :class="markdownEditorPreferences.paragraphIndentEnabled ? 'translate-x-5' : 'translate-x-0.5'"
                          />
                        </button>
                      </div>
                      <input
                        type="number"
                        class="mt-3 h-8 w-full rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-2.5 text-sm text-[var(--text-main)] outline-none focus:border-[var(--accent-main)] focus:ring-1 focus:ring-[var(--accent-main)] focus:ring-opacity-20 disabled:opacity-50"
                        :value="markdownEditorPreferences.paragraphIndentEm"
                        min="0"
                        max="4"
                        step="0.25"
                        :disabled="!markdownEditorPreferences.paragraphIndentEnabled"
                        @input="updateEditorNumber('paragraphIndentEm', ($event.target as HTMLInputElement).value, 0, 4)"
                      >
                    </div>
                  </div>
                </div>
                ...
                <div class="grid gap-3 border-t border-[var(--border-color)] pt-4">
                  <div class="flex flex-wrap items-center justify-between gap-4">
                    <div class="max-w-xl">
                      <h3 class="text-base font-semibold text-[var(--text-main)]">
                        {{ t("settings.editor.monacoTitle") }}
                      </h3>
                      <p class="mt-1 text-xs text-[var(--text-secondary)]">
                        {{ t("settings.editor.monacoDescription") }}
                      </p>
                    </div>
                    <button
                      type="button"
                      class="inline-flex h-8 items-center gap-1.5 rounded-lg border border-[var(--border-color)] bg-[var(--bg-panel)] px-3 text-xs text-[var(--text-secondary)] transition-colors hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)]"
                      @click="resetMonacoPreferences"
                    >
                      <span class="i-lucide-rotate-ccw h-3.5 w-3.5" />
                  ...
                  <div class="group flex items-center gap-4 rounded-2xl border border-[var(--border-color)] bg-[var(--bg-panel)] px-5 py-4 shadow-sm transition-all duration-300 hover:shadow-md">
                    <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[var(--bg-input)] text-[var(--text-secondary)] transition-colors group-hover:bg-[var(--accent-bg)] group-hover:text-[var(--accent-main)]">
                      <span class="i-lucide-code-2 h-5 w-5" />
                    </div>
                    <div class="min-w-0 flex-1">
                      <div class="text-sm font-medium text-[var(--text-main)]">
                        {{ t("settings.editor.monacoFontTitle") }}
                      </div>
                      <div class="mt-0.5 text-xs text-[var(--text-secondary)]">
                        {{ t("settings.editor.monacoFontDescription") }}
                      </div>
                    </div>
                    <div class="w-72 shrink-0">
                      <input
                      ...
                      <datalist id="monaco-editor-font-options">
                        <option
                          v-for="option in monacoFontOptions"
                          :key="option.value"
                          :value="option.value"
                        >
                          {{ option.label }}
                  ...
                  <div class="grid gap-3 md:grid-cols-3">
                    <label class="rounded-2xl border border-[var(--border-color)] bg-[var(--bg-panel)] px-5 py-4 shadow-sm">
                      ...
                      <input
                        type="number"
                        class="mt-3 h-8 w-full rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-2.5 text-sm text-[var(--text-main)] outline-none focus:border-[var(--accent-main)] focus:ring-1 focus:ring-[var(--accent-main)] focus:ring-opacity-20"
                        :value="monacoEditorPreferences.fontSize"
                        min="10"
                        max="32"
                        step="1"
                        @input="updateMonacoNumber('fontSize', ($event.target as HTMLInputElement).value, 10, 32)"
                      >
                    </label>
                    ...
                    <label class="rounded-2xl border border-[var(--border-color)] bg-[var(--bg-panel)] px-5 py-4 shadow-sm">
                      ...
                      <input
                        type="number"
                        class="mt-3 h-8 w-full rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-2.5 text-sm text-[var(--text-main)] outline-none focus:border-[var(--accent-main)] focus:ring-1 focus:ring-[var(--accent-main)] focus:ring-opacity-20"
                        :value="monacoEditorPreferences.lineHeight"
                        min="16"
                        max="56"
                        step="1"
                        @input="updateMonacoNumber('lineHeight', ($event.target as HTMLInputElement).value, 16, 56)"
                      >
                    </label>
                    ...
                    <label class="rounded-2xl border border-[var(--border-color)] bg-[var(--bg-panel)] px-5 py-4 shadow-sm">
                      ...
                      <input
                        type="number"
                        class="mt-3 h-8 w-full rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-2.5 text-sm text-[var(--text-main)] outline-none focus:border-[var(--accent-main)] focus:ring-1 focus:ring-[var(--accent-main)] focus:ring-opacity-20"
                        :value="monacoEditorPreferences.tabSize"
                        min="2"
                        max="8"
                        step="1"
                        @input="updateMonacoNumber('tabSize', ($event.target as HTMLInputElement).value, 2, 8)"
                      >
                    </label>
                  </div>
                  ...
                  <div class="grid gap-3 md:grid-cols-2">
                    <button
                      type="button"
                      class="flex items-center justify-between gap-4 rounded-2xl border border-[var(--border-color)] bg-[var(--bg-panel)] px-5 py-4 text-left shadow-sm transition-all hover:bg-[var(--bg-hover)]"
                      @click="updateMonacoPreferences({ wordWrap: !monacoEditorPreferences.wordWrap })"
                    >
                      ...
                      <span
                        ...
                        :class="monacoEditorPreferences.wordWrap ? 'bg-[var(--status-success)]' : 'bg-[var(--text-muted)]'"
                      />
                    </button>
                    <button
                      type="button"
                      class="flex items-center justify-between gap-4 rounded-2xl border border-[var(--border-color)] bg-[var(--bg-panel)] px-5 py-4 text-left shadow-sm transition-all hover:bg-[var(--bg-hover)]"
                      @click="updateMonacoPreferences({ minimapEnabled: !monacoEditorPreferences.minimapEnabled })"
                    >
                      ...
                      <span
                        class="h-2.5 w-2.5 rounded-full"
                        :class="monacoEditorPreferences.minimapEnabled ? 'bg-[var(--status-success)]' : 'bg-[var(--text-muted)]'"
                      />
                    </button>
                    <button
                      type="button"
                      class="flex items-center justify-between gap-4 rounded-2xl border border-[var(--border-color)] bg-[var(--bg-panel)] px-5 py-4 text-left shadow-sm transition-all hover:bg-[var(--bg-hover)]"
                      @click="updateMonacoPreferences({ lineNumbers: !monacoEditorPreferences.lineNumbers })"
                    >
                      ...
                      <span
                        class="h-2.5 w-2.5 rounded-full"
                        :class="monacoEditorPreferences.lineNumbers ? 'bg-[var(--status-success)]' : 'bg-[var(--text-muted)]'"
                      />
                    </button>
                    <button
                      type="button"
                      class="flex items-center justify-between gap-4 rounded-2xl border border-[var(--border-color)] bg-[var(--bg-panel)] px-5 py-4 text-left shadow-sm transition-all hover:bg-[var(--bg-hover)]"
                      @click="updateMonacoPreferences({ renderWhitespace: !monacoEditorPreferences.renderWhitespace })"
                    >
                      ...
                      <span
                        class="h-2.5 w-2.5 rounded-full"
                        :class="monacoEditorPreferences.renderWhitespace ? 'bg-[var(--status-success)]' : 'bg-[var(--text-muted)]'"
                      />
                    </button>
                  </div>
                </div>
              ...
              <div
                v-else-if="activeSection === 'desktop' && desktopAvailable"
                key="desktop"
                class="space-y-4 pt-1"
              >
                <div class="rounded-2xl border border-[var(--status-info-border)] bg-[var(--status-info-bg)] px-5 py-4 text-[var(--text-main)]">
                  <div class="flex items-start gap-3">
                    <span class="i-lucide-panels-top-left mt-0.5 h-5 w-5 shrink-0 text-[var(--status-info)]" />
                    <div class="min-w-0">
                      <h3 class="text-sm font-semibold">
                      ...
                      <p
                        v-if="desktopStatus"
                        class="mt-2 text-[11px] text-[var(--text-muted)]"
                      >
                        {{ desktopStatus.connection === "local" ? t("settings.desktop.localStatus") : t("settings.desktop.remoteStatus") }} · {{ desktopStatus.version }}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <label class="rounded-2xl border border-[var(--border-color)] bg-[var(--bg-panel)] px-5 py-4 shadow-sm">
                  ...
                  <input
                    class="mt-4 w-full accent-[var(--accent-main)]"
                    type="range"
                    min="0.75"
                    max="2"
                    step="0.05"
                    :value="desktopSettings.zoomFactor"
                    @change="updateDesktopSettings({ zoomFactor: Number(($event.target as HTMLInputElement).value) })"
                  >
                </label>

                <button
                  type="button"
                  class="flex w-full items-center justify-between gap-4 rounded-2xl border border-[var(--border-color)] bg-[var(--bg-panel)] px-5 py-4 text-left shadow-sm transition-colors hover:bg-[var(--bg-hover)]"
                  @click="updateDesktopSettings({ trayEnabled: !desktopSettings.trayEnabled })"
                >
                  <span>
                  ...
                  <span
                    ...
                    :class="desktopSettings.trayEnabled ? 'bg-[var(--status-success)]' : 'bg-[var(--text-muted)]'"
                  />
                </button>

                <div class="rounded-2xl border border-[var(--border-color)] bg-[var(--bg-panel)] px-5 py-4 shadow-sm">
                  <div class="text-sm font-medium text-[var(--text-main)]">
                    {{ t("settings.desktop.closeTitle") }}
                  </div>
                  <div class="mt-1 text-xs text-[var(--text-secondary)]">
                    {{ t("settings.desktop.closeDescription") }}
                  </div>
                  <div class="mt-3 max-w-sm">
                    <FormSelect
                      :model-value="desktopSettings.closeBehavior"
                      :options="desktopCloseOptions"
                      @update:model-value="updateDesktopCloseBehavior"
                    />
                  </div>
                </div>
              </div>
              ...
              >
                <NovelIdeEmbeddingSettingsPanel
                  ref="embeddingSettingsPanelRef"
                  :key="`embedding:${settingsPanelKey}`"
                  :scope="activeScope === 'project' ? 'project' : 'global'"
                  :target-query="targetQuery"
                  :target-label="targetLabel"
                />
              ...
              >
                <NovelIdeCostSettingsPanel
                  ref="costSettingsPanelRef"
                  :key="`cost:${settingsPanelKey}`"
                  :target-query="targetQuery"
                />
              </div>
              ...
              >
                <NovelIdeWebSettingsPanel
                  ref="webSettingsPanelRef"
                  :key="`web-tools:${settingsPanelKey}`"
                  :target-query="targetQuery"
                  :target-label="targetLabel"
                />
              </div>
              ...
              <div
                ...
                key="agent-profile-models"
              >
                <NovelIdeAgentProfileModelSettingsPanel
                  ref="agentProfileModelSettingsPanelRef"
                  :key="`profile-models:${settingsPanelKey}`"
                  :scope="activeScope === 'project' ? 'project' : 'global'"
                  :target-query="targetQuery"
                  :target-label="targetLabel"
                />
              ...
              <div
                ...
                key="observability"
              >
                <NovelIdeObservabilitySettingsPanel
                  ref="observabilitySettingsPanelRef"
                  :key="`observability:${settingsPanelKey}`"
                  :target-query="targetQuery"
                />
              </div>
            </Transition>
  ...
  >
    ...
    <p class="text-sm text-[var(--text-secondary)]">
      {{ t("settings.frontend.themeDeleteMessage", { name: themeDeleteTarget?.name ?? "" }) }}
    </p>
  </Dialog>
</template>
