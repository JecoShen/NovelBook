...
<template>
  ...
  <div class="contents">
    <aside
      v-if="activeTab"
      v-bind="attrs"
      class="relative z-10 flex shrink-0 flex-col border-r border-[var(--border-color)] bg-[var(--bg-panel)]"
    ...
    >
      ...
      >
        <div
          class="ml-0.5 h-full w-[2px] bg-[var(--accent-main)] opacity-0 transition-all duration-150 group-hover:opacity-100"
      ...
      <div class="shrink-0 border-b border-[var(--border-color)]">
        <div class="flex h-9 items-center gap-1 border-b border-[var(--border-color)] px-2">
          <Tooltip
            :text="t('ide.header.bookshelfTitle')"
            placement="bottom"
          >
            <button
              type="button"
              class="flex h-7 w-7 shrink-0 items-center justify-center rounded-md text-[var(--text-muted)] hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)]"
              @click="emit('openHome')"
            >
              <span class="i-lucide-library h-4 w-4" />
            </button>
          </Tooltip>
          <Dropdown
          ...
          >
            <button
              type="button"
              class="flex h-7 w-full min-w-0 items-center gap-1.5 rounded-md px-2 text-left text-xs text-[var(--text-secondary)] hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)]"
              :title="props.workspaceTitle"
            >
              <span class="i-lucide-book-open h-3.5 w-3.5 shrink-0" />
              <span class="min-w-0 flex-1 truncate">{{ props.workspaceTitle }}</span>
              <span class="i-lucide-chevron-down h-3.5 w-3.5 shrink-0" />
            </button>
          </Dropdown>
          <div
            v-else
            class="min-w-0 flex-1 truncate px-2 text-xs text-[var(--text-secondary)]"
          >
        ...
        <div class="flex items-center justify-between px-3 py-2">
          ...
          <div class="flex items-center gap-0.5">
            <input
              ...
              class="hidden"
              type="file"
              @change="(event) => void handleSingleFileSelected(event)"
            >
            <input
              ...
              multiple
              webkitdirectory
              @change="(event) => void handleProjectDirectorySelected(event)"
            >
            <input
              ...
              @change="(event) => void handleProjectZipSelected(event)"
            >

            <Tooltip
              :text="t('ide.toolPanel.uploadSingleTitle')"
              placement="bottom"
            >
              <button
                class="rounded-2 p-1 text-[var(--text-muted)] transition-colors hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)] disabled:cursor-not-allowed disabled:opacity-50"
                :disabled="uploadingSingleFile"
                @click="openSingleFileUpload"
              >
                <span
                  :class="uploadingSingleFile ? 'i-lucide-loader-2 animate-spin' : 'i-lucide-file-up'"
                  class="h-4 w-4"
                />
              </button>
            </Tooltip>
            <Dropdown
              ...
              menu-class="right-0 top-full mt-1 w-36"
              @select="selectProjectUploadMode"
            >
              <Tooltip
                :text="t('ide.toolPanel.uploadProjectTitle')"
                placement="bottom"
              >
                <button
                  class="rounded-2 p-1 text-[var(--text-muted)] transition-colors hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)] disabled:cursor-not-allowed disabled:opacity-50"
                ...
                >
                  <span
                    :class="uploadingProject ? 'i-lucide-loader-2 animate-spin' : 'i-lucide-folder-up'"
                    class="h-4 w-4"
                  />
                </button>
              </Tooltip>
            ...
            >
              ...
              >
                <span
                  :class="downloadingWorkspace ? 'i-lucide-loader-2 animate-spin' : 'i-lucide-download'"
                  class="h-4 w-4"
                />
              </button>
            </Tooltip>
            <Tooltip
              v-if="props.userAssetsMode"
              :text="t('ide.toolPanel.syncAssetsTitle')"
              placement="bottom"
            >
              <button
                class="rounded-2 p-1 text-[var(--text-muted)] transition-colors hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)] disabled:cursor-not-allowed disabled:opacity-50"
                :disabled="syncingAssets"
                @click="void syncSystemAssets()"
              >
                <span
                  :class="syncingAssets ? 'i-lucide-loader-2 animate-spin' : 'i-lucide-folder-sync'"
                  class="h-4 w-4"
                />
              </button>
            </Tooltip>
            <Tooltip
              v-if="props.userAssetsMode && lastSyncWarnings.length"
              :text="t('ide.toolPanel.viewSyncConflicts')"
              placement="bottom"
            >
              <button
                class="rounded-2 p-1 text-[var(--text-muted)] transition-colors hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)]"
                @click="syncWarningsOpen = true"
              >
                <span class="i-lucide-triangle-alert h-4 w-4" />
              </button>
            </Tooltip>
            <Tooltip
              :text="t('ide.toolPanel.collapsePanel')"
              placement="bottom"
            >
              <button
                class="rounded-2 p-1 text-[var(--text-muted)] transition-colors hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)]"
      ...
      <div
        class="contain-layout-paint min-h-0 flex-1 overflow-hidden"
      ...
      >
        ...
        <NovelPlotPanel
          v-else-if="activeTab === 'plot' && !props.userAssetsMode"
    ...
    <Dialog
      v-model="downloadConfirmOpen"
      :title="t('ide.toolPanel.downloadTitle', { target: downloadTargetLabel })"
      width="420px"
      show-cancel
      :busy="downloadingWorkspace"
      @confirm="confirmDownloadWorkspace"
    >
      <p>{{ t("ide.toolPanel.downloadConfirm", { target: downloadTargetLabel }) }}</p>
      <p
        v-if="!props.userAssetsMode"
        class="mt-3 rounded-md border border-[var(--status-warning-border)] bg-[var(--status-warning-bg)] px-3 py-2 text-sm leading-5 text-[var(--status-warning)]"
      >
        {{ t("ide.toolPanel.downloadProjectHistoryWarning") }}
      </p>
    </Dialog>

    <Dialog
      v-model="syncWarningsOpen"
      :title="t('ide.toolPanel.syncDetailsTitle')"
      width="620px"
      :show-footer="false"
    >
      <div class="space-y-3">
        <p class="m-0 text-sm text-[var(--text-secondary)]">
          {{ t("ide.toolPanel.syncDetailsDescription") }}
        </p>
        <div class="max-h-[52vh] space-y-2 overflow-auto pr-1">
          ...
          >
            ...
            <div class="mt-1 truncate font-mono text-xs text-[var(--text-muted)]">
              {{ item.path }}
            </div>
