...
<template>
  ...
  <div
    ref="rootRef"
    class="ide-prompt-bar z-20 shrink-0 px-4"
  >
    <div
      v-if="props.expanded"
      class="relative mx-auto w-full max-w-4xl pb-5 pt-7"
    >
      <button
      ...
      <div class="w-full overflow-visible rounded-xl border border-[var(--border-color)] bg-[var(--bg-panel)] shadow-2xl shadow-black/10 transition-all focus-within:border-[var(--accent-main)] focus-within:ring-1 focus-within:ring-[var(--accent-main)]">
        ...
        <div
          v-if="hasLivePanel"
          class="border-b border-[var(--border-color)] bg-[var(--bg-sidebar)] px-4 py-2 text-xs text-[var(--text-secondary)]"
        >
          <div class="mb-2 flex items-center justify-between gap-3">
            <div class="flex min-w-0 items-center gap-2 font-medium text-[var(--text-main)]">
              <span
                :class="props.running ? 'i-lucide-loader-circle animate-spin text-[var(--accent-text)]' : 'i-lucide-message-square-text text-[var(--status-success)]'"
                class="h-3.5 w-3.5 shrink-0"
              />
              <span class="truncate">{{ livePanelTitle }}</span>
            </div>
            <button
              class="inline-flex h-6 shrink-0 items-center gap-1.5 rounded-md border border-[var(--border-color)] bg-[var(--bg-panel)] px-2 text-[11px] text-[var(--text-secondary)] transition-colors hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)]"
              type="button"
              @click="emit('open-session-chat')"
            >
              <span class="i-lucide-panel-right-open h-3 w-3" />
              <span>{{ t("ide.inlineAi.openSessionChat") }}</span>
            </button>
          </div>
          <div class="max-h-48 space-y-2 overflow-y-auto pr-1">
            <div
              v-if="props.liveView.thinking"
              class="rounded-md border border-[var(--border-color)]/70 bg-[var(--bg-panel)] px-2.5 py-2"
            >
              ...
              <AgentMarkdownContent
                :content="props.liveView.thinking"
                :streaming="props.liveView.status === 'streaming'"
              />
            </div>
            <div
              v-if="props.liveView.content"
              class="rounded-md border border-[var(--border-color)]/70 bg-[var(--bg-panel)] px-2.5 py-2 text-[var(--text-secondary)]"
            >
              <AgentMarkdownContent
                :content="props.liveView.content"
                :streaming="props.liveView.status === 'streaming'"
              />
            </div>
            <div
              v-if="displayEditPreview"
              class="rounded-md border border-[var(--border-color)]/70 bg-[var(--bg-panel)] px-2.5 py-2"
            >
              <div class="mb-1 flex items-center gap-1.5 text-[10px] font-medium uppercase text-[var(--text-muted)]">
            ...
            <div
              v-if="displayResultText && !displayEditPreview"
              class="rounded-md border border-[var(--status-success-border)] bg-[var(--status-success-bg)] px-2.5 py-2 text-[var(--text-secondary)]"
            >
              <div class="mb-1 flex items-center gap-1.5 text-[10px] font-medium uppercase text-[var(--status-success)]">
                <span class="i-lucide-check-check h-3 w-3" />
        ...
        <div class="border-b border-[var(--border-color)] px-4 py-1.5">
          <div class="flex min-h-5 flex-wrap items-center gap-x-2 gap-y-1">
            ...
            >
              <span class="i-lucide-text-select h-3.5 w-3.5 shrink-0 text-[var(--accent-text)]" />
            ...
            <span
              ...
              class="inline-flex h-5 items-center text-xs text-[var(--text-muted)]"
            >
        ...
        <div class="flex items-center justify-between gap-3 border-t border-[var(--border-color)] bg-[var(--bg-sidebar)] px-3 py-2">
          <div class="flex min-w-0 flex-wrap items-center gap-1.5">
            <Dropdown
              ...
              menu-class="left-0 bottom-full mb-1.5 w-40"
              compact
              @select="selectTask"
            >
              <button
                class="inline-flex h-8 items-center gap-1.5 rounded-md px-2.5 text-xs text-[var(--text-secondary)] transition-colors hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)]"
                type="button"
                :title="currentTask.description"
              >
                <span
                  :class="currentTask.iconClass"
                  class="h-3.5 w-3.5 text-[var(--accent-text)]"
                />
                <span>{{ currentTask.label }}</span>
                <span class="i-lucide-chevron-up h-3 w-3 text-[var(--text-muted)]" />
              </button>
            </Dropdown>
            ...
            <Dropdown
              ...
              root-class="relative inline-block"
              menu-class="left-0 bottom-full mb-1.5 w-56"
              compact
            ...
            >
              ...
              >
                <span
                  :class="props.sessionLoading ? 'i-lucide-loader-circle animate-spin' : 'i-lucide-message-square-more'"
          ...
          <div class="flex items-center gap-2">
            ...
            >
              <span
                v-if="props.running"
                class="i-lucide-square h-3.5 w-3.5"
              />
              <span
                v-else
                class="i-lucide-send h-3.5 w-3.5"
              />
            </button>
          </div>
        </div>
