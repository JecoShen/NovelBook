  ...
  <div class="flex min-h-0 flex-1 flex-col overflow-y-auto bg-[var(--bg-main)] text-[var(--text-main)]">
    ...
    <main class="mx-auto flex w-full max-w-[1200px] flex-col gap-8 px-5 py-8 sm:px-6 sm:py-10 lg:px-8">
      ...
      <section class="flex flex-col gap-5 border-b border-[var(--border-color)] pb-7 sm:flex-row sm:items-end sm:justify-between">
        <div class="min-w-0">
          <h1 class="font-serif text-2xl font-bold text-[var(--text-main)]">
            {{ t("ide.picker.title") }}
          </h1>
        ...
        <div class="flex w-full shrink-0 flex-col gap-2 sm:w-auto sm:flex-row">
          <button
            type="button"
            class="inline-flex h-10 items-center justify-center gap-2 rounded-md border border-[var(--border-color)] bg-[var(--bg-panel)] px-4 text-sm font-medium text-[var(--text-secondary)] transition-colors hover:border-[var(--border-strong)] hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent-main)]"
            @click="emit('open-user-assets')"
          >
            <span class="i-lucide-folder-cog h-4 w-4" />
            {{ t("ide.picker.openUserAssets") }}
          </button>
          <button
            type="button"
            class="inline-flex h-10 items-center justify-center gap-2 rounded-md bg-[var(--accent-main)] px-4 text-sm font-medium text-[var(--text-inverse)] transition-opacity hover:opacity-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent-main)] focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--bg-main)] disabled:cursor-not-allowed disabled:opacity-60"
            :disabled="isLoading || Boolean(loadError) || isCreating"
            :aria-expanded="isCreateFormOpen"
            @click="void openCreateForm()"
          >
            <span class="i-lucide-book-plus h-4 w-4" />
      ...
      <form
        ...
        class="rounded-lg border border-[var(--border-accent)] bg-[var(--bg-panel)] p-4 sm:p-5"
        @keydown.esc.stop.prevent="cancelCreateForm"
        @submit.prevent="void handleCreateNovel()"
      >
        <div class="mb-4 flex items-center gap-2 text-sm font-semibold text-[var(--text-main)]">
          <span class="i-lucide-book-plus h-4 w-4 text-[var(--accent-text)]" />
          {{ t("ide.bookshelf.createBook") }}
        </div>
        <div
          v-if="createRecoveryNotice"
          class="mb-4 rounded-md border border-[var(--status-warning-border)] bg-[var(--status-warning-bg)] px-3 py-2 text-sm text-[var(--status-warning)]"
          role="status"
        >
        ...
        <div
          v-if="createRecovery"
          class="mb-4 space-y-2 rounded-md border border-[var(--status-danger-border)] bg-[var(--status-danger-bg)] px-3 py-2 text-sm text-[var(--status-danger)]"
          role="alert"
        >
          <p>{{ createRecovery.error || t("ide.picker.createRecoveryRequired") }}</p>
          <button
            type="button"
            class="inline-flex h-8 items-center justify-center gap-1.5 rounded-md border border-[var(--status-danger-border)] bg-[var(--bg-panel)] px-3 text-xs font-medium hover:bg-[var(--bg-hover)] disabled:opacity-50"
            :disabled="isCreating"
            @click="void retryCreateRecovery()"
          >
            <span
        ...
        <div class="grid gap-4 lg:grid-cols-[minmax(0,0.8fr)_minmax(0,1.4fr)_auto] lg:items-end">
          <label class="block text-xs text-[var(--text-secondary)]">
            ...
            <input
              ...
              v-model="createTitle"
              class="h-10 w-full rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-3 text-sm text-[var(--text-main)] outline-none transition-colors focus:border-[var(--accent-main)]"
              maxlength="120"
              :disabled="isCreating || Boolean(createRecovery)"
              autofocus
            >
          </label>
          <label class="block text-xs text-[var(--text-secondary)]">
            ...
            <textarea
              ...
              class="h-20 w-full resize-none rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-3 py-2 text-sm leading-5 text-[var(--text-main)] outline-none transition-colors focus:border-[var(--accent-main)] lg:h-10"
              maxlength="2000"
              :disabled="isCreating || Boolean(createRecovery)"
            />
          </label>
          <div class="grid grid-cols-2 gap-2 lg:flex">
            <button
              type="button"
              class="inline-flex h-10 items-center justify-center gap-1.5 rounded-md border border-[var(--border-color)] px-3 text-sm text-[var(--text-secondary)] transition-colors hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)] disabled:opacity-60"
              :disabled="isCreating || Boolean(createRecovery)"
            ...
            <button
              type="submit"
              class="inline-flex h-10 items-center justify-center gap-1.5 rounded-md bg-[var(--accent-main)] px-4 text-sm font-medium text-[var(--text-inverse)] transition-opacity hover:opacity-90 disabled:opacity-60"
              :disabled="isCreating || Boolean(createRecovery)"
            >
              <span
                v-if="isCreating"
                class="i-lucide-loader-2 h-4 w-4 animate-spin"
              />
              <span
                v-else
      ...
      >
        <div class="mb-5 flex items-center justify-between gap-4">
          <div class="h-5 w-24 animate-pulse rounded-md bg-[var(--bg-input)]" />
        ...
        <div class="grid grid-cols-2 gap-x-5 gap-y-9 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
          <div
            v-for="index in 10"
            :key="index"
            class="min-w-0 animate-pulse"
          >
            <div class="aspect-[2/3] rounded-[4px] border border-[var(--border-color)] bg-[var(--bg-input)]" />
            <div class="mt-4 h-4 w-4/5 rounded-md bg-[var(--bg-input)]" />
            <div class="mt-2 h-3 w-1/2 rounded-md bg-[var(--bg-input)]" />
            <div class="mt-3 h-3 w-2/3 rounded-md bg-[var(--bg-input)]" />
          </div>
        </div>
      </section>
      ...
      <section
        v-else-if="loadError"
        class="flex min-h-[240px] flex-col items-center justify-center rounded-lg border border-[var(--status-danger-border)] bg-[var(--status-danger-bg)] px-6 py-10 text-center"
        role="alert"
      >
        <span class="i-lucide-cloud-alert h-7 w-7 text-[var(--status-danger)]" />
        <h2 class="mt-4 text-base font-semibold text-[var(--text-main)]">
          {{ t("ide.picker.loadFailed") }}
        </h2>
        <p class="mt-2 max-w-[560px] break-words text-sm leading-6 text-[var(--text-secondary)]">
          {{ loadError }}
        </p>
        <button
          type="button"
          class="mt-5 inline-flex h-9 items-center justify-center gap-2 rounded-md border border-[var(--status-danger-border)] bg-[var(--bg-panel)] px-4 text-sm font-medium text-[var(--status-danger)] transition-colors hover:bg-[var(--bg-hover)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--status-danger)]"
          @click="void loadProjects()"
        >
      ...
      <section
        ...
        class="flex min-h-[300px] flex-col items-center justify-center rounded-lg border border-dashed border-[var(--border-color)] bg-[var(--bg-subtle)] px-6 py-10 text-center"
      >
        <div class="flex h-12 w-12 items-center justify-center rounded-lg border border-[var(--border-color)] bg-[var(--bg-panel)] text-[var(--accent-text)]">
          <span class="i-lucide-book-open-text h-6 w-6" />
        </div>
        <h2 class="mt-4 text-base font-semibold text-[var(--text-main)]">
          {{ t("ide.picker.emptyTitle") }}
        </h2>
        <p class="mt-2 text-sm leading-6 text-[var(--text-secondary)]">
          {{ t("ide.picker.empty") }}
        </p>
        <button
          type="button"
          class="mt-5 inline-flex h-10 items-center justify-center gap-2 rounded-md bg-[var(--accent-main)] px-4 text-sm font-medium text-[var(--text-inverse)] transition-opacity hover:opacity-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent-main)] focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--bg-main)]"
          @click="void openCreateForm()"
        >
          <span class="i-lucide-book-plus h-4 w-4" />
          {{ t("ide.bookshelf.createBook") }}
        ...
        <div class="grid grid-cols-2 gap-x-5 gap-y-9 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
          ...
          >
            <button
              ...
              :aria-label="t('ide.picker.openProject', { title: novel.title })"
              @click="emit('open', novel.projectRoot)"
            >
              ...
              <span class="project-cover relative block aspect-[2/3] overflow-hidden rounded-[4px] border border-[var(--border-color)] bg-[var(--bg-panel)] transition-transform duration-200 group-hover:-translate-y-1 group-focus-within:-translate-y-1">
                <img
                  v-if="novel.cover && !failedCoverRoots.has(novel.projectRoot)"
                  :key="`${novel.projectRoot}:${String(coverRefreshVersions[novel.projectRoot] ?? 0)}`"
                  class="h-full w-full object-cover"
                  :src="projectCoverUrl(novel.projectRoot)"
                  :alt="t('ide.picker.coverAlt', { title: novel.title })"
                  loading="lazy"
                  decoding="async"
                  @error="handleCoverError(novel.projectRoot)"
                >
                <span
                  v-else
                  class="project-cover-fallback absolute inset-0 flex flex-col items-center justify-between overflow-hidden px-4 py-6 text-center"
                >
                  <span class="flex w-full items-center gap-2 text-[10px] font-medium text-[var(--text-muted)]">
                    ...
                    <span class="i-lucide-feather h-3.5 w-3.5 text-[var(--accent-text)]" />
                    <span class="h-px flex-1 bg-[var(--border-color)]" />
                  </span>
                  ...
                  <span class="h-1 w-9 bg-[var(--accent-main)]" />
                </span>
                <span class="project-cover-spine absolute inset-y-0 left-0 w-2 border-r border-[var(--border-color)]" />
                <span class="project-cover-page absolute inset-y-2 right-0 w-1 border-l border-[var(--border-color)]" />
              </span>
              <span
                class="project-shelf-board block h-2"
                aria-hidden="true"
              />
              ...
              <span class="mt-3 block min-w-0">
                ...
                >{{ novel.summary }}</span>
                <span
                  v-if="formatDate(novel.manifestUpdatedAt)"
                  class="mt-2 flex min-w-0 items-center gap-1.5 text-[11px] text-[var(--text-muted)]"
                  :title="t('ide.picker.manifestUpdatedAtTitle', { time: formatDate(novel.manifestUpdatedAt) })"
                >
                  <span class="i-lucide-clock-3 h-3.5 w-3.5 shrink-0" />
            ...
            <div class="project-card-actions absolute right-2 top-2 z-10 flex gap-1.5 transition-opacity sm:opacity-0 sm:group-hover:opacity-100 sm:group-focus-within:opacity-100">
              <button
                type="button"
                class="flex h-9 w-9 items-center justify-center rounded-md border border-[var(--border-color)] bg-[var(--bg-panel)] text-[var(--text-muted)] transition-colors hover:border-[var(--border-accent)] hover:bg-[var(--bg-hover)] hover:text-[var(--accent-text)] focus-visible:border-[var(--border-accent)] focus-visible:text-[var(--accent-text)] focus-visible:outline-none"
                :title="t('ide.picker.setCover')"
                :aria-label="t('ide.picker.setCover')"
                @click="openCoverDialog(novel)"
              >
                <span class="i-lucide-image-plus h-4 w-4" />
              </button>
              <button
                type="button"
                class="flex h-9 w-9 items-center justify-center rounded-md border border-[var(--border-color)] bg-[var(--bg-panel)] text-[var(--text-muted)] transition-colors hover:border-[var(--status-danger-border)] hover:bg-[var(--status-danger-bg)] hover:text-[var(--status-danger)] focus-visible:border-[var(--status-danger-border)] focus-visible:bg-[var(--status-danger-bg)] focus-visible:text-[var(--status-danger)] focus-visible:outline-none disabled:opacity-50"
                :title="t('ide.bookshelf.deleteBook')"
                :aria-label="t('ide.bookshelf.deleteBook')"
                :disabled="deleteBusyRoots.has(novel.projectRoot) || Boolean(deleteRecoveryFor(novel.projectRoot))"
                @click="void handleDeleteNovel(novel.projectRoot, novel.title)"
              >
                <span
                  :class="deleteBusyRoots.has(novel.projectRoot) ? 'i-lucide-loader-circle animate-spin' : 'i-lucide-trash-2'"
                  class="h-4 w-4"
                />
              </button>
            </div>
            <div
              v-if="deleteRecoveryFor(novel.projectRoot)"
              class="mt-3 space-y-2 rounded-md border border-[var(--status-danger-border)] bg-[var(--status-danger-bg)] px-3 py-2 text-xs text-[var(--status-danger)]"
              role="alert"
            >
              <p>{{ deleteRecoveryFor(novel.projectRoot)?.error || t("ide.picker.deleteRecoveryRequired") }}</p>
              <button
                type="button"
                class="inline-flex h-7 items-center gap-1.5 rounded-md border border-[var(--status-danger-border)] bg-[var(--bg-panel)] px-2.5 font-medium disabled:opacity-50"
                :disabled="deleteBusyRoots.has(novel.projectRoot)"
                @click="void retryDeleteRecovery(novel.projectRoot)"
              >
                <span
                  class="i-lucide-refresh-cw h-3.5 w-3.5"
                  :class="deleteBusyRoots.has(novel.projectRoot) ? 'animate-spin' : ''"
                />{{ t("ide.picker.mutationRecoveryRetry") }}
              </button>
            </div>
          </article>
      ...
      <section class="rounded-lg border border-[var(--border-color)] bg-[var(--bg-panel)]">
        <button
          type="button"
          class="flex w-full items-center justify-between gap-4 px-4 py-3 text-left sm:px-5"
          :aria-expanded="recoveryExpanded"
          @click="void toggleRecovery()"
        >
          <span class="min-w-0">
            <span class="flex items-center gap-2 text-sm font-semibold text-[var(--text-main)]">
              <span class="i-lucide-message-square-warning h-4 w-4 text-[var(--status-warning)]" />
              ...
              >{{ t("ide.picker.recoveryCount", { count: recoveryTotal }) }}</span>
            </span>
            <span class="mt-1 block text-xs leading-5 text-[var(--text-secondary)]">{{ t("ide.picker.recoverySummary") }}</span>
          </span>
          <span
            class="i-lucide-chevron-down h-4 w-4 shrink-0 text-[var(--text-muted)] transition-transform"
            :class="recoveryExpanded ? 'rotate-180' : ''"
          />
        </button>

        <div
          v-if="recoveryExpanded"
          class="border-t border-[var(--border-color)] px-4 py-4 sm:px-5"
        >
          <div
            v-if="recoveryLoading && !recoveryLoaded"
            class="flex items-center justify-center gap-2 py-8 text-sm text-[var(--text-muted)]"
            role="status"
          >
          ...
          <div
            ...
            class="rounded-md border border-[var(--status-danger-border)] bg-[var(--status-danger-bg)] px-3 py-3 text-sm text-[var(--status-danger)]"
          >
            <p>{{ recoveryError }}</p>
            <button
              type="button"
              class="mt-3 rounded-md border border-[var(--status-danger-border)] bg-[var(--bg-panel)] px-3 py-1.5 text-xs"
              @click="void loadRecoverySessions(0, false)"
            >
              {{ t("ide.picker.recoveryRetry") }}
            </button>
          </div>
          <div
            v-else-if="recoverySessions.length === 0"
          ...
          >
            <article
              v-for="session in recoverySessions"
              :key="session.sessionId"
              class="rounded-md border border-[var(--border-color)] bg-[var(--bg-main)] p-3"
            >
              <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                ...
                <div class="flex min-w-0 flex-col gap-2 sm:flex-row sm:items-center">
                  <select
                    v-model="recoveryTargets[session.sessionId]"
                    class="h-9 min-w-0 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-3 text-xs text-[var(--text-main)] sm:w-[240px]"
                    :disabled="recoveryActionId === session.sessionId"
                  >
                    <option value="">
                      {{ t("ide.picker.recoveryProject") }}
                    </option>
                    <option
                      v-for="novel in novels"
                      :key="novel.projectRoot"
                  ...
                  <button
                    type="button"
                    class="h-9 rounded-md bg-[var(--accent-main)] px-3 text-xs font-medium text-[var(--text-inverse)] disabled:opacity-50"
                    :disabled="recoveryActionId === session.sessionId || !recoveryTargets[session.sessionId]"
                    @click="void recoverSession(session, false)"
                  >
                    {{ t("ide.picker.recoveryConfirm") }}
                  </button>
                  <button
                    type="button"
                    class="h-9 rounded-md border border-[var(--border-color)] bg-[var(--bg-panel)] px-3 text-xs text-[var(--text-secondary)] hover:bg-[var(--bg-hover)] disabled:opacity-50"
                    :disabled="recoveryActionId === session.sessionId"
                    @click="void recoverSession(session, true)"
                  >
            ...
            <button
              v-if="recoveryHasMore"
              type="button"
              class="mx-auto flex h-9 items-center gap-2 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-4 text-xs text-[var(--text-secondary)] disabled:opacity-50"
              :disabled="recoveryLoading"
              @click="void loadRecoverySessions(recoveryOffset, true)"
            >
              <span
    ...
    <Dialog
      :model-value="coverDialogOpen"
      size="md"
      :title="t('ide.picker.coverDialogTitle')"
      :busy="coverBusy"
      :show-footer="false"
      overlay-type="opaque"
      @request-close="closeCoverDialog"
      @update:model-value="updateCoverDialogOpen"
    >
      <div
        v-if="coverDialogProject"
        class="space-y-4"
      >
        <button
          v-if="coverPreviewUrl || coverDialogProject.cover"
          type="button"
          class="mx-auto block w-full max-w-[240px] focus-visible:outline-none"
          :aria-label="t('ide.imagePreview.openOriginal')"
          @click="previewCoverOriginal"
        >
          <span class="relative block aspect-[2/3] overflow-hidden rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] focus-visible:ring-2 focus-visible:ring-[var(--accent-main)]">
            <img
              :src="coverPreviewUrl || projectCoverUrl(coverDialogProject.projectRoot)"
              :alt="t('ide.picker.coverAlt', { title: coverDialogProject.title })"
              class="h-full w-full object-cover"
              decoding="async"
            >
        ...
        <div
          v-else
          class="mx-auto flex aspect-[2/3] w-full max-w-[240px] flex-col items-center justify-center gap-3 rounded-md border border-dashed border-[var(--border-color)] bg-[var(--bg-subtle)] text-[var(--text-muted)]"
        >
        ...
        <input
          ...
          :disabled="coverBusy || coverNeedsRefresh"
          @change="selectCoverFile"
        >
        <div class="flex flex-wrap items-center justify-center gap-2">
          <button
            type="button"
            class="inline-flex h-9 items-center justify-center gap-2 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-3 text-sm text-[var(--text-main)] hover:bg-[var(--bg-hover)] disabled:opacity-50"
            :disabled="coverBusy || coverNeedsRefresh"
            @click="coverInput?.click()"
          >
            <span class="i-lucide-folder-open h-4 w-4" />
            {{ coverDialogProject.cover ? t("ide.picker.replaceCover") : t("ide.picker.chooseCover") }}
          </button>
          <button
            v-if="coverDialogProject.cover"
            type="button"
            class="inline-flex h-9 items-center justify-center gap-2 rounded-md border border-[var(--status-danger-border)] bg-[var(--status-danger-bg)] px-3 text-sm text-[var(--status-danger)] hover:opacity-85 disabled:opacity-50"
            :disabled="coverBusy || coverNeedsRefresh"
            @click="void clearCover()"
          >
        ...
        <div
          v-if="coverRecoveryNotice"
          class="rounded-md border border-[var(--status-warning-border)] bg-[var(--status-warning-bg)] px-3 py-2 text-sm text-[var(--status-warning)]"
          role="status"
        >
          {{ coverRecoveryNotice }}
        </div>
        <div
          v-if="coverNeedsRefresh"
          class="space-y-2 rounded-md border border-[var(--status-danger-border)] bg-[var(--status-danger-bg)] px-3 py-2 text-sm text-[var(--status-danger)]"
          role="alert"
        >
          ...
          <button
            ...
            class="inline-flex h-8 items-center justify-center gap-1.5 rounded-md border border-[var(--status-danger-border)] bg-[var(--bg-panel)] px-3 text-xs font-medium hover:bg-[var(--bg-hover)] disabled:opacity-50"
            :disabled="coverBusy"
            @click="void refreshCoverMutationState(coverDialogProject.projectRoot)"
          >
            <span
              class="i-lucide-refresh-cw h-3.5 w-3.5"
              :class="coverBusy ? 'animate-spin' : ''"
            />{{ t("ide.picker.coverRecoveryRetry") }}
          </button>
        </div>
        <div
          v-if="coverError"
          class="rounded-md border border-[var(--status-danger-border)] bg-[var(--status-danger-bg)] px-3 py-2 text-sm text-[var(--status-danger)]"
          role="alert"
        >
          {{ coverError }}
        </div>
        ...
        <div class="flex items-center justify-end gap-2 border-t border-[var(--border-color)] pt-4">
          <button
            type="button"
            class="inline-flex h-9 items-center justify-center rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-4 text-sm text-[var(--text-main)] hover:bg-[var(--bg-hover)] disabled:opacity-50"
            :disabled="coverBusy"
          ...
          <button
            type="button"
            class="inline-flex h-9 items-center justify-center gap-2 rounded-md bg-[var(--accent-main)] px-4 text-sm font-medium text-[var(--text-inverse)] hover:opacity-90 disabled:opacity-50"
            :disabled="coverBusy || coverNeedsRefresh || !coverFile"
            @click="void uploadCover()"
          >
            <span
              v-if="coverBusy"
              class="i-lucide-loader-circle h-4 w-4 animate-spin"
