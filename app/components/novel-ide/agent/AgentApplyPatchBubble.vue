...
<template>
  <div class="mt-2 space-y-3">
    ...
    <div class="flex flex-wrap items-center gap-2">
      <span
        v-for="filePath in touchedFiles"
        :key="filePath"
        class="rounded border border-[var(--accent-main)]/30 bg-[var(--bg-main)] px-2 py-1 font-mono text-[11px] text-[var(--accent-main)]"
      >
        <span class="mr-1 inline-block h-3 w-3 align-text-bottom i-lucide-file-diff" />
        {{ filePath }}
      </span>
      <span
        v-if="touchedFiles.length === 0"
        class="rounded border border-[var(--accent-main)]/30 bg-[var(--bg-main)] px-2 py-1 font-mono text-[11px] text-[var(--accent-main)]"
      >{{ t("agent.tool.resolvingPath") }}</span>
    </div>

    <!-- Patch Preview -->
    <div class="rounded border border-[var(--border-color)] bg-[var(--bg-main)]/60">
      <div class="border-b border-[var(--border-color)]/50 px-2 py-1 text-[10px] uppercase tracking-[0.24em] text-[var(--text-muted)]">
        Patch
      </div>
      ...
      <div
        v-if="previewNotice"
        class="px-3 pb-3 text-[11px] text-[var(--status-info)]"
      >
        {{ previewNotice }}
      </div>
    </div>
