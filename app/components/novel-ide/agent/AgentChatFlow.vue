...
<template>
  ...
  <div
    ref="scrollRef"
    class="flex flex-1 flex-col overflow-y-auto p-4 pb-12 bg-[var(--bg-panel)]"
    @scroll="onScroll"
  >
    <!-- 更早历史局部状态；失败不会遮断当前已加载对话。 -->
    <div
      v-if="props.historyHasPrevious || props.historyLoading || props.historyError"
      class="mb-4 flex shrink-0 items-center justify-center"
    >
      <button
        type="button"
      ...
      >
        <span
          :class="props.historyLoading ? 'i-lucide-loader-circle animate-spin' : props.historyError ? 'i-lucide-rotate-ccw' : 'i-lucide-chevron-up'"
          class="h-3.5 w-3.5"
        />
      ...
      <span
        v-if="props.historyError && !props.historyLoading"
        class="ml-2 max-w-[360px] truncate text-xs text-[var(--status-danger)]"
        :title="props.historyError"
      >{{ props.historyError }}</span>
    </div>
    ...
    <div
      v-else
      class="flex h-full flex-col items-center justify-center space-y-6 px-4 text-center"
    >
      <!-- main 模式空状态 -->
      <template v-if="props.mode === 'main'">
        <div class="flex h-12 w-12 items-center justify-center rounded-2xl border border-[var(--border-color)] bg-[var(--bg-input)] shadow-sm">
          <span class="i-lucide-bot h-6 w-6 text-[var(--text-muted)]" />
        </div>
        <div class="space-y-2">
          <h3 class="text-base font-medium text-[var(--text-main)]">
            {{ t("agent.chat.startTitle") }}
          </h3>
          <p class="text-sm leading-relaxed text-[var(--text-muted)]">
            {{ t("agent.chat.startDescription") }}
          </p>
        </div>
      </template>
      <!-- compact 模式空状态 -->
