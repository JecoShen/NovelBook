...
<template>
  ...
  <div class="relative shrink-0 bg-[var(--bg-panel)] px-2 pb-1">
    <!-- pending 引导/队列 -->
    <div
      v-if="!hasPendingUserInput && props.queuedMessages.length > 0"
      class="flex min-w-0 flex-wrap gap-1 px-1 pb-1.5"
    >
      <div
      ...
      >
        <span
          :class="queuedMessageIcon(item)"
          class="h-3 w-3 shrink-0 text-[var(--accent-text)]"
        />
    ...
    <AgentWorkspaceChanges
      ...
      :refresh-key="props.historyInboxRefreshKey"
      :active="props.historyInboxActive"
      @open-full="emit('open-history-inbox')"
    ...
    >
      ...
      >
        <span
          :class="availabilityBannerView.icon"
        ...
        >
          <span
            :class="availabilityBannerView.actionIcon"
            class="h-3 w-3"
          />
      ...
      <div
        ...
        class="flex min-w-0 gap-1.5 overflow-x-auto border-b border-[var(--border-color)]/50 px-2 py-1.5"
      >
        <div
        ...
        >
          <img
            v-if="composerImageUrl(image.target)"
            :src="composerImageUrl(image.target) || undefined"
            :alt="image.label"
      ...
      <div
        v-if="images.metadataError.value"
      ...
      >
        <span class="i-lucide-image-off h-3.5 w-3.5 shrink-0" />
        <span
          class="min-w-0 flex-1 truncate"
          :title="images.metadataError.value"
        >{{ images.metadataError.value }}</span>
        <button
          type="button"
    ...
    <div class="mt-1.5 flex flex-wrap items-center justify-center gap-1 text-[9px] text-[var(--text-muted)]">
      ...
      <button
        ...
        class="inline-flex max-w-full items-center gap-1 rounded-full border border-[var(--border-color)] bg-[var(--bg-input)] px-1.5 py-0.5 transition-colors hover:bg-[var(--bg-hover)]"
        @click="emit('open-context-inspector')"
      >
        <span class="i-lucide-gauge h-3 w-3 shrink-0" />
        ...
        <span
          ...
          class="rounded-full bg-[var(--accent-bg)] px-1 py-[1px] text-[8px] font-semibold text-[var(--accent-text)]"
        >{{ props.contextPercentCompactLabel }}</span>
      </button>
      <div
        :title="props.cumulativeUsageExactLabel"
        class="inline-flex items-center gap-1 rounded-full border border-[var(--border-color)] bg-[var(--bg-input)] px-1.5 py-0.5"
      >
      ...
      <div
        ...
        class="inline-flex items-center gap-1 rounded-full border border-[var(--border-color)] bg-[var(--bg-input)] px-1.5 py-0.5"
      >
        <span class="i-lucide-wifi h-3 w-3" />
        <span>{{ props.connectionStatusLabel }}</span>
      </div>
      <template v-if="props.connectionNeedsAction">
        <button
          class="inline-flex items-center gap-1 rounded-full border border-[var(--border-color)] bg-[var(--bg-input)] px-1.5 py-0.5 text-[var(--text-secondary)] transition-colors hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)]"
        ...
        >
          <span class="i-lucide-history h-3 w-3" />
