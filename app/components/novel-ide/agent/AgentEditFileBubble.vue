  ...
  <div class="mt-2 space-y-3">
    ...
    <div class="flex items-center gap-2">
      <span class="rounded bg-[var(--bg-main)] px-2 py-1 font-mono text-[11px] text-[var(--accent-main)] border border-[var(--accent-main)]/30">
        <span class="i-lucide-file-edit h-3 w-3 mr-1 inline-block align-text-bottom" />
        {{ filePathText || t("agent.tool.resolvingPath") }}
      </span>
      <span
        v-if="(parsedArgs.edits?.length ?? 0) > 1"
        class="rounded border border-[var(--border-color)] bg-[var(--bg-panel)] px-2 py-1 font-mono text-[10px] text-[var(--text-muted)]"
      >{{ parsedArgs.edits?.length }} edits</span>
    </div>

    <!-- Diff 预览：old/new 都允许在半截 JSON 阶段逐步增长 -->
    <div class="grid grid-cols-2 gap-2 mt-2">
      <div class="rounded border border-[var(--border-color)] bg-[var(--status-danger-bg)]">
        <div class="border-b border-[var(--border-color)]/50 px-2 py-1 text-[10px] uppercase text-[var(--status-danger)]">
          Old String
        </div>
      ...
      <div class="rounded border border-[var(--border-color)] bg-[var(--status-success-bg)]">
        <div class="border-b border-[var(--border-color)]/50 px-2 py-1 text-[10px] uppercase text-[var(--status-success)]">
          New String
        </div>
    ...
    <div
      v-if="previewNotice"
      class="text-[11px] text-[var(--status-info)]"
    >
      {{ previewNotice }}
    </div>

    <div
      v-if="diffDetails?.diffPreview"
      class="rounded border border-[var(--border-color)] bg-[var(--bg-panel)]"
    >
      <div class="border-b border-[var(--border-color)]/50 px-2 py-1 text-[10px] uppercase text-[var(--text-muted)]">
        Diff Preview
      </div>
      ...
      <div
        v-if="diffDetails.diffOmitted"
        class="px-2 pb-2 text-[11px] text-[var(--status-info)]"
      >
        仅显示预览 · 原 diff {{ formatByteCount(diffDetails.diffBytes) }}
      </div>
    </div>

    <div
      v-if="props.toolCall.error"
      class="mt-2 break-all whitespace-pre-wrap rounded border border-[var(--status-danger-border)] bg-[var(--status-danger-bg)] p-2 font-mono text-xs text-[var(--status-danger)]"
    >
    ...
    <div
      v-if="resultText"
      class="whitespace-pre-wrap rounded border border-[var(--border-color)] bg-[var(--bg-panel)] p-2 font-mono text-xs leading-5 text-[var(--text-secondary)]"
    >
      {{ resultText }}
    </div>

    <div
      v-if="props.toolCall.status === 'success'"
      class="mt-2 flex items-center gap-1.5 text-[11px] font-medium text-[var(--status-success)]"
    >
