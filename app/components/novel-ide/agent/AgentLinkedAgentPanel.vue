  ...
  <div class="sticky top-0 z-10 flex flex-col gap-3 border-b border-[var(--border-color)] bg-[var(--bg-main)] p-4 shadow-sm">
    ...
    <div class="flex items-center justify-between">
      <span class="flex items-center gap-2 text-xs font-semibold tracking-wider text-[var(--accent-text)]">
        <span class="i-lucide-boxes h-4 w-4" />
        {{ t("agent.linkedAgents.title") }}
        <span
          v-if="totalRelations"
          class="rounded-sm bg-[var(--accent-main)] px-1.5 py-0.5 text-[9px] font-bold text-[var(--text-inverse)]"
        >{{ totalRelations }}</span>
      </span>
      <div class="flex shrink-0 gap-1.5">
        ...
        <button
          class="ml-1 flex h-6 w-6 items-center justify-center rounded text-[var(--text-muted)] transition-colors hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)]"
          @click="emit('close')"
        >
    ...
    <div
      ...
      class="max-h-[260px] space-y-3 overflow-y-auto pr-1 custom-scrollbar"
    >
      ...
      <section>
        ...
        <div
          v-if="props.ownedAgents.length > 0"
          class="space-y-2"
        >
          <button
            v-for="item in props.ownedAgents"
            :key="`owned-${item.sessionId}`"
            type="button"
            class="group flex w-full cursor-pointer items-center gap-3 rounded-lg border border-[var(--border-color)] bg-[var(--bg-panel)] px-3 py-2 text-left transition-all hover:border-[var(--accent-main)] hover:bg-[var(--bg-hover)] hover:shadow-sm"
          ...
          >
            <div class="flex h-7 w-7 shrink-0 items-center justify-center rounded border border-[var(--border-color)] bg-[var(--bg-input)] transition-colors group-hover:border-[var(--accent-main)] group-hover:bg-[var(--accent-bg)]">
              <span class="i-lucide-arrow-up-right h-4 w-4 text-[var(--text-muted)] transition-colors group-hover:text-[var(--accent-main)]" />
            </div>
            <div class="flex min-w-0 flex-1 flex-col justify-center">
              <div class="flex items-center gap-2">
                ...
                <span
                  v-if="profileAvailabilityLabel(item)"
                  class="inline-flex shrink-0 items-center gap-1 rounded border border-[var(--status-warning-border)] bg-[var(--status-warning-bg)] px-1.5 py-0.5 text-[10px] text-[var(--status-warning)]"
                  :title="profileAvailabilityTitle(item)"
                >
                  <span class="i-lucide-lock h-3 w-3" />
                  {{ profileAvailabilityLabel(item) }}
                </span>
              </div>
              <div class="mt-0.5 truncate font-mono text-[10px] text-[var(--text-muted)] opacity-70">
        ...
        <div
          v-else
          class="rounded-lg border border-dashed border-[var(--border-color)] bg-[var(--bg-panel)] py-4 text-center text-[11px] text-[var(--text-muted)]"
        >
      ...
      <section>
        ...
        <div
          v-if="props.linkedByAgents.length > 0"
          class="space-y-2"
        >
          <button
            v-for="item in props.linkedByAgents"
            :key="`owner-${item.sessionId}`"
            type="button"
            class="group flex w-full cursor-pointer items-center gap-3 rounded-lg border border-[var(--border-color)] bg-[var(--bg-panel)] px-3 py-2 text-left transition-all hover:border-[var(--accent-main)] hover:bg-[var(--bg-hover)] hover:shadow-sm"
            @click="emit('select', item.sessionId)"
          >
            <div class="flex h-7 w-7 shrink-0 items-center justify-center rounded border border-[var(--border-color)] bg-[var(--bg-input)] transition-colors group-hover:border-[var(--accent-main)] group-hover:bg-[var(--accent-bg)]">
              <span class="i-lucide-arrow-down-left h-4 w-4 text-[var(--text-muted)] transition-colors group-hover:text-[var(--accent-main)]" />
            </div>
            <div class="flex min-w-0 flex-1 flex-col justify-center">
              <div class="flex items-center gap-2">
                <span class="truncate text-xs font-medium text-[var(--text-main)] transition-colors group-hover:text-[var(--accent-main)]">{{ item.title || `Agent #${item.sessionId}` }}</span>
    ...
    <div
      v-else
      class="rounded-lg border border-dashed border-[var(--border-color)] bg-[var(--bg-panel)] py-8 text-center text-xs text-[var(--text-muted)]"
    >
