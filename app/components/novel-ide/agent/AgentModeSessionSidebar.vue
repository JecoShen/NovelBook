...
<template>
  ...
  >
    ...
    <div class="flex shrink-0 items-center justify-between border-b border-[var(--border-color)] px-3 py-3">
      <div class="min-w-0">
        <div class="text-sm font-semibold text-[var(--text-main)]">
          {{ t("agent.session.sidebarTitle") }}
        </div>
        <div class="text-[11px] text-[var(--text-muted)]">
          {{ t("agent.session.currentProjectWorkspace") }}
        </div>
      </div>
      <div class="flex shrink-0 items-center gap-1">
        <button
          type="button"
          class="flex h-8 w-8 items-center justify-center rounded-md text-[var(--text-muted)] hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)] disabled:opacity-40"
        ...
        <button
          type="button"
          class="flex h-8 w-8 items-center justify-center rounded-md bg-[var(--accent-bg)] text-[var(--accent-text)] hover:opacity-80 disabled:opacity-40"
    ...
    <div class="shrink-0 border-b border-[var(--border-color)] p-3">
      <div class="flex h-9 items-center gap-2 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-2.5">
        ...
        <input
          v-model="searchQuery"
          type="text"
          :placeholder="t('agent.session.searchPlaceholder')"
    ...
    <div
      class="contain-layout-paint min-h-0 flex-1 overflow-y-auto p-2 custom-scrollbar"
    ...
    >
      ...
      >
        <span
          :class="pinnedSet.has(session.sessionId) ? 'i-lucide-pin text-[var(--accent-text)]' : 'i-lucide-message-square text-[var(--text-muted)]'"
        ...
        <span class="min-w-0 flex-1">
          <span class="flex min-w-0 items-center gap-1.5">
            ...
            <span
              ...
              class="inline-flex shrink-0 items-center gap-1 rounded border border-[var(--status-warning-border)] bg-[var(--status-warning-bg)] px-1 py-0.5 text-[9px] text-[var(--status-warning)]"
        ...
        <span class="flex shrink-0 flex-col gap-1 opacity-0 transition-opacity group-hover:opacity-100">
          <button
            type="button"
            class="flex h-6 w-6 items-center justify-center rounded text-[var(--text-muted)] hover:bg-[var(--bg-input)] hover:text-[var(--accent-text)]"
            :title="pinnedSet.has(session.sessionId) ? t('agent.session.unpin') : t('agent.session.pin')"
            @click.stop="togglePin(session.sessionId)"
          >
            <span class="i-lucide-pin h-3.5 w-3.5" />
          </button>
          <button
            type="button"
            class="flex h-6 w-6 items-center justify-center rounded text-[var(--text-muted)] hover:bg-[var(--bg-input)] hover:text-[var(--accent-text)] disabled:opacity-40"
          ...
          <button
            type="button"
            class="flex h-6 w-6 items-center justify-center rounded text-[var(--text-muted)] hover:bg-[var(--status-danger-bg)] hover:text-[var(--status-danger)] disabled:opacity-40"
      ...
      <div
        ...
        class="mt-6 rounded-md border border-dashed border-[var(--border-color)] px-3 py-6 text-center text-[12px] text-[var(--text-muted)]"
      >
