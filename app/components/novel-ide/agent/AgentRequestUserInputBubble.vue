...
<template>
  ...
  <div class="mt-2 space-y-2">
    <div class="rounded-lg border border-[var(--border-color)] bg-[var(--bg-main)] p-3">
      <div class="mb-1 text-[9px] uppercase tracking-[0.24em] text-[var(--text-muted)]">
        {{ isToolApproval ? "Approval" : "Question" }}
      </div>
      <div
        v-if="questionText"
        class="text-sm leading-6 text-[var(--text-main)]"
      >
        {{ questionText }}
      </div>
      <div
        v-else
        class="text-xs whitespace-pre-wrap break-all font-mono leading-5 text-[var(--text-secondary)]"
      >
      ...
      <div class="mt-3 space-y-2">
        ...
        <div class="flex flex-wrap gap-1.5 text-[10px] text-[var(--text-muted)]">
          <span
            v-if="questionOptions.length === 0"
            class="rounded border border-[var(--border-color)] bg-[var(--bg-input)] px-1.5 py-0.5"
          >{{ t("agent.userInput.openAnswer") }}</span>
    ...
    >
      <div class="mb-1 text-[9px] uppercase tracking-[0.24em] text-[var(--text-muted)]">
        {{ isToolApproval ? "Decision" : "Answer" }}
      </div>

      <div
        v-if="isPendingQuestion"
        class="flex items-center gap-2 text-xs leading-5 text-[var(--status-warning)]"
      >
        <span class="i-lucide-clock h-3.5 w-3.5 shrink-0" />
        <span>{{ isToolApproval ? t("agent.userInput.waitingApproval") : t("agent.userInput.waitingAnswer") }}</span>
      </div>

      <div
        v-else-if="answerViews.length > 0"
        class="space-y-3"
      >
        <div
        ...
        >
          <div
            v-if="answerViews.length > 1 && answer.question"
            class="text-xs leading-5 text-[var(--text-muted)]"
          >
          ...
          <div
            ...
            class="text-sm leading-6 text-[var(--text-main)]"
          >
            {{ t("agent.userInput.ignored") }}
          </div>
          <div
            v-else-if="answer.openAnswer && answer.text"
            class="text-sm leading-6 text-[var(--text-main)]"
          >
            {{ t("agent.userInput.answerPrefix", { text: answer.text }) }}
          </div>
          <div
            v-else-if="answer.openAnswer && answer.note"
            class="text-sm leading-6 text-[var(--text-main)]"
          >
            {{ t("agent.userInput.answerPrefix", { text: answer.note }) }}
          </div>
          <div
            v-else
            class="text-sm text-[var(--text-main)]"
          >
            {{ t("agent.userInput.choicePrefix", { text: answer.selectedLabel || t("agent.userInput.openAnswer") }) }}
          </div>
          <div
            v-if="answer.note && !answer.openAnswer"
            class="text-xs leading-5 text-[var(--text-muted)]"
          >
            {{ t("agent.userInput.notePrefix", { text: answer.note }) }}
          </div>
          <div
            v-if="answer.omitted"
            class="flex items-center gap-1 text-[11px] text-[var(--status-info)]"
          >
            <span class="i-lucide-info h-3 w-3" /><span>仅显示预览</span>
          </div>
        </div>
      </div>
      ...
      <div
        v-else
        class="text-xs whitespace-pre-wrap break-all text-[var(--text-secondary)]"
      >
