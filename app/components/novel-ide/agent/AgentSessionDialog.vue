  ...
  >
    <template #header>
      <div class="flex min-w-0 flex-1 items-center justify-between gap-3">
        <div class="min-w-0">
          <div class="text-base font-semibold leading-snug tracking-wide text-[var(--text-main)]">
            {{ t("agent.session.title") }}
          </div>
        </div>
        <button
          class="flex h-7 w-7 items-center justify-center rounded-md text-[var(--text-muted)] transition-colors hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)]"
          @click="close"
        >
    ...
    <div class="flex min-h-0 flex-1 flex-col space-y-4 pt-4">
      ...
      <div class="relative flex items-center gap-2">
        <div class="flex h-11 min-w-0 flex-1 items-center gap-2 rounded-lg border border-[var(--border-color)] bg-[var(--bg-input)] px-3">
          ...
          <input
            ...
            type="text"
            :placeholder="t('agent.session.dialogSearchPlaceholder')"
          ...
          >
            <span class="i-lucide-list-filter h-4 w-4" />
          </button>
        </div>
        <Dropdown
          ...
          @select="emit('create', $event)"
        >
          <button
            class="inline-flex h-11 items-center justify-center gap-1.5 rounded-lg bg-[var(--accent-bg)] px-4 text-sm text-[var(--accent-text)] transition-opacity hover:opacity-80 disabled:opacity-40"
        ...
        <transition name="fade">
          <div
            ...
            ref="filterPanelRef"
            class="absolute left-0 top-[calc(100%+8px)] z-40 w-[240px] rounded-lg border border-[var(--border-color)] bg-[var(--bg-panel)] p-2.5 shadow-2xl"
          >
            <div class="flex items-center justify-between">
              <div class="text-[12px] font-semibold text-[var(--text-main)]">
                {{ t("agent.session.filter") }}
              </div>
              <button
                type="button"
                class="inline-flex h-6 w-6 items-center justify-center rounded-md text-[var(--text-muted)] hover:bg-[var(--bg-hover)]"
            ...
            <div class="mt-2.5 space-y-2.5">
              <section>
                <div class="mb-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-[var(--text-muted)]">
                  {{ t("agent.session.profile") }}
                </div>
                <div class="grid grid-cols-2 gap-1.5">
                  <button
                    ...
                    :class="profileFilter === item.value ? 'border-[var(--accent-main)] bg-[var(--accent-bg)] text-[var(--accent-text)]' : 'border-[var(--border-color)] bg-[var(--bg-input)] text-[var(--text-secondary)] hover:bg-[var(--bg-hover)]'"
                    @click="profileFilter = item.value"
                  >
                    {{ item.label }}
                  </button>
                </div>
              ...
              <section>
                ...
                <div class="grid grid-cols-2 gap-1.5">
                  <button
                    ...
                    type="button"
                    class="rounded-md border px-2 py-1.5 text-[11px] transition-colors"
                    :class="statusFilter === item.value ? 'border-[var(--accent-main)] bg-[var(--accent-bg)] text-[var(--accent-text)]' : 'border-[var(--border-color)] bg-[var(--bg-input)] text-[var(--text-secondary)] hover:bg-[var(--bg-hover)]'"
              ...
              <section>
                ...
                <div class="grid grid-cols-3 gap-1.5">
                  <button
                    ...
                    type="button"
                    class="rounded-md border px-2 py-1.5 text-[11px] transition-colors"
                    :class="relationFilter === item.value ? 'border-[var(--accent-main)] bg-[var(--accent-bg)] text-[var(--accent-text)]' : 'border-[var(--border-color)] bg-[var(--bg-input)] text-[var(--text-secondary)] hover:bg-[var(--bg-hover)]'"
                    @click="relationFilter = item.value"
                  >
                    {{ item.label }}
                  </button>
                </div>
              ...
              <div class="grid grid-cols-2 gap-2 text-[11px] text-[var(--text-secondary)]">
                <button
                  type="button"
                  class="rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-2 py-1.5 hover:bg-[var(--bg-hover)]"
                  @click="resetFilters"
                >
      ...
      <div class="min-h-0 flex-1 space-y-2 overflow-y-auto px-1.5 py-1 custom-scrollbar">
          ...
          <div class="min-w-0 flex-1">
            <div class="flex min-w-0 items-center gap-3">
              <div
                ...
                :class="session.parentSessionId ? 'border-[var(--status-info-border)] bg-[var(--status-info-bg)] text-[var(--status-info)]' : 'border-[var(--status-warning-border)] bg-[var(--status-warning-bg)] text-[var(--status-warning)]'"
              >
                <span
                  :class="session.parentSessionId ? 'i-lucide-bot' : 'i-lucide-crown'"
                  class="h-4.5 w-4.5"
                />
              </div>
              ...
              <span
                ...
                class="rounded bg-[var(--accent-main)] px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-[0.1em] text-[var(--text-inverse)] shadow-sm"
              >{{ t("agent.session.active") }}</span>
              <span
                class="rounded px-1.5 py-0.5 text-[10px] font-medium"
                :class="statusClass(session.status)"
              >{{ statusLabel(session.status) }}</span>
              <span
                v-if="profileAvailabilityLabel(session)"
                class="inline-flex shrink-0 items-center gap-1 rounded border border-[var(--status-warning-border)] bg-[var(--status-warning-bg)] px-1.5 py-0.5 text-[10px] font-medium text-[var(--status-warning)]"
                :title="profileAvailabilityTitle(session)"
              >
                <span class="i-lucide-lock h-3 w-3" />
                {{ profileAvailabilityLabel(session) }}
              </span>
            </div>
            <div class="mt-2 line-clamp-2 text-xs leading-relaxed text-[var(--text-secondary)]">
            ...
            <div class="mt-2.5 flex flex-wrap items-center gap-2 text-[10px] uppercase tracking-[0.16em] text-[var(--text-muted)]">
              ...
              <span
                ...
                class="rounded border border-[var(--border-color)] px-1.5 py-0.5 text-[10px] font-medium tracking-normal text-[var(--text-secondary)]"
              >{{ t("agent.session.parentSession", { id: session.parentSessionId }) }}</span>
            </div>
          </div>
          <div class="flex shrink-0 flex-col gap-1">
            <button
              class="flex h-8 w-8 items-center justify-center rounded-lg text-[var(--text-muted)] opacity-50 transition-all hover:bg-[var(--bg-input)] hover:text-[var(--accent-text)] hover:opacity-100 group-hover:opacity-100 disabled:opacity-40"
            ...
            <button
              ...
              class="flex h-8 w-8 items-center justify-center rounded-lg text-[var(--text-muted)] opacity-50 transition-all hover:bg-[var(--status-success-bg)] hover:text-[var(--status-success)] hover:opacity-100 group-hover:opacity-100 disabled:opacity-40"
              :disabled="actionId === session.sessionId || loading"
              :title="t('agent.session.restore')"
              @click.stop="emit('restore', session)"
            >
              <span
                v-if="actionId === session.sessionId"
                class="i-lucide-loader-circle h-4 w-4 animate-spin"
              />
              <span
                v-else
                class="i-lucide-archive-restore h-4 w-4"
              />
            </button>
            <button
              v-else
              class="flex h-8 w-8 items-center justify-center rounded-lg text-[var(--text-muted)] opacity-50 transition-all hover:bg-[var(--status-danger-bg)] hover:text-[var(--status-danger)] hover:opacity-100 group-hover:opacity-100 disabled:opacity-40"
            ...
            >
              ...
              <span
                v-else
                class="i-lucide-archive h-4 w-4"
              />
            </button>
          </div>
        </div>
        ...
        >
          <button
            type="button"
            class="inline-flex h-8 items-center justify-center gap-1.5 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-3 text-xs font-medium text-[var(--text-secondary)] hover:bg-[var(--bg-hover)] disabled:opacity-50"
          ...
          >
            ...
            <span
              v-else
              class="i-lucide-list-plus h-3.5 w-3.5"
            />
        ...
        <div
          v-if="listedSessions.length === 0"
          class="rounded-lg border border-dashed border-[var(--border-color)] bg-[var(--bg-sidebar)] px-4 py-10 text-center text-sm text-[var(--text-muted)]"
        >
          {{ t("agent.session.noMatching") }}
          <div class="mt-3 flex justify-center gap-2">
            <button
              type="button"
              class="rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-2 py-1 text-xs text-[var(--text-secondary)] hover:bg-[var(--bg-hover)]"
              @click="resetFilters"
            >
              {{ t("agent.session.resetFilters") }}
            </button>
    ...
    <template #footer>
      <div class="flex w-full items-center justify-between">
        <div class="text-[11px] text-[var(--text-muted)]">
          {{ t("agent.session.recentCount", { filtered: listedSessions.length, total: props.total }) }}
        </div>
        <button
          class="inline-flex h-8 cursor-pointer items-center justify-center rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-4 text-[13px] font-medium text-[var(--text-main)] transition-colors duration-200 hover:bg-[var(--bg-hover)] active:scale-95"
          @click="close"
        >
