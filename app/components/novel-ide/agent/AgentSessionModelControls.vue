...
<template>
  ...
  >
    ...
    <div
      v-if="props.sessionModelPopoverOpen"
      class="absolute bottom-full left-0 z-40 mb-2 rounded-xl border border-[var(--border-color)] bg-[var(--bg-panel)] p-3 shadow-2xl"
      :class="props.popoverClass"
    >
      <div class="mb-3 flex items-center justify-between gap-3">
        <div>
          <div class="text-sm font-medium text-[var(--text-main)]">
            {{ t("agent.composer.sessionModelParams") }}
          </div>
          <div class="mt-1 text-[11px] text-[var(--text-muted)]">
            {{ t("agent.composer.sessionModelDescription") }}
          </div>
        </div>
        <button
      ...
      <div class="space-y-3">
        ...
        <div class="space-y-1.5">
          ...
          <select
            :value="props.sessionModelDraft.reasoningEffort ?? ''"
            class="h-8 w-full rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-2.5 text-[12px] text-[var(--text-main)] outline-none transition-colors focus:border-[var(--accent-main)] focus:ring-1 focus:ring-[var(--accent-main)]/20 disabled:cursor-not-allowed disabled:opacity-50"
            :disabled="actionDisabled"
            @change="updateSessionModelDraft({ reasoningEffort: (($event.target as HTMLSelectElement).value || null) as AgentSessionModelDraft['reasoningEffort'] })"
          >
            <option
      ...
      <div class="mt-4 flex items-center justify-between gap-2">
        <button
          class="inline-flex h-8 items-center justify-center rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-3 text-xs font-medium text-[var(--text-secondary)] transition-colors hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)] disabled:cursor-not-allowed disabled:opacity-50"
          :disabled="actionDisabled"
        ...
        <button
          class="inline-flex h-8 items-center justify-center rounded-md bg-[var(--accent-main)] px-3 text-xs font-medium text-[var(--text-inverse)] transition-opacity hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50"
          :disabled="actionDisabled"
          @click="emit('apply-session-model-settings')"
        >
          <span
            v-if="props.sessionModelSaving"
            class="i-lucide-loader-2 mr-1.5 h-3.5 w-3.5 animate-spin"
          />
