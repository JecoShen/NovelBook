  ...
  >
      ...
      <div class="flex min-w-0 flex-1 items-center justify-between gap-4">
        <div class="flex min-w-0 items-center gap-3">
          <span class="flex h-9 w-9 shrink-0 items-center justify-center rounded-md border border-[var(--border-color)] bg-[var(--bg-panel)] text-[var(--accent-text)]">
            <span class="i-lucide-git-branch h-4.5 w-4.5" />
          </span>
          <div class="min-w-0">
            <div class="text-base font-semibold leading-snug text-[var(--text-main)]">
              {{ t("agent.sessionTree.title") }}
            </div>
        ...
        <button
          type="button"
          class="flex h-8 w-8 items-center justify-center rounded-md text-[var(--text-muted)] transition-colors hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)]"
    ...
    <div class="flex min-h-0 flex-1 bg-[var(--bg-main)]">
      ...
      <div class="flex min-w-0 flex-1 flex-col border-r border-[var(--border-color)] bg-[var(--bg-main)]">
        <div class="sticky top-0 z-20 flex flex-wrap items-center gap-2 border-b border-[var(--border-color)] bg-[var(--toolbar-bg)]/95 px-3 py-2 backdrop-blur">
          <div class="flex h-8 min-w-[280px] flex-1 items-center gap-2 rounded-md border border-[var(--border-color)] bg-[var(--bg-panel)] px-2.5 shadow-sm">
            ...
            <input
              ...
              :placeholder="t('agent.sessionTree.searchPlaceholder')"
            >
          </div>
        ...
        >
          <div
            ...
            class="overflow-hidden rounded-md border border-[var(--border-color)] bg-[var(--bg-panel)] shadow-sm"
          >
            <div
            ...
            >
              <span
                class="mr-0.5 flex shrink-0 self-stretch"
                aria-hidden="true"
              >
                <span
                  v-for="(part, guideIndex) in renderedGuideParts(row)"
                  :key="`${row.node.id}-guide-${guideIndex}`"
                  class="relative w-4 shrink-0"
                  :class="guideCellClass(row, part, guideIndex)"
                >
                  <span
                    v-if="guideCellHasTopLine(row, part, guideIndex)"
                    class="absolute left-1/2 top-0 h-1/2 w-px -translate-x-1/2 bg-current opacity-75"
                  />
                  <span
                    v-if="guideCellHasBottomLine(row, part, guideIndex)"
                    class="absolute bottom-0 left-1/2 h-1/2 w-px -translate-x-1/2 bg-current opacity-75"
                  />
                  <span
                    v-if="guideCellHasLeftHorizontalLine(row, part, guideIndex)"
                    class="absolute left-0 right-1/2 top-1/2 h-px bg-current opacity-80"
                  />
                  <span
                    v-if="guideCellHasRightHorizontalLine(row, part, guideIndex)"
                    class="absolute left-1/2 right-0 top-1/2 h-px bg-current opacity-80"
                  />
                  <span
                    v-if="guideCellHasDot(row, part, guideIndex)"
                    class="absolute left-1/2 top-1/2 z-10 h-2.5 w-2.5 -translate-x-1/2 -translate-y-1/2 rounded-full border"
                    :class="nodeDotClass(row.node)"
                  />
                </span>
              </span>
              <button
              ...
              >
                <span
                  :class="row.collapsed ? 'i-lucide-chevron-right' : 'i-lucide-chevron-down'"
                  class="h-3.5 w-3.5"
                />
              </button>
              <span
              ...
              <button
                type="button"
                class="grid min-w-0 flex-1 grid-cols-[16px_auto_minmax(0,1fr)_minmax(46px,auto)_52px] items-center gap-2 rounded px-1.5 py-0.5 text-left transition-colors focus-visible:outline focus-visible:outline-2 focus-visible:outline-[var(--accent-main)]"
                :title="rowTitle(row)"
                @click="selectNode(row.node)"
                @dblclick="activateSelected"
              >
                <span
                  :class="nodeIcon(row.node)"
                  class="h-3.5 w-3.5 shrink-0 text-[var(--text-muted)]"
                />
                <span
                  class="inline-flex h-5 max-w-[80px] shrink-0 items-center justify-center truncate rounded px-1.5 text-[10px] font-medium leading-none"
                  :class="roleToneClass(row.node)"
                  :title="roleTitle(row.node)"
                >{{ roleLabel(row.node) }}</span>
                <span
                  class="min-w-0 truncate text-[13px] leading-5"
                  :class="previewToneClass(row)"
                >{{ nodePreview(row.node) }}</span>
                <span class="flex min-w-[46px] justify-end">
                  <span
                    v-if="row.isBranchPoint"
                    class="inline-flex h-5 shrink-0 items-center gap-1 rounded border border-[var(--border-color)] bg-[var(--bg-input)] px-1.5 text-[10px] font-medium text-[var(--text-secondary)]"
                    :title="branchBadgeTitle(row)"
                  >
                    <span class="i-lucide-git-fork h-3 w-3 text-[var(--text-muted)]" />
                    {{ row.node.childCount }}
                    <span
                      v-if="row.collapsed && row.hiddenDescendantCount > 0"
                      class="text-[var(--accent-text)]"
                    >+{{ row.hiddenDescendantCount }}</span>
                  </span>
                  <span
                    v-else-if="row.branchSiblingCount > 1 && row.branchIndex !== null"
                    class="inline-flex h-5 shrink-0 items-center rounded border border-[var(--border-color)] bg-[var(--bg-input)] px-1.5 text-[10px] font-medium text-[var(--text-muted)]"
                    :title="branchBadgeTitle(row)"
                  >
                ...
                <span
                  class="justify-self-end truncate whitespace-nowrap font-mono text-[10px] leading-4 text-[var(--text-muted)] opacity-60"
                  :title="formatTimestamp(row.node.timestamp)"
                >{{ formatTimestamp(row.node.timestamp).split(' ')[1] || formatTimestamp(row.node.timestamp) }}</span>
              </button>
            </div>
          </div>
          <div
            v-else
            class="rounded-md border border-dashed border-[var(--border-color)] bg-[var(--bg-panel)] px-4 py-12 text-center text-sm text-[var(--text-muted)]"
          >
      ...
      <aside class="flex w-[440px] shrink-0 flex-col bg-[var(--bg-panel)]">
          ...
          <div class="flex min-w-0 items-center justify-between gap-3">
            <div class="min-w-0">
              <div class="text-sm font-semibold text-[var(--text-main)]">
                {{ t("agent.sessionTree.details") }}
              </div>
              <div class="mt-1 truncate font-mono text-[11px] text-[var(--text-muted)]">
                {{ selectedEntryLabel }}
              </div>
            </div>
            <button
              v-if="selectedNode"
              type="button"
              class="flex h-7 w-7 shrink-0 items-center justify-center rounded-md text-[var(--text-muted)] transition-colors hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)]"
        ...
        >
          <div class="mb-4 rounded-md border border-[var(--border-color)] bg-[var(--bg-main)] p-3">
            <div class="flex min-w-0 items-center gap-2">
              <span
                :class="nodeIcon(selectedNode)"
                class="h-4 w-4 shrink-0 text-[var(--accent-text)]"
              />
              <span
                class="min-w-0 truncate rounded border px-2 py-1 text-xs font-medium"
                :class="roleToneClass(selectedNode)"
                :title="roleTitle(selectedNode)"
              >{{ roleLabel(selectedNode) }}</span>
              <span
                v-if="selectedNode.id === props.activeLeafId"
                class="rounded bg-[var(--accent-bg)] px-2 py-1 text-[10px] font-medium text-[var(--accent-text)]"
              >LEAF</span>
            </div>
            <div class="mt-3 min-w-0 truncate text-sm font-medium text-[var(--text-main)]">
              {{ nodePreview(selectedNode) }}
            </div>
            <div class="mt-1 text-xs text-[var(--text-muted)]">
              {{ detailStatusLabel(selectedNode) }}
            </div>
          </div>
          ...
          <div class="mt-4 space-y-2 rounded-md border border-[var(--border-color)] bg-[var(--bg-main)] p-3 text-xs">
            <div class="flex items-center justify-between gap-3">
              ...
              <button
                class="min-w-0 truncate font-mono text-[var(--accent-text)]"
                :title="selectedNode.id"
                @click="void copyId(selectedNode.id)"
              >
                {{ shortEntryId(selectedNode.id) }}
              </button>
            </div>
            <div class="flex items-center justify-between gap-3">
              <span class="text-[var(--text-muted)]">Parent</span>
              <button
                class="min-w-0 truncate font-mono text-[var(--text-secondary)]"
                :title="selectedNode.parentId ?? '-'"
                @click="void copyId(selectedNode.parentId)"
              >
                {{ selectedNode.parentId ? shortEntryId(selectedNode.parentId) : "-" }}
              </button>
            </div>
            <div class="flex items-center justify-between gap-3">
        ...
        <div
          v-else
          class="flex flex-1 items-center justify-center px-4 text-sm text-[var(--text-muted)]"
        >
        ...
        <div class="border-t border-[var(--border-color)] bg-[var(--bg-main)] px-4 py-3">
          <button
            type="button"
            class="inline-flex h-9 w-full items-center justify-center gap-2 rounded-md bg-[var(--accent-main)] text-sm font-medium text-[var(--text-inverse)] shadow-sm transition-opacity hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50"
            :disabled="props.running || !props.canActivate || !selectedNode"
            @click="activateSelected"
          >
            <span class="i-lucide-git-branch h-4 w-4" />
