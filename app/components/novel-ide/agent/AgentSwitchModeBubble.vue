  ...
  <div class="min-w-0 w-full">
    <div class="min-w-0 w-full rounded-xl border border-[var(--border-color)] bg-[var(--chat-ai-bg)] px-3 py-2.5 shadow-sm">
      <div class="mb-1.5 flex min-w-0 items-center gap-2 text-[11px] leading-5 text-[var(--text-muted)]">
        <span
          :class="isPendingQuestion ? 'i-lucide-clock text-[var(--status-warning)]' : 'i-lucide-file-check-2 text-[var(--status-success)]'"
          class="h-3.5 w-3.5 shrink-0"
        />
        <span class="shrink-0 font-medium text-[var(--text-main)]">{{ statusLabel }}</span>
        <span
          v-if="planFilePath"
          class="min-w-0 truncate font-mono text-[11px] text-[var(--text-muted)]"
        >{{ planFilePath }}</span>
        <button
        ...
        >
          <span
            :class="showApprovedPreview ? 'i-lucide-chevron-up' : 'i-lucide-chevron-down'"
            class="h-3.5 w-3.5"
          />
        </button>
      </div>
      ...
      <div
        v-if="shouldShowPlanPreview && hasPlanFilePreview"
        class="max-h-[320px] min-w-0 overflow-y-auto pr-2 text-xs leading-relaxed text-[var(--text-main)]"
      >
        <AgentMarkdownContent :content="planContent" />
        <div class="mt-2 text-[11px] text-[var(--text-muted)]">
          完整计划文件 · {{ formatByteCount(planContentBytes) }}
        </div>
      </div>
      <div
        v-else-if="planSummary"
        class="line-clamp-2 break-words text-xs leading-5 text-[var(--text-secondary)]"
      >
        {{ planSummary || t("agent.planApproval.collapsed") }}
      </div>
      <div
        v-else-if="isExitToNormal"
      ...
      <div
        ...
        class="mt-2 flex items-center gap-2 text-[11px] leading-5 text-[var(--status-warning)]"
      >
        <span class="i-lucide-clock h-3.5 w-3.5 shrink-0" />
        <span>{{ t("agent.planApproval.waiting") }}</span>
      </div>
      <div
        v-else-if="parsedAnswer"
        class="mt-2 flex flex-wrap items-center gap-x-2 gap-y-1 text-[11px] leading-5 text-[var(--text-muted)]"
      >
