...
<template>
  ...
  <section
    ...
    class="shrink-0 border-b border-[var(--border-color)] bg-[var(--bg-main)] px-4 py-3"
  >
    <div class="flex items-center justify-between gap-3">
      <div class="flex min-w-0 items-center gap-2 text-xs font-medium text-[var(--text-main)]">
        <span class="i-lucide-terminal-square h-4 w-4 shrink-0 text-[var(--accent-text)]" />
        <span>{{ t("agent.systemPrompt.title") }}</span>
      </div>
      <div class="flex shrink-0 items-center gap-1">
        <button
          type="button"
          class="rounded p-1.5 text-[var(--text-muted)] transition-colors hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)] disabled:cursor-wait disabled:opacity-50"
    ...
    <div class="mt-2 max-h-[min(42vh,420px)] overflow-y-auto rounded-md border border-[var(--border-color)] bg-[var(--bg-panel)] px-3 py-2">
      <div
        v-if="props.loading"
        class="flex items-center gap-2 py-3 text-xs text-[var(--text-muted)]"
      >
      ...
      <div
        ...
        class="flex items-center justify-between gap-3 py-2 text-xs text-[var(--status-danger)]"
      >
        <span class="min-w-0 break-words">{{ props.error }}</span>
        <button
          type="button"
          class="shrink-0 rounded border border-[var(--status-danger-border)] bg-[var(--status-danger-bg)] px-2 py-1 transition-opacity hover:opacity-80"
          @click="emit('load')"
        >
          {{ t("agent.systemPrompt.retry") }}
        </button>
      </div>
      ...
      <div
        v-else
        class="py-3 text-xs text-[var(--text-muted)]"
      >
        {{ t("agent.systemPrompt.empty") }}
      </div>
    </div>
  </section>
</template>
