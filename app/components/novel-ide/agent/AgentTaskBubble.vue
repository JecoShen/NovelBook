  ...
  <div class="mt-2 min-w-0 w-full max-w-full rounded-xl border border-[var(--border-color)] bg-[var(--bg-main)] px-2.5 py-2 shadow-sm">
    <div class="flex min-w-0 items-center justify-between gap-2">
      ...
      <div class="flex shrink-0 items-center gap-1.5">
        ...
        >
          <span
            :class="isCollapsed ? 'i-lucide-chevron-down' : 'i-lucide-chevron-up'"
            class="h-3.5 w-3.5"
          />
        </button>
      </div>
    ...
    <div
      v-if="taskList && !isCollapsed"
      class="mt-2 min-w-0 space-y-1.5"
    >
      <div
        v-if="taskList.title"
        class="min-w-0 rounded-md border border-[var(--border-color)]/60 bg-[var(--bg-input)] px-2 py-1.5"
      >
        <div class="break-words text-xs leading-5 text-[var(--text-main)]">
          {{ taskList.title }}
        </div>
      </div>
        ...
        >
          <div class="flex min-w-0 items-start gap-2">
            <div
              ...
              :class="taskStatusClass(step.status)"
            >
              <span
                :class="taskStatusIcon(step.status)"
                class="h-2.5 w-2.5"
              />
            </div>
            <div class="min-w-0 flex-1">
              ...
              <div class="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-[11px] leading-5 text-[var(--text-muted)]">
                ...
                >{{ t("agent.tasks.note", { note: step.note }) }}</span>
    ...
    <div
      ...
      class="mt-2 min-w-0 break-words rounded-md border border-[var(--border-color)]/70 bg-[var(--bg-input)] px-2 py-1.5 text-[11px] leading-5 text-[var(--text-muted)]"
    >
      {{ toolCall.result?.trim() || t("agent.tasks.parsing") }}
    </div>

    <div
      v-if="toolCall.error && !isCollapsed"
      class="mt-2 break-all whitespace-pre-wrap rounded-md border border-[var(--status-danger-border)] bg-[var(--status-danger-bg)] p-2 font-mono text-[11px] text-[var(--status-danger)]"
    >
      {{ toolCall.error }}
    </div>
  </div>
