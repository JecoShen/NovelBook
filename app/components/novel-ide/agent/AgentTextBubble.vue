...
<template>
  ...
  <div
    ...
    class="group flex min-w-0 w-full flex-col pl-6"
  ...
  >
    ...
    <div
      v-show="!isSystemCollapsed"
      class="mt-2 min-w-0 w-full"
    >
      <div
  ...
  <div
    v-else
    class="group flex min-w-0 w-full flex-col items-start"
  >
    <!-- 消息头部 -->
    <div class="mb-1.5 ml-1 flex w-full items-center gap-2">
      ...
      <span
        v-if="props.node.message.model"
        class="rounded border border-[var(--border-color)] bg-[var(--bg-input)] px-1.5 py-0.5 text-[10px] text-[var(--text-muted)]"
      >
        {{ props.node.message.model }}
      </span>
      <span
        v-if="props.node.message.timestamp"
        class="text-[10px] text-[var(--text-muted)]"
      >{{ props.node.message.timestamp }}</span>
      <span
        v-if="messageStatusLabel(props.node.message)"
        class="rounded border border-[var(--border-color)] bg-[var(--bg-sidebar)] px-1.5 py-0.5 text-[10px] text-[var(--text-muted)]"
      >
      ...
      <div class="mr-4 flex items-center gap-1 text-[var(--text-muted)]">
        <button
          v-if="isUnknownDelivery"
          class="rounded p-1 text-[var(--status-warning)] transition-colors hover:bg-[var(--bg-hover)]"
          title="确认可能重复后重新发送"
          @click="emit('resend-unknown', props.node.message)"
        >
          <span class="i-lucide-send h-3.5 w-3.5" />
        </button>
        <button
          v-if="isUnknownDelivery"
          class="rounded p-1 transition-colors hover:bg-[var(--bg-hover)] hover:text-[var(--status-danger)]"
          title="移除本地未知占位"
          @click="emit('dismiss-unknown', props.node.message)"
        >
        ...
        <button
          ...
          :title="isContentOmitted ? t('agent.textBubble.copyPreview') : t('agent.textBubble.copy')"
          @click="emit('copy', props.node.message)"
        >
        ...
        <button
          ...
          class="rounded p-1 transition-colors hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)] disabled:cursor-not-allowed disabled:opacity-40"
          :disabled="props.actionDisabled || props.runActionDisabled"
          :title="t('agent.textBubble.retry')"
          @click="emit('retry', props.node.message)"
        >
        ...
        <button
          class="rounded p-1 transition-colors hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)] disabled:cursor-not-allowed disabled:opacity-40"
          :disabled="props.actionDisabled || props.runActionDisabled"
          :title="t('agent.textBubble.branchFromHere')"
          @click="emit('branch-from-here', props.node.message)"
        >
          <span class="i-lucide-git-branch-plus h-3.5 w-3.5" />
        </button>
      </div>
    </div>
    ...
    >
      <div class="px-0.5 py-0.5">
        ...
        >
          <span
            :class="isThinkingCollapsed ? 'i-lucide-chevron-right' : 'i-lucide-chevron-down'"
            class="h-3 w-3 shrink-0"
          />
          <span class="i-lucide-brain-circuit h-3 w-3 shrink-0" />
          <span
          ...
          <span
            v-else
            class="text-[10px] normal-case tracking-normal text-[var(--text-muted)]/65"
          >{{ t("agent.textBubble.collapse") }}</span>
        </button>

        <div
          v-if="!isThinkingCollapsed"
          class="mt-1.5 border-l border-[var(--border-color)]/40 pl-3 text-[13px] leading-relaxed text-[var(--text-muted)]/85"
        >
          <AgentMarkdownContent
            :content="props.node.message.thinking ?? ''"
            :streaming="props.node.message.status === 'streaming'"
            :open-reference="props.openReference"
          />
        </div>
      </div>
      ...
      >
        ...
        >
          ...
          <template v-if="props.node.message.contentBlocks?.length">
            <div
              v-for="(block, blockIndex) in props.node.message.contentBlocks"
          ...
          <div
            v-if="isContentOmitted"
            class="mt-3 flex items-center gap-1.5 border-t border-[var(--border-color)] pt-2 text-[11px] text-[var(--status-info)]"
          >
          ...
          <div
            v-if="props.node.message.status === 'interrupted'"
            class="mt-2 flex items-center gap-1.5 text-[11px] text-[var(--text-muted)]"
          >
            <span class="i-lucide-square h-3.5 w-3.5 shrink-0" />
            <span>{{ t("agent.textBubble.interrupted") }}</span>
          </div>
          <div
            v-if="(props.node.message.omittedToolCalls ?? 0) > 0"
            class="mt-2 flex items-center gap-1.5 text-[11px] text-[var(--status-info)]"
          >
            <span class="i-lucide-info h-3.5 w-3.5 shrink-0" /><span>另有 {{ props.node.message.omittedToolCalls }} 个工具调用未在历史预览中显示</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div
      v-if="messageUsage"
      class="mt-1 flex w-full items-center pl-6 text-[var(--text-muted)]"
    >
      <div class="flex-1" />
      <div
        class="flex items-center gap-1 text-[10px] text-[var(--text-muted)]"
        :title="messageUsageTitle"
      >
