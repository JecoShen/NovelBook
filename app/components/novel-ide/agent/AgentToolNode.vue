...
<template>
  <div
    v-if="renderConfig.mode !== 'hidden'"
    class="w-full overflow-hidden rounded-xl border border-[var(--border-color)] bg-[var(--chat-ai-bg)] shadow-sm"
  >
    <!-- 容器头部：点击切换展开 -->
    <button
      class="flex w-full items-center justify-between px-3 py-1.5 text-left transition-colors hover:bg-[var(--bg-hover)]"
      @click="emit('toggle')"
    >
      <div class="flex min-w-0 items-center gap-2.5 overflow-hidden">
        <div
          ...
          :class="displayedStatusClass"
        >
          <span
            :class="[displayedStatusIcon, isRunning ? 'animate-spin' : '']"
            class="h-3 w-3"
          />
        </div>
        <!-- 紧凑单行 -->
        <div class="flex min-w-0 items-center gap-2">
          ...
          <span
            v-if="!props.expanded && renderConfig.mode === 'inline'"
            class="truncate font-mono text-[11px] text-[var(--text-muted)] opacity-80 min-w-0"
          >
            {{ props.toolCall.argsJson ?? props.toolCall.argsText }}
          </span>
          <span
            v-else-if="!props.expanded && collapsedPreview"
            class="truncate font-mono text-[11px] text-[var(--text-muted)] opacity-80 min-w-0"
          >
            {{ collapsedPreview }}
          </span>
      ...
      <span
        :class="props.expanded ? 'i-lucide-chevron-up' : 'i-lucide-chevron-down'"
        class="ml-2 h-4 w-4 shrink-0 text-[var(--text-muted)]"
      />
    </button>
    ...
    <div
      v-if="props.expanded"
      class="border-t border-[var(--border-color)]/50 bg-[var(--bg-input)]/50 px-3 pb-3 pt-1"
    >
      <!-- block 模式：使用注册表声明的专用组件 -->
      <component
        :is="renderConfig.component"
        v-if="renderConfig.mode === 'block' && renderConfig.component"
      ...
      <template v-else>
        <div class="mt-2 space-y-2 overflow-y-auto max-h-[500px] pr-1">
          ...
          <div>
            ...
            <JsonViewer
              v-if="parsedArgs !== null"
              :value="parsedArgs"
              :max-height="300"
            />
            <div
              v-else
              class="break-all whitespace-pre-wrap rounded border border-[var(--border-color)] bg-[var(--bg-main)] p-2 font-mono text-xs text-[var(--text-secondary)]"
            >
              {{ props.toolCall.argsJson ?? props.toolCall.argsText }}
            </div>
          </div>
        ...
        <div class="mt-2 flex items-center justify-start gap-1 text-[var(--text-muted)]">
          <button
            class="rounded p-1 transition-colors hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)]"
