...
<template>
  ...
  >
    <header class="flex min-w-0 items-center justify-between gap-3 border-b border-[var(--status-warning-border)] bg-[var(--status-warning-bg)] px-3 py-2">
      <div class="min-w-0">
        <div class="flex min-w-0 flex-wrap items-center gap-x-2 gap-y-1 text-[11px] font-medium text-[var(--status-warning)]">
          <span class="inline-flex items-center gap-1.5"><span class="i-lucide-message-square-more h-3.5 w-3.5" />{{ t("agent.userInput.pendingTitle") }}</span>
        ...
        <div class="mt-0.5 text-xs font-medium text-[var(--text-main)]">
          {{ questionTypeLabel }}
        </div>
      </div>
      <div class="flex shrink-0 items-center gap-1 text-[var(--text-muted)]">
        <button
          type="button"
          class="flex h-7 w-7 items-center justify-center rounded transition-colors hover:bg-[var(--bg-hover)] disabled:cursor-not-allowed disabled:opacity-35"
          :disabled="activeIndex === 0"
          :title="t('agent.userInput.previous')"
          @click="switchItem(activeIndex - 1)"
        >
          <span class="i-lucide-chevron-left h-3.5 w-3.5" />
        </button>
        <button
          type="button"
          class="flex h-7 w-7 items-center justify-center rounded transition-colors hover:bg-[var(--bg-hover)] disabled:cursor-not-allowed disabled:opacity-35"
    ...
    <div>
      <div
        v-if="!props.canResolve && props.blockedMessage"
        class="flex items-start gap-2 border-b border-[var(--status-danger-border)] bg-[var(--status-danger-bg)] px-3 py-2 text-xs leading-5 text-[var(--status-danger)]"
        role="status"
      >
        <span class="i-lucide-octagon-alert mt-0.5 h-3.5 w-3.5 shrink-0" />
        <span>{{ props.blockedMessage }}</span>
      </div>
      <div
        v-if="props.submissionIssue"
        class="flex items-start gap-2 border-b px-3 py-2 text-xs leading-5"
        :class="props.submissionIssue.kind === 'unknown' ? 'border-[var(--status-warning-border)] bg-[var(--status-warning-bg)] text-[var(--status-warning)]' : 'border-[var(--status-danger-border)] bg-[var(--status-danger-bg)] text-[var(--status-danger)]'"
        role="status"
      >
        <span
          :class="props.submissionIssue.kind === 'unknown' ? 'i-lucide-circle-help' : 'i-lucide-circle-alert'"
          class="mt-0.5 h-3.5 w-3.5 shrink-0"
        />
        <span class="min-w-0 flex-1">{{ props.submissionIssue.message }}</span>
        <button
          v-if="props.submissionIssue.kind === 'unknown'"
          type="button"
          class="shrink-0 rounded border border-current px-2 py-0.5 font-medium hover:bg-[var(--bg-hover)]"
          @click="emit('resync')"
        >
          {{ t("agent.userInput.resync") }}
        </button>
      </div>
    </div>
    ...
    <div class="min-h-0">
      <!-- Low-Code Form 项目 -->
      <div
        v-if="activeForm && activeForm.session.form"
        ref="contentScrollRef"
        class="h-full overflow-y-auto px-3 py-3"
      >
        <LowCodeForm
          :form="activeForm.session.form"
          :model-value="activeFormDraft.data"
          :disabled="answerControlsDisabled"
          @update:model-value="updateForm"
        />
        <div
          class="mt-3 flex items-center gap-2 text-xs"
          :class="activeFormDraft.confirmed ? 'text-[var(--status-success)]' : 'text-[var(--text-muted)]'"
        >
          <span
            :class="activeFormDraft.confirmed ? 'i-lucide-circle-check' : 'i-lucide-circle-dashed'"
            class="h-3.5 w-3.5"
          />
      ...
      >
        ...
        >
          <div
            v-if="activeQuestion.header"
            class="mb-1 text-[11px] font-medium text-[var(--text-muted)]"
          >
            {{ activeQuestion.header }}
          </div>
          <h3 class="break-words text-sm font-semibold leading-6 text-[var(--text-main)]">
          ...
          >
            ...
            <label
              ...
              :key="index"
              :for="optionId(index)"
              class="group flex cursor-pointer items-start gap-2 rounded-md px-2 py-1.5 transition-colors hover:bg-[var(--bg-hover)] has-[:disabled]:cursor-not-allowed has-[:disabled]:opacity-55"
              :class="activeAnswer.selectedOptionIndex === index ? 'bg-[var(--accent-bg)] shadow-[inset_2px_0_0_var(--accent-main)]' : ''"
            >
              <input
                :id="optionId(index)"
                type="radio"
                :name="`agent-pending-${activeItem.key}`"
                class="sr-only"
                :checked="activeAnswer.selectedOptionIndex === index"
                :disabled="answerControlsDisabled"
              ...
              <span class="min-w-0 flex-1">
                <span
                  ...
                  :class="activeAnswer.selectedOptionIndex === index ? 'text-[var(--text-main)]' : 'text-[var(--text-secondary)]'"
                >{{ option.label }}</span>
                <span
                  v-if="option.description"
                  class="block text-[11px] leading-4 text-[var(--text-muted)]"
                >{{ option.description }}</span>
              </span>
              <span
                class="mt-0.5 h-4 w-4 shrink-0 rounded-full border"
                :class="activeAnswer.selectedOptionIndex === index ? 'border-[var(--accent-main)] bg-[var(--accent-main)] shadow-[inset_0_0_0_2px_var(--bg-main)]' : 'border-[var(--border-color)] group-hover:border-[var(--text-muted)]'"
              />
            </label>

            <label
              v-if="activeItem.kind === 'question' || (activeItem.kind === 'approval' && activeQuestion.approvalAction === 'switch_mode' && activeQuestion.switchTargetMode === 'normal')"
              :for="optionId(NONE_OF_ABOVE_OPTION_INDEX)"
              class="group flex cursor-pointer items-start gap-2 rounded-md px-2 py-1.5 transition-colors hover:bg-[var(--bg-hover)] has-[:disabled]:cursor-not-allowed has-[:disabled]:opacity-55"
              :class="activeAnswer.selectedOptionIndex === NONE_OF_ABOVE_OPTION_INDEX ? 'bg-[var(--accent-bg)] shadow-[inset_2px_0_0_var(--accent-main)]' : ''"
            >
              <input
                :id="optionId(NONE_OF_ABOVE_OPTION_INDEX)"
                type="radio"
                :name="`agent-pending-${activeItem.key}`"
                class="sr-only"
                :checked="activeAnswer.selectedOptionIndex === NONE_OF_ABOVE_OPTION_INDEX"
                :disabled="answerControlsDisabled"
                @change="selectOption(NONE_OF_ABOVE_OPTION_INDEX)"
              >
              <span class="w-5 shrink-0 pt-0.5 text-right text-xs tabular-nums text-[var(--text-muted)]">{{ activeQuestion.options.length + 1 }}.</span>
              <span class="min-w-0 flex-1">
                <span
                  class="block text-[13px] font-semibold leading-5"
                  :class="activeAnswer.selectedOptionIndex === NONE_OF_ABOVE_OPTION_INDEX ? 'text-[var(--text-main)]' : 'text-[var(--text-secondary)]'"
                >{{ activeItem.kind === 'approval' ? t("agent.userInput.addSuggestion") : t("agent.userInput.otherAnswer") }}</span>
              ...
              <span
                class="mt-0.5 h-4 w-4 shrink-0 rounded-full border"
                :class="activeAnswer.selectedOptionIndex === NONE_OF_ABOVE_OPTION_INDEX ? 'border-[var(--accent-main)] bg-[var(--accent-main)] shadow-[inset_0_0_0_2px_var(--bg-main)]' : 'border-[var(--border-color)] group-hover:border-[var(--text-muted)]'"
              />
            </label>
          </fieldset>
        </div>
        ...
        <div
          v-if="showNoteInput"
          class="shrink-0 border-t border-[var(--border-color)] bg-[var(--bg-panel)]/50 px-3 py-2"
        >
    ...
    <footer class="flex min-w-0 flex-wrap items-center justify-between gap-2 border-t border-[var(--border-color)] bg-[var(--bg-panel)]/50 px-3 py-2">
      <button
        type="button"
        class="inline-flex h-8 items-center gap-1.5 rounded px-2 text-xs font-medium text-[var(--text-muted)] transition-colors hover:bg-[var(--status-danger-bg)] hover:text-[var(--status-danger)] disabled:cursor-not-allowed disabled:opacity-40"
        :disabled="props.submitting || !props.canAbort"
        @click="emit('cancel')"
      >
        <span class="i-lucide-square h-3.5 w-3.5" />
      ...
      >
        ...
        <span
          ...
          class="i-lucide-corner-down-left h-3.5 w-3.5"
        />
        <span
          v-else
          class="i-lucide-arrow-right h-3.5 w-3.5"
        />
      </button>
    </footer>
  </section>
