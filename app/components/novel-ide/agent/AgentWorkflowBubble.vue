...
<template>
  <div class="mt-2 space-y-3">
    ...
    <div class="rounded-lg border border-[var(--border-color)] bg-[var(--bg-main)] p-3">
      <div class="flex flex-wrap items-start justify-between gap-2">
        <div class="min-w-0">
          <div class="flex flex-wrap items-center gap-2">
            <span class="i-lucide-route h-4 w-4 text-[var(--accent-main)]" />
            ...
            <span
              v-if="parsedArgs.model"
              class="rounded border border-[var(--status-info-border)] bg-[var(--status-info-bg)] px-1.5 py-0.5 font-mono text-[10px] text-[var(--status-info)]"
            >{{ parsedArgs.model }}</span>
            <span
              v-if="parsedArgs.wait"
              class="rounded border border-[var(--border-color)] bg-[var(--bg-input)] px-1.5 py-0.5 font-mono text-[10px] text-[var(--text-muted)]"
            >wait:true</span>
            <span
              v-else-if="jobId"
          ...
          <p
            v-if="catalogDescription"
            class="mt-1 text-xs leading-5 text-[var(--text-secondary)]"
          >
            {{ catalogDescription }}
          </p>
          <div
            v-if="jobId"
            class="mt-1 break-all font-mono text-[10px] text-[var(--text-muted)]"
          >
            job {{ jobId }}
          </div>
          <div
        ...
        <span
          class="inline-flex shrink-0 items-center gap-1.5 rounded-full border px-2.5 py-1 text-xs"
          :class="statusToneClass"
        >
          <span
            :class="[statusIcon, status === 'running' || status === 'starting' ? 'animate-spin' : '']"
            class="h-3.5 w-3.5"
          />
          {{ statusLabel }}
        </span>
      ...
      <div
        v-if="parsedArgs.args !== undefined"
        class="mt-3 overflow-hidden rounded-lg border border-[var(--border-color)]"
      >
        <button
          type="button"
          class="flex w-full items-center justify-between gap-2 px-3 py-2 text-left hover:bg-[var(--bg-hover)]"
          @click="argsExpanded = !argsExpanded"
        >
          <span class="text-xs font-medium text-[var(--text-main)]">运行参数</span>
          <span
            :class="argsExpanded ? 'i-lucide-chevron-up' : 'i-lucide-chevron-down'"
            class="h-3.5 w-3.5 text-[var(--text-muted)]"
          />
        </button>
        <div
          v-if="argsExpanded"
          class="border-t border-[var(--border-color)] p-2"
        >
          <JsonViewer
            :value="parsedArgs.args"
            :max-height="220"
          />
        </div>
      </div>
      <div
        v-if="parsedArgs.script"
        class="mt-3 overflow-hidden rounded-lg border border-[var(--border-color)]"
      >
        <button
          type="button"
        ...
        >
          ...
          <span
            :class="scriptExpanded ? 'i-lucide-chevron-up' : 'i-lucide-chevron-down'"
            class="h-3.5 w-3.5 text-[var(--text-muted)]"
          />
        </button>
        <pre
          v-if="scriptExpanded"
          class="max-h-64 overflow-auto whitespace-pre-wrap break-all border-t border-[var(--border-color)] bg-[var(--bg-input)] p-2 font-mono text-[11px] leading-5 text-[var(--text-secondary)]"
        >{{ parsedArgs.script }}</pre>
      </div>
    </div>
    ...
    <div
      ...
      class="rounded-lg border border-[var(--status-warning-border)] bg-[var(--status-warning-bg)] p-3 text-xs leading-5 text-[var(--status-warning)]"
    >
      <div class="flex items-start gap-2">
        <span class="i-lucide-shield-question mt-0.5 h-4 w-4 shrink-0" />
    ...
    <div
      v-if="jobId"
      class="rounded-lg border border-[var(--border-color)] bg-[var(--bg-main)] p-3"
    >
      <div class="flex flex-wrap items-center justify-between gap-2">
        <div class="flex min-w-0 items-center gap-2">
          <span class="i-lucide-briefcase-business h-4 w-4 shrink-0 text-[var(--accent-main)]" />
          <span class="break-all font-mono text-xs text-[var(--text-main)]">{{ jobId }}</span>
          <span
            v-if="matchingJob"
            class="text-[11px] text-[var(--text-muted)]"
          >{{ matchingJob.title }}</span>
        </div>
        <button
          v-if="canCancelJob || cancelRequested"
          type="button"
          class="rounded border border-[var(--status-danger-border)] bg-[var(--status-danger-bg)] px-2.5 py-1 text-[11px] text-[var(--status-danger)] disabled:cursor-wait disabled:opacity-50"
          :disabled="cancelSubmitting || cancelRequested"
          @click="cancelJob"
        >
      ...
      <div
        v-if="matchingJob?.preview"
        class="mt-2 whitespace-pre-wrap break-all text-xs leading-5 text-[var(--text-secondary)]"
      >
        {{ matchingJob.preview }}
      </div>
      <div
        v-else-if="!jobUnavailable"
        class="mt-2 text-xs text-[var(--text-muted)]"
      >
        正在连接后台任务管理器…
      </div>
      <div
        v-if="matchingJob?.error"
        class="mt-2 whitespace-pre-wrap break-all text-xs leading-5 text-[var(--status-danger)]"
      >
        <span class="font-medium">后台任务错误：</span>{{ matchingJob.error }}
      </div>
      <div
        v-if="matchingJob"
        class="mt-2 flex flex-wrap gap-3 text-[10px] text-[var(--text-muted)]"
      >
    ...
    <div
      v-if="runningNow.length"
      class="flex flex-wrap items-center gap-2"
    >
      <span class="text-xs text-[var(--text-muted)]">正在运行</span>
      <span
        v-for="activity in runningNow"
        :key="activity.key"
        class="workflow-running-chip rounded-full border border-[var(--status-info-border)] bg-[var(--status-info-bg)] px-2.5 py-1 text-xs text-[var(--status-info)]"
      >
    ...
    <div
      v-if="matchingRunState || effectiveChart"
      class="overflow-hidden rounded-lg border border-[var(--border-color)] bg-[var(--bg-main)]"
    >
      <button
        type="button"
        class="flex w-full items-center justify-between gap-2 px-3 py-2 text-left hover:bg-[var(--bg-hover)]"
      ...
      >
        <span class="flex items-center gap-2 text-xs font-medium text-[var(--text-main)]">
          <span class="i-lucide-git-branch h-3.5 w-3.5 text-[var(--accent-main)]" />
          Workflow 可视化
        </span>
        <span
          :class="chartExpanded ? 'i-lucide-chevron-up' : 'i-lucide-chevron-down'"
          class="h-3.5 w-3.5 text-[var(--text-muted)]"
        />
      </button>
      <div
    ...
    <div
      v-else-if="status === 'running' || status === 'starting'"
      class="flex items-center gap-2 rounded border border-[var(--status-info-border)] bg-[var(--status-info-bg)] px-3 py-2 text-xs text-[var(--status-info)]"
    >
      <span class="i-lucide-loader-circle h-3.5 w-3.5 animate-spin" />
      正在连接 workflow run…
    </div>
    ...
    >
      <div
        ...
        :key="ask.key"
        class="rounded-lg border border-[var(--status-warning-border)] bg-[var(--status-warning-bg)] p-3"
      >
        <div class="text-sm font-semibold text-[var(--status-warning)]">
          {{ ask.spec.title }}
        </div>
        <AgentMarkdownContent
          v-if="ask.spec.description"
          class="mt-2"
          :content="ask.spec.description"
        />
        <div
          v-if="ask.spec.kind === 'select'"
      ...
      <div
        v-if="pendingAsks.length === 0 && pendingAskTitles.length"
        class="rounded border border-[var(--status-warning-border)] bg-[var(--status-warning-bg)] px-3 py-2 text-xs text-[var(--status-warning)]"
      >
    ...
    <div
      v-if="workflowError"
      class="whitespace-pre-wrap break-all rounded border border-[var(--status-danger-border)] bg-[var(--status-danger-bg)] p-2 font-mono text-xs text-[var(--status-danger)]"
    >
      {{ workflowError }}
    </div>
    <div
    ...
    >
      ...
      >
        <div
          v-if="usage"
          class="flex flex-wrap gap-2 text-[11px] text-[var(--text-muted)]"
        >
        ...
        >
          <div
            ...
            class="flex flex-wrap items-center gap-2 rounded border border-[var(--border-color)] bg-[var(--bg-input)] px-2 py-1.5 text-[11px]"
          >
            ...
            <span
              v-if="session.title && session.profileKey"
              class="font-mono text-[var(--text-muted)]"
            >{{ session.profileKey }}</span>
            <span
              v-if="session.tokens"
              class="ml-auto text-[var(--text-muted)]"
            >{{ session.tokens.totalTokens.toLocaleString() }} tokens</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div
      v-if="effectiveResult !== undefined"
      class="overflow-hidden rounded-lg border border-[var(--border-color)] bg-[var(--bg-main)]"
    >
      <button
        type="button"
      ...
      >
        ...
        <span
          :class="resultExpanded ? 'i-lucide-chevron-up' : 'i-lucide-chevron-down'"
          class="h-3.5 w-3.5 text-[var(--text-muted)]"
        />
      </button>
      <div
    ...
    <div
      v-if="status === 'not_started' && props.toolCall.result"
      class="whitespace-pre-wrap break-all rounded border border-[var(--border-color)] bg-[var(--bg-main)] p-2 text-xs text-[var(--text-secondary)]"
    >
