...
<template>
  <section
    v-if="waitingCount || feed.error"
    class="border-t border-[var(--border-color)] bg-[var(--bg-panel)] px-3 py-3"
  >
    <div class="flex items-center justify-between gap-2">
      <div class="flex min-w-0 items-center gap-2 text-sm font-semibold text-[var(--text-main)]">
        <span class="i-lucide-inbox h-4 w-4 shrink-0 text-[var(--status-warning)]" />
        <span>Workflow 待处理</span>
        <span
          v-if="waitingCount"
          class="rounded-full bg-[var(--status-warning-bg)] px-1.5 py-0.5 text-[10px] text-[var(--status-warning)]"
        >{{ waitingCount }}</span>
      </div>
    ...
    <div
      ...
      :key="job.jobId"
      class="mt-3 border-t border-[var(--border-color)] pt-3 first:mt-2 first:border-t-0 first:pt-0"
    >
      <div class="flex flex-wrap items-center justify-between gap-2">
        <div class="flex min-w-0 items-center gap-2">
          <span class="i-lucide-route h-3.5 w-3.5 shrink-0 text-[var(--accent-main)]" />
      ...
      <template v-if="readRunRef(job) && stateFor(readRunRef(job)!.runId)">
        <div
          v-for="ask in asksFor(readRunRef(job)!.runId)"
          :key="ask.key"
          class="mt-2 rounded border border-[var(--status-warning-border)] bg-[var(--status-warning-bg)] p-3"
        >
          ...
          >
            <button
              ...
              @click="toggleOption(readRunRef(job)!.runId, ask, option.id)"
            >
              {{ option.label }}
            </button>
          </div>
          ...
          >
            <button
              ...
              :disabled="submittingRuns.has(readRunRef(job)!.runId) || submittedRuns.has(readRunRef(job)!.runId)"
              @click="setDraft(readRunRef(job)!.runId, ask.key, true)"
            >
              同意
            </button>
            <button
              ...
              :disabled="submittingRuns.has(readRunRef(job)!.runId) || submittedRuns.has(readRunRef(job)!.runId)"
              @click="setDraft(readRunRef(job)!.runId, ask.key, false)"
            >
              否决
            </button>
        ...
        <div
          v-if="asksFor(readRunRef(job)!.runId).length === 0"
        ...
        <div class="mt-2 flex flex-wrap items-center justify-between gap-2">
          <span
            v-if="runErrors[readRunRef(job)!.runId]"
            class="text-xs text-[var(--status-danger)]"
          >{{ runErrors[readRunRef(job)!.runId] }}</span>
          <span
            v-else-if="submittedRuns.has(readRunRef(job)!.runId)"
            class="text-xs text-[var(--status-info)]"
          >已提交，正在继续…</span>
          <span
            v-else
    ...
    <div
      v-if="feed.error"
      class="mt-2 text-xs text-[var(--status-danger)]"
    >
      {{ feed.error }}
    </div>
  </section>
