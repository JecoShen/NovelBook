...
<template>
  ...
  <Transition name="workspace-card">
    <section
      v-if="visible"
      class="mb-1.5 overflow-hidden rounded-lg border border-[var(--border-color)] bg-[var(--bg-panel)]"
      style="box-shadow: 0 2px 8px color-mix(in srgb, var(--shadow-color) 6%, transparent);"
    >
      <!-- 摘要标题与批量操作 -->
      <div class="flex min-h-7 items-center gap-1.5 px-2 py-1">
        <button
          type="button"
          class="flex min-w-0 flex-1 items-center gap-1.5 rounded-md text-left outline-none transition-colors hover:text-[var(--text-main)] focus-visible:ring-1 focus-visible:ring-[var(--accent-main)]"
        ...
        >
          <span class="flex h-5 w-5 shrink-0 items-center justify-center rounded border border-[var(--status-info-border)] bg-[var(--status-info-bg)] text-[var(--status-info)]">
            <span class="i-lucide-file-diff h-3 w-3" />
          </span>
          ...
          <span
            v-if="groups[0]"
            class="max-w-[38%] truncate font-mono text-[10px] text-[var(--text-muted)]"
            :title="groups[0].path"
          >{{ groups[0].path }}</span>
          <span
        ...
        <button
          ...
          type="button"
          class="inline-flex h-6 shrink-0 items-center gap-1 rounded border border-[var(--status-success-border)] bg-[var(--status-success-bg)] px-1.5 text-[10px] font-medium text-[var(--status-success)] transition-all duration-150 hover:border-[var(--status-success)] active:scale-95 disabled:cursor-not-allowed disabled:opacity-45"
          :disabled="acceptingAll || busyPath !== null"
          @click="void acceptAll()"
        >
          <span
            :class="acceptingAll ? 'i-lucide-loader-2 animate-spin' : 'i-lucide-check-check'"
            class="h-2.5 w-2.5"
          />
        ...
        >
          <div class="workspace-body-clip">
            <div class="workspace-scroll custom-scrollbar max-h-[14rem] overflow-y-auto border-t border-[var(--border-color)]">
              <p
                v-if="error"
                class="m-1.5 rounded border border-[var(--status-danger)] px-2 py-1 text-[10px] text-[var(--status-danger)]"
              >
                {{ error }}
              </p>
              <TransitionGroup
              ...
              >
                <article
                  ...
                  :key="`${group.path}:${group.revision}`"
                  class="border-b border-[var(--border-color)] last:border-b-0"
                >
                  <div class="flex min-h-6 items-center gap-1.5 px-2 py-1 transition-colors hover:bg-[var(--bg-hover)]">
                    <button
                      type="button"
                      class="flex min-w-0 flex-1 items-center gap-1.5 rounded-sm text-left outline-none focus-visible:ring-1 focus-visible:ring-[var(--accent-main)]"
                      :aria-expanded="selectedPath === group.path"
                      @click="void selectGroup(group)"
                    >
                      <span class="i-lucide-file h-3 w-3 shrink-0 text-[var(--text-muted)]" />
                      <span
                        class="min-w-0 flex-1 truncate font-mono text-[10px] text-[var(--text-main)]"
                        :title="group.path"
                      >{{ group.path }}</span>
                      ...
                      <span
                        class="i-lucide-chevron-right h-2.5 w-2.5 text-[var(--text-muted)] transition-transform duration-150"
                        :class="selectedPath === group.path ? 'rotate-90' : ''"
                      />
                    </button>
                    <button
                      type="button"
                      class="inline-flex h-5 shrink-0 items-center gap-0.5 rounded border border-[var(--border-color)] px-1.5 text-[10px] font-medium text-[var(--accent-text)] transition-colors hover:bg-[var(--bg-hover)] focus-visible:ring-1 focus-visible:ring-[var(--accent-main)] disabled:cursor-not-allowed disabled:text-[var(--text-muted)] disabled:opacity-45"
                      :disabled="group.endHash === null"
                      :title="group.endHash === null ? t('agent.workspaceChanges.openDeleted') : t('agent.workspaceChanges.openFile')"
                      @click="emit('open-file', group.path)"
                    >
                      <span class="i-lucide-external-link h-2.5 w-2.5" />
                      <span>{{ t("agent.workspaceChanges.open") }}</span>
                    </button>
                    <button
                      type="button"
                      class="inline-flex h-5 shrink-0 items-center gap-0.5 rounded border border-[var(--status-success-border)] px-1.5 text-[10px] font-medium text-[var(--status-success)] transition-all duration-150 hover:bg-[var(--status-success-bg)] active:scale-95 disabled:cursor-not-allowed disabled:opacity-45"
                      :disabled="busyPath !== null || acceptingAll"
                      @click="void acceptGroup(group)"
                    >
                      <span
                        :class="busyPath === group.path ? 'i-lucide-loader-2 animate-spin' : 'i-lucide-check'"
                        class="h-2.5 w-2.5"
                      />
                  ...
                  <Transition name="workspace-diff">
                    <div
                      v-if="selectedPath === group.path"
                      class="workspace-diff border-t border-[var(--border-color)] bg-[var(--bg-subtle)] px-2 py-1.5"
                    >
                      <p
                        v-if="diffRequests.state(diffIdentity(group)).loading"
                        class="text-[10px] text-[var(--text-muted)]"
                      >
                        {{ t("agent.workspaceChanges.loadingDiff") }}
                      </p>
                      <p
                        v-else-if="diffRequests.state(diffIdentity(group)).error"
                        class="text-[10px] text-[var(--status-danger)]"
                      >
                        {{ diffRequests.state(diffIdentity(group)).error }}
                      </p>
                      <template v-else-if="diffFor(group)?.status === 'available'">
                        <div class="workspace-diff-scroll custom-scrollbar max-h-36 overflow-auto rounded border border-[var(--border-color)] bg-[var(--source-bg)] font-mono text-[10px] leading-4">
                          <div
                            v-for="(line, index) in previewLines(availableDiff(group)?.changes ?? [])"
                            :key="index"
                            class="flex min-w-max px-1.5"
                            :class="line.kind === 'added' ? 'bg-[var(--status-success-bg)] text-[var(--status-success)]' : line.kind === 'removed' ? 'bg-[var(--status-danger-bg)] text-[var(--status-danger)]' : 'text-[var(--text-muted)]'"
                          >
                      ...
                      <p
                        v-else-if="diffFor(group)?.status === 'blocked'"
                        class="rounded border border-[var(--status-warning)] px-2 py-1 text-[10px] text-[var(--status-warning)]"
                      >
                        {{ t("agent.workspaceChanges.blocked") }}
                      </p>
                      <p
                        v-else-if="diffFor(group)?.status === 'too_large'"
                        class="text-[10px] text-[var(--text-muted)]"
                      >
                        {{ t("agent.workspaceChanges.tooLarge") }}
                      </p>
                      <p
                        v-else-if="diffFor(group)?.status === 'unavailable'"
                        class="text-[10px] text-[var(--text-muted)]"
                      >
                        {{ t("agent.workspaceChanges.unavailable") }}
                      </p>
                    </div>
                  </Transition>
                </article>
              ...
              <div class="flex min-h-6 items-center justify-between border-t border-[var(--border-color)] bg-[var(--bg-subtle)] px-2 py-1">
                ...
                <button
                  type="button"
                  class="inline-flex items-center gap-0.5 text-[10px] font-medium text-[var(--accent-text)] transition-opacity hover:opacity-70"
                  @click="emit('open-full')"
                >
                  <span>{{ t("agent.workspaceChanges.openFull") }}</span>
                  <span class="i-lucide-arrow-up-right h-2.5 w-2.5" />
                </button>
              </div>
            </div>
          </div>
