    ...
    <template v-else>
      ...
      <div class="shrink-0 border-b border-[var(--border-color)] px-4 pt-3">
        <div class="flex items-center gap-2">
          <span
            ...
            :class="traceStatusDotClass(props.record.status)"
          />
          ...
          >
            <span class="i-lucide-messages-square h-3 w-3" />
            <span>{{ t("agent.traceViewer.openSession", { id: props.record.correlation.sessionId }) }}</span>
          </button>
        </div>
        <div
          v-if="props.record.response.errorMessage"
          class="mt-2 rounded-md border border-[var(--status-danger-border)] bg-[var(--status-danger-bg)] px-2.5 py-1.5 text-xs text-[var(--status-danger)]"
        >
          {{ props.record.response.errorMessage }}
        </div>
      ...
      <div class="min-h-0 flex-1 overflow-y-auto p-4">
        ...
        <dl
          ...
          class="grid grid-cols-[auto_minmax(0,1fr)] gap-x-4 gap-y-1.5 text-xs"
        >
          <template
          ...
          >
            ...
            <dd class="break-all text-[var(--text-main)]">
              {{ row.value }}
            </dd>
          </template>
        </dl>
        ...
        <div v-else-if="activeTab === 'system'">
          <pre
            v-if="contextView?.systemPrompt"
            class="whitespace-pre-wrap break-words rounded-lg border border-[var(--border-color)] bg-[var(--bg-input)] bg-opacity-40 p-3 text-xs leading-relaxed text-[var(--text-main)]"
          >{{ contextView.systemPrompt }}</pre>
          <p
            v-else
            class="text-xs text-[var(--text-muted)]"
          >
            {{ t("agent.traceViewer.emptySection") }}
          </p>
        </div>
        ...
        >
          <p
            v-if="!contextView || contextView.messages.length === 0"
            class="text-xs text-[var(--text-muted)]"
          >
            {{ t("agent.traceViewer.emptySection") }}
          </p>
          <article
            v-for="(message, index) in contextView?.messages ?? []"
            :key="messageKey(message, index)"
            class="rounded-lg border border-[var(--border-color)] border-opacity-60"
          >
            ...
            <div class="space-y-2 p-2.5">
              ...
              >
                <pre
                  ...
                  class="max-h-72 overflow-y-auto whitespace-pre-wrap break-words text-xs leading-relaxed text-[var(--text-main)]"
                >{{ block.text }}</pre>
                <pre
                  v-else-if="block.kind === 'thinking'"
                  class="max-h-48 overflow-y-auto whitespace-pre-wrap break-words border-l-2 border-[var(--border-color)] pl-2 text-xs italic leading-relaxed text-[var(--text-muted)]"
                >{{ block.text }}</pre>
                <div
                ...
                >
                  <div class="flex items-center gap-1.5 text-[11px] font-medium text-[var(--text-secondary)]">
                    <span class="i-lucide-wrench h-3 w-3" />
        ...
        >
          <p
            v-if="!contextView || contextView.tools.length === 0"
            class="text-xs text-[var(--text-muted)]"
          >
            {{ t("agent.traceViewer.emptySection") }}
          </p>
          <details
            v-for="tool in contextView?.tools ?? []"
            :key="tool.name"
            class="rounded-lg border border-[var(--border-color)] border-opacity-60 px-2.5 py-2"
          >
            <summary class="cursor-pointer text-xs">
              <span class="font-medium text-[var(--text-main)]">{{ tool.name }}</span>
              <span
                v-if="tool.description"
                class="ml-2 text-[var(--text-muted)]"
              >{{ tool.description.slice(0, 120) }}</span>
            </summary>
            <div class="mt-2">
              <JsonViewer
                :value="tool.raw"
        ...
        >
          <JsonViewer
            v-if="props.record.request.payload !== undefined"
            :value="props.record.request.payload"
            :max-height="0"
          ...
          <p
            ...
            class="text-xs text-[var(--text-muted)]"
          >
            {{ t("agent.traceViewer.notCaptured") }}
          </p>
        </div>
