...
<template>
  ...
  <div class="flex h-full flex-col">
    ...
    >
      <span class="i-lucide-inbox h-6 w-6 text-[var(--text-muted)]" />
      <span class="text-xs text-[var(--text-muted)]">{{ t("agent.traceViewer.empty") }}</span>
    </div>

    <div
      v-else
      class="flex-1 space-y-3 overflow-y-auto p-2"
    >
      <!-- 每个 invocation 分组 -->
      <section
        v-for="(group, groupIndex) in groups"
        :key="group.invocationId ?? `single-${String(groupIndex)}`"
        class="space-y-1"
      >
        <div class="flex items-center gap-1.5 px-1.5 text-[10px] font-semibold uppercase tracking-wide text-[var(--text-muted)]">
          <span class="i-lucide-corner-down-right h-3 w-3" />
        ...
        >
          <div class="flex items-center gap-2">
            <span
              ...
              :class="traceStatusDotClass(entry.status)"
            />
          ...
          <div class="mt-1 flex items-center gap-2 pl-4 text-[10px] text-[var(--text-muted)]">
            ...
            <span
              ...
              class="max-w-[40%] truncate rounded bg-[var(--bg-input)] px-1 text-[var(--text-secondary)]"
            ...
            <span
              v-if="entry.stopReason"
              class="truncate"
            >{{ entry.stopReason }}</span>
          </div>
        </button>
      </section>
    </div>
