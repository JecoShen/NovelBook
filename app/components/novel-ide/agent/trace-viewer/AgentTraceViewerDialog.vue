    ...
    <template #header-extra>
      <div class="flex items-center gap-2">
        ...
        <button
          ...
          :title="t('agent.traceViewer.refresh')"
          @click="void initialize(selectedScope ?? undefined)"
        >
          <span class="i-lucide-refresh-cw h-4 w-4" />
        </button>
        <button
          type="button"
          class="rounded p-1.5 text-[var(--text-muted)] transition-colors hover:bg-[var(--bg-hover)] hover:text-[var(--status-danger)] disabled:cursor-not-allowed disabled:opacity-40"
          :title="t('agent.traceViewer.clear')"
          :disabled="!selectedScope || selectedScope === RECENT_SCOPE || loadingList"
          @click="void clearCurrentBucket()"
        >
