...
<template>
  ...
  >
    <div class="space-y-4">
      ...
      <div
        v-if="error"
        class="rounded-lg border border-[var(--status-danger-border)] bg-[var(--status-danger-bg)] px-3 py-2 text-xs text-[var(--status-danger)]"
      >
        {{ error }}
      </div>
      ...
      >
        <section class="rounded-xl border border-[var(--border-color)] bg-[var(--bg-input)]/20 p-3">
          ...
          <div class="mt-2 space-y-2 text-xs">
            <div
              v-for="field in response.schema.fields"
              :key="field.key"
              class="rounded-lg border border-[var(--border-color)] bg-[var(--bg-panel)] px-2.5 py-2"
            >
              <div class="font-medium text-[var(--text-main)]">
                {{ field.label }}
              </div>
              <div class="mt-1 text-[var(--text-secondary)]">
                {{ field.description || "无额外说明" }}
              </div>
    ...
    <template #footer="{ cancel }">
      ...
      <button
        class="inline-flex items-center justify-center h-8 px-4 rounded-md text-[13px] font-medium cursor-pointer border border-[var(--border-color)] bg-[var(--bg-input)] text-[var(--text-main)] transition-colors duration-200 hover:bg-[var(--bg-hover)] active:scale-95 disabled:opacity-50"
      ...
      <button
        class="inline-flex items-center justify-center h-8 px-4 rounded-md text-[13px] font-medium cursor-pointer border border-transparent bg-[var(--accent-main)] text-[var(--text-inverse)] transition-all duration-200 hover:opacity-90 hover:shadow-md active:scale-95 disabled:opacity-50"
        :disabled="!response"
        @click="applyDraft"
      >
        应用草稿
