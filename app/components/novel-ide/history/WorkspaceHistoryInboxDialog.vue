...
<template>
  ...
  <Dialog
    ...
    :title="t('ide.historyInbox.title')"
    size="xl"
    :show-footer="false"
  ...
  >
    <template #header-extra>
      <button
        ...
        :title="t('ide.historyInbox.refresh')"
        @click="void load()"
      >
    ...
    <div class="flex h-[70vh] min-h-0 flex-1">
      <aside class="flex w-[340px] shrink-0 flex-col overflow-y-auto border-r border-[var(--border-color)]">
        <p
          v-if="error"
          class="m-3 rounded border border-[var(--status-danger)] px-3 py-2 text-xs text-[var(--status-danger)]"
        >
          {{ error }}
        </p>
        <p
          v-else-if="loading && groups.length === 0"
          class="m-3 text-sm text-[var(--text-muted)]"
        >
          {{ t("ide.historyInbox.loading") }}
        </p>
        <p
          v-else-if="groups.length === 0"
          class="m-3 text-sm text-[var(--text-muted)]"
        >
          {{ t("ide.historyInbox.empty") }}
        </p>
        <button
          v-for="group in groups"
          :key="group.path"
          type="button"
          class="flex flex-col gap-1 border-b border-[var(--border-color)] px-3 py-2.5 text-left transition-colors hover:bg-[var(--bg-hover)]"
          :class="group.path === selectedPath ? 'bg-[var(--bg-hover)]' : ''"
          @click="void select(group)"
        >
          <span class="flex items-center gap-2">
            <span
              class="min-w-0 flex-1 truncate text-[13px] text-[var(--text-main)]"
              :title="group.path"
            >{{ group.path }}</span>
            <span
              v-if="group.endHash === null"
              class="shrink-0 rounded px-1.5 py-0.5 text-[10px] text-[var(--status-danger)] border border-[var(--status-danger)]"
            >{{ t("ide.historyInbox.deleted") }}</span>
          </span>
          ...
          <span class="flex items-center gap-2">
            <button
              type="button"
              class="rounded border border-[var(--border-color)] px-2 py-0.5 text-[11px] text-[var(--status-success)] transition-colors hover:bg-[var(--bg-hover)] disabled:cursor-not-allowed disabled:opacity-40"
              :disabled="busyPath !== null"
              @click.stop="void acceptGroup(group)"
            >{{ t("ide.historyInbox.accept") }}</button>
            <button
              type="button"
              ...
              :disabled="busyPath !== null"
              @click.stop="void revertGroup(group)"
            >{{ t("ide.historyInbox.revert") }}</button>
          </span>
        </button>
      </aside>
      <section class="flex min-w-0 flex-1 flex-col">
        <p
          v-if="!selectedGroup"
          class="m-4 text-sm text-[var(--text-muted)]"
        >
          {{ t("ide.historyInbox.noSelection") }}
        </p>
        <p
          v-else-if="selectedDiffState.loading"
          class="m-4 text-sm text-[var(--text-muted)]"
        >
          {{ t("ide.historyInbox.loading") }}
        </p>
        <p
          v-else-if="selectedDiffState.error"
          class="m-4 text-sm text-[var(--status-danger)]"
        >
          {{ selectedDiffState.error }}
        </p>
        <p
          v-else-if="selectedDiff?.status === 'blocked'"
          class="m-4 rounded border border-[var(--status-warning)] px-3 py-2 text-sm text-[var(--status-warning)]"
        >
          {{ t("ide.historyInbox.diffBlocked") }}
        </p>
        <p
          v-else-if="selectedDiff && selectedDiff.status !== 'available'"
          class="m-4 text-sm text-[var(--text-muted)]"
        >
          {{ t("ide.historyInbox.diffUnavailable") }}
        </p>
        <SharedDiffEditor
          v-else-if="selectedDiff?.status === 'available'"
          class="min-h-0 flex-1"
          :model-key="`history-inbox:${selectedGroup.path}:${selectedGroup.revision}`"
          :original-content="selectedDiff.original"
          :modified-content="selectedDiff.modified"
          :original-label="t('ide.historyInbox.baseLabel')"
          :modified-label="t('ide.historyInbox.currentLabel')"
          :theme="props.theme"
        />
      </section>
    </div>
