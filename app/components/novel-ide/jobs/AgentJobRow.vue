...
<template>
  ...
  <div class="border-b border-[var(--border-color)]">
    <div
      class="flex cursor-pointer flex-col gap-1 px-4 py-2.5 transition-colors hover:bg-[var(--bg-hover)]"
      @click="expanded = !expanded"
    >
      <!-- 行1：kind 图标 + 标题 + 状态 chip -->
      <div class="flex items-center gap-2">
        <span
          :class="KIND_ICONS[job.kind]"
        ...
        >{{ job.title }}</span>
        <span
          class="flex shrink-0 items-center gap-1 rounded-full border px-2 py-0.5 text-[11px]"
          :class="statusChip.chipClass"
        >
          <span
            v-if="statusChip.icon"
            :class="statusChip.icon"
            class="h-3 w-3"
          />
      ...
      <div class="flex items-center gap-1.5 text-[11px] text-[var(--text-muted)]">
        ...
        <button
          ...
          @click.stop="void cancel()"
        >
          {{ cancelling || cancelRequested ? t("ide.agentJobs.cancelling") : t("ide.agentJobs.cancel") }}
        </button>
      </div>
      <!-- 行3：收起态 preview / error 摘要 -->
      <p
        v-if="job.preview && !expanded"
        class="line-clamp-2 text-[12px] text-[var(--text-secondary)]"
      >
        {{ job.preview }}
      </p>
      <p
        v-if="job.error && !expanded"
        class="truncate text-[12px] text-[var(--status-danger)]"
      >
        {{ job.error }}
      </p>
      <p
        v-if="job.deliveryStatus !== 'not_required'"
      ...
      <p
        ...
        class="text-[12px] text-[var(--status-warning)]"
      >
        ↳ {{ t("ide.agentJobs.waitingHint", { session: ownerLabel }) }}
      </p>
    </div>
    ...
    <div
      v-if="expanded"
      class="flex flex-col gap-2 border-t border-dashed border-[var(--border-color)] bg-[var(--bg-input)] px-4 py-3"
    >
      <!-- ref 观测指针 + 复制 -->
      <div
        v-for="entry in refEntries"
        :key="entry.key"
        class="flex items-center gap-2 text-[12px]"
      >
        <span class="shrink-0 text-[var(--text-muted)]">{{ entry.label }}</span>
        <code
          class="min-w-0 flex-1 truncate rounded bg-[var(--bg-main)] px-1.5 py-0.5 text-[11px] text-[var(--text-secondary)]"
          :title="entry.value"
        >{{ entry.value }}</code>
        <button
          type="button"
          class="flex h-6 w-6 shrink-0 items-center justify-center rounded text-[var(--text-muted)] transition-colors hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)]"
          @click="void copyText(entry.value)"
        >
      ...
      <p
        v-if="job.preview"
        class="whitespace-pre-wrap break-words text-[12px] leading-relaxed text-[var(--text-secondary)]"
      >
        {{ job.preview }}
      </p>
      <!-- error 全文 + 复制 -->
      ...
      >
        <p class="min-w-0 flex-1 whitespace-pre-wrap break-words text-[12px] text-[var(--status-danger)]">
          {{ job.error }}
        </p>
        <button
          type="button"
          class="flex h-6 w-6 shrink-0 items-center justify-center rounded text-[var(--text-muted)] transition-colors hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)]"
          @click="void copyText(job.error)"
        >
          <span class="i-lucide-copy h-3.5 w-3.5" />
        </button>
      </div>
      ...
      >
        ...
        <div
          v-if="detailLoading"
          class="flex items-center py-1 text-[var(--text-muted)]"
        >
          <span class="i-lucide-loader-2 h-4 w-4 animate-spin" />
        </div>
        <p
          v-else-if="detailUnavailable"
          class="text-[12px] text-[var(--text-muted)]"
        >
          {{ t("ide.agentJobs.resultUnavailable") }}
        </p>
        <p
          v-else-if="detailError"
          class="text-[12px] text-[var(--status-danger)]"
        >
          {{ detailError }}
        </p>
        <JsonViewer
          v-else-if="detail && detail.result !== undefined"
          :value="detail.result"
          :max-height="240"
        />
        <p
