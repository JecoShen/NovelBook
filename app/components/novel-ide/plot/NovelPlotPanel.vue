  ...
  <div class="flex h-full min-h-0 flex-col">
    ...
    <div class="flex shrink-0 items-center justify-between gap-3 border-b border-[var(--border-color)] bg-[var(--bg-panel)] px-3 py-2">
      ...
      <div class="flex shrink-0 items-center gap-2">
        ...
        >
          <span class="i-lucide-scroll-text h-3.5 w-3.5" />
        ...
        >
          <span
            :class="loadingWorkbench ? 'i-lucide-loader-2 animate-spin' : 'i-lucide-git-branch-plus'"
            class="h-4 w-4"
          />
          <span>剧本工作台</span>
        </button>
    ...
    <div
      v-if="chapterChips.length || actNodes.length"
      class="flex shrink-0 items-center gap-1.5 overflow-x-auto border-b border-[var(--border-color)] bg-[var(--bg-panel)] px-3 py-1.5"
    >
      ...
      >
        <span
          :class="chip.hasBrief ? 'bg-[var(--status-success)]' : 'bg-[var(--text-muted)]/40'"
    ...
    >
      <template #footer>
        <button
          class="inline-flex items-center justify-center h-8 px-4 rounded-md text-[13px] font-medium cursor-pointer border border-[var(--border-color)] bg-[var(--bg-input)] text-[var(--text-main)] transition-colors hover:bg-[var(--bg-hover)] active:scale-95 disabled:opacity-50"
          :disabled="savingAct"
          @click="actDialogVisible = false"
        >
          取消
        </button>
        <button
          class="inline-flex items-center justify-center h-8 min-w-[92px] px-4 rounded-md text-[13px] font-medium cursor-pointer border border-transparent bg-[var(--accent-main)] text-[var(--text-inverse)] transition-all hover:opacity-90 active:scale-95 disabled:opacity-50"
          :disabled="savingAct"
          @click="saveAct"
        >
          <span
      ...
      <div class="space-y-3 px-1 mt-1">
        ...
        <label class="block space-y-1">
          ...
          <input
            v-model="actDraftTitle"
            placeholder="如 第一卷 · 序章"
            class="h-8 w-full rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-2.5 text-[12px] text-[var(--text-main)] outline-none focus:border-[var(--accent-main)]"
          >
        </label>
        <label class="block space-y-1">
          <span class="text-[10px] font-medium uppercase tracking-[0.14em] text-[var(--text-muted)]">name(可空,自动派生)</span>
          <input
            v-model="actDraftName"
