...
const emit = defineEmits<{
  (e: 'selectChapter' | 'selectScene', id: string): void
}>()
  ...
  <div class="grid min-h-0 gap-4 xl:grid-cols-[280px_minmax(0,1fr)]">
    <section class="flex min-h-0 flex-col overflow-hidden rounded-[22px] border border-[var(--border-color)] bg-[var(--bg-panel)] shadow-[0_18px_50px_color-mix(in_srgb,var(--shadow-color)_8%,transparent)]">
      ...
      <div class="min-h-0 flex-1 overflow-y-auto px-3 py-3">
        <div class="space-y-2">
          ...
          >
            <div class="text-[11px] uppercase tracking-[0.16em] opacity-80">
              {{ chapter.volumeTitle }}
            </div>
    ...
    <section class="flex min-h-0 flex-col overflow-hidden rounded-[22px] border border-[var(--border-color)] bg-[var(--bg-panel)] shadow-[0_18px_50px_color-mix(in_srgb,var(--shadow-color)_8%,transparent)]">
      ...
      <div class="min-h-0 flex-1 overflow-y-auto px-5 py-5">
        ...
        >
          ...
          >
            ...
            >
              ...
              <div
                class="mt-3 text-base font-semibold text-[var(--text-main)]"
                :class="props.selectedSceneId === scene.id ? 'text-[var(--accent-text)]' : ''"
              >
                {{ scene.title }}
              </div>
            ...
            <div class="mt-4 grid gap-2 text-xs leading-6 text-[var(--text-secondary)]">
              <div
                v-if="scene.purpose"
                class="rounded-lg border border-[var(--border-color)] bg-[var(--bg-panel)] px-3 py-2"
              >
                目的：{{ scene.purpose }}
              </div>
              <div
                v-if="scene.writingTip"
                class="rounded-lg border border-[var(--border-color)] bg-[var(--bg-panel)] px-3 py-2"
              >
        ...
        <div
          v-else
          class="flex min-h-[260px] items-center justify-center rounded-[22px] border border-dashed border-[var(--border-color)] bg-[var(--bg-input)]/30 px-6 text-center"
        >
          <div>
            ...
            <div class="mt-2 text-sm leading-7 text-[var(--text-secondary)]">
              这个视图会以 `Chapter -> Scene` 的方式展示一章内部承载。目前适合用来观察章级 Scene 是否过空或过满。
            </div>
          </div>
        </div>
      </div>
