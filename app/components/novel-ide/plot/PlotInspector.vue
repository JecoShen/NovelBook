...
<template>
  ...
  <aside class="flex min-h-0 flex-col overflow-hidden rounded-[22px] border border-[var(--border-color)] bg-[var(--bg-panel)] shadow-[0_18px_50px_color-mix(in_srgb,var(--shadow-color)_8%,transparent)]">
    ...
    >
      ...
      <div
        v-if="focus.meta.length > 0"
        class="mt-4 rounded-2xl border border-[var(--border-color)] bg-[var(--bg-input)] px-4 py-4"
      >
        <div class="text-[10px] uppercase tracking-[0.18em] text-[var(--text-muted)]">
          Meta
        </div>
      ...
      >
        ...
        <div class="mt-3 space-y-2">
          ...
          >
            ...
            <div
              v-if="refItem.note"
              class="mt-1 text-xs leading-6 text-[var(--text-secondary)]"
            >
              {{ refItem.note }}
            </div>
          </div>
