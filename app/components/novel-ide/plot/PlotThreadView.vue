...
const emit = defineEmits<{
  (e: 'selectThread' | 'selectScene' | 'jumpChapter', payload: string): void
}>()
...
<template>
  ...
  <div class="grid min-h-0 gap-4 xl:grid-cols-[280px_minmax(0,1fr)]">
          ...
          >
            <div class="flex items-center justify-between gap-3">
              ...
              <span
                ...
                :class="PLOT_TONE_STYLES[thread.tone].chipClass"
              >
                {{ thread.isMainThread ? "主线" : "支线" }}
              </span>
            </div>
            <div class="mt-2 line-clamp-2 text-xs leading-6">
              {{ thread.summary }}
            </div>
          </button>
        </div>
    ...
    <section class="flex min-h-0 flex-col overflow-hidden rounded-[22px] border border-[var(--border-color)] bg-[var(--bg-panel)] shadow-[0_18px_50px_color-mix(in_srgb,var(--shadow-color)_8%,transparent)]">
      <div
        v-if="selectedThread"
        class="border-b border-[var(--border-color)] px-5 py-4"
      >
        <div class="flex flex-wrap items-center gap-2">
          <span
            class="rounded-full px-2.5 py-1 text-[11px] font-semibold"
            :class="PLOT_TONE_STYLES[selectedThread.tone].chipClass"
          >
        ...
        <div class="space-y-4">
          ...
          >
            <div class="flex flex-wrap items-center justify-between gap-3">
              ...
              <span
                v-else
                class="rounded-full border border-dashed border-[var(--border-color)] px-2.5 py-1 text-[11px] text-[var(--text-muted)]"
              >
                未挂章
              </span>
            </div>

            <div class="mt-3 text-base font-semibold text-[var(--text-main)]">
              {{ scene.title }}
            </div>
            <div class="mt-2 text-sm leading-7 text-[var(--text-secondary)]">
              {{ scene.summary }}
      ...
      <div
        v-else
        class="flex min-h-0 flex-1 items-center justify-center px-6 py-8 text-center"
      >
        <div>
          ...
          <div class="mt-2 text-sm leading-7 text-[var(--text-secondary)]">
            该视图会按 `threadSortOrder` 展示 Scene 序列，方便快速规划一条线的前后推进。
          </div>
        </div>
      </div>
    </section>
