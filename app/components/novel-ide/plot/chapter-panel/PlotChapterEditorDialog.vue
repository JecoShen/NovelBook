  ...
  >
    ...
    <div class="space-y-3 px-1 mt-1">
      ...
      <FormField label="name(供 Prose frontmatter 反指)">
        <FormInput
          v-model="draft.name"
          placeholder="如 001-volume-001-chapter(小写字母/数字/连字符)"
        />
      </FormField>
      <FormField label="备注">
        <FormTextarea
      ...
      <div class="mt-1 border-t border-[var(--border-color)] pt-3">
        <div class="mb-2 text-[11px] font-semibold uppercase tracking-[0.12em] text-[var(--text-muted)]">
          章级写作指令(ChapterBrief)
        </div>
        ...
        <div class="grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)] gap-2">
          ...
          <FormField label="语气 / 风格">
            <FormTextarea
              v-model="draft.tone"
              :rows="2"
              placeholder="情绪温度、风格约束(可空)"
            />
          </FormField>
        </div>
        <FormField label="节奏 / 下一章牵引">
          <FormTextarea
            v-model="draft.pacing"
            :rows="2"
            placeholder="节奏、悬念、下一章牵引(可空)"
          />
        </FormField>

        <!-- 信息控制:防全知的按章控制面,四项全空时 brief status 会停在 needs_chapter_brief -->
        <div class="mt-1 mb-1 text-[11px] font-medium text-[var(--text-muted)]">
          信息控制(至少填一项,防止 writer 越界泄露)
        </div>
        <div class="grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)] gap-2">
          <FormField label="读者已知">
            <FormTextarea
              v-model="draft.readerKnows"
              :rows="2"
              placeholder="读者此刻应知道什么(可空)"
            />
          </FormField>
          <FormField label="主角已知">
            <FormTextarea
              v-model="draft.protagonistKnows"
              :rows="2"
              placeholder="主角此刻知道什么(可空)"
            />
          </FormField>
          <FormField label="必须隐藏">
            <FormTextarea
              v-model="draft.mustHide"
              :rows="2"
              placeholder="本章绝不能泄露的事实(可空)"
            />
          </FormField>
          <FormField label="可暗示不可明说">
            <FormTextarea
              v-model="draft.hintOnly"
              :rows="2"
              placeholder="可埋伏笔但不点破(可空)"
            />
          </FormField>
        </div>
        ...
        <div class="grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)] gap-2">
          <FormField label="开场钩子">
            <FormTextarea
              v-model="draft.opening"
              :rows="2"
              placeholder="本章如何开场(可空)"
            />
          </FormField>
          <FormField label="结尾定句 / 落点">
            <FormTextarea
              ...
              :rows="2"
              placeholder="本章如何收尾(可空)"
            />
          </FormField>
        </div>
        <FormField label="禁写事项">
          <FormTextarea
            v-model="draft.doNotWrite"
            :rows="2"
            placeholder="不要写的内容(可空)"
          />
        </FormField>
      </div>
      ...
      <div
        ...
        class="mt-1 border-t border-[var(--border-color)] pt-3"
      >
        <div class="mb-2 flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.12em] text-[var(--text-muted)]">
          ...
          <span
            v-if="loadingProse"
            class="i-lucide-loader-circle h-3 w-3 animate-spin"
          />
        </div>
        <div
          v-if="proseError"
        ...
        <div
          v-else-if="!loadingProse && proseNodes.length === 0"
        ...
        >
          <li
            v-for="prose in proseNodes"
            :key="prose.indexPath"
            class="flex items-center justify-between gap-2 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-2 py-1 text-[11px]"
          >
            <span
              class="min-w-0 truncate text-[var(--text-secondary)]"
              :title="prose.path"
            >{{ prose.title || prose.path }}</span>
