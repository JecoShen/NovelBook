  ...
  >
    ...
    <template #footer>
      ...
      >
        ...
        <template v-else>
          <span class="i-lucide-gavel h-3.5 w-3.5" />
          <span>落槌拍板</span>
        </template>
      </button>
    </template>
    ...
    <div class="mt-1 space-y-3.5 px-1">
      ...
      <div class="space-y-1.5">
        ...
        >
          <div class="flex items-start gap-2.5">
            <span
              ...
              :class="choice === index ? 'border-[var(--accent-main)]' : 'border-[var(--border-strong)]'"
            >
              <span
                v-if="choice === index"
                class="h-1.5 w-1.5 rounded-full bg-[var(--accent-main)]"
              />
            </span>
            <span class="min-w-0">
              <span
                class="block text-[12px] leading-5"
                :class="choice === index ? 'font-semibold text-[var(--text-main)]' : 'text-[var(--text-secondary)]'"
              >{{ option.option }}</span>
              <span
                v-if="option.note"
                class="mt-0.5 block text-[11px] leading-4 text-[var(--text-muted)]"
              >{{ option.note }}</span>
            </span>
          </div>
        </button>
        ...
        >
          <div class="flex items-start gap-2.5">
            <span
              class="mt-0.5 flex h-3.5 w-3.5 shrink-0 items-center justify-center rounded-full border"
              :class="choice === 'new' ? 'border-[var(--accent-main)]' : 'border-[var(--border-strong)]'"
            >
              <span
                v-if="choice === 'new'"
                class="h-1.5 w-1.5 rounded-full bg-[var(--accent-main)]"
            ...
            <span class="min-w-0">
              <span
                class="block text-[12px] leading-5"
                :class="choice === 'new' ? 'font-semibold text-[var(--text-main)]' : 'text-[var(--text-secondary)]'"
              >全新方案(不在候选中)</span>
      ...
      <div class="space-y-1.5">
        <div class="flex items-center justify-between gap-2">
          ...
          <button
            v-if="chosenOptionText !== null"
            type="button"
            class="inline-flex items-center gap-1 rounded px-1.5 py-0.5 text-[10px] text-[var(--accent-text)] transition-colors hover:bg-[var(--bg-hover)]"
            @click="fillDecisionFromChoice"
          >
            <span class="i-lucide-corner-left-down h-2.5 w-2.5" />
            把候选文本填入
          </button>
        </div>
        <FormTextarea
          ...
          :rows="3"
          placeholder="最终怎么定(写进决策记录的结论)"
        />
        <div
          v-if="fieldErrors.decision"
          class="text-[11px] text-[var(--status-danger)]"
        >
          {{ fieldErrors.decision }}
        </div>
      ...
      <div class="space-y-1.5">
        ...
        <FormTextarea
          ...
          :rows="2"
          placeholder="为什么这样定(供后续审查与接手者理解)"
        />
        <div
          v-if="fieldErrors.motivation"
          class="text-[11px] text-[var(--status-danger)]"
        >
          {{ fieldErrors.motivation }}
        </div>
      ...
      <div class="space-y-1.5 rounded-md border border-[var(--status-warning-border)] bg-[var(--status-warning-bg)] p-2.5">
        ...
        <FormTextarea
          v-model="draft.risk"
          :rows="2"
          placeholder="沿此结论写下去要注意什么、哪里容易写崩"
        />
        <div
          v-if="fieldErrors.risk"
          class="text-[11px] text-[var(--status-danger)]"
        >
          {{ fieldErrors.risk }}
        </div>
      ...
      <div class="flex items-start gap-1.5 rounded-md border border-[var(--status-info-border)] bg-[var(--status-info-bg)] px-2.5 py-1.5 text-[11px] text-[var(--status-info)]">
        <span class="i-lucide-info mt-0.5 h-3 w-3 shrink-0" />
