  ...
  >
    ...
    <div class="mt-1 space-y-3 px-1">
      ...
      <FormField :label="props.mode === 'create' ? 'name(必填,供互指引用)' : 'name(供互指引用)'">
        <FormInput
          v-model="draft.name"
          placeholder="如 d-liya-truth(小写字母/数字/连字符)"
        />
        <!-- 改名破坏按名引用的行内警示,仅编辑态且已改动时出现;不举 decision:// 协议例(serves/dependsOn 互指按 id,不受改名影响) -->
        <div
          v-if="nameChanged"
          class="mt-1 flex items-start gap-1.5 rounded-md border border-[var(--status-warning-border)] bg-[var(--status-warning-bg)] px-2 py-1 text-[11px] text-[var(--status-warning)]"
        >
          <span class="i-lucide-triangle-alert mt-0.5 h-3 w-3 shrink-0" />
      ...
      <FormField label="待决问题(question,必填)">
        <FormTextarea
          ...
          :rows="3"
          placeholder="要拍板的问题本身,如「莉娅的身世在第几卷、以什么方式揭晓」"
        />
      </FormField>
      ...
      <FormField label="候选方案(options)">
        <div class="space-y-1.5">
          <div
            ...
            :key="index"
            class="rounded-md border border-[var(--border-color)] bg-[var(--bg-subtle)] p-2"
          >
            <div class="flex items-center gap-1.5">
              ...
              <div class="min-w-0 flex-1">
                <FormInput
                  v-model="row.option"
                  placeholder="候选方案内容(必填)"
                />
              </div>
              <button
                type="button"
                class="flex h-6 w-6 shrink-0 items-center justify-center rounded text-[var(--text-muted)] transition-colors hover:bg-[var(--status-danger-bg)] hover:text-[var(--status-danger)]"
                title="移除该候选"
                @click="optionRows.splice(index, 1)"
              >
          ...
          <button
            type="button"
            class="inline-flex items-center gap-1 rounded-md border border-dashed border-[var(--border-color)] px-2.5 py-1 text-[11px] text-[var(--text-muted)] transition-colors hover:border-[var(--border-strong)] hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)]"
            @click="optionRows.push({ option: '', note: '' })"
          >
      ...
      <FormField label="主锚点(anchor)">
        <div class="grid grid-cols-[120px_minmax(0,1fr)] gap-2">
          <FormSelect
            :model-value="draft.anchorKind"
            :options="anchorKindOptions"
            @update:model-value="changeAnchorKind"
          />
          <div
            v-if="draft.anchorKind === 'story'"
            class="flex h-7 items-center rounded-md border border-dashed border-[var(--border-color)] px-2.5 text-[11px] text-[var(--text-muted)]"
          >
            锚在全书层,无需选择载体
          </div>
          <FormSelect
            v-else-if="draft.anchorKind === 'chapter'"
            v-model="draft.anchorId"
            :options="chapterOptions"
            placeholder="选择章节"
          />
          <FormSelect
            v-else-if="draft.anchorKind === 'thread'"
            v-model="draft.anchorId"
            :options="threadOptions"
            placeholder="选择线程"
          />
          <FormSelect
            v-else-if="draft.anchorKind === 'scene'"
            v-model="draft.anchorId"
            :options="sceneOptions"
            placeholder="选择场景(线名 · 场名)"
          />
          <FormSelect
            v-else-if="draft.anchorKind === 'promise'"
            v-model="draft.anchorId"
          ...
          <FormSelect
            v-else-if="draft.anchorKind === 'act' && props.acts.length > 0"
            v-model="draft.anchorId"
            :options="actOptions"
            placeholder="选择卷"
          />
          <div
            v-else-if="draft.anchorKind === 'act'"
            class="flex h-7 items-center rounded-md border border-dashed border-[var(--border-color)] px-2.5 text-[11px] text-[var(--text-muted)]"
          >
            暂无卷(Act):先在剧情面板章节条「+卷」创建
          </div>
          <FormInput
            v-else
            v-model="draft.anchorPath"
            placeholder="内容节点相对路径,如 lorebook/character/chen-yao/"
          />
        </div>
      </FormField>
      ...
      <FormField label="拍板期限章">
        <FormSelect
          v-model="draft.deadlineChapterId"
      ...
      <FormField label="服务对象(serves)">
        <div class="space-y-1.5">
          ...
          >
            <div class="flex items-center gap-1.5">
              <div class="min-w-0 flex-1">
                <FormInput
                  v-model="row.value"
                  placeholder="promise://{id} / decision://{id} / thread://{id} / scene://{id} 或内容节点相对路径"
                />
              </div>
              <button
                type="button"
                class="flex h-6 w-6 shrink-0 items-center justify-center rounded text-[var(--text-muted)] transition-colors hover:bg-[var(--status-danger-bg)] hover:text-[var(--status-danger)]"
                title="移除该引用"
                @click="servesRows.splice(index, 1)"
              >
                <span class="i-lucide-trash-2 h-3.5 w-3.5" />
              </button>
            </div>
            <div
              v-if="isDeadRefRow(row)"
              class="mt-1 flex items-start gap-1.5 rounded-md border border-[var(--status-danger-border)] bg-[var(--status-danger-bg)] px-2 py-1 text-[11px] text-[var(--status-danger)]"
            >
              <span class="i-lucide-unlink mt-0.5 h-3 w-3 shrink-0" />
          ...
          <button
            type="button"
            class="inline-flex items-center gap-1 rounded-md border border-dashed border-[var(--border-color)] px-2.5 py-1 text-[11px] text-[var(--text-muted)] transition-colors hover:border-[var(--border-strong)] hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)]"
            @click="servesRows.push({ value: '', deadTarget: null })"
          >
            <span class="i-lucide-plus h-3 w-3" />
            添加服务对象
          </button>
      ...
      <FormField label="依赖前置(dependsOn)">
        <div class="space-y-1.5">
          <div
            v-for="(row, index) in dependsOnRows"
            :key="index"
          >
            <div class="flex items-center gap-1.5">
              <div class="min-w-0 flex-1">
              ...
              <button
                ...
                class="flex h-6 w-6 shrink-0 items-center justify-center rounded text-[var(--text-muted)] transition-colors hover:bg-[var(--status-danger-bg)] hover:text-[var(--status-danger)]"
                title="移除该引用"
                @click="dependsOnRows.splice(index, 1)"
              >
                <span class="i-lucide-trash-2 h-3.5 w-3.5" />
              </button>
            </div>
          ...
          <button
            type="button"
            class="inline-flex items-center gap-1 rounded-md border border-dashed border-[var(--border-color)] px-2.5 py-1 text-[11px] text-[var(--text-muted)] transition-colors hover:border-[var(--border-strong)] hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)]"
            @click="dependsOnRows.push({ value: '', deadTarget: null })"
          >
            <span class="i-lucide-plus h-3 w-3" />
            添加依赖
          </button>
      ...
      <FormField label="备注">
        ...
        <div
          v-if="props.decision?.status === 'dropped'"
          class="mt-1 text-[11px] text-[var(--text-muted)]"
        >
          当前决策已作废:备注承载失效原因,清空会被服务层拒绝。
        </div>
      </FormField>
      ...
      >
        ...
        <FormField label="动机(motivation)">
          <FormTextarea
            ...
            :rows="2"
            placeholder="为什么这样定(decided 态必须非空)"
          />
        </FormField>
        <FormField label="风险(risk · writer 的刹车点)">
          <FormTextarea
            v-model="draft.risk"
            :rows="2"
            placeholder="沿此结论写下去要注意什么(decided 态必须非空)"
          />
        </FormField>
        <FormField label="否决记录(rejectedAlternatives)">
          <div class="space-y-1.5">
            <div
              v-for="(row, index) in rejectedRows"
              :key="index"
              class="rounded-md border border-[var(--border-color)] bg-[var(--bg-subtle)] p-2"
            >
              <div class="flex items-center gap-1.5">
                ...
                <button
                  ...
                  class="flex h-6 w-6 shrink-0 items-center justify-center rounded text-[var(--text-muted)] transition-colors hover:bg-[var(--status-danger-bg)] hover:text-[var(--status-danger)]"
                  title="移除该否决记录"
                  @click="rejectedRows.splice(index, 1)"
                >
                  <span class="i-lucide-trash-2 h-3.5 w-3.5" />
                </button>
              </div>
              <div class="mt-1.5">
                <FormInput
                  v-model="row.whyRejected"
                  placeholder="否决理由(可空=待补理由)"
                />
              </div>
            </div>
            <button
              type="button"
              class="inline-flex items-center gap-1 rounded-md border border-dashed border-[var(--border-color)] px-2.5 py-1 text-[11px] text-[var(--text-muted)] transition-colors hover:border-[var(--border-strong)] hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)]"
              @click="rejectedRows.push({ option: '', whyRejected: '' })"
            >
              <span class="i-lucide-plus h-3 w-3" />
              添加否决记录
            </button>
