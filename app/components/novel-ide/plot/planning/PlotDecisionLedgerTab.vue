...
<template>
  ...
  <div class="flex h-full min-h-0 text-[var(--text-secondary)]">
    ...
    <div class="flex w-[320px] shrink-0 flex-col border-r border-[var(--border-color)] bg-[var(--bg-sidebar)]">
      ...
      <div class="flex shrink-0 items-center justify-between gap-2 border-b border-[var(--border-color)] px-3 py-2">
        <div class="flex min-w-0 items-center gap-1.5">
          <span class="i-lucide-gavel h-3.5 w-3.5 shrink-0 text-[var(--text-muted)]" />
        ...
        <button
          type="button"
          class="inline-flex shrink-0 items-center gap-1 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-2 py-1 text-[11px] font-medium text-[var(--text-main)] transition-colors hover:bg-[var(--bg-hover)]"
          @click="openCreate"
        >
      ...
      <div
        v-if="listError"
        class="border-b border-[var(--status-danger-border)] bg-[var(--status-danger-bg)] px-3 py-2 text-[11px] text-[var(--status-danger)]"
      >
        <div class="flex items-center justify-between gap-2">
          <span class="min-w-0">{{ listError }}</span>
          <button
            type="button"
            class="shrink-0 rounded-md border border-[var(--status-danger-border)] px-2 py-0.5 text-[10px] font-semibold text-[var(--status-danger)] transition-colors hover:bg-[var(--status-danger-bg)]"
            @click="loadDecisions()"
          >
            重试
          </button>
      ...
      <div class="custom-scrollbar min-h-0 flex-1 overflow-y-auto px-2 py-2">
        <div
          v-if="loadingList && decisions.length === 0"
          class="flex items-center justify-center py-8 text-[var(--text-muted)]"
        >
          <span class="i-lucide-loader-circle h-4 w-4 animate-spin" />
        </div>
        <div
          v-else-if="decisions.length === 0 && !listError"
          class="px-2 py-8 text-center"
        >
          <span class="i-lucide-gavel mx-auto block h-7 w-7 text-[var(--text-muted)] opacity-50" />
          <div class="mt-2 text-[12px] font-medium text-[var(--text-main)]">
            还没有决策记录
          </div>
          <div class="mt-1 text-[11px] leading-5 text-[var(--text-muted)]">
            用 ADR 式记录待决问题;拍板后留下结论、动机与风险,防止 writer 把未决问题写死。
          </div>
          <button
            type="button"
            class="mt-3 inline-flex items-center gap-1 rounded-md border border-transparent bg-[var(--accent-main)] px-3 py-1.5 text-[11px] font-medium text-[var(--text-inverse)] transition-all hover:opacity-90"
            @click="openCreate"
          >
            <span class="i-lucide-plus h-3 w-3" />
            新建决策
        ...
        <template v-else>
          ...
          >
            ...
            <button
              ...
              type="button"
              class="flex w-full items-center gap-1 px-1 py-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-[var(--text-muted)] transition-colors hover:text-[var(--text-main)]"
              @click="closedExpanded = !closedExpanded"
            >
              <span
                class="i-lucide-chevron-right h-3 w-3 transition-transform"
                :class="closedExpanded ? 'rotate-90' : ''"
              />
            ...
            <div
              v-else
              class="flex items-center gap-1 px-1 py-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-[var(--text-muted)]"
            >
              <span>{{ group.label }}</span>
              <span class="font-normal">{{ group.rows.length }}</span>
            </div>
            ...
            <div
              v-if="!group.collapsible || closedExpanded"
              class="space-y-1"
            >
              <button
              ...
              >
                <div class="flex items-center gap-1.5">
                  <span
                    ...
                    :class="PLANNING_TONE_CLASSES[DECISION_STATUS_META[row.status].tone].chip"
                  >{{ DECISION_STATUS_META[row.status].label }}</span>
                ...
                <div class="mt-1 flex flex-wrap items-center gap-1">
                  <span
                    class="inline-flex max-w-full items-center gap-1 rounded px-1.5 py-px text-[10px]"
                    :class="PLANNING_TONE_CLASSES.muted.chip"
                  >
                    <span class="i-lucide-anchor h-2.5 w-2.5 shrink-0" />
                  ...
                  <template v-if="row.deadlineChapterId">
                    <span
                      v-if="row.deadlineChapter"
                      class="inline-flex min-w-0 items-center gap-1 text-[10px] text-[var(--text-muted)]"
                    >
                      <span class="i-lucide-flag h-2.5 w-2.5 shrink-0" />
                      <span class="truncate">{{ decisionDeadlineName(row, nameMaps) }}</span>
                    </span>
                    <span
                      v-else
                      class="inline-flex items-center rounded px-1.5 py-px text-[10px]"
                      :class="PLANNING_TONE_CLASSES.danger.chip"
                    >期限章已删除</span>
                  </template>
                </div>
              </button>
    ...
    <div class="flex min-w-0 flex-1 flex-col bg-[var(--bg-main)]">
      ...
      >
        <div class="text-center">
          <span class="i-lucide-gavel mx-auto block h-8 w-8 text-[var(--text-muted)] opacity-40" />
          <div class="mt-2 text-[12px] text-[var(--text-muted)]">
            选择左侧决策查看 ADR 详情
          </div>
        </div>
      </div>
      <template v-else>
          ...
          <div class="flex items-start justify-between gap-3">
            <div class="min-w-0">
              <div class="flex min-w-0 items-center gap-2">
                ...
                <span
                  ...
                  :class="PLANNING_TONE_CLASSES[DECISION_STATUS_META[detail.status].tone].chip"
                >{{ DECISION_STATUS_META[detail.status].label }}</span>
              </div>
              <div
                v-if="detail"
                class="mt-1 flex flex-wrap items-center gap-x-2.5 gap-y-1 text-[11px] text-[var(--text-muted)]"
              >
                <span class="font-mono">{{ detail.name }}</span>
                <!-- 锚点 chip:kind=scene 时可点击跳回线程规划 -->
                <button
                  v-if="detail.anchorKind === 'scene' && detail.anchorTargetId"
                  type="button"
                  class="inline-flex min-w-0 items-center gap-1 rounded px-1.5 py-px text-[10px] transition-opacity hover:opacity-80"
                  :class="PLANNING_TONE_CLASSES.info.chip"
                  title="跳到该场景(切回线程规划)"
                  @click="emit('selectScene', detail.anchorTargetId)"
                >
                  <span class="i-lucide-anchor h-2.5 w-2.5 shrink-0" />
                ...
                <span
                  v-else
                  class="inline-flex min-w-0 items-center gap-1 rounded px-1.5 py-px text-[10px]"
                  :class="PLANNING_TONE_CLASSES.muted.chip"
                >
                  <span class="i-lucide-anchor h-2.5 w-2.5 shrink-0" />
                ...
                <template v-if="detail.deadlineChapterId">
                  <span
                    v-if="detail.deadlineChapter"
                    class="inline-flex items-center gap-1"
                  >
                    <span class="i-lucide-flag h-3 w-3" />
                    期限 {{ decisionDeadlineName(detail, nameMaps) }}
                  </span>
                  <span
                    v-else
                    class="inline-flex items-center gap-1 text-[var(--status-danger)]"
                  >
                    <span class="i-lucide-flag-off h-3 w-3" />
                    期限章已删除
                  </span>
                </template>
            ...
            >
              <button
                v-if="canDecide"
                type="button"
                class="inline-flex items-center gap-1 rounded-md border border-transparent bg-[var(--accent-main)] px-2.5 py-1 text-[11px] font-medium text-[var(--text-inverse)] transition-all hover:opacity-90 hover:shadow-md active:scale-95"
                @click="openDecide"
              >
                <span class="i-lucide-gavel h-3 w-3" />
                拍板
              </button>
              <button
                type="button"
                class="inline-flex items-center gap-1 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-2.5 py-1 text-[11px] font-medium text-[var(--text-main)] transition-colors hover:bg-[var(--bg-hover)]"
                @click="openEdit"
              >
              ...
              <button
                ...
                @click="openDrop"
              >
                <span class="i-lucide-ban h-3 w-3" />
                作废
              </button>
              <button
                v-if="canReopen"
              ...
              <button
                type="button"
                class="flex h-6.5 w-6.5 items-center justify-center rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] text-[var(--text-muted)] transition-colors hover:border-[var(--status-danger-border)] hover:bg-[var(--status-danger-bg)] hover:text-[var(--status-danger)]"
                title="删除决策(物理删除)"
                @click="removeDecision"
              >
        ...
        <div class="custom-scrollbar min-h-0 flex-1 overflow-y-auto px-4 py-3">
          ...
          <div
            v-if="detailError"
            class="rounded-md border border-[var(--status-danger-border)] bg-[var(--status-danger-bg)] px-3 py-2 text-[11px] text-[var(--status-danger)]"
          >
            <div class="flex items-center justify-between gap-2">
          ...
          <div
            v-else-if="loadingDetail && !detail"
    ...
    <!-- 拍板对话框(仅 open 态可开) -->
    <PlotDecisionDecideDialog
      v-model:visible="decideVisible"
      :decision="detail"
      :saving="savingDecide"
      :error="decideError"
      @save="handleDecideSave"
    />
    ...
    >
      <template #footer>
        <button
          class="inline-flex items-center justify-center h-8 px-4 rounded-md text-[13px] font-medium cursor-pointer border border-[var(--border-color)] bg-[var(--bg-input)] text-[var(--text-main)] transition-colors duration-200 hover:bg-[var(--bg-hover)] active:scale-95 disabled:cursor-not-allowed disabled:opacity-50"
          :disabled="savingDrop"
      ...
      <div class="mt-1 space-y-3 px-1">
        <div class="text-[12px] text-[var(--text-secondary)]">
          作废「{{ detail?.title ?? "" }}」:问题因剧情改道不再需要回答。作废后仍可在详情中重开。
        </div>
        <div
          v-if="dropError"
          class="rounded-md border border-[var(--status-danger-border)] bg-[var(--status-danger-bg)] px-2.5 py-1.5 text-[11px] text-[var(--status-danger)]"
        >
          {{ dropError }}
        </div>
        <FormField label="失效原因(必填,写入备注)">
          <FormTextarea
            v-model="dropReason"
            :rows="3"
            placeholder="为什么这个问题不再需要回答,如「该支线已在第 12 章改道删除」"
          />
        </FormField>
      </div>
    </Dialog>
