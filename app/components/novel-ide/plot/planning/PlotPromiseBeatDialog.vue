    ...
    <div class="space-y-3 px-1 mt-1">
      ...
      <FormField label="推进指示(note)">
        <FormTextarea
          v-model="draft.note"
          :rows="2"
          placeholder="给该场 writer 的具体指示,如范围限制(可空)"
        />
      </FormField>

      <!-- autoFulfill 开关:仅 kind=payoff 时出现,默认开 -->
      <div
        v-if="draft.kind === 'payoff'"
        class="flex items-center justify-between gap-3 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-3 py-2"
      >
        <div class="min-w-0">
          <div class="text-[12px] font-medium text-[var(--text-main)]">
            自动兑现
          </div>
          <div class="text-[11px] text-[var(--text-muted)]">
            {{ draft.autoFulfill ? "保存后承诺将自动标记为已兑现" : "仅记录兑现节拍,承诺保持进行中(里程碑式兑现)" }}
          </div>
        </div>
        <button
          type="button"
        ...
        >
          <span
            class="absolute top-1/2 h-3.5 w-3.5 -translate-y-1/2 rounded-full bg-[var(--bg-panel)] shadow transition-all"
            :class="draft.autoFulfill ? 'left-[18px]' : 'left-[3px]'"
          />
        </button>
      </div>
    </div>
