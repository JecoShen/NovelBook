  ...
  >
    ...
    <div class="space-y-3 px-1 mt-1">
      ...
      <FormField :label="props.mode === 'create' ? 'name(必填,供互指引用)' : 'name(供互指引用)'">
        <FormInput
          v-model="draft.name"
          placeholder="如 silver-key-mystery(小写字母/数字/连字符)"
        />
      ...
      <FormField label="承诺内容(向读者许了什么)">
        <FormTextarea
          ...
          :rows="3"
          placeholder="读者读到什么会形成期待,如「银钥匙能打开的东西终会揭晓」"
        />
      </FormField>

      <FormField label="兑现预期(只给兑现场的 writer)">
        <FormTextarea
          v-model="draft.payoffExpectation"
          :rows="2"
          placeholder="兑现时的预期戏剧效果(可空)"
        />
      </FormField>
      ...
      <FormField label="标签">
        <TagInput
          v-model="draft.tags"
          placeholder="伏笔四词表:setup_payoff / prophecy / motif / mirror,回车添加"
        />
      </FormField>
    </div>
  </Dialog>
