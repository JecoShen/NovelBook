...
<template>
  ...
  <div
    ...
    data-testid="plot-promise-ledger"
  >
    <!-- 左:承诺列表 -->
    <aside class="flex min-h-0 w-[360px] shrink-0 flex-col border-r border-[var(--border-color)] bg-[var(--bg-panel)]/78">
      <div class="shrink-0 space-y-2.5 border-b border-[var(--border-color)] px-3 py-3">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-1.5 text-[11px] font-semibold text-[var(--text-main)]">
            ...
            <span
              v-if="listLoading"
              class="i-lucide-loader-circle h-3 w-3 animate-spin text-[var(--text-muted)]"
            />
          </div>
          <button
            type="button"
            data-testid="plot-promise-create"
        ...
        <div class="flex items-center gap-1.5">
          <div class="flex flex-1 items-center gap-0.5 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] p-0.5">
            <button
              ...
              :class="statusFilter === item.value ? 'bg-[var(--accent-bg)] font-semibold text-[var(--accent-text)]' : 'text-[var(--text-muted)] hover:text-[var(--text-main)]'"
      ...
      <div class="min-h-0 flex-1 overflow-y-auto px-2 py-2 custom-scrollbar">
        <div
          v-if="listError"
          class="space-y-2 rounded-md border border-[var(--status-danger-border)] bg-[var(--status-danger-bg)] px-3 py-2.5 text-[11px] text-[var(--status-danger)]"
        >
          <div>{{ listError }}</div>
          <button
            type="button"
            class="rounded-md border border-[var(--status-danger-border)] px-2 py-1 text-[10px] font-semibold transition-colors hover:bg-[var(--status-danger-bg)]"
            @click="loadPromises()"
          >
            重试
          </button>
        ...
        <div
          v-else-if="listLoading && promises.length === 0"
          class="flex items-center justify-center gap-2 px-4 py-10 text-[11px] text-[var(--text-muted)]"
        >
        ...
        <!-- 空账本 / 筛选无结果 -->
        <div
          v-else-if="visiblePromises.length === 0"
          class="flex flex-col items-center gap-2 px-4 py-10 text-center"
        >
          <span class="i-lucide-scroll-text h-7 w-7 text-[var(--text-muted)] opacity-50" />
          <div class="text-[12px] text-[var(--text-secondary)]">
            {{ promises.length === 0 ? "账本还是空的" : "当前筛选下没有承诺" }}
          </div>
          <div class="text-[11px] leading-relaxed text-[var(--text-muted)]">
            {{ promises.length === 0 ? "承诺是对读者的债务:许下 → 埋设 → 推进 → 兑现。" : "换一个状态或重要性筛选试试。" }}
          </div>
          <button
            v-if="promises.length === 0"
            type="button"
            class="mt-1 inline-flex h-7 items-center gap-1 rounded-md border border-transparent bg-[var(--accent-main)] px-3 text-[11px] font-medium text-[var(--text-inverse)] transition-all hover:opacity-90"
            @click="openCreateEditor"
          >
            <span class="i-lucide-plus h-3 w-3" />
            许下第一个承诺
          ...
          >
            ...
            <div class="flex min-w-0 items-center gap-2">
              <span
                ...
                :class="PLANNING_TONE_CLASSES[PROMISE_STATUS_META[promise.status].tone].dot"
                :title="PROMISE_STATUS_META[promise.status].label"
              />
              ...
              <span
                ...
                :class="PLANNING_TONE_CLASSES[PROMISE_DERIVED_STAGE_META[promise.derivedStage].tone].chip"
              >{{ PROMISE_DERIVED_STAGE_META[promise.derivedStage].label }}</span>
              <span
                class="shrink-0 rounded-full px-1.5 py-0.5 text-[10px]"
                :class="PLANNING_TONE_CLASSES[PROMISE_IMPORTANCE_META[promise.importance].tone].chip"
              >{{ PROMISE_IMPORTANCE_META[promise.importance].label }}</span>
            </div>
            ...
            <div class="mt-1.5 flex min-w-0 items-center gap-2.5 text-[10px] text-[var(--text-muted)]">
              <span class="flex shrink-0 items-center gap-1.5">
                <span
                  v-for="[kind, meta] in beatKindEntries"
                  :key="kind"
                  class="inline-flex items-center gap-0.5"
                  :class="promise.beatStats[kind] === 0 ? 'opacity-40' : ''"
                  :title="`${meta.label} × ${promise.beatStats[kind]}`"
                >
                  <span
                    class="h-3 w-3"
              ...
              <span
                class="inline-flex shrink-0 items-center gap-0.5 text-[var(--status-warning)]"
                :class="promise.beatStats.planned === 0 ? 'opacity-40' : ''"
                :title="`计划节拍 × ${promise.beatStats.planned}`"
              >
                <span class="h-1.5 w-1.5 rounded-full bg-[var(--status-warning)]" />
                {{ promise.beatStats.planned }}
              </span>
              <span
                class="inline-flex shrink-0 items-center gap-0.5 text-[var(--status-success)]"
                :class="promise.beatStats.factual === 0 ? 'opacity-40' : ''"
                :title="`事实节拍 × ${promise.beatStats.factual}`"
              >
                <span class="h-1.5 w-1.5 rounded-full bg-[var(--status-success)]" />
                {{ promise.beatStats.factual }}
              </span>
              <span
                v-if="promise.deadlineChapterId"
                class="ml-auto inline-flex min-w-0 items-center gap-0.5"
                :class="promise.deadlineChapter ? '' : 'text-[var(--status-danger)]'"
                :title="`兑现期限:${deadlineLabel(promise)}`"
              >
                <span class="i-lucide-flag h-3 w-3 shrink-0" />
