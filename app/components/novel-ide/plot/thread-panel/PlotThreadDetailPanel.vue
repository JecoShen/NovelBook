  ...
  >
    <template #header>
      <template v-if="props.detail?.scene">
        ...
        >
          <span class="i-lucide-clapperboard h-3.5 w-3.5" />
        </span>
        <span
          v-if="isDirty"
    ...
    <template #actions>
      ...
      <button
        ...
        @click="openAiDialog"
      >
        <span class="i-lucide-sparkles h-3 w-3" />
      </button>
      <button
        v-if="panelHeight > 44 && hasActivePanel"
        type="button"
    ...
    <template #default>
      ...
      >
        ...
        <div class="grid grid-cols-2 gap-2 text-[10px] text-[var(--text-muted)]">
          ...
          <!-- 节奏字段只读 chip:有值才显示 -->
          <span
            v-if="outcomeTypeLabel"
            class="truncate rounded-full border border-[var(--border-color)] bg-[var(--bg-input)] px-2 py-1"
            :title="outcomeTypeLabel"
          >{{ outcomeTypeLabel }}</span>
          <span
            v-if="pacingRoleLabel"
            class="truncate rounded-full border border-[var(--border-color)] bg-[var(--bg-input)] px-2 py-1 text-right"
            :title="pacingRoleLabel"
          >{{ pacingRoleLabel }}</span>
        </div>

        <!-- 本场服务的承诺线(promise beats):只读芯片,点击跳承诺账本聚焦该 Promise -->
        <div
          v-if="props.detail.promiseBeats.length"
          class="space-y-1"
        >
