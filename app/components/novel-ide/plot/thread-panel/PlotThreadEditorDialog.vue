...
<template>
  ...
  >
    ...
    <!-- Thread 表单 -->
    <div
      v-if="props.target === 'thread'"
      class="space-y-3 px-1 mt-1"
    >
    ...
    >
      ...
      <div class="grid grid-cols-2 gap-2">
        <FormField label="结果类型">
          <FormSelect
            v-model="sceneDraft.outcomeType"
      ...
      <section class="space-y-2 rounded-lg border border-[var(--border-color)] bg-[var(--bg-panel)] p-3">
        <div class="flex items-center justify-between gap-3">
          <div>
            <div class="text-[11px] font-semibold uppercase tracking-[0.16em] text-[var(--text-muted)]">
              Refs
            </div>
          </div>
          <button
        ...
        <div
          v-if="sceneRefsDraft.length"
          class="space-y-1.5"
        >
          <div
            v-for="refItem in sceneRefsDraft"
            :key="refItem.id"
            class="rounded-md border border-[var(--border-color)] bg-[var(--bg-panel)] px-2.5 py-2"
          >
        ...
        <div
          v-else
          class="rounded-md border border-dashed border-[var(--border-color)] bg-[var(--bg-panel)]/50 px-3 py-4 text-center text-[12px] text-[var(--text-muted)]"
        >
          当前 Scene 还没有 Ref。
        </div>
      </section>
