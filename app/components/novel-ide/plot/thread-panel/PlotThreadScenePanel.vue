...
const emit = defineEmits<{
  (e: 'selectThread' | 'selectScene' | 'editScene', payload: string): void
  (e: 'createScene' | 'editThread'): void
...
<template>
  ...
  <div class="min-h-0 flex flex-1 flex-col">
      ...
      <div class="space-y-1.5">
        <div class="flex items-center justify-between gap-2">
          ...
          <div class="flex items-center gap-1">
            ...
            >
              <span class="i-lucide-ellipsis h-3.5 w-3.5" />
            </button>
          </div>
        </div>
        ...
        <Dropdown
          :items="threadItems"
          menu-class="left-0 top-full mt-1.5 w-full"
        ...
        >
          ...
          >
            ...
            <span class="ml-2 flex items-center gap-1.5">
              ...
              <span class="i-lucide-chevron-down h-3.5 w-3.5 text-[var(--text-muted)]" />
            </span>
          </button>
        </Dropdown>
        ...
        <div
          v-if="selectedThread"
          class="flex flex-wrap items-center gap-1 text-[10px] text-[var(--text-muted)]"
        >
          <span
        ...
        <div
          v-if="selectedThread"
          class="rounded-md border border-[var(--border-color)] bg-[var(--bg-input)]/55 px-2.5 py-2"
        >
          <div class="flex items-start justify-between gap-2">
            ...
            <button
              ...
              @click="emit('editThread')"
            >
              <span class="i-lucide-square-pen h-3 w-3" />
            </button>
          </div>
        </div>
      </div>
    ...
    <!-- Scene 列表 -->
    <div
      class="min-h-0 flex-1 overflow-y-auto p-1.5 custom-scrollbar"
      @contextmenu.prevent="emit('openRootMenu', $event)"
    >
      <DragDropProvider
      ...
      <div
        v-else-if="visibleScenes.length"
        class="space-y-1.5"
      >
        <PlotThreadSortableSceneRow
      ...
      <div
        v-else
        class="flex h-full min-h-[180px] flex-col items-center justify-center gap-2 rounded-2xl border border-dashed border-[var(--border-color)] bg-[var(--bg-input)]/20 px-5 text-center"
      >
