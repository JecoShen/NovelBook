...
<template>
  ...
  >
    <button
      ...
      @click.stop
    >
      <span class="i-lucide-grip-vertical h-3 w-3" />
    </button>

    <div class="min-w-0 flex-1">
      <div class="flex items-start justify-between gap-2">
        <div class="min-w-0">
          <div class="truncate text-[12px] font-semibold leading-5 text-[var(--text-main)]">
            {{ scene.title }}
          </div>
          <div class="mt-0.5 line-clamp-2 text-[10px] leading-4 text-[var(--text-secondary)]">
            {{ scene.summary }}
          </div>
        </div>
        <div class="flex shrink-0 items-center gap-1">
          ...
          >
            <span class="i-lucide-ellipsis h-3 w-3" />
          </button>
        </div>
      </div>
      ...
      <div class="mt-1.5 flex flex-wrap items-center gap-1 text-[10px] text-[var(--text-muted)]">
        ...
        <span
          v-if="chapter"
          class="rounded-full border border-[var(--border-color)] px-1.5 py-0.5"
        >{{ chapter.numberLabel }}</span>
        <span
          v-else
          class="rounded-full border border-dashed border-[var(--border-color)] px-1.5 py-0.5"
        >未挂章</span>
        <span
          v-if="scene.purpose"
          class="rounded-full border border-[var(--border-color)] px-1.5 py-0.5"
        ...
        <span
          ...
          :class="hasWorldAnchor ? 'border-[var(--status-success-border)] bg-[var(--status-success-bg)] text-[var(--status-success)]' : 'border-dashed border-[var(--border-color)]'"
        >
