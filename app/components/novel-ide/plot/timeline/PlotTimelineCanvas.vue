...
const emit = defineEmits<{
  (e: 'selectThread' | 'selectScene' | 'selectChapter', payload: string): void
}>()
...
<template>
  ...
  <div class="overflow-auto rounded-[28px] border border-[var(--border-color)] bg-[var(--bg-panel)] shadow-[0_24px_80px_color-mix(in_srgb,var(--shadow-color)_12%,transparent)]">
    <div class="min-w-max px-4 py-4">
        ...
        >
          ...
          >
            <div
              ...
              :class="segment.kind === 'draft' ? 'text-[var(--text-muted)]' : 'opacity-80'"
            >
      ...
      <!-- 泳道列表 -->
      <div
        v-if="props.timeline.lanes.length"
        class="space-y-4"
      >
        <PlotTimelineLane
      ...
      <div
        v-else
        class="flex min-h-[340px] items-center justify-center rounded-[24px] border border-dashed border-[var(--border-color)] bg-[var(--bg-input)]/70 px-6 py-10 text-center"
      >
        <div class="max-w-[320px]">
          <div class="mx-auto flex h-14 w-14 items-center justify-center rounded-full border border-[var(--border-color)] bg-[var(--bg-panel)] text-[var(--text-muted)]">
            <span class="i-lucide-waypoints h-6 w-6" />
          </div>
          <div class="mt-4 text-sm font-semibold text-[var(--text-main)]">
            当前 StoryPhase 还没有可展示的 Thread
          </div>
          <div class="mt-2 text-sm leading-7 text-[var(--text-secondary)]">
            时间轴按单 StoryPhase 工作。请切换到其他 Phase，或先为这个阶段补充 Thread 与 Scene。
          </div>
        </div>
      </div>
    </div>
