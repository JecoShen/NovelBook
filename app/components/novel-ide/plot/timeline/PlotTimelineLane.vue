...
<template>
  ...
  <div class="grid grid-cols-[220px_minmax(0,1fr)] gap-3">
    <!-- 左侧泳道标题 -->
    <div
      class="sticky left-0 z-10 flex min-h-[188px] flex-col justify-between rounded-2xl border bg-[var(--bg-panel)] px-4 py-4 shadow-[0_12px_36px_color-mix(in_srgb,var(--shadow-color)_8%,transparent)]"
    ...
    >
      ...
      >
        ...
        <div class="mt-3 text-base font-semibold text-[var(--text-main)]">
          {{ props.lane.thread.title }}
        </div>
        <div class="mt-2 line-clamp-3 text-sm leading-6 text-[var(--text-secondary)]">
          {{ props.lane.thread.summary }}
        </div>
      </button>
    ...
    <div class="overflow-x-hidden">
      ...
      >
        ...
        >
          <div class="mb-2 text-[10px] uppercase tracking-[0.16em] text-[var(--text-muted)]">
            {{ slot.label }}
          </div>

          <div
            v-if="props.lane.slotCards[slotIndex]?.length"
            class="space-y-2"
          >
