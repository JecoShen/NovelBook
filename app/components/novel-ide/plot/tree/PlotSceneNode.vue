...
<template>
  ...
  >
    <Handle
      ...
      class="!h-3 !w-3 !border-[var(--border-strong)] !bg-[var(--border-strong)]"
    />
    <Handle
      type="source"
      :position="Position.Right"
      class="!h-3 !w-3 !border-[var(--accent-main)] !bg-[var(--accent-main)]"
    />
    ...
    <!-- Scene 内容 -->
    <div class="mt-3 text-sm font-semibold leading-6 text-[var(--text-main)]">
      {{ props.data.scene.title }}
    </div>
    <div class="mt-2 line-clamp-3 text-xs leading-6 text-[var(--text-secondary)]">
      {{ props.data.scene.summary }}
    </div>
