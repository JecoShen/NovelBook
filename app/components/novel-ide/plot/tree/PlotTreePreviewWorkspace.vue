      ...
      <div class="flex flex-wrap items-center justify-between gap-3 rounded-[24px] border border-[var(--border-color)] bg-[var(--bg-panel)] px-4 py-3">
        <div>
          ...
          <div class="mt-1 text-sm text-[var(--text-secondary)]">
            当前测试数据覆盖主线推进、跨线程入口 fork、游离 Scene，以及 toolbar 手动布局。
          </div>
        </div>
      ...
      <section class="rounded-[24px] border border-[var(--border-color)] bg-[var(--bg-panel)] px-4 py-4 shadow-[0_18px_50px_color-mix(in_srgb,var(--shadow-color)_8%,transparent)]">
        ...
        <template v-if="selectedScene">
          <div class="mt-3 flex flex-wrap items-center gap-2 text-xs">
            ...
            <span
              ...
              :class="selectedScene.isMainBranch ? 'border-[var(--border-accent)] bg-[var(--accent-bg)] text-[var(--accent-text)]' : 'border-[var(--border-color)] bg-[var(--bg-input)] text-[var(--text-muted)]'"
            >
              {{ selectedScene.isMainBranch ? "主线分支" : "支线分支" }}
            </span>
          </div>

          <div class="mt-4 text-sm leading-7 text-[var(--text-secondary)]">
          ...
          <div class="mt-4 grid gap-3">
            ...
            <div class="rounded-2xl border border-[var(--border-color)] bg-[var(--bg-input)] px-3 py-3">
              ...
              <div class="mt-1 text-sm text-[var(--text-main)]">
                {{ selectedScene.chapterLabel ?? "未挂章" }}
              </div>
            </div>
          </div>
        </template>
        ...
        <template v-else-if="selectedThread">
          <div class="mt-3 flex flex-wrap items-center gap-2 text-xs">
            <span
              class="rounded-full border px-3 py-1"
              :class="selectedThread.isMainThread ? 'border-[var(--border-accent)] bg-[var(--accent-bg)] text-[var(--accent-text)]' : 'border-[var(--border-color)] bg-[var(--bg-input)] text-[var(--text-muted)]'"
            >
