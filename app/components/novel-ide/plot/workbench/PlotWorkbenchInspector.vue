  ...
  <aside class="flex min-h-0 w-[380px] shrink-0 flex-col border-l border-[var(--border-color)] bg-[var(--bg-panel)]/88">
    <div class="min-h-0 flex-1 overflow-y-auto px-3 py-3 custom-scrollbar">
      <div class="mb-4 flex items-center justify-between gap-3">
        <div class="min-w-0">
          <div class="text-[11px] font-medium uppercase tracking-[0.14em] text-[var(--text-muted)]">
            当前编辑：{{ props.mode }}
          </div>
          <div class="mt-0.5 truncate text-[13px] font-semibold text-[var(--text-main)]">
            {{ currentTitle }}
          </div>
        </div>
      ...
      <section
        v-if="props.mode === 'thread' && props.thread"
        class="space-y-3"
      >
        <div class="grid grid-cols-[minmax(0,1fr)_112px] gap-2">
          <FormField label="标题">
            <FormInput
              :model-value="props.thread.title"
              placeholder="Thread 标题"
              @update:model-value="updateThread({ title: $event })"
            />
          </FormField>
          <FormField label="状态">
            <FormSelect
              :model-value="props.thread.status"
              :options="threadStatusOptions"
              @update:model-value="updateThread({ status: $event as StoryThreadStatusDto })"
            />
          </FormField>
        </div>
        <FormField label="线型(MICE)">
          <FormSelect
            :model-value="props.thread.miceType ?? ''"
            :options="THREAD_MICE_TYPE_OPTIONS"
            @update:model-value="updateThread({ miceType: ($event || null) as StoryThreadMiceTypeDto | null })"
          />
        </FormField>
        <FormField label="摘要">
      ...
      <section
        v-else-if="props.mode === 'scene' && props.scene"
        class="space-y-3"
      >
        <div class="grid grid-cols-[minmax(0,1fr)_112px] gap-2">
          <FormField label="标题">
            <FormInput
              :model-value="props.scene.title"
              placeholder="Scene 标题"
              @update:model-value="updateScene({ title: $event })"
            />
          </FormField>
          <FormField label="状态">
            <FormSelect
              :model-value="props.scene.status"
              :options="sceneStatusOptions"
              @update:model-value="updateScene({ status: $event as StorySceneStatusDto })"
            />
          </FormField>
        </div>
        <div class="grid grid-cols-1 gap-2">
          <FormField label="所属章节">
            <FormSelect
              :model-value="props.scene.chapterId ?? ''"
              :options="chapterOptions"
              @update:model-value="updateScene({ chapterId: $event || null })"
            />
          </FormField>
        </div>
        ...
        <div class="grid grid-cols-2 gap-2">
          <FormField label="结果类型">
            <FormSelect
              :model-value="props.scene.outcomeType ?? ''"
              :options="SCENE_OUTCOME_TYPE_OPTIONS"
              @update:model-value="updateScene({ outcomeType: ($event || null) as StorySceneOutcomeTypeDto | null })"
            />
          </FormField>
          <FormField label="节奏角色">
            <FormSelect
              :model-value="props.scene.pacingRole ?? ''"
              :options="SCENE_PACING_ROLE_OPTIONS"
              @update:model-value="updateScene({ pacingRole: ($event || null) as StoryScenePacingRoleDto | null })"
            />
          </FormField>
        </div>
        <!-- 本场服务的承诺线(promise beats):只读芯片,点击跳承诺账本聚焦;节拍增删在账本侧 -->
        <div
          v-if="(props.scenePromiseBeats ?? []).length"
          class="space-y-1"
        >
          <div class="text-[10px] font-medium uppercase tracking-[0.14em] text-[var(--text-muted)]">
            服务承诺
          </div>
        ...
        <FormField label="摘要">
          ...
          />
          <div class="mt-1 text-right text-[10px] text-[var(--text-muted)]">
            {{ props.scene.summary.length }}/5000
          </div>
        </FormField>
        <FormField label="目的">
          <StructuredTextEditor
        ...
        <div class="mt-4 space-y-3 border-t border-[var(--border-color)] pt-4">
          <div class="flex items-center justify-between">
            ...
            <button
              type="button"
              class="inline-flex h-7 items-center gap-1.5 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-2.5 text-[11px] text-[var(--text-main)] hover:bg-[var(--bg-hover)]"
              @click="worldContextVisible = !worldContextVisible"
            >
              <span class="i-lucide-orbit h-3.5 w-3.5" />
              上下文
            </button>
          </div>
          <div class="grid grid-cols-1 gap-2">
            <FormField label="开始时间">
              <FormInput
                :model-value="props.scene.worldAnchor.startTime ?? ''"
                placeholder="项目日历时间，留空表示未连接"
                @update:model-value="updateWorldAnchor({ startTime: $event.trim() || null, startInstant: null })"
              />
            </FormField>
            <FormField label="结束时间">
              <FormInput
                :model-value="props.scene.worldAnchor.endTime ?? ''"
                placeholder="项目日历时间，留空表示未连接"
                @update:model-value="updateWorldAnchor({ endTime: $event.trim() || null, endInstant: null })"
              />
            </FormField>
          </div>
          <FormField label="出场 Subjects">
            <SubjectMultiSelect
              ...
              :model-value="props.scene.worldAnchor.subjectIds"
              @update:model-value="updateWorldAnchor({ subjectIds: $event })"
            />
          </FormField>
          <FormField label="地点">
            <SubjectSingleSelect
              :project-root="props.projectRoot"
              :model-value="props.scene.worldAnchor.locationSubjectId"
              @update:model-value="updateWorldAnchor({ locationSubjectId: $event })"
            />
          </FormField>
          <div
            v-if="props.scene.worldAnchor.unresolvedSubjectIds.length"
            class="rounded-md border border-[var(--status-warning-border)] bg-[var(--status-warning-bg)] px-2.5 py-2 text-[11px] leading-relaxed text-[var(--status-warning)]"
          >
            <div class="mb-1 flex items-center gap-1 font-semibold">
              <span class="i-lucide-alert-triangle h-3.5 w-3.5" />
              World Engine subject 尚未接入
            </div>
            <div class="break-all font-mono">
              {{ props.scene.worldAnchor.unresolvedSubjectIds.join("，") }}
            </div>
          </div>
          <WorldEngineContextPanel
            v-if="worldContextVisible"
            :project-root="props.projectRoot"
      ...
      <section
        v-else
        class="rounded-lg border border-dashed border-[var(--border-color)] bg-[var(--bg-input)]/30 px-4 py-8 text-center text-[12px] text-[var(--text-muted)]"
      >
        请选择一个对象开始编辑。
      </section>
      ...
      >
        <div class="space-y-2">
          <div class="flex items-center justify-between text-[10px] font-medium uppercase tracking-[0.14em] text-[var(--text-muted)]">
            ...
            <button
              type="button"
              class="inline-flex items-center gap-0.5 text-[var(--text-secondary)] transition-colors hover:text-[var(--text-main)]"
              @click="addManualRef"
            >
          ...
          <div
            v-if="manualRefsDraft.length"
            class="space-y-2"
          >
            <div
              v-for="refItem in manualRefsDraft"
              :key="refItem.id"
              class="relative"
              :class="activeRefId === refItem.id ? 'z-50' : 'z-10'"
            >
              <!-- DISPLAY CARD -->
              <div
              ...
              >
                <div class="flex items-start gap-2">
                  <span
                    class="mt-0.5 shrink-0 text-[14px] text-[var(--accent-main)] opacity-90"
                    :class="getTargetIcon(refItem.target)"
                  />
                  <div class="min-w-0 flex-1">
                    <div
                      class="flex items-center gap-1.5"
                      :title="refItem.target || '尚未选择目标路径'"
                    >
                    ...
                    <div
                      v-if="getTargetDescription(refItem.target)"
                      class="mt-1 text-[11px] leading-relaxed text-[var(--text-secondary)] line-clamp-2"
                    >
                      {{ getTargetDescription(refItem.target) }}
                    </div>
                    <div
                      v-if="refItem.note"
                      class="mt-1.5 rounded-md bg-[var(--bg-sidebar)] px-2 py-1 text-[11px] leading-relaxed text-[var(--text-main)]"
                    >
                      {{ refItem.note }}
                    </div>
                  </div>
                ...
                <div class="absolute right-1.5 top-1.5 flex opacity-0 transition-opacity group-hover:opacity-100">
                  <button
                    type="button"
                    class="flex h-5 w-5 items-center justify-center rounded-md text-[var(--text-muted)] hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)]"
                    title="删除"
                    @click.stop="removeManualRef(refItem.id)"
                  >
          ...
          <div
            v-else
            class="rounded-md border border-dashed border-[var(--border-color)] bg-transparent px-3 py-3 text-center text-[11px] text-[var(--text-muted)]"
          >
            暂无手动引用
          </div>
        </div>
        ...
        <div class="space-y-2">
          ...
          <div
            v-if="visibleRefGroups.length"
            class="space-y-2.5"
          >
            <div
              v-for="group in visibleRefGroups"
              :key="group.kind"
              class="space-y-1"
            >
              ...
              <div class="space-y-1.5">
                <div
                  v-for="refItem in group.items"
                  :key="refItem.id"
                  class="flex flex-col gap-1.5 rounded-lg border border-[var(--border-color)] bg-[var(--bg-panel)] p-2.5 transition-all hover:border-[var(--border-strong)] hover:shadow-sm"
                >
                  <div class="flex items-start gap-2">
                    <span
                      class="mt-0.5 shrink-0 text-[14px] text-[var(--accent-main)]"
                      :class="getTargetIcon(refItem.target)"
                    />
                    <div class="min-w-0 flex-1">
                      ...
                      <div class="mt-0.5 truncate font-mono text-[9px] text-[var(--text-muted)] opacity-60">
                        {{ refItem.target }}
                      </div>
                      <div class="mt-1 flex items-center gap-1.5">
                        <span class="i-lucide-corner-down-right h-3 w-3 shrink-0 text-[var(--text-muted)] opacity-50" />
