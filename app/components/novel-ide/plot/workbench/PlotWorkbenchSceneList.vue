...
<template>
  ...
  <main class="min-w-0 flex-1 overflow-y-auto bg-transparent px-3 py-3 custom-scrollbar">
    <section
      v-if="props.thread"
      class="mb-3 flex flex-col overflow-hidden rounded-lg border border-[var(--border-color)] bg-[var(--bg-panel)] shadow-sm xl:flex-row"
    >
      <!-- Thread 摘要区 -->
      <div class="flex min-w-0 flex-1 flex-col p-3.5">
        <div class="flex flex-wrap items-center gap-2.5">
          <span class="flex h-7 w-7 shrink-0 items-center justify-center rounded-[6px] border border-[var(--border-accent)] bg-[var(--accent-bg)] text-[var(--accent-main)]">
            <span class="i-lucide-drama h-4 w-4" />
          </span>
          <h2 class="min-w-0 truncate text-[15px] font-semibold text-[var(--text-main)]">
            {{ props.thread.title }}
          </h2>
        ...
        <div class="mt-3 flex flex-col gap-1.5 text-[12px] leading-relaxed text-[var(--text-secondary)]">
          <div class="flex items-start gap-2">
            <span class="i-lucide-align-left mt-[3px] h-3.5 w-3.5 shrink-0 text-[var(--text-muted)] opacity-70" />
            <p class="min-w-0 flex-1">
              {{ displayInlineText(props.thread.summary) }}
            </p>
          </div>
          <div class="flex items-start gap-2">
            <span class="i-lucide-lightbulb mt-[3px] h-3.5 w-3.5 shrink-0 text-[var(--status-warning)]" />
            <p class="min-w-0 flex-1 text-[var(--text-muted)]">
              {{ displayInlineText(props.thread.writingTip ?? "保持清晰的因果链，避免场景只承担说明功能。") }}
            </p>
          </div>
        </div>
      </div>
      ...
      <!-- 分割线 -->
      <div class="h-[1px] w-full bg-[var(--border-color)] xl:h-auto xl:w-[1px]" />
      ...
      <div class="flex w-full shrink-0 flex-col bg-[var(--bg-main)]/20 p-3.5 xl:w-[320px]">
        <div class="mb-3 flex items-center justify-between">
          <div class="flex items-center gap-1.5">
            <div class="flex h-5 w-5 items-center justify-center rounded bg-[var(--bg-panel)] shadow-sm ring-1 ring-inset ring-[var(--border-color)]">
              <span class="i-lucide-target h-3 w-3 text-[var(--status-info)]" />
            </div>
        ...
        <div class="flex flex-col gap-2 rounded-md border border-[var(--border-color)] bg-[var(--bg-panel)] p-2.5 shadow-sm">
          <div class="flex items-start justify-between gap-2">
            ...
            <span
              v-if="focusScene"
              class="shrink-0 rounded-[4px] border px-1.5 py-0.5 text-[10px] font-medium"
              :class="sceneStatusClass[focusScene.status]"
            >
          ...
          <div class="flex flex-wrap items-center gap-1.5 text-[10.5px]">
            <span class="flex items-center gap-1 rounded bg-[var(--bg-main)] px-1.5 py-0.5 text-[var(--text-muted)] ring-1 ring-inset ring-[var(--border-color)]/50">
              <span class="i-lucide-book-open h-3 w-3 opacity-70" />
              {{ focusScene?.chapterId ? (chapterMap.get(focusScene.chapterId)?.numberLabel ?? "未识别章节") : "未挂章" }}
            </span>
            <span class="flex items-center gap-1 rounded bg-[var(--bg-main)] px-1.5 py-0.5 text-[var(--text-muted)] ring-1 ring-inset ring-[var(--border-color)]/50">
              <span class="i-lucide-hash h-3 w-3 opacity-70" />
              {{ focusScene ? `#${focusScene.threadSortOrder + 1}` : "未排序" }}
            </span>
          </div>

          <div
            v-if="focusScene?.purpose"
            class="text-[11px] leading-relaxed text-[var(--text-secondary)]"
          >
    ...
    <div class="mt-2.5 flex flex-wrap items-center justify-end gap-1.5">
      <span
        v-if="sceneCheckMessage"
        class="mr-auto inline-flex min-h-7 items-center rounded-md border border-[var(--border-color)] bg-[var(--bg-panel)] px-2.5 text-[11px] leading-5 text-[var(--text-secondary)]"
      >
        {{ sceneCheckMessage }}
      </span>
      <button
        type="button"
        class="inline-flex h-7 items-center gap-1.5 rounded-md border border-[var(--border-color)] bg-[var(--bg-panel)] px-3 text-[11.5px] font-medium text-[var(--text-main)] shadow-sm transition-colors hover:bg-[var(--bg-hover)]"
        @click="createScene"
      >
        <span class="i-lucide-plus h-3.5 w-3.5 opacity-70" />
        Scene
      </button>
      <button
        type="button"
        class="inline-flex h-7 items-center gap-1.5 rounded-md border border-[var(--border-color)] bg-[var(--bg-panel)] px-2.5 text-[11.5px] font-medium text-[var(--text-main)] shadow-sm transition-colors hover:bg-[var(--bg-hover)]"
        @click="autoSortScenes"
      >
        <span class="i-lucide-route h-3.5 w-3.5 text-[var(--status-info)]" />
        自动排序
      </button>
      <button
        type="button"
        class="inline-flex h-7 items-center gap-1.5 rounded-md border border-[var(--border-color)] bg-[var(--bg-panel)] px-2.5 text-[11.5px] font-medium text-[var(--text-main)] shadow-sm transition-colors hover:bg-[var(--bg-hover)]"
        @click="checkSceneDependencies"
      >
        <span class="i-lucide-list-checks h-3.5 w-3.5 text-[var(--status-success)]" />
        依赖检查
      </button>
    </div>
