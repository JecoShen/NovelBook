  ...
  <aside class="relative flex min-h-0 w-[292px] shrink-0 flex-col border-r border-[var(--border-color)] bg-[var(--bg-panel)]/78">
    <div class="shrink-0 space-y-3 border-b border-[var(--border-color)] px-3 py-3">
      <div class="flex items-center gap-2 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-2.5 py-2">
        ...
        <button
          ref="filterButtonRef"
          type="button"
          class="inline-flex h-6 w-6 items-center justify-center rounded-md text-[var(--text-muted)] hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)]"
          title="筛选"
      ...
      <section>
        <div class="mb-2 flex items-center justify-between text-[11px] font-semibold text-[var(--text-main)]">
          ...
          <button
            type="button"
            class="inline-flex h-5 items-center gap-1 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-1.5 text-[10px] text-[var(--text-secondary)] hover:bg-[var(--bg-hover)]"
        ...
        >
          <div class="flex items-center justify-between gap-2">
            <span class="flex min-w-0 items-center gap-2">
              <span class="i-lucide-crown h-3.5 w-3.5 shrink-0 text-[var(--accent-main)]" />
            ...
            <span class="flex shrink-0 items-center gap-1">
              <span
                ...
                :class="threadStatusClass[selectedThread.status]"
              >
              ...
              <button
                ...
                class="inline-flex h-6 w-6 items-center justify-center rounded-md text-[var(--text-muted)] hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)]"
                title="编辑 Thread"
                @click.stop="emit('editThread', selectedThread.id)"
              >
    ...
    <div class="min-h-0 flex-1 overflow-y-auto px-3 py-3 custom-scrollbar">
      <div class="mb-2 flex items-center justify-between text-[11px] font-semibold text-[var(--text-main)]">
        ...
        <button
          ...
          @click="toggleFilterPanel"
        >
          <span class="i-lucide-sliders-horizontal h-3 w-3" />
          管理
        </button>
      </div>
      ...
      <div
        ...
        class="mb-3 space-y-1.5 border-b border-[var(--border-color)] pb-3"
      >
        <div class="text-[10px] font-semibold uppercase tracking-[0.14em] text-[var(--text-muted)]">
          Pinned
        </div>
        <button
        ...
        >
          <div class="flex items-center justify-between gap-2">
            <span class="flex min-w-0 items-center gap-2">
              <span class="i-lucide-pin h-3.5 w-3.5 shrink-0 text-[var(--accent-main)]" />
              <span class="truncate text-[12px] font-semibold text-[var(--text-main)]">{{ thread.title }}</span>
            </span>
            <span class="flex shrink-0 items-center gap-1">
              <span
                class="rounded-full px-1.5 py-0.5 text-[10px] font-semibold"
                :class="thread.isMainThread ? 'workbench-main-chip' : PLOT_THREAD_TONE_STYLES[thread.tone].chipClass"
              >
                {{ thread.isMainThread ? "主线" : "支线" }}
              </span>
              <button
                type="button"
                class="inline-flex h-5 w-5 items-center justify-center rounded-md text-[var(--text-muted)] hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)]"
                title="编辑 Thread"
                @click.stop="emit('editThread', thread.id)"
              >
      ...
      <div class="space-y-1.5">
        ...
        >
          <div class="flex items-center justify-between gap-2">
            <span class="flex min-w-0 items-center gap-2">
              <span
                :class="thread.isMainThread ? 'i-lucide-crown text-[var(--accent-main)]' : 'i-lucide-waypoints text-[var(--text-muted)]'"
    ...
    <transition name="fade">
      <div
        ...
        ref="filterPanelRef"
        class="absolute left-[calc(100%+8px)] top-3 z-40 w-[260px] rounded-xl border border-[var(--border-color)] bg-[var(--bg-panel)] p-3 shadow-2xl"
      >
        ...
        <div class="mt-3 space-y-1.5">
          <div class="flex items-center justify-between text-[10px] font-semibold uppercase tracking-[0.14em] text-[var(--text-muted)]">
            ...
            <button
              type="button"
              class="text-[10px] text-[var(--text-muted)] hover:text-[var(--text-main)]"
          ...
          <div class="grid grid-cols-2 gap-1.5 text-[11px]">
            ...
            <div class="rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-2 py-1.5">
              草稿 {{ statusCounts.draft ?? 0 }}
            </div>
            <div class="rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-2 py-1.5">
              暂停 {{ statusCounts.paused ?? 0 }}
            </div>
            <div class="rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-2 py-1.5">
              完成 {{ statusCounts.done ?? 0 }}
            </div>
          </div>
        </div>
        ...
        <div class="mt-3 border-t border-[var(--border-color)] pt-3">
          ...
          <div class="mt-1 max-h-28 space-y-1 overflow-y-auto pr-1 custom-scrollbar">
            <div
              v-for="thread in props.threads.filter((item) => pinnedThreadSet.has(item.id))"
              :key="thread.id"
              class="flex items-center justify-between rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-2 py-1.5 text-[11px]"
            >
              <span class="min-w-0 truncate">{{ thread.title }}</span>
              <button
                type="button"
                class="text-[var(--text-muted)] hover:text-[var(--text-main)]"
                @click="emit('toggleThreadPin', thread.id)"
              >
                取消
              </button>
            </div>
            <div
              v-if="!props.threads.some((item) => pinnedThreadSet.has(item.id))"
              class="rounded-md border border-dashed border-[var(--border-color)] px-2 py-2 text-center text-[10px] text-[var(--text-muted)]"
            >
              还没有 Pin 的 Thread
            </div>
          </div>
