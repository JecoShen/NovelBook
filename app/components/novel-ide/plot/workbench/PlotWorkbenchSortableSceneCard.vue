    ...
    <div class="flex items-start gap-3 p-3.5">
      ...
      <div class="flex shrink-0 flex-col items-center gap-2 pt-0.5">
        ...
        <button
          ref="handleRef"
          type="button"
          class="flex cursor-grab items-center justify-center rounded py-1 opacity-40 transition-opacity hover:bg-[var(--bg-hover)] hover:opacity-100 group-hover:opacity-70 active:cursor-grabbing"
          title="拖拽排序 Scene"
          @click.stop
        >
          <span class="i-lucide-grip-vertical h-4 w-4 text-[var(--text-muted)]" />
        </button>
      </div>
      ...
      <div class="flex min-w-0 flex-1 flex-col">
        <div class="flex items-start justify-between gap-4">
          ...
          <div class="flex min-w-0 flex-col gap-1.5">
            <div class="flex items-center gap-2.5">
              <h3 class="truncate text-[15px] font-semibold tracking-tight text-[var(--text-main)]">
                {{ props.scene.title }}
              </h3>
              <span
                class="shrink-0 rounded-md border px-1.5 py-0.5 text-[10px] font-medium"
                :class="sceneStatusClass[props.scene.status]"
              >
            ...
            <div class="flex flex-wrap items-center gap-x-3 gap-y-1 text-[11.5px] font-medium text-[var(--text-muted)]">
              <span class="flex items-center gap-1.5">
                <span class="i-lucide-book-open h-3.5 w-3.5 opacity-70" />
                {{ props.chapter ? props.chapter.numberLabel : "未挂章" }}
              </span>
              <span class="flex items-center gap-1.5">
                <span class="i-lucide-hash h-3.5 w-3.5 opacity-70" />
          ...
          <div class="flex shrink-0 items-center gap-0.5 opacity-70 transition-opacity group-hover:opacity-100">
            <button
              type="button"
              class="inline-flex h-7 w-7 items-center justify-center rounded-md text-[var(--text-muted)] hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)] disabled:cursor-not-allowed disabled:opacity-35 disabled:hover:bg-transparent disabled:hover:text-[var(--text-muted)]"
              title="上移 Scene"
              :disabled="!props.canMoveUp"
              @click.stop="emit('moveScene', { sceneId: props.scene.id, direction: 'up' })"
            >
            ...
            <button
              ...
              @click.stop="emit('moveScene', { sceneId: props.scene.id, direction: 'down' })"
            >
            ...
            <button
              ...
              title="编辑 Scene"
              @click.stop="emit('editScene', props.scene.id)"
            >
        ...
        <div class="mt-2 flex flex-wrap items-center gap-1.5 text-[11px]">
          <template v-if="hasWorldAnchor">
            ...
            <span
              v-if="props.scene.worldAnchor.locationSubjectId"
              class="inline-flex items-center gap-1 rounded px-1.5 py-0.5 ring-1 ring-inset"
              :class="locationResolved ? 'bg-[var(--status-success-bg)] text-[var(--status-success)] ring-[var(--status-success-border)]' : 'bg-[var(--status-warning-bg)] text-[var(--status-warning)] ring-[var(--status-warning-border)]'"
            >
            ...
            <span
              ...
              class="inline-flex items-center gap-1 rounded bg-[var(--status-warning-bg)] px-1.5 py-0.5 text-[var(--status-warning)] ring-1 ring-inset ring-[var(--status-warning-border)]"
            >
          ...
          <span
            v-else
            class="inline-flex items-center gap-1 rounded bg-[var(--bg-main)] px-1.5 py-0.5 text-[var(--text-muted)] ring-1 ring-inset ring-[var(--border-color)]/50"
          >
            <span class="i-lucide-unlink h-3 w-3" />
            未连接到世界引擎
          </span>
        </div>
        ...
        <!-- 场景摘要框 -->
        <div
          v-if="props.scene.summary"
          class="mt-3 rounded-lg bg-[var(--bg-main)]/40 transition-colors group-hover:bg-[var(--bg-main)]/60"
        >
          <div class="px-3 py-2.5">
            <p class="whitespace-pre-wrap text-[13px] leading-relaxed text-[var(--text-secondary)]">
              {{ displayInlineText(props.scene.summary) }}
            </p>
          </div>
        </div>
        <div
          v-else
          class="mt-3 flex cursor-pointer items-center gap-2 rounded-lg border border-dashed border-[var(--border-color)] bg-transparent px-3 py-2 text-[12px] text-[var(--text-muted)] opacity-0 transition-all hover:bg-[var(--bg-main)]/30 hover:text-[var(--text-main)] group-hover:opacity-100"
          @click.stop="emit('editScene', props.scene.id)"
        >
