...
<template>
  ...
  <div class="space-y-2">
    <div class="flex min-h-8 items-center justify-between gap-2 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-2 py-1.5">
      <div class="min-w-0">
        <div class="truncate text-[12px] text-[var(--text-main)]">
          {{ selectedSubject?.name ?? "未选择地点" }}
        </div>
        <div
          v-if="selectedSubject"
          class="truncate font-mono text-[10px] text-[var(--text-muted)]"
        >
          {{ selectedSubject.type }} · {{ selectedSubject.id }}
        </div>
      </div>
      <button
        v-if="selectedSubject"
        type="button"
        class="inline-flex h-6 w-6 shrink-0 items-center justify-center rounded-md text-[var(--text-muted)] hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)]"
        title="清空地点"
    ...
    <div class="max-h-36 overflow-y-auto rounded-md border border-[var(--border-color)] bg-[var(--bg-panel)] custom-scrollbar">
      ...
      >
        <span
          ...
          :class="props.modelValue === subject.id ? 'i-lucide-map-pin-check text-[var(--accent-main)]' : 'i-lucide-map-pin text-[var(--text-muted)]'"
        />
      ...
      <div
        ...
        class="px-2 py-3 text-center text-[11px] text-[var(--text-muted)]"
      >
        暂无 location subject
      </div>
      <div
