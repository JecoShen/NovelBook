  ...
  <div class="rounded-md border border-[var(--border-color)] bg-[var(--bg-input)]/35 p-2.5">
    <div class="mb-2 flex items-center justify-between gap-2">
      ...
      <div class="flex shrink-0 items-center gap-1">
        <button
          type="button"
          data-testid="plot-world-context-open-workbench"
          class="inline-flex h-6 items-center gap-1 rounded-md px-2 text-[11px] text-[var(--text-secondary)] hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)]"
          @click="emit('openWorldEngine')"
        >
          <span class="i-lucide-external-link h-3 w-3" />
          打开
        </button>
        <button
          type="button"
          class="inline-flex h-6 items-center gap-1 rounded-md px-2 text-[11px] text-[var(--text-secondary)] hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)]"
          :disabled="loading"
          @click="void loadContext()"
        >
          <span
            class="i-lucide-refresh-cw h-3 w-3"
    ...
    <div
      v-if="loading"
      class="py-4 text-center text-[11px] text-[var(--text-muted)]"
    >
      正在查询...
    </div>
    <div
      v-else-if="error"
      class="rounded-md border border-[var(--status-danger-border)] bg-[var(--status-danger-bg)] px-2 py-2 text-[11px] leading-relaxed text-[var(--status-danger)]"
    >
      {{ error }}
    </div>
    <div
    ...
    >
      <div
        v-if="unresolvedSubjectIds.length"
        class="rounded-md border border-[var(--status-warning-border)] bg-[var(--status-warning-bg)] px-2 py-2 text-[11px] leading-relaxed text-[var(--status-warning)]"
      >
        ...
        <div class="break-all text-center font-mono">
          {{ unresolvedSubjectIds.join("，") }}
        </div>
      </div>
      <div class="rounded-md border border-dashed border-[var(--border-color)] px-2 py-4 text-center text-[11px] text-[var(--text-muted)]">
        暂无匹配的 slices 或 subject 状态
      </div>
    </div>
    <div
      v-else
    ...
    >
      ...
      <section class="space-y-1.5">
        <div class="text-[10px] font-medium uppercase tracking-[0.12em] text-[var(--text-muted)]">
          Slices
        </div>
        <div
          v-if="context?.slices.length"
          class="space-y-1.5"
        >
          <div
            v-for="slice in context.slices"
            :key="slice.id"
            class="rounded-md border border-[var(--border-color)] bg-[var(--bg-panel)] px-2 py-1.5"
          >
            ...
            <div
              v-if="slice.summary"
              class="mt-1 line-clamp-2 text-[11px] leading-relaxed text-[var(--text-secondary)]"
            >
              {{ slice.summary }}
            </div>
          </div>
      ...
      <section class="space-y-1.5">
        ...
        <div
          v-if="context?.subjectStates.length"
          class="space-y-1.5"
        >
          <div
            v-for="subject in context.subjectStates"
          ...
          >
            ...
            <div class="mt-1 line-clamp-2 text-[11px] leading-relaxed text-[var(--text-secondary)]">
              {{ attrsSummary(subject.attrs) }}
            </div>
          </div>
        </div>
        <div
