  ...
  >
    <template v-if="mode === 'prepare'">
      <div class="flex gap-3 rounded-md border border-[var(--status-danger-border)] bg-[var(--status-danger-bg)] p-3 text-[var(--status-danger)]">
        <span class="i-lucide-triangle-alert mt-0.5 h-5 w-5 shrink-0" />
        <div class="space-y-1">
          <p class="text-sm font-semibold">
            {{ t("ide.profile.keys.lossWarningTitle") }}
          </p>
          <p class="text-xs leading-relaxed">
            {{ t("ide.profile.keys.lossWarning") }}
          </p>
        </div>
      </div>
      <div
      ...
      >
        ...
        <div class="rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] p-3 font-mono text-sm leading-relaxed text-[var(--text-main)] [overflow-wrap:anywhere]">
          {{ recoveryCode }}
        </div>
        <div class="flex flex-wrap gap-2">
          <button
            type="button"
            class="inline-flex h-8 items-center gap-1.5 rounded-md border border-[var(--border-color)] px-3 text-xs text-[var(--text-main)] hover:bg-[var(--bg-hover)]"
            @click="copyRecoveryCode"
          >
            <span class="i-lucide-copy h-3.5 w-3.5" />{{ t("ide.profile.keys.copy") }}
          </button>
          <button
            type="button"
            class="inline-flex h-8 items-center gap-1.5 rounded-md border border-[var(--border-color)] px-3 text-xs text-[var(--text-main)] hover:bg-[var(--bg-hover)]"
          ...
          >
            <span class="i-lucide-download h-3.5 w-3.5" />{{ t("ide.profile.keys.download") }}
          </button>
        </div>
        <p
        ...
        <label
          ...
          :class="deliverySucceeded ? 'text-[var(--text-main)]' : 'text-[var(--text-muted)]'"
        >
          <input
            v-model="acknowledged"
            type="checkbox"
            class="mt-0.5 accent-[var(--accent-main)]"
            :disabled="!deliverySucceeded"
          >
    ...
    <template v-else-if="mode === 'import'">
      <p class="text-xs">
        {{ t("ide.profile.keys.importDescription", { keyId }) }}
      </p>
      <textarea
        ...
        autocomplete="off"
        spellcheck="false"
        class="w-full resize-y rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] p-3 font-mono text-sm text-[var(--text-main)] outline-none focus:border-[var(--accent-main)]"
        :placeholder="t('ide.profile.keys.importPlaceholder')"
      />
    </template>
    ...
    <template v-else>
      ...
      >
        <p class="text-xs">
          {{ t("ide.profile.keys.exportDescription", { keyId }) }}
        </p>
        <input
          v-model="password"
          type="password"
          autocomplete="current-password"
          class="h-8 w-full rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-3 text-sm text-[var(--text-main)] outline-none focus:border-[var(--accent-main)]"
          :placeholder="t('ide.profile.keys.passwordPlaceholder')"
          @keyup.enter="exportRecoveryCode"
        >
      </div>
      <div
        v-else
      ...
      >
        <div class="flex gap-2 rounded-md border border-[var(--status-warning-border)] bg-[var(--status-warning-bg)] p-3 text-xs text-[var(--status-warning)]">
          <span class="i-lucide-key-round mt-0.5 h-4 w-4 shrink-0" />{{ t("ide.profile.keys.exportSaveReminder") }}
        </div>
    ...
    <p
      v-if="error"
      class="rounded-md border border-[var(--status-danger-border)] bg-[var(--status-danger-bg)] px-3 py-2 text-xs text-[var(--status-danger)]"
    >
      {{ error }}
    </p>
    ...
    <template #footer>
      <button
        type="button"
        class="inline-flex h-8 items-center justify-center rounded-md border border-[var(--border-color)] px-4 text-xs text-[var(--text-main)] hover:bg-[var(--bg-hover)] disabled:opacity-50"
        :disabled="busy"
        @click="close"
      >
        {{ t("common.cancel") }}
      ...
      <button
        v-if="mode === 'prepare'"
        type="button"
        class="inline-flex h-8 items-center justify-center rounded-md bg-[var(--accent-main)] px-4 text-xs font-medium text-[var(--text-inverse)] disabled:opacity-40"
        :disabled="busy || !canConfirmPreparedKey"
        @click="confirmPreparedKey"
      >
        {{ t("ide.profile.keys.confirmAndContinue") }}
      </button>
      <button
        v-else-if="mode === 'import'"
        type="button"
        class="inline-flex h-8 items-center justify-center rounded-md bg-[var(--accent-main)] px-4 text-xs font-medium text-[var(--text-inverse)] disabled:opacity-40"
        :disabled="busy || !importedCode.trim()"
        @click="importRecoveryCode"
      >
        {{ t("ide.profile.keys.importAction") }}
      </button>
      <button
        ...
        @click="exportRecoveryCode"
      >
        {{ t("ide.profile.keys.verifyAndShow") }}
      </button>
      <button
        v-else
        type="button"
        class="inline-flex h-8 items-center justify-center rounded-md bg-[var(--accent-main)] px-4 text-xs font-medium text-[var(--text-inverse)]"
        @click="close"
      >
        {{ t("common.done") }}
      </button>
