  ...
  <aside class="flex w-[360px] shrink-0 flex-col border-l border-[var(--border-color)] bg-[var(--bg-panel)]">
    <div class="flex h-12 shrink-0 items-center justify-between border-b border-[var(--border-color)] px-3">
      <div>
        ...
        <div class="text-[10px] text-[var(--text-muted)]">
          {{ selection.kind === "chunk" ? "条目与向量元数据" : selection.kind === "search" ? "召回候选详情" : "Project RAG 状态" }}
        </div>
      </div>
      <button
    ...
    <div class="min-h-0 flex-1 overflow-auto p-3">
      ...
      <template v-else-if="selection.kind === 'chunk'">
        <div class="mb-3 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] p-3">
          <div class="mb-2 text-[12px] font-semibold text-[var(--text-main)]">
            索引条目 #{{ selection.chunk.id }}
          </div>
        ...
        <div class="mb-3 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] p-3">
          ...
          <div
            v-if="!selection.chunk.vector.embeddingModel"
            class="mt-2 rounded border border-[var(--status-warning-border)] bg-[var(--status-warning-bg)] p-2 text-[11px] leading-5 text-[var(--status-warning)]"
          >
            这条缓存来自旧版索引，chunk 正文可以查看，但条目级 embedding 元数据需要重建索引后才会补齐。
          </div>
        ...
        <div class="rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] p-3">
          ...
          <div class="whitespace-pre-wrap text-[12px] leading-5 text-[var(--text-secondary)]">
      ...
      <template v-else>
        <div
          v-if="inspector.index.readError"
        ...
        >
          SQLite 读取失败：{{ inspector.index.readError }}
        </div>
        <div class="mb-3 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] p-3">
          <div class="mb-2 text-[12px] font-semibold text-[var(--text-main)]">
            当前嵌入配置
    ...
    <div class="shrink-0 border-t border-[var(--border-color)] p-3">
      ...
      <div class="grid grid-cols-2 gap-2">
        <button
          type="button"
          class="rounded-md border border-[var(--border-color)] px-2 py-1.5 text-[11px] text-[var(--text-main)] hover:bg-[var(--bg-hover)] disabled:opacity-50"
          :disabled="actionBusy || !inspector?.selectedSubjectPath"
          @click="emit('debug', 'mark-dirty', '标记当前 subject 待索引')"
        >
          标记待索引
        </button>
        <button
          ...
          class="rounded-md border border-[var(--border-color)] px-2 py-1.5 text-[11px] text-[var(--text-main)] hover:bg-[var(--bg-hover)] disabled:opacity-50"
          :disabled="actionBusy || !inspector?.selectedSubjectPath"
          @click="emit('debug', 'delete-subject-index', '删除当前 subject 索引缓存')"
        >
          删除本角色索引
        </button>
        <button
          ...
          class="rounded-md border border-[var(--border-color)] px-2 py-1.5 text-[11px] text-[var(--text-main)] hover:bg-[var(--bg-hover)] disabled:opacity-50"
          :disabled="actionBusy || !inspector?.selectedSubjectPath"
          @click="emit('debug', 'clear-index-cache-and-rebuild', '清空缓存并重建当前 subject')"
        >
          清空并重建
        </button>
        <button
          type="button"
          class="rounded-md border border-[var(--status-danger-border)] px-2 py-1.5 text-[11px] text-[var(--status-danger)] hover:bg-[var(--status-danger-bg)] disabled:opacity-50"
          :disabled="actionBusy"
          @click="emit('debug', 'clear-index-cache', '清空 Project RAG SQLite 缓存')"
        >
          清空缓存
        </button>
      </div>
      <div class="mt-2 text-[10px] leading-4 text-[var(--text-muted)]">
        这些操作只影响 RAG 缓存或 dirty state，不会修改 events.jsonl / memory.jsonl。
      </div>
    </div>
  </aside>
</template>
