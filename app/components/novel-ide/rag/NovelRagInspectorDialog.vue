...
<template>
  ...
  >
    <template #header>
      ...
      <div class="flex min-w-0 flex-1 items-center gap-3">
        <span class="rag-workbench-accent-icon">
          <span class="i-lucide-brain-circuit h-4 w-4" />
        </span>
        ...
        <button
          type="button"
          class="inline-flex h-8 w-8 items-center justify-center rounded-md text-[var(--text-muted)] hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)] disabled:opacity-50"
          title="刷新 RAG Inspector"
          :disabled="loading"
          @click="void loadInspector()"
        >
          <span
        ...
        <button
          type="button"
          class="inline-flex h-8 w-8 items-center justify-center rounded-md text-[var(--text-muted)] hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)]"
    ...
    <div class="flex min-h-0 flex-1 flex-col overflow-hidden bg-[color-mix(in_srgb,var(--bg-main)_96%,white)]">
      <div
        v-if="error"
        class="shrink-0 border-b border-[var(--status-danger-border)] bg-[var(--status-danger-bg)] px-3 py-2 text-[12px] text-[var(--status-danger)]"
      >
        {{ error }}
      </div>
    ...
    <Dialog
      v-model="confirmOpen"
      title="确认 RAG Debug 操作"
      width="420px"
      show-cancel
      :busy="actionBusy"
      @confirm="confirmDebugAction"
    >
