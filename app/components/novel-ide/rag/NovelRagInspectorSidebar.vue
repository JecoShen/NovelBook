...
<template>
  ...
  <aside class="flex w-[286px] shrink-0 flex-col border-r border-[var(--border-color)] bg-[var(--bg-panel)]/92">
    <div class="flex h-12 shrink-0 items-center justify-between border-b border-[var(--border-color)] px-3">
      ...
      <span
        :class="loading ? 'i-lucide-loader-2 animate-spin' : 'i-lucide-database'"
    ...
    <div class="min-h-0 flex-1 overflow-auto p-2">
      ...
      >
        <div class="flex items-center justify-between gap-2">
          ...
          <span
            :class="statusClass(subject)"
            class="shrink-0 text-[10px]"
          >{{ statusLabel(subject) }}</span>
        </div>
        ...
        <div
          v-if="subject.errors[0]"
          class="mt-1 truncate text-[10px] text-[var(--status-danger)]"
        >
          {{ subject.errors[0].source }}: {{ subject.errors[0].message }}
        </div>
      </button>
    </div>
  </aside>
