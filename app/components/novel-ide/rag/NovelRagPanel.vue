...
<template>
  <div class="flex h-full min-h-0 bg-[var(--bg-panel)] text-[var(--text-main)]">
    ...
    <div class="flex w-[42%] min-w-[150px] max-w-[240px] shrink-0 flex-col border-r border-[var(--border-color)]">
      <div class="border-b border-[var(--border-color)] p-2">
        <div class="flex items-center justify-between gap-2">
          <div class="text-[11px] font-semibold uppercase text-[var(--text-secondary)]">
            Subjects
          </div>
          <button
            type="button"
            class="rounded-md p-1 text-[var(--text-muted)] hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)]"
            title="刷新"
            :disabled="loadingOverview"
            @click="void loadOverview()"
          >
            <span
              :class="loadingOverview ? 'i-lucide-loader-2 animate-spin' : 'i-lucide-refresh-cw'"
              class="h-3.5 w-3.5"
            />
          </button>
        </div>
        <div class="mt-2 grid grid-cols-2 gap-1 text-[11px] text-[var(--text-muted)]">
          ...
          <div :class="sourceStatusClass(selectedSubject?.sourceStatuses)">
            {{ selectedSubject ? subjectStatusLabel(selectedSubject) : "无选中" }}
          </div>
        </div>
      </div>
      ...
      <div
        v-if="error"
        class="border-b border-[var(--status-danger-border)] bg-[var(--status-danger-bg)] p-2 text-[11px] text-[var(--status-danger)]"
      >
        {{ error }}
      </div>
      ...
      <div class="min-h-0 flex-1 overflow-auto p-2">
        <div
          v-if="!currentProjectRoot"
          class="py-8 text-center text-[12px] text-[var(--text-muted)]"
        >
          当前没有 Project Workspace。
        </div>
        <div
          v-else-if="!loadingOverview && subjects.length === 0"
          class="py-8 text-center text-[12px] text-[var(--text-muted)]"
        >
          当前 Project 暂无 subject RAG 数据。
        </div>
        <button
        ...
        >
          <div class="truncate text-[12px] font-medium text-[var(--text-main)]">
            {{ subject.subjectId }}
          </div>
    ...
    <div class="flex min-w-0 flex-1 flex-col">
      <div class="flex shrink-0 items-center justify-between gap-2 border-b border-[var(--border-color)] px-3 py-2">
        <div class="min-w-0">
          <div class="truncate text-[12px] font-semibold">
            {{ selectedSubject?.subjectId ?? "RAG" }}
          </div>
          <div class="truncate text-[10px] text-[var(--text-muted)]">
            {{ selectedSubjectPath || "请选择 subject" }}
          </div>
        </div>
        <div class="flex shrink-0 items-center gap-1">
          <button
            type="button"
            class="rounded-md p-1 text-[var(--text-muted)] hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)] disabled:opacity-50"
            title="重建当前 subject 索引"
            :disabled="!selectedSubjectPath || actionBusy"
            @click="void rebuildRag('subject')"
          >
            <span
              :class="actionBusy ? 'i-lucide-loader-2 animate-spin' : 'i-lucide-refresh-cw'"
              class="h-3.5 w-3.5"
            />
          </button>
          <button
            type="button"
            class="rounded-md p-1 text-[var(--text-muted)] hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)] disabled:opacity-50"
            title="重建当前 Project 索引"
            :disabled="subjects.length === 0 || actionBusy"
            @click="void rebuildRag('project')"
          >
            <span class="i-lucide-database-backup h-3.5 w-3.5" />
          </button>
        </div>
      </div>
      ...
      <div class="flex shrink-0 border-b border-[var(--border-color)] px-2 pt-2">
        <button
          ...
          :class="activeTab === 'events' ? 'border-b-2 border-[var(--accent-main)] text-[var(--text-main)]' : 'text-[var(--text-muted)]'"
        ...
        <button
          type="button"
          class="px-3 py-1.5 text-[12px]"
          :class="activeTab === 'memory' ? 'border-b-2 border-[var(--accent-main)] text-[var(--text-main)]' : 'text-[var(--text-muted)]'"
        ...
        <button
          type="button"
          class="px-3 py-1.5 text-[12px]"
          :class="activeTab === 'search' ? 'border-b-2 border-[var(--accent-main)] text-[var(--text-main)]' : 'text-[var(--text-muted)]'"
      ...
      <div class="min-h-0 flex-1 overflow-auto p-3">
        ...
        <template v-else-if="subjectDetail && activeTab === 'events'">
          <div class="mb-3 flex items-center justify-between gap-2">
            <div
              class="text-[11px]"
              :class="sourceStatusClass(subjectDetail.sourceStatuses)"
            >
              {{ currentStatusLabel }}
            </div>
            <button
              type="button"
              class="rounded-md border border-[var(--border-color)] px-2 py-1 text-[11px] hover:bg-[var(--bg-hover)] disabled:opacity-50"
              :disabled="selectedSubjectHasEventError"
              @click="openCreateEvent"
            >
              <span class="i-lucide-plus mr-1 inline-block h-3 w-3 align-[-2px]" />Event
            </button>
          </div>
          <div
            v-if="subjectDetail.events.length === 0"
            class="py-8 text-center text-[12px] text-[var(--text-muted)]"
          >
            暂无 events。
          </div>
          <div
            v-for="(event, index) in subjectDetail.events"
            :key="`${event.line}:${event.text}`"
            class="mb-2 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] p-2"
          >
            <div class="mb-1 flex items-center justify-between gap-2 text-[10px] text-[var(--text-muted)]">
              ...
              <span class="flex items-center gap-1">
                <button
                  type="button"
                  class="rounded p-1 hover:bg-[var(--bg-hover)] disabled:opacity-40"
                  title="上移"
                  :disabled="index === 0 || selectedSubjectHasEventError"
                  @click="void reorderEvent(index, -1)"
                ><span class="i-lucide-arrow-up h-3 w-3" /></button>
                <button
                  type="button"
                  class="rounded p-1 hover:bg-[var(--bg-hover)] disabled:opacity-40"
                  title="下移"
                  :disabled="index === subjectDetail.events.length - 1 || selectedSubjectHasEventError"
                  @click="void reorderEvent(index, 1)"
                ><span class="i-lucide-arrow-down h-3 w-3" /></button>
                <button
                  type="button"
                  class="rounded p-1 hover:bg-[var(--bg-hover)]"
                  title="编辑"
                  :disabled="selectedSubjectHasEventError"
                  @click="openEditEvent(event, index)"
                ><span class="i-lucide-pencil h-3 w-3" /></button>
                <button
                  type="button"
                  class="rounded p-1 text-[var(--status-danger)] hover:bg-[var(--status-danger-bg)]"
                  title="删除"
                  :disabled="selectedSubjectHasEventError"
                  @click="queueDeleteEvent(event, index)"
                ><span class="i-lucide-trash-2 h-3 w-3" /></button>
              </span>
            </div>
            <div class="whitespace-pre-wrap text-[12px] leading-5">
              {{ event.text }}
            </div>
          </div>
        </template>
        ...
        <template v-else-if="subjectDetail && activeTab === 'memory'">
          <div class="mb-3 flex items-center justify-between gap-2">
            ...
            <button
              type="button"
              class="rounded-md border border-[var(--border-color)] px-2 py-1 text-[11px] hover:bg-[var(--bg-hover)] disabled:opacity-50"
              :disabled="selectedSubjectHasMemoryError"
              @click="openCreateMemory"
            >
              <span class="i-lucide-plus mr-1 inline-block h-3 w-3 align-[-2px]" />Memory
            </button>
          </div>
          <div
            v-if="subjectDetail.memories.length === 0"
            class="py-8 text-center text-[12px] text-[var(--text-muted)]"
          >
            暂无 memory。
          </div>
          <div
            v-for="memory in subjectDetail.memories"
            :key="memory.topic"
            class="mb-2 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] p-2"
          >
            <div class="mb-1 flex items-start justify-between gap-2">
              <div class="min-w-0">
                ...
                <div
                  v-if="memory.aliases?.length"
                  class="mt-1 flex flex-wrap gap-1"
                >
                  <span
                    v-for="alias in memory.aliases"
                    :key="alias"
                    class="rounded-sm bg-[var(--bg-panel)] px-1.5 py-0.5 text-[10px] text-[var(--text-muted)]"
                  >{{ alias }}</span>
                </div>
              </div>
              <div class="flex shrink-0 items-center gap-1 text-[var(--text-muted)]">
                ...
                <button
                  ...
                  title="删除"
                  :disabled="selectedSubjectHasMemoryError"
                  @click="queueDeleteMemory(memory)"
                >
        ...
        <template v-else-if="subjectDetail && activeTab === 'search'">
          <div class="mb-3 flex gap-2">
            <input
              v-model="searchQuery"
              class="min-w-0 flex-1 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-2 py-1.5 text-[12px] outline-none focus:border-[var(--accent-main)]"
              placeholder="输入 RAG query"
              @keydown.enter="void searchRag()"
            >
            <button
              type="button"
              class="rounded-md border border-[var(--border-color)] px-2 py-1.5 text-[11px] hover:bg-[var(--bg-hover)] disabled:opacity-50"
              :disabled="searching || !searchQuery.trim()"
              @click="void searchRag()"
            >
              <span
                :class="searching ? 'i-lucide-loader-2 animate-spin' : 'i-lucide-search'"
                class="mr-1 inline-block h-3 w-3 align-[-2px]"
              />Search
            </button>
          </div>
          <div
            v-if="searchError"
            class="mb-3 rounded-md border border-[var(--status-danger-border)] bg-[var(--status-danger-bg)] p-2 text-[11px] text-[var(--status-danger)]"
          >
            {{ searchError }}
          </div>
          <div
            v-if="searchResult && searchResult.candidates.length === 0"
            class="py-8 text-center text-[12px] text-[var(--text-muted)]"
          >
            没有召回候选。
          </div>
          <div
            ...
            :key="`${candidate.rank}:${candidate.source}:${candidate.text}`"
            class="mb-2 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] p-2"
          >
    ...
    <Dialog
      ...
      :title="eventEditorMode === 'create' ? '新增 Event' : '编辑 Event'"
      width="460px"
      show-cancel
      :busy="actionBusy"
      @confirm="saveEvent"
    >
      <div class="space-y-3">
        <input
          v-model="eventForm.tick"
          class="w-full rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-2 py-1.5 text-sm"
          placeholder="tick"
        >
        <input
        ...
        <textarea
          ...
          class="min-h-28 w-full resize-y rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-2 py-1.5 text-sm"
          placeholder="text"
        />
      </div>
    </Dialog>
    ...
    <Dialog
      ...
      :title="memoryEditorMode === 'create' ? '新增 Memory' : '编辑 Memory'"
      width="520px"
      show-cancel
      :busy="actionBusy"
      @confirm="saveMemory"
    >
      <div class="space-y-3">
        <input
          v-model="memoryForm.topic"
          class="w-full rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-2 py-1.5 text-sm"
          placeholder="topic"
        >
        <input
          v-model="memoryForm.aliases"
          class="w-full rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-2 py-1.5 text-sm"
          placeholder="aliases，用逗号分隔"
        >
        <textarea
          v-model="memoryForm.view"
          class="min-h-36 w-full resize-y rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-2 py-1.5 text-sm"
          placeholder="view"
        />
      </div>
    </Dialog>
