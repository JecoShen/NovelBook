  ...
  <div class="space-y-4 pb-8">
    ...
    <section class="rounded-2xl border border-[var(--border-color)] bg-[var(--bg-panel)] p-5 shadow-sm">
      <div class="mb-4 border-b border-[var(--border-color)] pb-4">
        <h4 class="text-sm font-semibold text-[var(--text-main)]">
          {{ t("settings.panels.defaultProfile.title") }}
        </h4>
        <p class="mt-1 text-xs text-[var(--text-secondary)]">
          {{ isProjectScope ? t("settings.panels.defaultProfile.projectDescription") : t("settings.panels.defaultProfile.globalDescription") }}
        </p>
      </div>
      ...
      <div class="grid gap-4 md:grid-cols-[minmax(0,1fr)_minmax(0,0.7fr)]">
        <div class="space-y-1.5">
          ...
          <FormSelect
            :model-value="props.defaultProfileKey"
            :options="props.defaultProfileOptions"
            :placeholder="t('settings.panels.defaultProfile.selectPlaceholder')"
            @update:model-value="emit('update:defaultProfileKey', $event)"
          />
        </div>
        <div class="space-y-1.5">
          ...
          <div class="flex h-7 w-full items-center gap-2 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)]/30 px-2.5 text-[12px] select-all">
            <span class="h-1.5 w-1.5 shrink-0 animate-pulse rounded-full bg-[var(--status-success)]" />
    ...
    <section class="rounded-2xl border border-[var(--border-color)] bg-[var(--bg-panel)] p-5 shadow-sm">
      <div class="mb-4 flex flex-wrap items-center justify-between gap-2 border-b border-[var(--border-color)] pb-4">
        <div>
          <h4 class="text-sm font-semibold text-[var(--text-main)]">
            {{ t("settings.panels.profileModels.defaultParameters") }}
          </h4>
          <p class="mt-1 text-xs text-[var(--text-secondary)]">
            {{ isProjectScope ? t("settings.panels.profileModels.projectDefaultDescription") : t("settings.panels.profileModels.globalDefaultDescription") }}
          </p>
        </div>
        <button
    ...
    <!-- 通用运行默认值 -->
    <section
      v-if="props.runtimeEffective && props.runtimeSources"
      class="rounded-2xl border border-[var(--border-color)] bg-[var(--bg-panel)] p-5 shadow-sm"
    >
      <div class="mb-4 border-b border-[var(--border-color)] pb-4">
        <h4 class="text-sm font-semibold text-[var(--text-main)]">
          {{ t("settings.panels.profileModels.runtime.defaultsTitle") }}
        </h4>
        <p class="mt-1 text-xs text-[var(--text-secondary)]">
          {{ isProjectScope ? t("settings.panels.profileModels.runtime.projectDefaultsDescription") : t("settings.panels.profileModels.runtime.globalDefaultsDescription") }}
        </p>
      </div>
