...
<template>
  ...
  <aside class="sticky top-0 flex flex-col self-start rounded-2xl border border-[var(--border-color)] bg-[var(--bg-panel)] p-2 shadow-sm">
    ...
    <div class="px-1">
      ...
      >
        <div
          ...
          :class="isDefaultsActive ? 'opacity-100' : 'scale-y-0 opacity-0'"
        />
        <span
          class="i-lucide-sliders-horizontal h-4 w-4 shrink-0 transition-transform duration-300 group-hover:scale-110"
          :class="isDefaultsActive ? 'text-[var(--accent-main)]' : 'text-[var(--text-muted)]'"
        />
        <div class="min-w-0 flex-1">
          <div
            class="truncate text-[13px] font-medium"
            :class="isDefaultsActive ? 'text-[var(--accent-text)]' : 'text-[var(--text-main)]'"
          >
            {{ t("settings.panels.profileModels.nav.defaults") }}
          </div>
          <div class="mt-0.5 truncate text-[11px] opacity-70">
            {{ t("settings.panels.profileModels.nav.defaultsDescription") }}
          </div>
        </div>
        <span
          v-if="props.defaultsDirty"
          class="h-1.5 w-1.5 shrink-0 rounded-full bg-[var(--status-warning)]"
          :title="t('settings.panels.profileModels.unsavedChanges')"
        />
      </button>
    </div>
    ...
    <div class="px-3 pb-1.5 pt-3">
      ...
      <div class="mt-1 text-[11px] leading-4 text-[var(--text-secondary)] opacity-80">
        {{ t("settings.panels.profileModels.nav.profilesHint") }}
      </div>
    </div>
    ...
    <div class="flex min-h-[120px] flex-col gap-1 overflow-y-auto px-1 pb-1 custom-scrollbar">
      ...
      >
        ...
        <div
          class="absolute left-0 top-1/2 h-1/2 w-[3px] -translate-y-1/2 rounded-r-full bg-[var(--accent-main)] transition-all duration-300"
          :class="props.activeKey === item.profileKey ? 'opacity-100' : 'scale-y-0 opacity-0'"
        />

        <span
          class="h-1.5 w-1.5 shrink-0 rounded-full"
          :class="statusDotClass(item.status)"
          :title="t(`settings.panels.profileModels.status.${item.status}`)"
        />

        <div class="min-w-0 flex-1">
          <div
            class="truncate text-[13px] font-medium"
            :class="props.activeKey === item.profileKey ? 'text-[var(--accent-text)]' : 'text-[var(--text-main)]'"
          >
            {{ item.name }}
          </div>
        ...
        <div class="flex shrink-0 items-center gap-1">
          <span
            v-if="item.isDefault"
            class="i-lucide-star h-3 w-3 text-[var(--accent-main)]"
            :title="t('settings.panels.profileModels.currentDefault')"
          />
          <span
            v-if="item.dirty"
          ...
          <span
            ...
            class="rounded-full bg-[var(--bg-input)] px-1.5 py-0.5 text-[10px] font-medium text-[var(--text-secondary)]"
            :title="t('settings.panels.profileModels.overrideCount', { count: item.overrideCount })"
          >{{ item.overrideCount }}</span>
        </div>
      </button>

      <div
        v-if="filteredItems.length === 0"
        class="m-1 rounded-xl border border-dashed border-[var(--border-color)] px-4 py-8 text-center text-xs leading-6 text-[var(--text-secondary)]"
      >
