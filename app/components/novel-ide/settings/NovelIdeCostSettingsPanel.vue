    ...
    <div class="flex flex-wrap items-center justify-between gap-4">
      <div class="max-w-xl">
        <h3 class="text-base font-semibold text-[var(--text-main)]">
          {{ t("settings.panels.cost.title") }}
        </h3>
        <p class="mt-1 text-xs text-[var(--text-secondary)]">
          {{ t("settings.panels.cost.description") }}
        </p>
      </div>
    </div>
    ...
    >
      <section class="rounded-xl border border-[var(--border-color)] border-opacity-60 bg-[var(--bg-input)] bg-opacity-20 p-5 space-y-5 shadow-sm transition-all duration-300 hover:shadow-[0_4px_20px_color-mix(in_srgb,var(--shadow-color)_2%,transparent)]">
        <div class="space-y-1.5">
          <div class="flex items-center gap-2">
            <span class="flex h-5 w-5 items-center justify-center rounded bg-[var(--accent-bg)] text-[var(--accent-text)]">
              <span class="i-lucide-circle-dollar-sign h-3.5 w-3.5" />
            </span>
            <h4 class="text-xs font-bold tracking-wider text-[var(--text-main)]">
              {{ t("settings.panels.cost.currencyTitle") }}
            </h4>
          </div>
          <p class="text-xs leading-relaxed text-[var(--text-secondary)]">
            {{ t("settings.panels.cost.currencyDescription") }}
          </p>
        </div>
        ...
        <div class="grid gap-4 md:grid-cols-[minmax(0,1fr)_auto] md:items-end">
          ...
          >
            <span
              v-if="exchangeRateLoading"
              class="i-lucide-loader-2 mr-1.5 h-3.5 w-3.5 animate-spin"
            />
            <span
              v-else
              class="i-lucide-refresh-cw mr-1.5 h-3.5 w-3.5"
            />
        ...
        <div class="rounded-lg border border-[var(--border-color)] border-opacity-40 bg-[var(--bg-input)] bg-opacity-25 px-3 py-2 text-xs leading-5 text-[var(--text-secondary)]">
          ...
          <div
            v-if="exchangeRateFetchedLabel"
            class="text-[10px] text-[var(--text-muted)]"
          >
            {{ exchangeRateFetchedLabel }}
          </div>
        </div>
      </section>
    </div>
