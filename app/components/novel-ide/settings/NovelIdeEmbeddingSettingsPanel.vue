      ...
      <section class="rounded-xl border border-[var(--border-color)] border-opacity-60 bg-[var(--bg-input)] bg-opacity-20 p-5 shadow-sm">
        <div class="mb-4 flex items-center gap-2">
          <span class="flex h-5 w-5 items-center justify-center rounded bg-[var(--accent-bg)] text-[var(--accent-text)]">
            <span class="i-lucide-folder-cog h-3.5 w-3.5" />
          </span>
          <h4 class="text-xs font-bold tracking-wider text-[var(--text-main)]">
            {{ t("settings.panels.embedding.projectOverride") }}
          </h4>
        </div>
        ...
        <div class="grid gap-3 md:grid-cols-2">
          ...
          <label class="space-y-1.5">
            ...
            <FormInput
              v-model="projectDraft.dimensions"
              type="number"
              min="1"
              step="1"
              :placeholder="t('settings.panels.embedding.inheritGlobal')"
      ...
      <section class="rounded-xl border border-[var(--border-color)] border-opacity-60 bg-[var(--bg-input)] bg-opacity-20 p-5 shadow-sm">
        <div class="mb-4 flex items-start justify-between gap-4">
          <div class="flex items-center gap-2">
            <span class="flex h-5 w-5 items-center justify-center rounded bg-[var(--accent-bg)] text-[var(--accent-text)]">
              <span class="i-lucide-binary h-3.5 w-3.5" />
            </span>
            <h4 class="text-xs font-bold tracking-wider text-[var(--text-main)]">
              {{ t("settings.panels.embedding.serviceConfig") }}
            </h4>
          </div>
          <FormCheckbox
            v-model="globalDraft.enabled"
            :label="globalDraft.enabled ? t('settings.panels.embedding.enabled') : t('settings.panels.embedding.disabled')"
          />
        </div>
        ...
        <div class="grid gap-3 md:grid-cols-2">
          ...
          <label class="space-y-1.5">
            <span class="text-xs font-medium text-[var(--text-secondary)]">{{ t("settings.panels.embedding.dimensions") }}</span>
            <FormInput
              v-model="globalDraft.dimensions"
              type="number"
              min="1"
              step="1"
          ...
          <label class="space-y-1.5 md:col-span-2">
            ...
            <div class="flex gap-2">
              <FormInput
                v-model="globalDraft.apiKey"
                type="password"
                :placeholder="globalDraft.apiKeyConfigured ? globalDraft.apiKeyMaskedValue ?? t('settings.panels.embedding.apiKeyConfigured') : 'sk-...'"
              />
              <button
                type="button"
          ...
          <label class="space-y-1.5 md:col-span-2">
            ...
            <textarea
              v-model="globalDraft.requestOptions"
              rows="5"
