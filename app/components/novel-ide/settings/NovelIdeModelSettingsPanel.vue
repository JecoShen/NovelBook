...
<template>
  ...
  <div class="space-y-4 pt-1">
    ...
    <div class="flex flex-wrap items-center justify-between gap-4">
      <div class="max-w-xl">
        <h3 class="text-base font-semibold text-[var(--text-main)]">
          {{ isProjectScope ? t("settings.panels.models.projectTitle") : t("settings.panels.models.globalTitle") }}
        </h3>
        <p class="mt-1 text-xs text-[var(--text-secondary)]">
          {{ isProjectScope ? t("settings.panels.models.projectDescription", { target: props.targetLabel || t("settings.panels.models.currentProject") }) : t("settings.panels.models.globalDescription") }}
        </p>
      </div>
    </div>
    ...
    <div
      ...
      class="flex min-h-9 items-center gap-2 rounded-lg border border-[var(--status-warning-border)] bg-[var(--status-warning-bg)] px-3 py-1.5 text-xs text-[var(--text-main)]"
      :title="validationIssueDetails"
    >
      <span class="i-lucide-triangle-alert h-4 w-4 shrink-0 text-[var(--status-warning)]" />
      ...
      <button
        class="inline-flex h-7 shrink-0 items-center justify-center rounded-md px-2 text-[var(--text-secondary)] transition-colors hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)]"
        @click="validationDialogOpen = true"
      >
        {{ t("settings.panels.models.viewAllIssues") }}
      </button>
      <button
        class="inline-flex h-7 shrink-0 items-center justify-center rounded-md border border-[var(--status-warning-border)] bg-[var(--bg-panel)] px-2.5 font-medium text-[var(--text-main)] transition-colors hover:bg-[var(--bg-hover)] disabled:opacity-50"
        :disabled="repairingModels"
        @click="void repairModelSettings()"
      >
        <span
          class="mr-1 h-3.5 w-3.5"
          :class="repairingModels ? 'i-lucide-loader-2 animate-spin' : 'i-lucide-wand-sparkles'"
        />
    ...
    <div
      class="grid gap-4"
      :class="isProjectScope ? '' : 'lg:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)]'"
    >
      <div class="group flex flex-col justify-center rounded-2xl border border-[var(--border-color)] bg-[var(--bg-panel)] px-5 py-4 shadow-sm transition-all duration-300 hover:shadow-md">
        <div class="mb-3 flex items-center gap-2">
          ...
          <div class="text-sm font-semibold text-[var(--text-main)]">
            {{ isProjectScope ? t("settings.panels.models.projectDefaultTitle") : t("settings.panels.models.globalDefaultTitle") }}
          </div>
        </div>
        <NovelIdeModelSelect
      ...
      <div
        v-if="!isProjectScope"
        class="group flex flex-col justify-center rounded-2xl border border-[var(--border-color)] bg-[var(--bg-panel)] px-5 py-4 shadow-sm transition-all duration-300 hover:shadow-md"
      >
        <div class="mb-3 flex items-center gap-2">
          <div class="flex h-6 w-6 items-center justify-center rounded-md bg-[var(--bg-input)] text-[var(--text-secondary)]">
            <span class="i-lucide-network h-3.5 w-3.5" />
          </div>
          <div class="text-sm font-semibold text-[var(--text-main)]">
            {{ t("settings.panels.models.addProvider") }}
          </div>
        </div>
        <div class="flex items-center gap-2">
          <div class="min-w-0 flex-1">
            <FormSelect
              ...
              :options="providerTemplateOptions.map((item) => ({ value: item.id, label: item.name }))"
              @update:model-value="selectedTemplate = $event"
            />
          </div>
          <button
            class="inline-flex h-7 shrink-0 items-center justify-center rounded-md border border-[var(--border-color)] bg-[var(--bg-panel)] px-3 text-xs font-medium text-[var(--text-main)] shadow-sm transition-colors hover:bg-[var(--bg-hover)] active:scale-95"
            @click="void addProvider()"
          >
            <span class="i-lucide-plus mr-1 h-3 w-3" />
    ...
    <AgentVisibleModelsEditor
      v-if="!isProjectScope"
      v-model="draft.agentVisibleModels"
      :models="defaultModelOptions"
    ...
    <div
      v-if="loading"
      class="flex min-h-[400px] flex-col items-center justify-center gap-4 rounded-2xl border border-[var(--border-color)] bg-[var(--bg-panel)] shadow-sm"
    >
      <span class="i-lucide-loader-2 h-8 w-8 animate-spin text-[var(--text-muted)]" />
    ...
    >
      ...
      <aside class="flex flex-col rounded-2xl border border-[var(--border-color)] bg-[var(--bg-panel)] p-2 shadow-sm">
        <div class="px-3 pb-3 pt-2">
          ...
          <div class="mt-1 text-xs text-[var(--text-secondary)] opacity-80">
            {{ t("settings.panels.models.providersHint") }}
          </div>
        </div>

        <div
          v-if="draft.providers.length === 0"
          class="m-2 rounded-xl border border-dashed border-[var(--border-color)] px-4 py-8 text-center text-xs leading-6 text-[var(--text-secondary)]"
        >
        ...
        <div class="flex flex-col gap-1 overflow-y-auto px-1 pb-1 custom-scrollbar">
          ...
          >
            ...
            <span
              class="h-4 w-4 shrink-0 transition-transform duration-300 group-hover:scale-110"
              :class="[provider.enabled ? 'i-lucide-server' : 'i-lucide-server-off', activeProviderKey === provider.localKey ? 'text-[var(--accent-main)]' : 'text-[var(--text-muted)]']"
            />
            <div class="min-w-0 flex-1">
              <div
                class="truncate text-[13px] font-medium"
                :class="activeProviderKey === provider.localKey ? 'text-[var(--accent-text)]' : 'text-[var(--text-main)]'"
              >
                {{ provider.name }}
              </div>
              <div class="mt-0.5 flex min-w-0 items-center gap-1 truncate text-[11px] opacity-70">
                ...
                <span
                  ...
                  class="shrink-0 rounded border border-[var(--status-warning-border)] bg-[var(--status-warning-bg)] px-1 text-[10px] text-[var(--status-warning)]"
                >{{ t("settings.panels.models.providerDisabled") }}</span>
              </div>
            </div>
          </button>
        </div>
      ...
      <div class="relative min-w-0">
        ...
        >
          ...
          >
            ...
            <div class="rounded-2xl border border-[var(--border-color)] bg-[var(--bg-panel)] p-5 shadow-sm transition-all duration-300 hover:shadow-md">
              ...
              <div class="flex flex-wrap items-center justify-between gap-2">
                <div
                  v-if="!activeProvider.enabled"
                  class="inline-flex min-h-8 min-w-0 items-center gap-1.5 rounded-lg border border-[var(--status-warning-border)] bg-[var(--status-warning-bg)] px-2.5 py-1 text-xs font-medium text-[var(--status-warning)]"
                >
                  <span class="i-lucide-server-off h-3.5 w-3.5" />
                  {{ t("settings.panels.models.providerDisabledHint") }}
                </div>

                <div class="ml-auto flex flex-wrap items-center justify-end gap-2">
                  <button
                    class="inline-flex h-8 items-center gap-1.5 rounded-lg border px-3 text-xs font-medium shadow-sm transition-all duration-200 hover:shadow active:scale-95"
                    :class="activeProvider.enabled ? 'border-[var(--status-warning-border)] bg-[var(--status-warning-bg)] text-[var(--status-warning)] hover:bg-[var(--status-warning-bg)]' : 'border-[var(--status-success-border)] bg-[var(--status-success-bg)] text-[var(--status-success)] hover:bg-[var(--status-success-bg)]'"
                    @click="toggleActiveProviderEnabled"
                  >
                    <span
                      class="h-3.5 w-3.5"
                      :class="activeProvider.enabled ? 'i-lucide-power-off' : 'i-lucide-power'"
                    />
                    {{ activeProvider.enabled ? t("settings.panels.models.disableProvider") : t("settings.panels.models.enableProvider") }}
                  </button>
                  <button
                    class="inline-flex h-8 items-center gap-1.5 rounded-lg border border-[var(--border-color)] bg-[var(--bg-input)] px-3 text-xs font-medium text-[var(--text-main)] shadow-sm transition-all duration-200 hover:bg-[var(--bg-hover)] hover:shadow active:scale-95 disabled:pointer-events-none disabled:opacity-60"
                    :disabled="providerDiscoveringId === activeProvider.id"
                    @click="void discoverModels()"
                  >
                    <span
                      v-if="providerDiscoveringId === activeProvider.id"
                      class="i-lucide-loader-2 h-3.5 w-3.5 animate-spin"
                    />
                    <span
                      v-else
                      class="i-lucide-cloud-lightning h-3.5 w-3.5 text-[var(--text-muted)]"
                    />
                    {{ providerDiscoveringId === activeProvider.id ? t("settings.panels.models.discovering") : t("settings.panels.models.discoverModels") }}
                  </button>
                  <button
                    v-if="activeProvider.sourceIndex !== undefined"
                    class="inline-flex h-8 items-center gap-1.5 rounded-lg border border-[var(--border-color)] bg-[var(--bg-input)] px-3 text-xs font-medium text-[var(--text-main)] shadow-sm transition-all duration-200 hover:bg-[var(--bg-hover)] hover:shadow active:scale-95"
                    @click="cloneActiveProviderConnection"
                  >
                    <span class="i-lucide-copy-plus h-3.5 w-3.5" />
                    {{ t("settings.panels.models.cloneProvider") }}
                  </button>
                  <button
                    class="inline-flex h-8 items-center gap-1.5 rounded-lg border border-[var(--status-danger-border)] bg-[var(--status-danger-bg)] px-3 text-xs font-medium text-[var(--status-danger)] shadow-sm transition-all duration-200 hover:bg-[var(--status-danger-bg)] hover:shadow active:scale-95"
                    @click="requestDeleteActiveProvider"
                  >
              ...
              <div class="mt-4 grid gap-4 md:grid-cols-2">
                <div class="group space-y-1.5">
                  ...
                  <FormInput
                    :model-value="activeProvider.id"
                    :readonly="activeProvider.sourceIndex !== undefined"
                    :placeholder="t('settings.panels.models.providerIdPlaceholder')"
                    @update:model-value="renameActiveProviderId"
                  />
                  <p
                    v-if="activeProvider.sourceIndex !== undefined"
                    class="text-[11px] leading-5 text-[var(--text-muted)]"
                  >
                    {{ t("settings.panels.models.providerIdentityHint") }}
                  </p>
                </div>
                <div class="group space-y-1.5">
                ...
                <div class="group space-y-1.5 md:col-span-2">
                  ...
                  <FormInput
                    v-model="activeProvider.options.baseURL"
                    :readonly="activeProvider.sourceIndex !== undefined"
                    :placeholder="t('settings.panels.models.apiBasePlaceholder')"
                  />
                </div>
                <div class="group space-y-1.5 md:col-span-2">
                  <label class="flex items-center gap-1 text-xs font-medium text-[var(--text-secondary)] transition-colors group-focus-within:text-[var(--text-main)]"><span>{{ t("settings.panels.models.providerModelApi") }}</span><span
                    class="text-[var(--status-danger)]"
                    aria-hidden="true"
                  >*</span></label>
                  <FormSelect
                    v-model="activeProvider.modelApi"
                    :options="modelApiOptions"
                    :placeholder="t('settings.panels.models.apiFormat')"
                  />
                  <p class="text-[11px] leading-5 text-[var(--text-muted)]">
                    {{ t("settings.panels.models.providerModelApiHint") }}
                  </p>
                </div>
                <div class="group space-y-1.5 md:col-span-2">
                  <div class="flex items-center justify-between gap-3">
                    ...
                    <button
                      v-if="activeProvider.options.apiKeyConfigured"
                      type="button"
                      class="text-[11px] text-[var(--status-danger)] transition-colors hover:opacity-80"
                      @click="clearActiveProviderApiKey"
                    >
                      {{ t("settings.panels.models.clearApiKey") }}
                    </button>
                  </div>
                  <FormInput
                    v-model="activeProvider.options.apiKey"
                    :placeholder="activeProvider.options.apiKeyConfigured ? t('settings.panels.models.configuredApiKeyPlaceholder', { value: activeProvider.options.apiKeyMaskedValue ?? '' }) : 'sk-...'"
                    type="password"
                  />
                </div>
                <div class="group space-y-1.5 md:col-span-2">
                  ...
                  <FormInput
                    v-model="activeProvider.options.proxy"
                    :readonly="activeProvider.sourceIndex !== undefined"
                ...
                <div class="group space-y-1.5">
                  ...
                  <FormInput
                    v-model="activeProvider.options.timeoutMs"
                ...
                <div class="group space-y-1.5">
                  ...
                  <FormInput
                    v-model="activeProvider.options.maxRetries"
                    :placeholder="t('settings.panels.models.defaultMaxRetries', { value: DEFAULT_PI_MAX_RETRIES })"
                    min="0"
                    step="1"
                    type="number"
                  />
                ...
                <div class="group space-y-1.5 md:col-span-2">
                  ...
                  <textarea
                    v-model="activeProvider.options.requestOptions"
                    rows="4"
                    placeholder="{&quot;temperature&quot;:0.7}"
                    class="w-full resize-y rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-2.5 py-2 font-mono text-[12px] text-[var(--text-main)] outline-none transition-colors placeholder:text-[var(--text-muted)] placeholder:opacity-80 focus:border-[var(--accent-main)] focus:ring-1 focus:ring-[var(--accent-main)]/20"
                  />
                </div>
              </div>
            </div>
          ...
          <section
            v-else
            class="flex min-h-[400px] flex-col items-center justify-center gap-4 rounded-2xl border border-[var(--border-color)] bg-[var(--bg-panel)] px-8 py-10 text-center shadow-sm"
          >
            <div class="flex h-16 w-16 items-center justify-center rounded-full bg-[var(--bg-input)] shadow-inner">
              <span class="i-lucide-server-off h-8 w-8 text-[var(--text-muted)] opacity-50" />
            </div>
            <div>
              <div class="text-base font-medium text-[var(--text-main)]">
                {{ t("settings.panels.models.noProviderSelected") }}
              </div>
              <div class="mt-1 text-sm text-[var(--text-secondary)]">
                {{ t("settings.panels.models.selectProviderHint") }}
              </div>
            </div>
          </section>
        </Transition>
  ...
  >
    ...
    <div class="h-full space-y-2 overflow-y-auto pr-1 custom-scrollbar">
      <div
        ...
        :key="`${issue.code}-${issue.path.join('.')}-${String(index)}`"
        class="rounded-lg border border-[var(--border-color)] bg-[var(--bg-panel)] px-3 py-2.5"
      >
        <div class="flex flex-wrap items-center gap-2 text-xs">
          <span class="rounded bg-[var(--status-warning-bg)] px-1.5 py-0.5 font-medium text-[var(--status-warning)]">{{ issue.code }}</span>
          <span
            v-if="issue.modelKey"
            class="font-mono text-[var(--text-main)]"
          >{{ issue.modelKey }}</span>
        </div>
        <p class="mt-1.5 text-sm text-[var(--text-main)]">
          {{ issue.message }}
        </p>
        <p class="mt-1 break-all font-mono text-[11px] text-[var(--text-muted)]">
          {{ issue.path.join(".") }}
        </p>
      </div>
    </div>
  ...
  >
    ...
    >
      <p class="text-sm text-[var(--text-secondary)]">
        {{ t("settings.panels.models.deleteProviderMessage", { name: activeProvider.name }) }}
      </p>
      <p class="rounded-lg border border-[var(--status-warning-border)] bg-[var(--status-warning-bg)] px-3 py-2 text-xs text-[var(--status-warning)]">
        {{ t("settings.panels.models.deleteProviderWarning") }}
      </p>
    </div>
  </Dialog>
