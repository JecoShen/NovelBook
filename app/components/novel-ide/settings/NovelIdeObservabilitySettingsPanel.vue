  ...
  <div class="space-y-4 pt-1">
    <div class="max-w-xl">
      <h3 class="text-base font-semibold text-[var(--text-main)]">
        {{ t("settings.panels.observability.title") }}
      </h3>
      <p class="mt-1 text-xs text-[var(--text-secondary)]">
        {{ t("settings.panels.observability.description") }}
      </p>
    </div>
    ...
    >
      ...
      <div class="flex items-center gap-4 rounded-2xl border border-[var(--border-color)] bg-[var(--bg-panel)] px-5 py-4 shadow-sm">
        <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[var(--bg-input)] text-[var(--text-secondary)]">
          <span class="i-lucide-activity h-5 w-5" />
        </div>
        <div class="min-w-0 flex-1">
          <div class="text-sm font-medium text-[var(--text-main)]">
            {{ t("settings.panels.observability.enabledTitle") }}
          </div>
          <div class="mt-0.5 text-xs text-[var(--text-secondary)]">
            {{ t("settings.panels.observability.enabledDescription") }}
          </div>
        </div>
        <button
          type="button"
          class="relative h-6 w-11 shrink-0 rounded-full border transition-colors"
          :class="enabled ? 'border-[var(--accent-main)] bg-[var(--accent-main)]' : 'border-[var(--border-color)] bg-[var(--bg-input)]'"
      ...
      <label class="rounded-2xl border border-[var(--border-color)] bg-[var(--bg-panel)] px-5 py-4 shadow-sm">
        ...
        <input
          type="number"
          class="mt-3 h-8 w-full max-w-[220px] rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-2.5 text-sm text-[var(--text-main)] outline-none focus:border-[var(--accent-main)] focus:ring-1 focus:ring-[var(--accent-main)] focus:ring-opacity-20"
          :value="maxRecords"
          min="0"
          max="10000"
          step="10"
          @input="updateMaxRecords(($event.target as HTMLInputElement).value)"
        >
      </label>
      ...
      <div class="rounded-lg border border-[var(--border-color)] border-opacity-40 bg-[var(--bg-input)] bg-opacity-25 px-3 py-2 text-xs leading-5 text-[var(--text-secondary)]">
        <span class="i-lucide-shield-alert mr-1 inline-block h-3.5 w-3.5 align-text-bottom" />{{ t("settings.panels.observability.privacyNote") }}
      </div>
    </div>
  </div>
</template>
