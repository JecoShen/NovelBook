...
<template>
  ...
  <div class="space-y-4 pt-1">
    <div class="flex flex-wrap items-center justify-between gap-4">
      <div class="max-w-xl">
        <h3 class="text-base font-semibold text-[var(--text-main)]">
          {{ t("settings.panels.web.title") }}
        </h3>
        <p class="mt-1 text-xs text-[var(--text-secondary)]">
          {{ t("settings.panels.web.description") }}
        </p>
      </div>
    </div>
    ...
    <div
      v-if="errorText"
      class="flex items-start gap-3 rounded-xl border border-[var(--status-danger-border)] bg-[var(--status-danger-bg)] px-4 py-3 shadow-sm"
    >
      <span class="i-lucide-alert-circle mt-0.5 h-4 w-4 shrink-0 text-[var(--status-danger)]" />
      <div class="text-sm text-[var(--status-danger)]">
        {{ errorText }}
      </div>
    </div>
    ...
    <div
      v-if="loading"
      class="flex min-h-[260px] flex-col items-center justify-center gap-4 rounded-2xl border border-[var(--border-color)] bg-[var(--bg-panel)] shadow-sm"
    >
    ...
    >
      <section class="rounded-xl border border-[var(--border-color)] border-opacity-60 bg-[var(--bg-input)] bg-opacity-20 p-5 shadow-sm">
          ...
          <div>
            <div class="flex items-center gap-2">
              <span class="flex h-5 w-5 items-center justify-center rounded bg-[var(--accent-bg)] text-[var(--accent-text)]">
                <span class="i-lucide-search h-3.5 w-3.5" />
              </span>
              <h4 class="text-xs font-bold tracking-wider text-[var(--text-main)]">
                {{ t("settings.panels.web.searchProvider") }}
              </h4>
            </div>
            <p class="mt-1.5 text-xs leading-relaxed text-[var(--text-secondary)]">
              {{ t("settings.panels.web.searchProviderDescription") }}
            </p>
          </div>
          <div class="w-[260px] shrink-0 space-y-1.5">
            <div class="text-[11px] font-semibold text-[var(--text-secondary)]">
              {{ t("settings.panels.web.defaultSearchProvider") }}
            </div>
            <FormSelect
            ...
            <div class="truncate text-[10px] text-[var(--text-muted)]">
              Fallback: {{ draft.order.map((key) => key === "tavily" ? "Tavily" : "Brave").join(" -> ") }}
            </div>
          </div>
        </div>
        ...
        <div class="grid gap-3">
          <div
            ...
            :key="item.key"
            class="rounded-xl border border-[var(--border-color)] bg-[var(--bg-panel)] p-4 shadow-sm"
          >
            <div class="flex flex-wrap items-start justify-between gap-3">
              <div class="flex min-w-0 items-start gap-3">
                ...
                <div class="min-w-0">
                  <div class="flex flex-wrap items-center gap-2">
                    <h5 class="text-sm font-semibold text-[var(--text-main)]">
                      {{ item.name }}
                    </h5>
                    <span
                      class="h-2 w-2 rounded-full"
                      :class="item.enabled ? 'bg-[var(--status-success)]' : 'bg-[var(--text-muted)]'"
                    />
              ...
              <div class="flex items-center gap-1.5">
                <button
                  type="button"
                  class="flex h-7 w-7 items-center justify-center rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] text-[var(--text-secondary)] hover:bg-[var(--bg-hover)] disabled:opacity-40"
                  :disabled="!canMove(item.key, -1)"
                  :title="t('settings.panels.web.moveUp')"
                  @click="moveProvider(item.key, -1)"
                >
                ...
                <button
                  ...
                  :class="item.enabled ? 'border-[var(--accent-main)] bg-[var(--accent-main)]' : 'border-[var(--border-color)] bg-[var(--bg-input)]'"
                  :title="item.enabled ? t('settings.panels.web.disableProvider') : t('settings.panels.web.enableProvider')"
            ...
            <div class="mt-4 grid gap-3 md:grid-cols-3">
              <label class="space-y-1.5 md:col-span-2">
                ...
                <div class="flex gap-2">
                  <FormInput
                    v-model="providerDraft(item.key).apiKey"
                    type="password"
                    :placeholder="providerDraft(item.key).apiKeyConfigured ? providerDraft(item.key).apiKeyMaskedValue ?? t('settings.panels.web.apiKeyConfigured') : t('settings.panels.web.notConfigured')"
                  />
                  <button
                    type="button"
                    class="inline-flex h-7 shrink-0 items-center gap-1.5 rounded-md border border-[var(--status-danger-border)] bg-[var(--status-danger-bg)] px-2.5 text-[11px] font-medium text-[var(--status-danger)] hover:bg-[var(--status-danger-bg)]"
                    @click="clearProviderApiKey(item.key)"
                  >
              ...
              <label class="space-y-1.5">
                ...
                <FormInput
                  v-model="providerDraft(item.key).timeoutMs"
                  type="number"
                  min="1000"
                  step="1000"
                  placeholder="15000"
              ...
              <template v-if="item.key === 'brave'">
                ...
                <label class="space-y-1.5">
                  ...
                  <FormInput
                    v-model="draft.brave.searchLang"
                    placeholder="en"
                  />
                </label>
              </template>
      ...
      <section class="rounded-xl border border-[var(--border-color)] border-opacity-60 bg-[var(--bg-input)] bg-opacity-20 p-5 shadow-sm">
        <div class="mb-4 flex items-center gap-2">
          <span class="flex h-5 w-5 items-center justify-center rounded bg-[var(--accent-bg)] text-[var(--accent-text)]">
            <span class="i-lucide-file-down h-3.5 w-3.5" />
          </span>
          <h4 class="text-xs font-bold tracking-wider text-[var(--text-main)]">
            {{ t("settings.panels.web.localFetch") }}
          </h4>
        </div>
        ...
        <div class="grid gap-3 md:grid-cols-3">
          <button
            type="button"
            class="flex items-center justify-between gap-4 rounded-xl border border-[var(--border-color)] bg-[var(--bg-panel)] px-4 py-3 text-left shadow-sm transition-all hover:bg-[var(--bg-hover)]"
            @click="draft.localFetch.enabled = !draft.localFetch.enabled"
          >
            ...
            <span
              ...
              :class="draft.localFetch.enabled ? 'bg-[var(--status-success)]' : 'bg-[var(--text-muted)]'"
            />
          </button>
          <label class="space-y-1.5">
            <span class="text-xs font-medium text-[var(--text-secondary)]">{{ t("settings.panels.web.timeoutMs") }}</span>
            <FormInput
              v-model="draft.localFetch.timeoutMs"
              type="number"
              min="1000"
              step="1000"
            />
          ...
          <label class="space-y-1.5">
            ...
            <FormInput
              v-model="draft.localFetch.maxRedirects"
              type="number"
              min="0"
              step="1"
            />
          ...
          <label class="space-y-1.5">
            ...
            <FormInput
              v-model="draft.localFetch.maxBytes"
              type="number"
              min="1024"
              step="1024"
            />
          ...
          <label class="space-y-1.5">
            ...
            <FormInput
              v-model="draft.localFetch.maxCharacters"
              type="number"
              min="1000"
              step="1000"
            />
          ...
          <label class="space-y-1.5">
            ...
            <FormInput
              v-model="draft.localFetch.minCharactersForLocal"
              type="number"
              min="0"
              step="100"
            />
      ...
      <section class="rounded-xl border border-[var(--border-color)] border-opacity-60 bg-[var(--bg-input)] bg-opacity-20 p-5 shadow-sm">
        <div class="mb-4 flex items-center justify-between gap-4">
          <div>
            <div class="flex items-center gap-2">
              <span class="flex h-5 w-5 items-center justify-center rounded bg-[var(--accent-bg)] text-[var(--accent-text)]">
                <span class="i-lucide-rotate-cw h-3.5 w-3.5" />
              </span>
              <h4 class="text-xs font-bold tracking-wider text-[var(--text-main)]">
                {{ t("settings.panels.web.tavilyFallback") }}
              </h4>
            </div>
            <p class="mt-1.5 text-xs text-[var(--text-secondary)]">
              {{ t("settings.panels.web.tavilyFallbackDescription") }}
            </p>
          </div>
          <button
            type="button"
            class="relative h-6 w-11 rounded-full border transition-colors"
            :class="draft.tavilyFallback.enabled ? 'border-[var(--accent-main)] bg-[var(--accent-main)]' : 'border-[var(--border-color)] bg-[var(--bg-input)]'"
            @click="draft.tavilyFallback.enabled = !draft.tavilyFallback.enabled"
          >
            <span
              class="absolute top-0.5 h-[18px] w-[18px] rounded-full bg-white shadow transition-transform"
              :class="draft.tavilyFallback.enabled ? 'translate-x-5' : 'translate-x-0.5'"
            />
          </button>
        </div>
        <div class="max-w-xs space-y-1.5">
          ...
          <FormInput
            v-model="draft.tavilyFallback.timeoutMs"
            type="number"
            min="1000"
            step="1000"
            placeholder="20000"
