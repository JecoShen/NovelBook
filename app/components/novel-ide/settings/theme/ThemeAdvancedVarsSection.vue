...
<template>
  ...
  <section>
    <p class="mb-3 text-xs text-[var(--text-secondary)]">
      {{ t("settings.themeEditor.advancedDescription") }}
    </p>
    ...
    <div class="space-y-2">
      <div
        ...
        :key="group.id"
        class="rounded-lg border border-[var(--border-color)] bg-[var(--bg-input)]"
      >
        <button
          type="button"
          class="flex w-full items-center justify-between gap-3 px-3 py-2 text-left text-xs font-medium text-[var(--text-main)]"
          @click="toggleGroup(group.id)"
        >
          <span>{{ t(group.labelKey) }}</span>
          <span
            class="i-lucide-chevron-down h-4 w-4 text-[var(--text-muted)] transition-transform"
        ...
        <div
          ...
          class="grid gap-3 border-t border-[var(--border-color)] p-3 sm:grid-cols-2"
        >
          <div
          ...
          >
            <div class="mb-1 flex items-center justify-between gap-2">
              ...
              <button
                type="button"
                class="inline-flex h-6 w-6 shrink-0 items-center justify-center rounded border border-[var(--border-color)] text-[var(--text-secondary)] transition-colors hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)]"
                :title="t('settings.themeEditor.copyValue')"
                @click="copyColorValue(modelValue[key] ?? '')"
              >
