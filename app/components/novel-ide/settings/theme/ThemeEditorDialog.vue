  ...
  >
    ...
    <div class="shrink-0 border-b border-[var(--border-color)] px-4 pb-3 pt-3">
      <div class="grid gap-3 sm:grid-cols-[minmax(0,1fr)_140px]">
        <label class="block">
          ...
          <input
            class="h-9 w-full rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-3 text-sm text-[var(--text-main)] outline-none focus:border-[var(--accent-main)]"
        ...
        <label class="block">
          ...
          <FormSelect
            :model-value="draftTheme.appearance"
      ...
      <div class="mt-2.5 flex items-center justify-between gap-3">
        <p
          class="min-w-0 flex-1 truncate text-[11px] text-[var(--text-muted)]"
          :title="t('settings.themeEditor.regenerateHint')"
        >
          {{ t("settings.themeEditor.floatHint") }}
        </p>
        <button
          type="button"
          class="inline-flex h-7 shrink-0 items-center gap-1.5 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-2.5 text-xs font-medium text-[var(--text-main)] transition-colors hover:bg-[var(--bg-hover)] disabled:opacity-50"
          :title="t('settings.themeEditor.regenerateHint')"
          :disabled="saving"
          @click="regenerateVars"
        >
      ...
      <p
        v-if="localError"
        class="mt-2.5 rounded-md border border-[var(--status-danger-border)] bg-[var(--status-danger-bg)] px-3 py-2 text-xs text-[var(--status-danger)]"
      >
        {{ localError }}
      </p>
    </div>
    ...
    <div class="flex shrink-0 items-center gap-1 border-b border-[var(--border-color)] px-4">
      <button
        ...
        :class="activeTab === tab.value ? 'border-[var(--accent-main)] text-[var(--accent-text)]' : 'border-transparent text-[var(--text-secondary)] hover:text-[var(--text-main)]'"
    ...
    <div class="min-h-0 flex-1 overflow-y-auto px-4 py-3">
      <ThemeCorePaletteSection
        v-if="activeTab === 'core'"
        :model-value="draftTheme.vars"
        :appearance="draftTheme.appearance"
        :picker-theme="pickerTheme"
        :disabled="saving"
        @update:model-value="updateVars"
        @valid-change="updateColorValidity"
      />
      <ThemeAdvancedVarsSection
    ...
    <template #footer>
      <button
        type="button"
        class="inline-flex h-8 items-center justify-center rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-4 text-[13px] font-medium text-[var(--text-main)] transition-colors hover:bg-[var(--bg-hover)] disabled:opacity-50"
        :disabled="saving"
        @click="cancelEditing"
      >
        {{ t("common.cancel") }}
      ...
      <button
        type="button"
        class="inline-flex h-8 items-center justify-center gap-1.5 rounded-md border border-transparent bg-[var(--accent-main)] px-4 text-[13px] font-medium text-[var(--text-inverse)] transition-opacity hover:opacity-90 disabled:opacity-50"
        :disabled="saving"
