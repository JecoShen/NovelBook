  ...
  <div class="flex h-full min-h-0 flex-col">
    ...
    <div class="flex shrink-0 items-center gap-2 border-b border-[var(--border-color)] bg-[var(--bg-panel)] px-3 py-2">
      <div class="relative min-w-0 flex-1">
        <span class="i-lucide-search absolute left-2 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-[var(--text-muted)]" />
        <input
          v-model="searchQuery"
          type="text"
          :placeholder="t('ide.workspace.character.searchPlaceholder')"
      ...
      <button
        type="button"
        class="rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-2 py-1.5 text-xs text-[var(--text-secondary)] hover:bg-[var(--bg-hover)]"
        :title="t('ide.workspace.character.create')"
        @click="void createCharacter()"
      >
        <span class="i-lucide-user-plus h-3.5 w-3.5" />
      </button>
      <button
        type="button"
        class="rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-2 py-1.5 text-xs text-[var(--text-secondary)] hover:bg-[var(--bg-hover)]"
    ...
    <!-- 角色列表 -->
    <div
      class="min-h-0 flex-1 overflow-y-auto p-2 custom-scrollbar"
      @contextmenu.prevent.stop="openRootMenu"
    >
      <div
        v-if="loadingWorkspaceTree && workspaceTree.length === 0"
      ...
      <div
        ...
        class="flex h-full min-h-[180px] items-center justify-center rounded-md border border-dashed border-[var(--border-color)] px-4 text-center text-xs leading-5 text-[var(--text-muted)]"
      >
      ...
      >
        <span class="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-lg border border-[var(--border-accent)] bg-[var(--accent-bg)] text-[var(--accent-text)]">
          <span class="i-lucide-user-round h-4 w-4" />
        </span>
        <span class="min-w-0 flex-1">
          <span class="flex min-w-0 items-center gap-2">
            ...
            <span
              v-if="node.status"
              class="shrink-0 rounded border border-[var(--border-color)] px-1.5 py-0.5 text-[9px] uppercase tracking-wide text-[var(--text-muted)]"
            >{{ node.status }}</span>
          </span>
          <span class="mt-0.5 block truncate text-[10px] font-mono text-[var(--text-muted)]">{{ node.path.replace(/\/index\.md$/i, "") }}</span>
          <span
            v-if="node.summary"
            class="mt-1 line-clamp-2 block text-[11px] leading-4 text-[var(--text-secondary)]"
          >{{ node.summary }}</span>
        </span>
      </button>
    </div>
