  ...
  >
    ...
    >
      <div class="mb-2 flex items-center gap-2">
        ...
        />
        <h2 class="min-w-0 flex-1 truncate text-[13px] font-medium leading-5 text-[var(--text-main)]">
          {{ dialogTitle }}
        </h2>
        <button
          class="flex h-6 w-6 items-center justify-center rounded text-[var(--text-muted)] hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)]"
          type="button"
          :disabled="props.busy"
          @click="closeDialog"
        >
      ...
      <p
        v-if="localError"
        class="mb-0 mt-2 text-[12px] leading-4 text-[var(--status-danger)]"
      >
        {{ localError }}
      </p>
      ...
      <div class="mt-3 flex items-center justify-end gap-2">
        <button
          class="h-7 rounded-md border border-[var(--border-color)] bg-transparent px-3 text-[12px] text-[var(--text-secondary)] hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)] disabled:cursor-not-allowed disabled:opacity-50"
          type="button"
          :disabled="props.busy"
          @click="closeDialog"
        >
        ...
        <button
          class="h-7 rounded-md border border-transparent bg-[var(--accent-main)] px-3 text-[12px] font-medium text-[var(--text-inverse)] hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50"
          type="submit"
          :disabled="props.busy"
        >
          {{ t("common.confirm") }}
