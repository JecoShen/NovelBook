  ...
  >
    <template #header>
      <div class="min-w-0 overflow-hidden">
        <div class="truncate text-xs font-semibold text-[var(--text-main)]">
          {{ props.node?.title || props.node?.path || t("ide.workspace.fileDetail.title") }}
        </div>
    ...
    <template #actions>
      <button
        ...
        type="button"
        @click="emit('create-index')"
      >
        {{ t("ide.workspace.fileDetail.convert") }}
      </button>
      <button
        v-if="canConvertFileToDirectory"
        class="rounded-md px-2 py-1 text-[10px] text-[var(--accent-text)] hover:bg-[var(--bg-hover)]"
        type="button"
        @click="emit('convert-file-to-directory')"
      >
        {{ t("ide.workspace.fileDetail.convertToDirectory") }}
      </button>
      <button
        v-if="isContentIndexFile"
        class="rounded-md px-2 py-1 text-[10px] text-[var(--accent-text)] hover:bg-[var(--bg-hover)] disabled:cursor-not-allowed disabled:opacity-45"
        type="button"
        :disabled="isManuscriptIndexFile ? savingFile : (!isFrontmatterDirty || savingFile)"
        @click="isManuscriptIndexFile ? void saveManuscriptForm() : void saveFrontmatter()"
      >
    ...
    <div
      v-if="props.node"
      class="grid min-w-0 gap-2 text-xs text-[var(--text-secondary)]"
    >
      <!-- 文件详情基础信息 -->
      <div class="min-w-0 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-2 py-1.5">
        <div class="flex min-w-0 items-center justify-between gap-2">
          <div class="text-[10px] uppercase tracking-[0.16em] text-[var(--text-muted)]">
            {{ props.node.isDirectory ? "Directory" : "File" }}
          </div>
          <div class="shrink-0 text-[10px] text-[var(--text-muted)]">
            {{ props.node.editable ? t("ide.workspace.fileDetail.editable") : t("ide.workspace.fileDetail.readonly") }}
          </div>
        </div>
        <div
          class="mt-1 max-w-full truncate font-mono text-[11px] text-[var(--text-main)]"
      ...
      <div
        v-if="isContentDirectoryWithoutIndex"
        class="rounded-md border border-[var(--status-warning-border)] bg-[var(--status-warning-bg)] p-2 text-[var(--status-warning)]"
      >
        {{ t("ide.workspace.fileDetail.contentDirectoryWithoutIndex") }}
      </div>
      <div
        v-else-if="isDirectoryWithoutIndex"
        class="rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] p-2 text-[var(--text-muted)]"
      >
        {{ t("ide.workspace.fileDetail.directoryWithoutIndex") }}
      </div>

      <div
        v-if="localFrontmatterError || props.node.frontmatterError"
        class="rounded-md border border-[var(--status-danger-border)] bg-[var(--status-danger-bg)] p-2 text-[var(--status-danger)]"
      >
      ...
      <!-- frontmatter 详情 -->
      <div
        v-if="isManuscriptIndexFile"
        class="min-w-0 space-y-2 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] p-2"
      >
        <div class="flex items-center justify-between gap-2">
          ...
          <div class="flex shrink-0 items-center gap-2">
            <span
              v-if="manuscriptStats.updatedAt"
              class="text-[10px] text-[var(--text-muted)]"
            >{{ manuscriptStats.updatedAt }}</span>
            <span
              v-if="isFrontmatterDirty"
              class="text-[10px] text-[var(--status-warning)]"
            >{{ t("ide.workspace.common.unsaved") }}</span>
            <button
              type="button"
              class="rounded-md border border-[var(--border-color)] px-1.5 py-0.5 text-[10px] text-[var(--text-muted)] hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)]"
              @click="refreshManuscriptStats"
            >
              {{ t("ide.workspace.fileDetail.updateStats") }}
            </button>
          </div>
        </div>
        <div class="grid grid-cols-5 gap-1.5">
          <div class="rounded-md border border-[var(--border-color)] bg-[var(--bg-panel)] px-2 py-1.5">
            <div class="text-[8px] uppercase tracking-[0.12em] text-[var(--text-muted)]">
              {{ t("ide.workspace.fileDetail.current") }}
            </div>
            <div class="mt-0.5 text-[var(--text-main)]">
              {{ manuscriptStats.currentWords }}
            </div>
          </div>
          <div class="rounded-md border border-[var(--border-color)] bg-[var(--bg-panel)] px-2 py-1.5">
            <div class="text-[8px] uppercase tracking-[0.12em] text-[var(--text-muted)]">
              {{ t("ide.workspace.fileDetail.total") }}
            </div>
            <div class="mt-0.5 text-[var(--text-main)]">
              {{ manuscriptStats.totalWords }}
            </div>
          </div>
          <div class="rounded-md border border-[var(--border-color)] bg-[var(--bg-panel)] px-2 py-1.5">
            <div class="text-[8px] uppercase tracking-[0.12em] text-[var(--text-muted)]">
              {{ t("ide.workspace.fileDetail.size") }}
            </div>
            <div class="mt-0.5 text-[var(--text-main)]">
              {{ manuscriptStats.totalSize }}
            </div>
          </div>
          <div class="rounded-md border border-[var(--border-color)] bg-[var(--bg-panel)] px-2 py-1.5">
            <div class="text-[8px] uppercase tracking-[0.12em] text-[var(--text-muted)]">
              {{ t("ide.workspace.fileDetail.chapters") }}
            </div>
            <div class="mt-0.5 text-[var(--text-main)]">
              {{ manuscriptStats.chapters }}
            </div>
          </div>
          <div class="rounded-md border border-[var(--border-color)] bg-[var(--bg-panel)] px-2 py-1.5">
            <div class="text-[8px] uppercase tracking-[0.12em] text-[var(--text-muted)]">
              {{ t("ide.workspace.fileDetail.files") }}
            </div>
            <div class="mt-0.5 text-[var(--text-main)]">
              {{ manuscriptStats.files }}
            </div>
          </div>
        </div>
        <div class="grid grid-cols-[minmax(0,1fr)_132px] gap-2">
          <div class="space-y-1">
            ...
            <input
              ...
              type="text"
              @blur="void saveManuscriptForm()"
            >
          </div>
          <div class="space-y-1">
            <label class="text-[11px] font-medium text-[var(--text-secondary)]">{{ t("ide.workspace.common.status") }}</label>
            <FormSelect
              :model-value="manuscriptForm.status || 'draft'"
              :options="manuscriptStatusOptions"
              @update:model-value="manuscriptForm.status = $event; void saveManuscriptForm()"
            />
          </div>
        </div>
        <div class="space-y-1">
      ...
      <div
        v-else-if="isContentIndexFile || props.node.contentNode"
        class="space-y-2 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] p-2"
      >
        <div class="flex items-center justify-between gap-2">
          <div class="text-[10px] uppercase tracking-[0.18em] text-[var(--text-muted)]">
            Frontmatter
          </div>
          <span
            v-if="isContentIndexFile && isFrontmatterDirty"
            class="text-[10px] text-[var(--status-warning)]"
          >{{ t("ide.workspace.common.unsaved") }}</span>
        </div>
        <div
          v-if="isContentIndexFile"
          class="flex items-center justify-between gap-2 rounded-md border border-[var(--border-color)] bg-[var(--bg-panel)] px-2 py-1.5"
        >
          ...
          <button
            type="button"
            class="rounded-md border border-[var(--border-color)] px-2 py-1 text-[10px] text-[var(--text-muted)] hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)]"
            @click="iconPickerVisible = true"
          >
            {{ t("ide.workspace.fileDetail.chooseIcon") }}
          </button>
        </div>
        <textarea
          v-if="isContentIndexFile"
        ...
        <pre
          v-else
          class="max-h-48 overflow-auto whitespace-pre-wrap rounded-md border border-[var(--border-color)] bg-[var(--bg-panel)] p-2 font-mono text-[11px] leading-5 text-[var(--text-main)] custom-scrollbar"
        >{{ readonlyFrontmatterText }}</pre>
      </div>

      <div
        v-if="props.node.summary"
        class="rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] p-2 leading-5"
      >
        {{ props.node.summary }}
      </div>
      ...
      >
        <div class="text-[10px] uppercase tracking-[0.18em] text-[var(--text-muted)]">
          {{ t("ide.workspace.fileDetail.validate") }}
        </div>
        <div
