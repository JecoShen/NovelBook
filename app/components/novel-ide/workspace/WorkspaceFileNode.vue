  ...
  >
    ...
    >
      ...
      <div class="min-w-0 flex flex-1 items-center gap-1.5">
        ...
        <span
          v-if="isLorebookEntry"
          class="max-w-[80px] shrink-0 truncate text-right text-[11px] opacity-60"
        >
          {{ nodeName }}
        </span>
        <span
          v-else-if="node.entryType"
          class="max-w-[84px] shrink-0 truncate text-[10px] opacity-60"
        >
          {{ node.entryType }}
        </span>
        <span
          v-else-if="directoryMeta"
          class="max-w-[72px] shrink-0 truncate text-right text-[10px] opacity-45"
        >
          {{ directoryMeta.label }}
        </span>
        <span
          v-else-if="isContentIndexFile"
          class="shrink-0 text-[10px] text-[var(--text-muted)] opacity-45"
        >
          node
        </span>
        <span
        ...
        <span
          ...
          class="ml-auto h-1.5 w-1.5 shrink-0 rounded-full"
          :class="node.status === 'active' ? 'bg-[var(--status-success)]' : node.status === 'pending' ? 'bg-[var(--status-info)]' : node.status === 'draft' ? 'bg-[var(--status-warning)]' : 'bg-[var(--text-muted)]'"
          :title="node.status"
        />
      </div>
    </div>
    ...
    >
      <div
        v-if="isOpen"
        class="relative -my-px overflow-hidden py-px"
      >
        <WorkspaceFileNode
