...
<template>
  ...
  >
      ...
      <div class="flex min-w-0 flex-1 items-center justify-between gap-4">
        <div class="flex min-w-0 items-center gap-4">
          <span class="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl border border-[var(--status-info-border)] bg-[var(--status-info-bg)] text-[var(--status-info)]">
            <span class="i-lucide-map-pinned h-7 w-7" />
          </span>
          <div class="min-w-0">
            <h2 class="truncate text-xl font-bold text-[var(--text-main)]">
              {{ draft?.title || t("ide.workspace.typedProfile.locationProfile") }}
            </h2>
            <div class="mt-1 truncate text-xs text-[var(--text-secondary)]">
              {{ draft?.path }}
            </div>
          </div>
        </div>
        <div class="flex shrink-0 items-center gap-1">
          ...
          <button
            ...
            :disabled="savingFile || Boolean(diagnostics)"
    ...
    >
      <div
        v-if="diagnostics"
        class="rounded-md border border-[var(--status-danger-border)] bg-[var(--status-danger-bg)] px-3 py-2 text-[var(--status-danger)]"
      >
        {{ diagnostics }}
      </div>
      <div
      ...
      <section class="rounded-xl border border-[var(--status-info-border)] bg-[var(--status-info-bg)] p-3">
        <div class="grid grid-cols-[minmax(0,1fr)_132px_150px] gap-2">
          <label class="space-y-1"><span>{{ t("ide.workspace.typedProfile.locationName") }}</span><input
            v-model="draft.title"
            class="field"
            @blur="void saveDraft()"
          ></label>
          <label class="space-y-1"><span>{{ t("ide.workspace.common.status") }}</span><FormSelect
            :model-value="draft.status"
            :options="statusOptions"
            @update:model-value="draft.status = $event; void saveDraft()"
          /></label>
          <label class="space-y-1"><span>{{ t("ide.workspace.typedProfile.type") }}</span><input
            :value="draft.subtype ?? ''"
            class="field"
            list="location-subtypes"
            @input="draft.subtype = ($event.target as HTMLInputElement).value || null"
            @blur="void saveDraft()"
          ></label>
        </div>
        ...
        <label class="mt-2 block space-y-1"><span>{{ t("ide.workspace.common.summary") }}</span><textarea
        ...
        <div class="mt-2 grid grid-cols-2 gap-2">
          <label class="space-y-1"><span>{{ t("ide.workspace.common.aliases") }}</span><TagInput
            :model-value="draft.aliases"
            :placeholder="t('ide.workspace.common.addAlias')"
            @update:model-value="draft.aliases = $event; void saveDraft()"
          /></label>
          <label class="space-y-1"><span>{{ t("ide.workspace.common.tags") }}</span><TagInput
            :model-value="draft.tags"
            :placeholder="t('ide.workspace.common.addTag')"
            accent-style
            @update:model-value="draft.tags = $event; void saveDraft()"
          /></label>
        </div>
      </section>
      ...
      <section class="grid grid-cols-2 gap-3">
        <div class="space-y-2 rounded-xl border border-[var(--border-color)] bg-[var(--bg-input)]/40 p-3">
          <div class="font-semibold text-[var(--text-main)]">
            {{ t("ide.workspace.typedProfile.spaceEnvironment") }}
          </div>
          <input
            :value="draft.location.parent ?? ''"
            class="field"
            :placeholder="t('ide.workspace.typedProfile.parentLocation')"
            @input="draft.location.parent = ($event.target as HTMLInputElement).value || null"
            @blur="void saveDraft()"
          >
          <input
            :value="draft.location.region ?? ''"
            class="field"
            :placeholder="t('ide.workspace.typedProfile.region')"
            @input="draft.location.region = ($event.target as HTMLInputElement).value || null"
            @blur="void saveDraft()"
          >
          <input
            :value="draft.location.terrain ?? ''"
            class="field"
            :placeholder="t('ide.workspace.typedProfile.terrain')"
            @input="draft.location.terrain = ($event.target as HTMLInputElement).value || null"
            @blur="void saveDraft()"
          >
          <input
            :value="draft.location.climate ?? ''"
            class="field"
            :placeholder="t('ide.workspace.typedProfile.climate')"
            @input="draft.location.climate = ($event.target as HTMLInputElement).value || null"
            @blur="void saveDraft()"
          >
          <textarea
            :value="draft.location.atmosphere ?? ''"
            rows="3"
            class="textarea"
            :placeholder="t('ide.workspace.typedProfile.atmosphere')"
            @input="draft.location.atmosphere = ($event.target as HTMLTextAreaElement).value || null"
            @blur="void saveDraft()"
          />
        </div>
        <div class="space-y-2 rounded-xl border border-[var(--border-color)] bg-[var(--bg-input)]/40 p-3">
          <div class="font-semibold text-[var(--text-main)]">
            {{ t("ide.workspace.typedProfile.availability") }}
          </div>
          <textarea
            :value="draft.location.access ?? ''"
            rows="2"
            class="textarea"
            :placeholder="t('ide.workspace.typedProfile.access')"
            @input="draft.location.access = ($event.target as HTMLTextAreaElement).value || null"
            @blur="void saveDraft()"
          />
          <TagInput
            :model-value="draft.location.landmarks"
            :placeholder="t('ide.workspace.typedProfile.landmarks')"
            @update:model-value="draft.location.landmarks = $event; void saveDraft()"
          />
          <TagInput
            :model-value="draft.location.risks"
            :placeholder="t('ide.workspace.typedProfile.risks')"
            @update:model-value="draft.location.risks = $event; void saveDraft()"
          />
          <TagInput
            :model-value="draft.location.resources"
            :placeholder="t('ide.workspace.typedProfile.resources')"
            @update:model-value="draft.location.resources = $event; void saveDraft()"
          />
        </div>
      </section>
      ...
      <section class="space-y-2 rounded-xl border border-[var(--border-color)] bg-[var(--bg-input)]/40 p-3">
        <div class="font-semibold text-[var(--text-main)]">
          {{ t("ide.workspace.typedProfile.aiReferences") }}
        </div>
        <div class="flex flex-wrap items-center gap-4">
          <label class="flex items-center gap-2 text-[var(--text-secondary)]">
            <input
              v-model="draft.retrieval.enabled"
              type="checkbox"
        ...
        <textarea
          :value="draft.retrieval.trigger ?? ''"
          rows="2"
          class="textarea"
          :placeholder="t('ide.workspace.common.retrievalTrigger')"
          @input="draft.retrieval.trigger = ($event.target as HTMLTextAreaElement).value || null"
          @blur="void saveDraft()"
        />
        <div
          v-for="(entryRef, index) in draft.refs"
          :key="index"
        ...
        >
          ...
          <button
            type="button"
            class="flex h-7 w-7 items-center justify-center rounded-md text-[var(--text-muted)] hover:bg-[var(--status-danger-bg)] hover:text-[var(--status-danger)]"
