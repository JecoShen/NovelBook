...
<template>
  ...
  >
    ...
    <template #actions>
      ...
      >
        {{ t("ide.workspace.common.save") }}
      </button>
    </template>

    <div
      v-if="editForm"
      class="space-y-4 p-3 pb-6 text-[11px]"
      :class="savingFile ? 'pointer-events-none opacity-80' : ''"
    >
      <div
        v-if="diagnostics"
        class="rounded-xl border border-[var(--status-warning-border)] bg-[var(--status-warning-bg)] px-3 py-2 text-[11px] leading-5 text-[var(--status-warning)]"
      >
        {{ diagnostics }}
      </div>
      <div
      ...
      <div class="space-y-3 rounded-2xl border border-[var(--border-color)] bg-[var(--bg-input)]/20 p-3">
        <div class="space-y-1">
          ...
          <div class="rounded-lg border border-[var(--border-color)]/70 bg-[var(--bg-panel)] px-2 py-1.5 font-mono text-[10px] text-[var(--text-muted)]">
            {{ editForm.path }}
          </div>
        </div>
        <div class="grid grid-cols-2 gap-4">
          <div class="space-y-1">
            ...
            <input
              ...
              type="text"
              disabled
              class="w-full rounded-lg border border-[var(--border-color)] bg-[var(--bg-panel)] px-2 py-1 text-xs font-mono text-[var(--text-muted)] outline-none"
            >
          </div>
          <div class="space-y-1">
            ...
            <input
              ...
              type="text"
              class="w-full rounded-lg border border-[var(--border-color)] bg-[var(--bg-panel)] px-2 py-1 text-xs text-[var(--text-main)] outline-none focus:border-[var(--accent-main)]"
              @blur="void saveDraft()"
            >
          </div>
        </div>
      ...
      <div class="grid grid-cols-3 gap-4">
        <div class="space-y-1">
          ...
          <FormSelect
            ...
            :options="typeOptions"
            @update:model-value="editForm.type = $event as LorebookFileDraft['type']; void saveDraft()"
          />
        </div>
        <div class="space-y-1">
          <label class="font-medium text-[var(--text-secondary)]">{{ t("ide.workspace.lorebookDetail.subtype") }}</label>
          <Combobox
            :model-value="editForm.subtype"
            :options="subtypeOptions"
            :placeholder="t('ide.workspace.lorebookDetail.subtypePlaceholder')"
            @update:model-value="editForm.subtype = $event; void saveDraft()"
          />
        </div>
        <div class="space-y-1">
      ...
      <div class="space-y-3">
        <div class="space-y-1">
          ...
          <TagInput
            ...
            :placeholder="t('ide.workspace.common.addThenEnter')"
      ...
      <div class="space-y-4">
        <div class="space-y-1">
          ...
          <textarea
            ...
            rows="5"
            class="w-full resize-y rounded-lg border border-[var(--border-color)] bg-[var(--bg-panel)] px-2 py-1.5 text-xs text-[var(--text-main)] outline-none focus:border-[var(--accent-main)]"
            @blur="void saveDraft()"
          />
        </div>
      </div>
      ...
      <div class="overflow-hidden rounded-xl border border-[var(--border-color)] bg-[var(--bg-panel)] shadow-sm">
        <button
          type="button"
          class="flex w-full items-center justify-between bg-[var(--bg-sidebar)] px-3 py-2 text-left hover:bg-[var(--bg-hover)]"
          @click="expandedSections.retrieval = !expandedSections.retrieval"
        >
          <span class="flex items-center gap-2 font-medium text-[var(--text-main)]"><span class="i-lucide-target h-3.5 w-3.5 text-[var(--text-muted)]" />{{ t("ide.workspace.lorebookDetail.retrievalTitle") }}</span>
          <span
            class="i-lucide-chevron-down h-3.5 w-3.5 text-[var(--text-muted)] transition-transform duration-200"
            :class="expandedSections.retrieval ? '' : '-rotate-90'"
          />
        </button>
        <div
          v-show="expandedSections.retrieval"
          class="space-y-3 bg-[var(--bg-input)]/30 p-2.5"
        >
          ...
          <textarea
            ...
            rows="2"
            class="w-full resize-y rounded-lg border border-[var(--border-color)] bg-[var(--bg-panel)] px-2 py-1.5 text-xs text-[var(--text-main)] outline-none focus:border-[var(--accent-main)]"
            :placeholder="t('ide.workspace.common.retrievalTrigger')"
      ...
      <div class="overflow-hidden rounded-xl border border-[var(--border-color)] bg-[var(--bg-panel)] shadow-sm">
        ...
        >
          <span class="flex items-center gap-2 font-medium text-[var(--text-main)]"><span class="i-lucide-link h-3.5 w-3.5 text-[var(--text-muted)]" />{{ t("ide.workspace.lorebookDetail.referencesTitle") }}</span>
          <span
            class="i-lucide-chevron-down h-3.5 w-3.5 text-[var(--text-muted)] transition-transform duration-200"
        ...
        <div
          ...
          class="bg-[var(--bg-input)]/30 p-2"
        >
          <div
            v-for="(entryRef, index) in editForm.refs"
            :key="index"
            class="group mb-1.5 flex items-center gap-1"
          >
            <input
              v-model="entryRef.relation"
              type="text"
              :placeholder="t('ide.workspace.common.relation')"
              class="w-[76px] rounded-lg border border-[var(--border-color)] bg-[var(--bg-panel)] px-1.5 py-1 text-[11px] outline-none"
              @blur="void saveDraft()"
            >
            <span class="i-lucide-arrow-right h-3 w-3 shrink-0 text-[var(--text-muted)]" />
            <input
              v-model="entryRef.target"
              type="text"
              :placeholder="t('ide.workspace.lorebookDetail.targetOrPending')"
              class="min-w-0 flex-1 rounded-lg border border-[var(--border-color)] bg-[var(--bg-panel)] px-1.5 py-1 text-[11px] font-mono outline-none"
              @blur="void saveDraft()"
            >
            <button
              type="button"
              class="flex h-6 w-6 shrink-0 items-center justify-center rounded-md text-[var(--text-muted)] hover:bg-[var(--status-danger-bg)] hover:text-[var(--status-danger)]"
          ...
          <button
            type="button"
            class="mt-2 flex items-center gap-1 px-1 text-[10px] font-bold uppercase tracking-wider text-[var(--text-muted)] hover:text-[var(--text-main)]"
            @click="addRef"
          >
      ...
      <div class="overflow-hidden rounded-xl border border-[var(--border-color)] bg-[var(--bg-panel)] shadow-sm">
        <button
          type="button"
          class="flex w-full items-center justify-between bg-[var(--bg-sidebar)] px-3 py-2 text-left hover:bg-[var(--bg-hover)]"
          @click="expandedSections.governance = !expandedSections.governance"
        >
          <span class="flex items-center gap-2 font-medium text-[var(--text-main)]"><span class="i-lucide-shield-check h-3.5 w-3.5 text-[var(--text-muted)]" />{{ t("ide.workspace.lorebookDetail.governanceTitle") }}</span>
          <span
            class="i-lucide-chevron-down h-3.5 w-3.5 text-[var(--text-muted)] transition-transform duration-200"
            :class="expandedSections.governance ? '' : '-rotate-90'"
          />
        </button>
        <div
          v-show="expandedSections.governance"
          class="space-y-3 bg-[var(--bg-input)]/30 p-2.5"
        >
          <div class="grid grid-cols-2 gap-3">
            <input
              ...
              type="text"
              class="rounded-lg border border-[var(--border-color)] bg-[var(--bg-panel)] px-2 py-1 text-xs outline-none"
              placeholder="source"
              @blur="void saveDraft()"
            >
            <input
