      ...
      <div class="flex min-w-0 flex-1 items-center justify-between gap-4">
        <div class="flex min-w-0 items-center gap-4">
          <span class="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl border border-[var(--status-danger-border)] bg-[var(--status-danger-bg)] text-[var(--status-danger)]">
            <span class="i-lucide-book-key h-7 w-7" />
          </span>
          <div class="min-w-0">
            <h2 class="truncate text-xl font-bold text-[var(--text-main)]">
              {{ draft?.title || t("ide.workspace.typedProfile.ruleProfile") }}
            </h2>
    ...
    >
      ...
      <section class="rounded-xl border border-[var(--status-danger-border)] bg-[var(--status-danger-bg)] p-3">
        <div class="grid grid-cols-[minmax(0,1fr)_132px_150px] gap-2">
          <label class="space-y-1"><span>{{ t("ide.workspace.typedProfile.ruleName") }}</span><input
            v-model="draft.title"
            class="field"
            @blur="void saveDraft()"
          ></label>
          ...
          /></label>
          <label class="space-y-1"><span>{{ t("ide.workspace.typedProfile.category") }}</span><input
            :value="draft.subtype ?? ''"
            class="field"
            list="rule-subtypes"
        ...
        <label class="mt-2 block space-y-1"><span>{{ t("ide.workspace.common.summary") }}</span><textarea
          ...
          @blur="void saveDraft()"
        /></label>
        <label class="mt-2 block space-y-1"><span>{{ t("ide.workspace.common.tags") }}</span><TagInput
      ...
      <section class="grid grid-cols-[1fr_1fr] gap-3">
        <div class="space-y-2 rounded-xl border border-[var(--border-color)] bg-[var(--bg-input)]/40 p-3">
          <div class="font-semibold text-[var(--text-main)]">
            {{ t("ide.workspace.typedProfile.triggerEffect") }}
          </div>
          <input
            :value="draft.rule.scope ?? ''"
            class="field"
            :placeholder="t('ide.workspace.typedProfile.scope')"
            @input="draft.rule.scope = ($event.target as HTMLInputElement).value || null"
            @blur="void saveDraft()"
          >
          <input
            :value="draft.rule.priority ?? ''"
            class="field"
            :placeholder="t('ide.workspace.typedProfile.priority')"
            @input="draft.rule.priority = ($event.target as HTMLInputElement).value || null"
            @blur="void saveDraft()"
          >
          <textarea
            :value="draft.rule.trigger ?? ''"
            rows="3"
            class="textarea"
            :placeholder="t('ide.workspace.typedProfile.trigger')"
            @input="draft.rule.trigger = ($event.target as HTMLTextAreaElement).value || null"
            @blur="void saveDraft()"
          />
          <textarea
            :value="draft.rule.effect ?? ''"
            rows="4"
            class="textarea"
            :placeholder="t('ide.workspace.typedProfile.effect')"
            @input="draft.rule.effect = ($event.target as HTMLTextAreaElement).value || null"
            @blur="void saveDraft()"
          />
        </div>
        <div class="space-y-2 rounded-xl border border-[var(--border-color)] bg-[var(--bg-input)]/40 p-3">
          <div class="font-semibold text-[var(--text-main)]">
            {{ t("ide.workspace.typedProfile.boundaries") }}
          </div>
          <textarea
            :value="draft.rule.limits ?? ''"
            rows="3"
            class="textarea"
            :placeholder="t('ide.workspace.typedProfile.limits')"
            @input="draft.rule.limits = ($event.target as HTMLTextAreaElement).value || null"
            @blur="void saveDraft()"
          />
          <TagInput
            :model-value="draft.rule.exceptions"
            :placeholder="t('ide.workspace.typedProfile.exceptions')"
            @update:model-value="draft.rule.exceptions = $event; void saveDraft()"
          />
          <TagInput
            :model-value="draft.rule.conflicts"
            :placeholder="t('ide.workspace.typedProfile.conflicts')"
            @update:model-value="draft.rule.conflicts = $event; void saveDraft()"
          />
          <TagInput
            :model-value="draft.rule.examples"
            :placeholder="t('ide.workspace.typedProfile.examples')"
            @update:model-value="draft.rule.examples = $event; void saveDraft()"
          />
        </div>
      </section>
      ...
      <section class="space-y-2 rounded-xl border border-[var(--border-color)] bg-[var(--bg-input)]/40 p-3">
        ...
        <div class="grid grid-cols-2 gap-2">
          <input
            v-model="draft.governance.source"
            class="field"
            placeholder="source"
            @blur="void saveDraft()"
          >
          <input
            v-model="draft.governance.review"
            class="field"
            placeholder="review"
            @blur="void saveDraft()"
          >
