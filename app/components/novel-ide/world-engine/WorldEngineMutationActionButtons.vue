...
<template>
  ...
  <div class="mt-2 flex flex-wrap gap-2">
    <button
      type="button"
      class="inline-flex h-8 min-w-[86px] flex-1 items-center justify-center gap-1 rounded-md border border-[var(--border-color)] px-2 text-[12px] text-[var(--text-main)] hover:bg-[var(--bg-hover)] disabled:opacity-50"
      :disabled="props.disabled"
      @click="runAddMutation('append')"
    >
      <span class="i-lucide-list-plus h-3.5 w-3.5" />
      追加
    </button>
    <button
      type="button"
      class="inline-flex h-8 min-w-[86px] flex-1 items-center justify-center gap-1 rounded-md border border-[var(--border-color)] px-2 text-[12px] text-[var(--text-main)] hover:bg-[var(--bg-hover)] disabled:opacity-50"
      :disabled="props.disabled || !canUseSelectedMutation"
      title="用 Builder 当前内容替换所选 mutation"
      @click="runReplaceSelectedMutation"
    >
    ...
    <button
      ...
      class="inline-flex h-8 min-w-[86px] flex-1 items-center justify-center gap-1 rounded-md border border-[var(--border-color)] px-2 text-[12px] text-[var(--text-main)] hover:bg-[var(--bg-hover)] disabled:opacity-50"
      :disabled="props.disabled || !canUseSelectedMutation"
      title="在所选 mutation 后插入 Builder 当前内容"
      @click="runInsertAfterSelectedMutation"
    >
      <span class="i-lucide-corner-down-right h-3.5 w-3.5" />
      插入其后
    </button>
    <button
      type="button"
    ...
    <button
      type="button"
      class="inline-flex h-8 min-w-[86px] flex-1 items-center justify-center gap-1 rounded-md border border-[var(--status-danger-border)] px-2 text-[12px] text-[var(--status-danger)] hover:bg-[var(--status-danger-bg)] disabled:opacity-50"
      :disabled="props.disabled || !canUseSelectedMutation"
      title="删除所选 mutation"
      @click="runDeleteSelectedMutation"
    >
    ...
    <button
      ...
      class="inline-flex h-8 min-w-[86px] flex-1 items-center justify-center gap-1 rounded-md border border-[var(--border-color)] px-2 text-[12px] text-[var(--text-main)] hover:bg-[var(--bg-hover)] disabled:opacity-50"
      :disabled="props.disabled"
      @click="runAddMutation('replace')"
    >
      <span class="i-lucide-refresh-ccw h-3.5 w-3.5" />
      替换全部
    </button>
  </div>
</template>
