...
import type { WorldMutationOp, WorldPreviewSchemaAttr } from 'nbook/app/utils/world-engine-preview'
...
<template>
  ...
  <div class="rounded-md border border-[var(--border-color)] bg-[var(--bg-panel)] p-3">
    <fieldset
      class="m-0 border-0 p-0 disabled:opacity-70"
      :disabled="props.disabled"
    >
      <div class="mb-2 flex items-center justify-between gap-2">
        <div class="flex min-w-0 items-center gap-2">
          <div class="text-[11px] font-semibold uppercase tracking-[0.16em] text-[var(--text-secondary)]">
            Mutation Builder
          </div>
      ...
      <div class="grid grid-cols-2 gap-2">
        <select
          :value="builder.subjectId"
          class="h-8 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-2 text-[12px] outline-none focus:border-[var(--accent-main)]"
          @change="updateBuilderField('subjectId', inputValue($event))"
        >
          <option
            v-for="subject in subjects"
            :key="`builder:${subject.id}`"
            :value="subject.id"
          >
        ...
        <select
          :value="builder.path"
          class="h-8 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-2 text-[12px] outline-none focus:border-[var(--accent-main)]"
          @change="updateBuilderField('path', inputValue($event))"
        >
          <option
            v-if="builder.path && !builderHasSchemaAttr"
            :value="builder.path"
          >
            自定义 · {{ builder.path }}
          </option>
          <option
            v-for="attr in builderAttrs"
            :key="`builder-attr:${attr.name}`"
            :value="attrPath(attr.name)"
          >
            {{ attr.name }}
          </option>
        </select>
        <input
          :value="builder.path"
          list="world-engine-builder-attrs"
          class="col-span-2 h-8 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-2 font-mono text-[12px] outline-none focus:border-[var(--accent-main)]"
          placeholder="JSON Pointer path, e.g. /equipment/weapon /memory/师门"
          @input="updateBuilderField('path', inputValue($event))"
        >
        <datalist id="world-engine-builder-attrs">
          <option
            v-for="attr in builderAttrs"
            :key="`builder-attr-option:${attr.name}`"
            :value="attrPath(attr.name)"
          />
        </datalist>
        <select
          :value="builder.op"
          class="h-8 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-2 text-[12px] outline-none focus:border-[var(--accent-main)]"
          @change="updateBuilderField('op', inputValue($event))"
        >
          <option
            v-for="op in builderOpOptions"
            :key="op"
            :value="op"
          >
            {{ op }}
        ...
        <input
          v-if="builderValueMode === 'number'"
          :value="builder.value"
          type="number"
          class="h-8 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-2 text-[12px] outline-none focus:border-[var(--accent-main)]"
          placeholder="value"
          @input="updateBuilderField('value', inputValue($event))"
        >
        <select
          v-else-if="builderValueMode === 'boolean'"
          :value="builder.value"
          class="h-8 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-2 text-[12px] outline-none focus:border-[var(--accent-main)]"
          @change="updateBuilderField('value', inputValue($event))"
        >
          <option value="true">
            true
          </option>
        ...
        <select
          v-else-if="builderValueMode === 'enum'"
          :value="builder.value"
          class="h-8 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-2 text-[12px] outline-none focus:border-[var(--accent-main)]"
          @change="updateBuilderField('value', inputValue($event))"
        >
          <option
            v-for="option in enumValueOptions"
            :key="option.value"
            :value="option.value"
          >
            {{ option.label }}
        ...
        <select
          v-else-if="builderValueMode === 'ref' && refValueOptions.length"
          :value="builder.value"
          class="h-8 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-2 text-[12px] outline-none focus:border-[var(--accent-main)]"
          @change="updateBuilderField('value', inputValue($event))"
        >
          <option
            v-for="option in refValueOptions"
            :key="option.value"
            :value="option.value"
          >
            {{ option.label }}
        ...
        <input
          v-else-if="builderValueMode === 'ref'"
          :value="builder.value"
          class="h-8 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-2 text-[12px] outline-none focus:border-[var(--accent-main)]"
          placeholder="subject://"
          @input="updateBuilderField('value', inputValue($event))"
        ...
        />
        <textarea
          v-else-if="builderValueMode === 'json'"
          :value="builder.value"
          rows="4"
          class="col-span-2 min-h-[96px] resize-y rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-2 py-1.5 font-mono text-[12px] leading-5 outline-none focus:border-[var(--accent-main)]"
          placeholder="{&quot;key&quot;: &quot;value&quot;}"
          title="当前 value 需要填写 JSON object"
          @input="updateBuilderField('value', inputValue($event))"
        />
        <input
          v-else-if="builderValueMode === 'text'"
          :value="builder.value"
          class="h-8 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-2 text-[12px] outline-none focus:border-[var(--accent-main)]"
          placeholder="value"
          @input="updateBuilderField('value', inputValue($event))"
        ...
        <input
          v-else
          class="h-8 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-2 text-[12px] opacity-50 outline-none"
          disabled
          placeholder="remove"
        >
      </div>
