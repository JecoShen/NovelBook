...
<template>
  <section class="p-5">
    ...
    <div
      v-if="lastContinueSaveNotice"
      data-testid="slice-composer-continue-save-notice"
      class="mb-4 flex items-center gap-2 rounded-md border border-[var(--we-success-border)] bg-[var(--we-success-soft)] px-3 py-2 text-[12px] text-[var(--we-success)]"
    >
      <span class="i-lucide-check-circle-2 h-4 w-4 shrink-0" />
      ...
      <aside class="space-y-3">
        <div class="rounded-md border border-[var(--border-color)] bg-[var(--bg-panel)] p-3">
          ...
          <div
            ...
            :key="`editor:${type.type}`"
            class="mb-3 last:mb-0"
          >
            <div class="mb-1 text-[12px] font-medium text-[var(--text-main)]">
              {{ type.type }}
            </div>
            <div class="flex flex-wrap gap-1">
              <button
                v-for="attr in type.attrs"
                :key="`editor:${type.type}:${attr.name}`"
                type="button"
                class="rounded-md border border-[var(--border-color)] px-2 py-1 text-[11px] text-[var(--text-secondary)] hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)] disabled:opacity-50"
                :disabled="schemaTypeShortcutDisabled(type.type)"
                :title="schemaTypeShortcutTitle(type.type)"
                @click="fillMutation(type.type, attr)"
              >
                {{ attr.name }}
              </button>
            </div>
