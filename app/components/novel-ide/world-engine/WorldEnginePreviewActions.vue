  ...
  <section class="min-w-0 rounded-md border border-[var(--border-color)] bg-[var(--bg-panel)]">
    ...
    <div class="space-y-5 p-4">
      ...
      <div class="space-y-2">
        ...
        <fieldset
          class="space-y-2 disabled:opacity-60"
          :disabled="loadingWorld || actionBusy"
        >
          <div class="grid grid-cols-2 gap-2">
            <input
              v-model="subjectForm.id"
              class="h-9 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-3 text-sm outline-none focus:border-[var(--accent-main)]"
              placeholder="id"
            >
            <select
              v-model="subjectForm.type"
              class="h-9 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-3 text-sm outline-none focus:border-[var(--accent-main)]"
            >
              <option
          ...
          <input
            ...
            class="h-9 w-full rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-3 text-sm outline-none focus:border-[var(--accent-main)]"
            placeholder="name"
          >
          <input
            v-model="subjectForm.time"
            class="h-9 w-full rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-3 text-sm outline-none focus:border-[var(--accent-main)]"
            placeholder="time"
          >
          <div class="flex flex-wrap gap-1">
            <button
              v-for="attr in selectedTypeAttrs.slice(0, 8)"
              :key="attr.name"
              type="button"
              class="rounded border border-[var(--border-color)] px-2 py-1 text-[11px] text-[var(--text-muted)]"
              disabled
            >
              {{ attr.name }}
            </button>
          ...
          <div
            v-if="subjectIdAlreadyExists"
            class="rounded-md border border-[var(--status-warning-border)] bg-[var(--status-warning-bg)] px-3 py-2 text-xs text-[var(--status-warning)]"
          >
            该 subject 已存在。点击左侧 subject 会载入查询上下文；新建 subject 请填写新的 id。
          </div>
          <button
            type="button"
            class="inline-flex h-9 w-full items-center justify-center gap-2 rounded-md border border-[var(--border-color)] px-3 text-sm hover:bg-[var(--bg-hover)] disabled:opacity-50"
            :disabled="!canCreateSubject"
            @click="emit('create-subject')"
          >
            <span class="i-lucide-circle-plus h-4 w-4" />
            创建 Subject
          </button>
        </fieldset>
      </div>
      ...
      <div class="space-y-2 border-t border-[var(--border-color)] pt-5">
        ...
        >
          <div class="flex items-center justify-between gap-2">
            <div class="text-xs font-semibold uppercase text-[var(--text-secondary)]">
              {{ editingSliceId ? "Edit Slice" : "Write Slice" }}
            </div>
            <button
              v-if="editingSliceId"
              type="button"
              class="rounded px-2 py-1 text-xs text-[var(--text-muted)] hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)]"
              @click="emit('clear-slice-edit-mode')"
            >
              取消编辑
            </button>
          </div>
          <div
            v-if="editingSliceId"
            class="rounded-md border border-[var(--accent-main)]/30 bg-[var(--accent-bg)] px-3 py-2 text-xs text-[var(--text-secondary)]"
          >
            当前整块替换 slice：{{ editingSliceId }}
          </div>
          <input
            v-model="sliceForm.time"
            class="h-9 w-full rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-3 text-sm outline-none focus:border-[var(--accent-main)]"
            placeholder="time"
          >
          <div class="grid grid-cols-[minmax(0,1fr)_96px] gap-2">
            <input
              v-model="sliceForm.title"
              class="h-9 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-3 text-sm outline-none focus:border-[var(--accent-main)]"
              placeholder="title"
            >
            <input
              v-model="sliceForm.kind"
              class="h-9 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-3 text-sm outline-none focus:border-[var(--accent-main)]"
              placeholder="kind"
            >
          </div>
          <textarea
            v-model="sliceForm.summary"
            class="min-h-16 w-full resize-y rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-3 py-2 text-sm outline-none focus:border-[var(--accent-main)]"
            placeholder="summary"
          />
          ...
          />
          <textarea
            v-model="sliceForm.mutations"
            class="min-h-40 w-full resize-y rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-3 py-2 font-mono text-xs outline-none focus:border-[var(--accent-main)]"
            spellcheck="false"
          />
          <button
            type="button"
            class="inline-flex h-9 w-full items-center justify-center gap-2 rounded-md border border-[var(--border-color)] px-3 text-sm hover:bg-[var(--bg-hover)] disabled:opacity-50"
            :disabled="!canWriteSlice"
            @click="emit('write-slice')"
          >
            <span
              :class="editingSliceId ? 'i-lucide-save' : 'i-lucide-send'"
              class="h-4 w-4"
            />
            {{ sliceActionLabel }}
          </button>
        ...
        <pre
          v-if="hasWriteResult"
          class="max-h-32 overflow-auto rounded bg-[var(--bg-input)] p-2 text-[11px] leading-5"
        >{{ writeResultJson }}</pre>
      </div>
      ...
      <div class="space-y-2 border-t border-[var(--border-color)] pt-5">
        ...
        >
          <input
            v-model="queryForm.subjectIds"
            class="h-9 w-full rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-3 text-sm outline-none focus:border-[var(--accent-main)]"
            placeholder="subjectIds"
          >
          <div class="grid grid-cols-2 gap-2">
            <input
            ...
            >
            <input
              v-model.number="queryForm.listLimit"
              type="number"
              min="1"
              max="100"
              class="h-9 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-3 text-sm outline-none focus:border-[var(--accent-main)]"
          ...
          <input
            v-model="queryForm.attrs"
            class="h-9 w-full rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-3 text-sm outline-none focus:border-[var(--accent-main)]"
            placeholder="attrs"
          >
          <input
            v-model="queryForm.at"
            class="h-9 w-full rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-3 text-sm outline-none focus:border-[var(--accent-main)]"
            placeholder="at"
          >
          <button
