...
import type { WorldMutationOp, WorldPreviewSchemaAttr, WorldPreviewStateSubject } from 'nbook/app/utils/world-engine-preview'
    ...
    >
      ...
      <div class="grid grid-cols-2 gap-2">
        <select
          ...
          class="h-8 rounded-md border border-[var(--border-color)] bg-[var(--bg-panel)] px-2 text-xs outline-none focus:border-[var(--accent-main)]"
          @change="emit('update-builder-field', 'subjectId', inputValue($event))"
        >
          <option :value="subjectFormId">
            {{ subjectFormId || "subject" }}
          </option>
          <option
        ...
        <select
          :value="builder.path"
          class="h-8 rounded-md border border-[var(--border-color)] bg-[var(--bg-panel)] px-2 text-xs outline-none focus:border-[var(--accent-main)]"
          @change="emit('update-builder-field', 'path', inputValue($event))"
        >
          <option
        ...
        <input
          :value="builder.path"
          list="world-engine-preview-builder-attrs"
          class="col-span-2 h-8 rounded-md border border-[var(--border-color)] bg-[var(--bg-panel)] px-2 font-mono text-xs outline-none focus:border-[var(--accent-main)]"
          placeholder="JSON Pointer path, e.g. /memory/师门"
        ...
        <select
          :value="builder.op"
          class="h-8 rounded-md border border-[var(--border-color)] bg-[var(--bg-panel)] px-2 text-xs outline-none focus:border-[var(--accent-main)]"
          @change="emit('update-builder-field', 'op', inputValue($event))"
        >
          <option
        ...
        <textarea
          v-if="valueRequiresJsonObject"
          :value="builder.value"
          rows="4"
          class="col-span-2 min-h-[92px] resize-y rounded-md border border-[var(--border-color)] bg-[var(--bg-panel)] px-2 py-1.5 font-mono text-xs leading-5 outline-none focus:border-[var(--accent-main)]"
          placeholder="{&quot;key&quot;: &quot;value&quot;}"
          title="当前 value 必须是 JSON object"
          @input="emit('update-builder-field', 'value', inputValue($event))"
        />
        <input
          v-else
          :value="builder.value"
          class="h-8 rounded-md border border-[var(--border-color)] bg-[var(--bg-panel)] px-2 text-xs outline-none focus:border-[var(--accent-main)] disabled:opacity-50"
          :disabled="builder.op === 'remove'"
          placeholder="value"
          @input="emit('update-builder-field', 'value', inputValue($event))"
        >
      </div>
