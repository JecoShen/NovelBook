  ...
  <section class="border-b border-[var(--border-color)] p-2">
    <div class="rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] p-3">
      <div class="mb-2 flex items-center justify-between gap-2">
        ...
        <span class="i-lucide-user-plus h-4 w-4 text-[var(--text-muted)]" />
      </div>
      <fieldset
      ...
      >
        <div class="grid grid-cols-2 gap-2">
          <input
            v-model="form.id"
            class="h-8 min-w-0 rounded-md border border-[var(--border-color)] bg-[var(--bg-panel)] px-2 text-[12px] outline-none focus:border-[var(--accent-main)]"
            placeholder="id"
          >
          <select
            v-model="form.type"
        ...
        <input
          v-model="form.name"
          class="h-8 w-full rounded-md border border-[var(--border-color)] bg-[var(--bg-panel)] px-2 text-[12px] outline-none focus:border-[var(--accent-main)]"
          placeholder="name"
        >
        <input
          v-model="form.time"
        ...
        <div class="flex max-h-14 flex-wrap gap-1 overflow-hidden">
          <span
            ...
            :key="attr.name"
            class="rounded border border-[var(--border-color)] px-1.5 py-0.5 text-[10px] text-[var(--text-muted)]"
          >{{ attr.name }}</span>
        </div>
        <button
          type="button"
          class="inline-flex h-8 w-full items-center justify-center gap-2 rounded-md border border-[var(--border-color)] px-2 text-[12px] text-[var(--text-main)] hover:bg-[var(--bg-hover)] disabled:opacity-50"
          :disabled="!canSubmit"
          @click="void createSubject()"
        >
          <span
            :class="creating ? 'i-lucide-loader-2 animate-spin' : 'i-lucide-circle-plus'"
            class="h-3.5 w-3.5"
          />
          创建 Subject
        </button>
