...
<template>
  <div class="group flex flex-col text-[12px]">
    <div
      class="flex items-center gap-2 px-2 py-1.5 transition-colors hover:bg-[var(--bg-hover)]"
    ...
    >
      ...
      <div class="flex w-[180px] shrink-0 items-center gap-1 font-mono text-[var(--text-secondary)]">
        <button
          v-if="hasChildren"
          type="button"
          class="inline-flex h-4 w-4 shrink-0 items-center justify-center rounded hover:bg-[var(--bg-hover)]"
          @click.stop="toggle"
        >
          <span
        ...
        >{{ props.attr.name }}</span>
      </div>

      <!-- Value -->
      <div
        class="min-w-0 flex-1 truncate font-mono text-[var(--text-main)]"
        :class="{ 'opacity-50': !hasValue }"
      >
        <span
          v-if="!hasValue && !hasDefault"
          class="text-[10px] italic"
        >undefined</span>
        <span
          v-else-if="!hasValue && hasDefault"
          :title="displayValue"
        >{{ displayValue }} <span class="text-[10px] opacity-60 italic">(default)</span></span>
        <span
          v-else
          :title="displayValue"
        >{{ displayValue }}</span>
      ...
      <!-- Desc -->
      <div
        class="w-[180px] shrink-0 truncate text-[11px] text-[var(--text-muted)]"
        :title="cleanDesc"
      >
        <span
          v-if="isRef"
          class="i-lucide-link h-3 w-3 mr-1 inline-block align-text-bottom"
        />
        {{ cleanDesc }}
      </div>
      ...
      <div class="w-[64px] shrink-0 text-right">
        <span
          class="rounded bg-[var(--bg-input)] px-1.5 py-0.5 text-[9px] uppercase tracking-wider text-[var(--text-muted)]"
          :title="props.attr.kind"
        >{{ props.attr.kind }}</span>
      </div>
    </div>
