...
<template>
  ...
  <section class="p-5">
    <div class="mb-4 flex items-center justify-between gap-3">
      <div>
        <h2 class="m-0 text-[18px] font-semibold text-[var(--text-main)]">
          Timeline
        </h2>
        <p class="m-0 mt-1 text-[12px] text-[var(--text-muted)]">
          {{ visibleSlices.length }} / {{ slices.length }} slices · {{ visibleMutationCount }} / {{ totalMutationCount }} mutations · {{ totalIssueCount }} issues
        </p>
      </div>
      <div class="flex items-center gap-2">
        <label class="inline-flex h-9 items-center gap-2 rounded-md border border-[var(--border-color)] px-3 text-[12px] text-[var(--text-secondary)] hover:bg-[var(--bg-hover)]">
          <input
            v-model="timelineOnlySelectedSubject"
            type="checkbox"
            class="h-4 w-4 accent-[var(--accent-main)]"
    ...
    <div class="mb-4 flex items-center gap-2 rounded-md border border-[var(--border-color)] bg-[var(--bg-panel)] px-3">
      ...
      <input
        v-model="timelineSearchText"
        class="h-9 min-w-0 flex-1 bg-transparent text-[13px] text-[var(--text-main)] outline-none placeholder:text-[var(--text-muted)]"
        placeholder="搜索 title / time / summary / mutation"
      >
      <button
        v-if="timelineSearchText"
        type="button"
        class="inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-md text-[var(--text-muted)] hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)]"
        title="清空搜索"
        @click="timelineSearchText = ''"
      >
    ...
    <div class="space-y-3">
      <div
        ...
        :class="selectedSliceId === slice.id ? 'border-[var(--accent-main)] bg-[var(--accent-bg)]' : 'border-[var(--border-color)] bg-[var(--bg-panel)] hover:bg-[var(--bg-hover)]'"
        @click="emit('select-slice', slice.id)"
        @keydown.enter="emit('select-slice', slice.id)"
      >
        <div class="flex items-start justify-between gap-4">
          <div class="min-w-0">
            <div class="truncate text-[14px] font-semibold text-[var(--text-main)]">
            ...
            <div
              v-if="slice.summary"
              class="mt-2 line-clamp-2 text-[12px] text-[var(--text-secondary)]"
            >
              {{ slice.summary }}
            </div>
          </div>
          <div class="flex shrink-0 items-center gap-2">
            <span
              ...
              class="inline-flex items-center gap-1 rounded-md border border-[var(--status-warning-border)] bg-[var(--status-warning-bg)] px-2 py-1 text-[11px] text-[var(--status-warning)]"
            >
            ...
            <button
              ...
              title="编辑 slice"
              aria-label="编辑 slice"
              @click.stop="emit('edit-slice', slice.id)"
            >
      ...
      <div
        v-if="!visibleSlices.length"
        class="rounded-md border border-[var(--border-color)] bg-[var(--bg-panel)] px-4 py-12 text-center text-[13px] text-[var(--text-muted)]"
      >
        {{ timelineEmptyText }}
      </div>
    </div>
