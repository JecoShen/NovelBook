  ...
  >
    <template #header>
      ...
      <div class="world-engine-workbench-theme flex min-w-0 flex-1 items-center gap-3">
        ...
        <div class="min-w-0">
          <div class="text-[16px] font-semibold text-[var(--text-main)]">
            World Engine Workbench
          </div>
          <div class="flex min-w-0 items-center gap-2 truncate text-[12px] text-[var(--text-muted)]">
            ...
            <span
              v-if="workbenchSchema.calendar.format"
              class="hidden rounded-md border border-[var(--border-color)] px-1.5 py-0.5 text-[10px] md:inline"
            >{{ workbenchSchema.calendar.format }}</span>
        ...
        <div class="ml-auto flex items-center gap-2">
          ...
          >
            <span class="i-lucide-list-todo h-3.5 w-3.5" />
          ...
          <button
            type="button"
            class="inline-flex h-8 items-center gap-1.5 rounded-md border border-[var(--border-color)] px-3 text-[12px] text-[var(--text-secondary)] transition-colors hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)] disabled:opacity-50"
            :disabled="workbenchActionBusy"
            @click="void refreshWorldForCurrentTimeline()"
          >
            <span
          ...
          <button
            type="button"
            class="inline-flex h-8 items-center gap-1.5 rounded-md border border-[var(--border-color)] px-3 text-[12px] text-[var(--text-secondary)] transition-colors hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)] disabled:opacity-50"
            :disabled="workbenchActionBusy || !schema"
            @click="openSliceComposer"
          >
            <span class="i-lucide-file-plus-2 h-3.5 w-3.5" />
            新建 Slice
          </button>
          <button
            type="button"
            class="hidden h-8 items-center gap-1.5 rounded-md border border-[var(--border-color)] px-3 text-[12px] text-[var(--text-secondary)] transition-colors hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)] disabled:opacity-50 lg:inline-flex"
            :disabled="workbenchActionBusy || !schema || !selectedSlice"
            @click="void openSelectedSliceComposer()"
          >
          ...
          <button
            type="button"
            class="hidden h-8 items-center gap-1.5 rounded-md border border-[var(--border-color)] px-3 text-[12px] text-[var(--text-secondary)] transition-colors hover:border-[var(--we-danger-border)] hover:bg-[var(--we-danger-soft)] hover:text-[var(--we-danger)] disabled:opacity-50 xl:inline-flex"
            :disabled="workbenchActionBusy || !selectedSlice"
            @click="void deleteSelectedSlice()"
          >
            <span
              :class="actionBusy ? 'i-lucide-loader-2 animate-spin' : 'i-lucide-trash-2'"
              class="h-3.5 w-3.5"
            />
            删除 Slice
          </button>
          ...
          >
            <span
              :class="inspectorVisible ? 'i-lucide-panel-right-close' : 'i-lucide-panel-right-open'"
              class="h-3.5 w-3.5"
            />
            {{ t("worldEngine.workbenchPreview.inspector") }}
            <span
              v-if="selectedSliceSubjectFileProposalCount"
              data-testid="world-workbench-inspector-proposal-count"
              class="rounded bg-[var(--we-bg-panel)] px-1.5 font-mono text-[10px]"
          ...
          <button
            type="button"
            data-testid="world-workbench-close"
            aria-label="关闭 World Engine Workbench"
            title="关闭 World Engine Workbench"
            class="inline-flex h-8 w-8 items-center justify-center rounded-md text-[var(--text-muted)] transition-colors hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)] disabled:opacity-50"
            :disabled="workbenchActionBusy"
            @click="void requestWorkbenchClose()"
          >
    ...
    <div class="world-engine-workbench-dialog world-engine-workbench-theme relative flex min-h-0 flex-1 flex-col overflow-hidden bg-[var(--we-bg-canvas)] text-[var(--we-text-main)]">
      ...
      <div
        v-if="error"
        class="border-b border-[var(--we-danger-border)] bg-[var(--we-danger-soft)] px-4 py-2 text-[13px] text-[var(--we-danger)]"
      >
        {{ error }}
      </div>

      <div
        v-if="sliceComposerVisible"
        data-testid="world-slice-composer"
        class="absolute inset-3 z-30 flex min-h-0 flex-col overflow-hidden rounded-md border border-[var(--we-border)] bg-[var(--we-bg-panel)] shadow-2xl"
        @input.capture="markSliceComposerDirtyFromInput"
        @change.capture="markSliceComposerDirtyFromInput"
      >
        <div class="flex h-11 shrink-0 items-center justify-between border-b border-[var(--we-border)] px-4">
          <div class="flex min-w-0 items-center gap-2">
            <span class="i-lucide-file-plus-2 h-4 w-4 text-[var(--we-accent)]" />
          ...
          <button
            type="button"
            data-testid="world-slice-composer-close"
            aria-label="关闭 Slice Composer"
            title="关闭 Slice Composer"
            class="inline-flex h-8 w-8 items-center justify-center rounded-md text-[var(--we-text-muted)] transition-colors hover:bg-[var(--we-bg-hover)] hover:text-[var(--we-text-main)]"
            @click="void closeSliceComposer()"
          >
            <span class="i-lucide-x h-4 w-4" />
          </button>
        </div>
      ...
      <div class="flex min-h-0 flex-1">
        ...
        >
          <template #actions>
            <section
              v-if="pendingSubjectSystemSummaries.length"
              data-testid="subject-system-sync-panel"
              class="rounded-md border border-[var(--we-warning-border)] bg-[var(--we-warning-soft)] p-2.5"
            >
              <div class="flex items-start justify-between gap-2">
                <div class="min-w-0">
                  <div class="text-[11px] font-semibold uppercase tracking-[0.12em] text-[var(--we-warning)]">
                    主体系统待接入
                  </div>
                  <div class="mt-1 text-[11px] text-[var(--we-text-secondary)]">
                    {{ pendingSubjectSystemSummaries.length }} 个 simulation/subjects 主体还没有 World Engine subject 身份。
                  </div>
                  <div class="mt-1 text-[11px] text-[var(--we-text-secondary)]">
                    同步只注册身份，不复制或改写六文件正文。
                  </div>
              ...
              <div class="mt-2 flex flex-wrap gap-1">
                <span
                  v-for="summary in pendingSubjectSystemSummaries.slice(0, 4)"
                  :key="`pending-subject:${summary.subjectId}`"
                  class="rounded border border-[var(--we-warning-border)] bg-[var(--we-bg-panel)] px-1.5 py-0.5 font-mono text-[10px] text-[var(--we-warning)]"
                >{{ summary.subjectId }}</span>
                <span
                  v-if="pendingSubjectSystemSummaries.length > 4"
                  class="rounded border border-[var(--we-warning-border)] bg-[var(--we-bg-panel)] px-1.5 py-0.5 font-mono text-[10px] text-[var(--we-warning)]"
                >+{{ pendingSubjectSystemSummaries.length - 4 }}</span>
              </div>
              <div class="mt-2 rounded border border-[var(--we-warning-border)] bg-[var(--we-bg-panel)] p-2 text-[11px]">
                ...
                <input
                  v-model.trim="subjectSystemSyncTimeOverride"
                  type="text"
                  class="h-7 w-full rounded border border-[var(--we-border)] bg-[var(--we-bg-data)] px-2 font-mono text-[11px] text-[var(--we-text-main)] outline-none transition-colors placeholder:text-[var(--we-text-muted)] focus:border-[var(--we-warning-border)] disabled:opacity-50"
                  :placeholder="subjectSystemDefaultSyncTime || '未配置'"
                  :disabled="workbenchActionBusy"
                  aria-label="同步主体系统初始化时间"
                >
              </div>
              <button
                type="button"
                class="mt-2 inline-flex h-8 w-full items-center justify-center gap-1.5 rounded-md border border-[var(--we-warning-border)] bg-[var(--we-bg-panel)] px-2 text-[12px] font-medium text-[var(--we-warning)] transition-colors hover:bg-[var(--we-bg-hover)] disabled:opacity-50"
                :disabled="workbenchActionBusy || !subjectSystemSyncTime"
                @click="void syncPendingSubjectSystemSubjects()"
              >
                <span
                  :class="actionBusy ? 'i-lucide-loader-2 animate-spin' : 'i-lucide-link-2'"
                  class="h-3.5 w-3.5"
                />
                同步主体系统
              </button>
            ...
            <details
              class="rounded-md border border-[var(--we-border)] bg-[var(--we-bg-panel)]"
              :open="subjectCreatorOpen"
              @toggle="updateSubjectCreatorOpen"
            >
              <summary class="cursor-pointer px-2.5 py-1.5 text-[11px] font-semibold uppercase tracking-[0.12em] text-[var(--we-text-secondary)]">
                创建 Subject
              </summary>
        ...
        <main class="flex min-w-0 flex-1 flex-col overflow-hidden">
          ...
          <div
            v-else
            class="flex min-h-0 flex-1 items-center justify-center border-t border-[var(--we-border)] bg-[var(--we-bg-panel)] px-6 text-center"
          >
            <div class="max-w-xl">
              <div class="text-[15px] font-semibold text-[var(--we-text-main)]">
                {{ emptySliceState.title }}
              </div>
              <div class="mt-2 text-[13px] text-[var(--we-text-muted)]">
                {{ emptySliceState.description }}
              </div>
              <div
                v-if="reviewQueueItems.length"
                data-testid="empty-slice-review-issues"
                class="mt-4 space-y-2 rounded-md border border-[var(--we-warning-border)] bg-[var(--we-warning-soft)] p-2.5 text-left"
              >
                <div class="flex flex-wrap items-center justify-between gap-2">
                  <div class="flex min-w-0 items-center gap-1.5 text-[11px] font-semibold text-[var(--we-warning)]">
                    <span class="i-lucide-circle-alert h-3.5 w-3.5 shrink-0" />
                ...
                >
                  <span
                    class="justify-self-start rounded border px-1.5 py-0.5 font-mono text-[10px] font-semibold"
                    :class="issueLevel(item.severity) === 'E' ? 'border-[var(--we-danger-border)] bg-[var(--we-danger-soft)] text-[var(--we-danger)]' : 'border-[var(--we-warning-border)] bg-[var(--we-warning-soft)] text-[var(--we-warning)]'"
                  >{{ issueLevel(item.severity) }}</span>
                ...
                <div
                  v-if="hiddenEmptyReviewItemCount"
                  class="rounded-md border border-[var(--we-warning-border)] bg-[var(--we-bg-panel)] px-2 py-1 text-[11px] text-[var(--we-text-muted)]"
                >
                  +{{ hiddenEmptyReviewItemCount }} issues
                </div>
              </div>
              <button
                v-if="emptySliceState.action === 'create-subject'"
                type="button"
                class="mt-4 inline-flex h-9 items-center gap-2 rounded-md border border-[var(--we-border)] bg-[var(--we-bg-panel)] px-3 text-[13px] text-[var(--we-text-main)] transition-colors hover:bg-[var(--we-bg-hover)] disabled:opacity-50"
                :disabled="workbenchActionBusy"
                @click="openSubjectCreatorPanel"
              >
                <span class="i-lucide-user-plus h-4 w-4" />
                创建 Subject
              </button>
              <button
                v-else-if="emptySliceState.action === 'create-world-subject'"
                type="button"
                class="mt-4 inline-flex h-9 items-center gap-2 rounded-md border border-[var(--we-accent-border)] bg-[var(--we-bg-panel)] px-3 text-[13px] font-medium text-[var(--we-accent-strong)] transition-colors hover:bg-[var(--we-bg-hover)] disabled:opacity-50"
                :disabled="workbenchActionBusy || !worldSubjectDefaultTime"
                @click="void createWorldSubject()"
              >
                <span
                  :class="actionBusy ? 'i-lucide-loader-2 animate-spin' : 'i-lucide-globe-2'"
                  class="h-4 w-4"
                />
                创建 world subject
              </button>
              <button
                v-else-if="emptySliceState.action === 'sync-subject-system'"
                type="button"
                class="mt-4 inline-flex h-9 items-center gap-2 rounded-md border border-[var(--we-warning-border)] bg-[var(--we-bg-panel)] px-3 text-[13px] font-medium text-[var(--we-warning)] transition-colors hover:bg-[var(--we-bg-hover)] disabled:opacity-50"
                :disabled="workbenchActionBusy || !subjectSystemSyncTime"
                @click="void syncPendingSubjectSystemSubjects()"
              >
                <span
              ...
              <div
                v-else-if="emptySliceState.action === 'new-slice'"
                class="mt-4 flex flex-wrap items-center justify-center gap-2"
              >
                <button
                  type="button"
                  class="inline-flex h-9 items-center gap-2 rounded-md border border-[var(--we-border)] bg-[var(--we-bg-panel)] px-3 text-[13px] text-[var(--we-text-main)] transition-colors hover:bg-[var(--we-bg-hover)] disabled:opacity-50"
                  :disabled="workbenchActionBusy || !schema"
                  @click="openSliceComposer"
                >
                ...
                <button
                  ...
                  type="button"
                  class="inline-flex h-9 items-center gap-2 rounded-md border border-[var(--we-border)] px-3 text-[13px] text-[var(--we-text-secondary)] transition-colors hover:bg-[var(--we-bg-hover)] hover:text-[var(--we-text-main)] disabled:opacity-50"
                  :disabled="workbenchActionBusy"
                  @click="clearSubjectFilter"
                >
                  <span class="i-lucide-filter-x h-4 w-4" />
                  清空 subject 过滤
                </button>
              </div>
            </div>
        ...
        <aside
          v-if="selectedSlice && !inspectorVisible"
          data-testid="world-inspector-restore-rail"
          class="flex w-10 shrink-0 flex-col items-center border-l border-[var(--we-border)] bg-[var(--we-bg-panel)] py-2"
        >
          <button
            type="button"
            class="inline-flex h-8 w-8 items-center justify-center rounded-md text-[var(--we-text-muted)] transition-colors hover:bg-[var(--we-bg-hover)] hover:text-[var(--we-text-main)]"
            :title="selectedSliceSubjectFileProposalCount ? '展开检查器并定位主体文件建议' : '展开检查器'"
            @click="toggleInspectorPanel"
          >
          ...
          <span
            v-if="selectedSliceSubjectFileProposalCount"
            data-testid="world-inspector-restore-proposal-count"
            class="mt-2 rounded border border-[var(--we-accent-border)] bg-[var(--we-accent-soft)] px-1.5 py-0.5 font-mono text-[10px] font-semibold text-[var(--we-accent-strong)]"
            title="当前切片有主体文件建议"
          >{{ selectedSliceSubjectFileProposalCount }}</span>
