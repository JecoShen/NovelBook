...
<template>
  ...
  >
    ...
    >
      <div
        class="ml-[3px] h-full w-[2px] bg-[var(--we-accent)] opacity-0 transition-opacity group-hover:opacity-100"
        :class="isResizing ? 'opacity-100' : ''"
      />
    </div>
    <div class="flex items-center justify-between gap-3 border-b border-[var(--we-border)] px-3 py-3">
      <div class="min-w-0">
        <div class="text-[11px] font-medium uppercase tracking-[0.14em] text-[var(--we-text-muted)]">
          {{ t("worldEngine.workbenchPreview.sliceContext") }}
        </div>
        <div class="mt-0.5 truncate text-[13px] font-semibold text-[var(--we-text-main)]">
          {{ props.slice.title || props.slice.id }}
        </div>
      </div>
      <div class="flex shrink-0 items-center gap-1.5">
        <span
          v-if="subjectFileProposalCount"
          data-testid="subject-file-proposal-count"
          class="inline-flex h-6 items-center gap-1 rounded-md border border-[var(--we-accent-border)] bg-[var(--we-accent-soft)] px-2 text-[10px] font-medium text-[var(--we-accent-strong)]"
          title="当前切片有主体文件建议"
        >
          <span class="i-lucide-file-check-2 h-3 w-3" />
        ...
        <button
          ...
          @click="emit('close')"
        >
          <span class="i-lucide-panel-right-close h-3.5 w-3.5" />
        </button>
      </div>
    </div>
    ...
    <div class="flex min-h-0 flex-1 flex-col gap-3 overflow-y-auto px-3 py-3 custom-scrollbar">
      <section class="rounded-md border border-[var(--we-border)] bg-[var(--we-bg-subtle)] p-3">
        <div class="mb-3 flex items-center justify-between gap-2">
          <div class="text-[11px] font-semibold uppercase tracking-[0.14em] text-[var(--we-text-muted)]">
            {{ t("worldEngine.workbenchPreview.metadata") }}
          </div>
        ...
        <div class="mt-3 flex items-center justify-between gap-2">
          <span
            class="inline-flex min-w-0 items-center gap-1.5 text-[11px]"
            :class="metadataDraftDirty ? 'text-[var(--we-warning)]' : 'text-[var(--we-text-muted)]'"
          >
            <span
              :class="metadataDraftDirty ? 'i-lucide-pencil-line' : 'i-lucide-check'"
          ...
          <div class="flex shrink-0 items-center gap-1.5">
            <button
              v-if="metadataDraftDirty"
              type="button"
              class="inline-flex h-8 items-center gap-1.5 rounded-md border border-[var(--we-border)] bg-[var(--we-bg-panel)] px-2.5 text-[12px] text-[var(--we-text-secondary)] transition-colors hover:bg-[var(--we-bg-hover)] hover:text-[var(--we-text-main)] disabled:cursor-not-allowed disabled:opacity-50"
              :disabled="props.busy"
              @click="resetDraft"
            >
            ...
            <button
              type="button"
              class="inline-flex h-8 items-center gap-1.5 rounded-md border px-3 text-[12px] font-medium transition-colors disabled:cursor-not-allowed disabled:opacity-50"
              :class="metadataDraftDirty && !props.busy ? 'border-[var(--we-accent-border)] bg-[var(--we-accent)] text-[var(--text-inverse)] hover:bg-[var(--we-accent-strong)]' : 'border-[var(--we-border)] bg-[var(--we-bg-muted)] text-[var(--we-text-muted)]'"
              :disabled="props.busy || !metadataDraftDirty"
              @click="applyPatch"
            >
              <span
                :class="props.busy ? 'i-lucide-loader-2 animate-spin' : 'i-lucide-check'"
                class="h-3.5 w-3.5"
              />
      ...
      <section
        v-if="subjectFileProposals.length"
        ref="subjectFileProposalsRef"
        data-testid="subject-file-proposals"
        class="order-6 rounded-md border border-[var(--we-border)] bg-[var(--we-bg-panel)] p-3"
      >
        <div class="mb-2 flex items-center justify-between gap-2">
          <div>
            ...
            <div class="mt-0.5 text-[11px] text-[var(--we-text-muted)]">
              仅生成建议，不会自动写入 simulation/subjects
            </div>
          </div>
          <div class="flex shrink-0 items-center gap-1.5">
            <button
              type="button"
              class="inline-flex h-7 items-center gap-1.5 rounded border border-[var(--we-border)] bg-[var(--we-bg-subtle)] px-2 text-[11px] text-[var(--we-text-secondary)] transition-colors hover:bg-[var(--we-bg-hover)] hover:text-[var(--we-text-main)]"
              title="复制全部主体文件建议"
              @click="void copyAllSubjectFileProposals()"
            >
              <span class="i-lucide-copy-check h-3.5 w-3.5" />
              复制全部
            </button>
        ...
        <div class="space-y-2">
          <details
            v-for="proposal in subjectFileProposals"
            :key="`subject-file-proposal:${proposal.subjectId}`"
            class="overflow-hidden rounded-md border border-[var(--we-border)] bg-[var(--we-bg-muted)]"
            open
          >
            <summary class="flex cursor-pointer items-center justify-between gap-2 bg-[var(--we-bg-subtle)] px-3 py-2">
              <span class="flex min-w-0 items-center gap-1.5">
                ...
                <span
                  class="min-w-0 max-w-[168px] truncate rounded border px-1.5 py-0.5 text-[10px]"
                  :class="proposal.sourceKind === 'direct-mutation' ? 'border-[var(--we-accent-border)] bg-[var(--we-accent-soft)] text-[var(--we-accent-strong)]' : 'border-[var(--we-warning-border)] bg-[var(--we-warning-soft)] text-[var(--we-warning)]'"
                  :title="proposal.sourceLabel"
                >{{ proposal.sourceLabel }}</span>
              </span>
            ...
            <div class="space-y-2 border-t border-[var(--we-border)] px-3 py-2 text-[11px]">
              <div class="flex items-center justify-end">
                <button
                  type="button"
                  class="inline-flex h-7 items-center gap-1.5 rounded border border-[var(--we-border)] bg-[var(--we-bg-panel)] px-2 text-[11px] text-[var(--we-text-secondary)] transition-colors hover:bg-[var(--we-bg-hover)] hover:text-[var(--we-text-main)]"
                  title="复制主体文件建议"
                  @click="void copySubjectFileProposal(proposal)"
                >
              ...
              <div>
                <div class="mb-1 flex items-center justify-between gap-2">
                  <div class="flex min-w-0 items-center gap-1.5 font-semibold text-[var(--we-text-secondary)]">
                    <span class="i-lucide-list-plus h-3.5 w-3.5 shrink-0" />
                    <span class="truncate">events.jsonl draft</span>
                  </div>
                  <div class="flex shrink-0 items-center gap-1">
                    <button
                      type="button"
                      class="inline-flex h-6 items-center gap-1 rounded border border-[var(--we-accent-border)] bg-[var(--we-accent-soft)] px-1.5 text-[10px] font-medium text-[var(--we-accent-strong)] transition-colors hover:bg-[var(--we-bg-active)] disabled:opacity-45"
                      :disabled="props.busy || committedSubjectEventKeySet.has(worldWorkbenchSubjectEventProposalKey(proposal))"
                      :title="committedSubjectEventKeySet.has(worldWorkbenchSubjectEventProposalKey(proposal)) ? '这条 events.jsonl 经历已在当前会话处理' : '确认后追加到 events.jsonl'"
                      @click="commitSubjectEventProposal(proposal)"
                    >
                      <span :class="committedSubjectEventKeySet.has(worldWorkbenchSubjectEventProposalKey(proposal)) ? 'i-lucide-check h-3 w-3' : 'i-lucide-file-plus-2 h-3 w-3'" />
                      {{ committedSubjectEventKeySet.has(worldWorkbenchSubjectEventProposalKey(proposal)) ? "已追加" : "追加" }}
                    </button>
                    <button
                      type="button"
                      class="inline-flex h-6 items-center gap-1 rounded border border-[var(--we-border)] bg-[var(--we-bg-panel)] px-1.5 text-[10px] text-[var(--we-text-secondary)] transition-colors hover:bg-[var(--we-bg-hover)] hover:text-[var(--we-text-main)]"
                      title="复制 events.jsonl 行"
                      @click="void copySubjectFileProposalText(proposal.eventJsonLine, 'events.jsonl 行已复制。')"
                    >
                    ...
                    <button
                      type="button"
                      class="inline-flex h-6 items-center gap-1 rounded border border-[var(--we-accent-border)] bg-[var(--we-accent-soft)] px-1.5 text-[10px] text-[var(--we-accent-strong)] transition-colors hover:bg-[var(--we-bg-active)] disabled:opacity-45"
                      :disabled="props.busy"
                      title="复制 events.jsonl 行并打开文件，确认后追加到文件末尾"
                      @click="void copySubjectFileProposalTextAndOpen(proposal.eventJsonLine, 'events.jsonl 行已复制，打开文件后确认并追加到末尾。', proposal.eventsPath)"
                    >
                ...
                <div class="rounded border border-[var(--we-border)] bg-[var(--we-bg-panel)] px-2 py-1.5 font-mono text-[10px] text-[var(--we-code-text)]">
                  {{ proposal.eventJsonLine }}
                </div>
                <div class="mt-1 rounded border border-[var(--we-warning-border)] bg-[var(--we-warning-soft)] px-2 py-1 text-[10px] text-[var(--we-warning)]">
                  写入前确认第一人称口吻、角色当时知道什么；确认后追加到 events.jsonl 末尾。
                </div>
                <div class="mt-1 rounded border border-[var(--we-border)] bg-[var(--we-bg-subtle)] px-2 py-1.5 text-[var(--we-text-secondary)]">
                  {{ proposal.eventDraft }}
                </div>
                <div class="mt-1 flex min-w-0 items-center gap-1.5">
                  <button
                    type="button"
                    class="inline-flex h-5 w-5 shrink-0 items-center justify-center rounded border border-[var(--we-border)] bg-[var(--we-bg-panel)] text-[var(--we-text-muted)] transition-colors hover:bg-[var(--we-bg-hover)] hover:text-[var(--we-text-main)] disabled:opacity-45"
                    :disabled="props.busy"
                    title="打开 events.jsonl"
                    @click="openSubjectFileProposalPath(proposal.eventsPath)"
                  >
                    <span class="i-lucide-folder-open h-3 w-3" />
                  </button>
                  <div
                    class="min-w-0 truncate font-mono text-[10px] text-[var(--we-text-muted)]"
                    :title="proposal.eventsPath"
                  >
                    {{ proposal.eventsPath }}
                  </div>
                </div>
              ...
              <div v-if="proposal.memoryFacts.length">
                <div class="mb-1 flex items-center justify-between gap-2">
                  <div class="flex min-w-0 items-center gap-1.5 font-semibold text-[var(--we-text-secondary)]">
                    <span class="i-lucide-brain h-3.5 w-3.5 shrink-0" />
                  ...
                  <div class="flex shrink-0 items-center gap-1">
                    <button
                      type="button"
                      class="inline-flex h-6 items-center gap-1 rounded border border-[var(--we-border)] bg-[var(--we-bg-panel)] px-1.5 text-[10px] text-[var(--we-text-secondary)] transition-colors hover:bg-[var(--we-bg-hover)] hover:text-[var(--we-text-main)]"
                      title="复制 memory.jsonl 候选行"
                      @click="void copySubjectFileProposalText(proposal.memoryJsonLines.join('\n'), 'memory.jsonl 候选行已复制。')"
                    >
                      <span class="i-lucide-copy h-3 w-3" />
                      复制行
                    </button>
                    <button
                      ...
                      class="inline-flex h-6 items-center gap-1 rounded border border-[var(--we-accent-border)] bg-[var(--we-accent-soft)] px-1.5 text-[10px] text-[var(--we-accent-strong)] transition-colors hover:bg-[var(--we-bg-active)] disabled:opacity-45"
                      :disabled="props.busy"
                      title="复制 memory.jsonl 候选行并打开文件，确认后追加新行或按 topic 改写"
                      @click="void copySubjectFileProposalTextAndOpen(proposal.memoryJsonLines.join('\n'), 'memory.jsonl 候选行已复制，打开文件后确认追加新行或按 topic 改写。', proposal.memoryPath)"
                    >
                      <span class="i-lucide-arrow-up-right h-3 w-3" />
                      复制并打开
                    </button>
                ...
                <ul class="space-y-1">
                  <li
                    v-for="line in proposal.memoryJsonLines"
                    :key="`memory-jsonl:${proposal.subjectId}:${line}`"
                    class="rounded border border-[var(--we-border)] bg-[var(--we-bg-panel)] px-2 py-1 font-mono text-[10px] text-[var(--we-code-text)]"
                  >
                    {{ line }}
                  </li>
                  <li class="rounded border border-[var(--we-warning-border)] bg-[var(--we-warning-soft)] px-2 py-1 text-[10px] text-[var(--we-warning)]">
                    memory.jsonl 是当前认知快照；写入前确认追加新行，还是按 topic 改写已有行。
                  </li>
                  <li
                    v-for="fact in proposal.memoryFacts"
                    :key="`memory-fact:${proposal.subjectId}:${fact}`"
                    class="rounded border border-[var(--we-border)] bg-[var(--we-bg-subtle)] px-2 py-1 text-[var(--we-text-secondary)]"
                  >
                    {{ fact }}
                  </li>
                </ul>
                <div class="mt-1 flex min-w-0 items-center gap-1.5">
                  <button
                    ...
                    class="inline-flex h-5 w-5 shrink-0 items-center justify-center rounded border border-[var(--we-border)] bg-[var(--we-bg-panel)] text-[var(--we-text-muted)] transition-colors hover:bg-[var(--we-bg-hover)] hover:text-[var(--we-text-main)] disabled:opacity-45"
                    :disabled="props.busy"
                    title="打开 memory.jsonl"
                    @click="openSubjectFileProposalPath(proposal.memoryPath)"
                  >
                    <span class="i-lucide-folder-open h-3 w-3" />
                  </button>
                  <div
                    class="min-w-0 truncate font-mono text-[10px] text-[var(--we-text-muted)]"
                    :title="proposal.memoryPath"
                  >
                    {{ proposal.memoryPath }}
                  </div>
                </div>
              ...
              <div v-if="proposal.stateReviewReasons.length">
                <div class="mb-1 flex items-center justify-between gap-2">
                  <div class="flex min-w-0 items-center gap-1.5 font-semibold text-[var(--we-warning)]">
                    <span class="i-lucide-eye h-3.5 w-3.5 shrink-0" />
                  ...
                  <div class="flex shrink-0 items-center gap-1">
                    <button
                      type="button"
                      class="inline-flex h-6 items-center gap-1 rounded border border-[var(--we-border)] bg-[var(--we-bg-panel)] px-1.5 text-[10px] text-[var(--we-text-secondary)] transition-colors hover:bg-[var(--we-bg-hover)] hover:text-[var(--we-text-main)]"
                      title="复制 state.md 审查提示"
                      @click="void copySubjectFileProposalText(proposal.stateReviewReasons.join('\n'), 'state.md 审查提示已复制。')"
                    >
                      <span class="i-lucide-copy h-3 w-3" />
                      复制提示
                    </button>
                    <button
                      ...
                      class="inline-flex h-6 items-center gap-1 rounded border border-[var(--we-accent-border)] bg-[var(--we-accent-soft)] px-1.5 text-[10px] text-[var(--we-accent-strong)] transition-colors hover:bg-[var(--we-bg-active)] disabled:opacity-45"
                      :disabled="props.busy"
                      title="复制 state.md 审查提示并打开文件，打开后检查对应区块"
                      @click="void copySubjectFileProposalTextAndOpen(proposal.stateReviewReasons.join('\n'), 'state.md 审查提示已复制，打开文件后检查对应区块。', proposal.statePath)"
                    >
                      <span class="i-lucide-arrow-up-right h-3 w-3" />
                      复制并打开
                    </button>
                ...
                <ul class="space-y-1">
                  <li
                    v-for="reason in proposal.stateReviewReasons"
                    :key="`state-review:${proposal.subjectId}:${reason}`"
                    class="rounded border border-[var(--we-warning-border)] bg-[var(--we-warning-soft)] px-2 py-1 text-[var(--we-warning)]"
                  >
                    {{ reason }}
                  </li>
                </ul>
                <div class="mt-1 flex min-w-0 items-center gap-1.5">
                  <button
                    ...
                    :disabled="props.busy"
                    title="打开 state.md"
                    @click="openSubjectFileProposalPath(proposal.statePath)"
                  >
                    <span class="i-lucide-folder-open h-3 w-3" />
                  </button>
                  <div
                    class="min-w-0 truncate font-mono text-[10px] text-[var(--we-text-muted)]"
                    :title="proposal.statePath"
                  >
                    {{ proposal.statePath }}
                  </div>
                </div>
      ...
      <section class="order-7 flex flex-col gap-2 rounded-md border border-[var(--we-border)] bg-[var(--we-bg-panel)] p-3">
        <div class="flex items-center justify-between gap-2">
          <div>
            <div class="text-[11px] font-semibold uppercase tracking-[0.14em] text-[var(--we-text-muted)]">
              {{ t("worldEngine.workbenchPreview.stateSnapshot") }}
            </div>
            <div class="mt-0.5 text-[11px] text-[var(--we-text-muted)]">
              {{ showFullState ? "完整世界状态" : "当前切片触及主体" }}
            </div>
          </div>
          <button
            type="button"
            class="inline-flex h-7 items-center gap-1.5 rounded-md border border-[var(--we-border)] bg-[var(--we-bg-subtle)] px-2 text-[11px] text-[var(--we-text-secondary)] transition-colors hover:bg-[var(--we-bg-hover)] hover:text-[var(--we-text-main)] disabled:opacity-45"
            :aria-pressed="showFullState"
            :disabled="props.busy || props.fullSnapshotLoading"
            @click="toggleFullState"
          >
            <span
              :class="props.fullSnapshotLoading && showFullState ? 'i-lucide-loader-2 animate-spin' : showFullState ? 'i-lucide-minimize-2' : 'i-lucide-expand'"
              class="h-3.5 w-3.5"
            />
        ...
        <div
          v-if="visibleSnapshotIssues.length"
          data-testid="snapshot-query-issues"
          class="space-y-1 rounded-md border border-[var(--we-danger-border)] bg-[var(--we-danger-soft)] px-2.5 py-2"
        >
          <div class="flex items-center gap-1.5 text-[11px] font-semibold text-[var(--we-danger)]">
            <span class="i-lucide-circle-alert h-3.5 w-3.5" />
            State issues · {{ visibleSnapshotIssues.length }}
          </div>
          <div
            v-for="(issue, issueIndex) in visibleSnapshotIssues"
            :key="`snapshot-issue:${issue.sliceId ?? 'query'}:${issueIndex}:${issue.subjectId}:${issue.attr}:${issue.code}`"
            class="grid grid-cols-[112px_minmax(0,1fr)] gap-2 text-[11px] text-[var(--we-danger)]"
          >
            <span
            ...
            <span
              ...
              :title="issue.message"
            >{{ subjectMap.get(issue.subjectId)?.name ?? issue.subjectId }} · {{ issue.attr }} · {{ issue.message }}</span>
          </div>
        </div>

        <div
          v-if="showFullState && props.fullSnapshotLoading"
          class="rounded-md border border-[var(--we-border)] bg-[var(--we-bg-subtle)] px-3 py-6 text-center text-[12px] text-[var(--we-text-muted)]"
        >
          正在读取完整世界状态...
        </div>
        <div
          v-else-if="showFullState && props.fullSnapshotError"
          class="rounded-md border border-[var(--we-danger)] bg-[var(--we-danger-soft)] px-3 py-3 text-[12px] text-[var(--we-danger)]"
        >
          {{ props.fullSnapshotError }}
        </div>
        <div
          v-else
          class="rounded-md border border-[var(--we-border)]"
        >
          <JsonViewer
            :value="rawSnapshotValue"
            :max-height="400"
          />
        </div>
      </section>
