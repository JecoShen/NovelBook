...
<template>
  ...
  >
    ...
    <div
      ...
      ref="resizeHandleRef"
      class="group absolute -top-1 left-0 right-0 z-20 h-2 cursor-row-resize"
    >
      <div
        class="mt-[3px] h-[2px] w-full bg-[var(--we-accent)] opacity-0 transition-opacity group-hover:opacity-100"
    ...
    <div class="flex h-10 items-center justify-between gap-3 border-b border-[var(--we-border)] px-3">
      <div class="flex min-w-0 flex-1 items-center gap-2 overflow-hidden">
        <span class="i-lucide-list-checks h-4 w-4 shrink-0 text-[var(--we-text-secondary)]" />
        ...
        <span
          class="hidden shrink-0 whitespace-nowrap rounded-md border px-2 py-0.5 text-[11px] xl:inline"
          :class="dirtyValueDraftCount || otherSliceDirtyDraftCount ? 'border-[var(--we-warning-border)] bg-[var(--we-warning-soft)] text-[var(--we-warning)]' : 'border-[var(--we-border)] bg-[var(--we-bg-subtle)] text-[var(--we-text-muted)]'"
        >{{ valueDraftStatusLabel }}</span>
      </div>
      <div class="flex shrink-0 items-center gap-2">
        <button
          v-if="nextOtherSliceDraft"
          data-testid="mutation-editor-next-draft-toolbar"
          type="button"
          class="inline-flex h-7 items-center gap-1.5 rounded-md border border-[var(--we-warning-border)] bg-[var(--we-warning-soft)] px-2 text-[11px] font-medium text-[var(--we-warning)] transition-colors hover:bg-[var(--we-bg-hover)] disabled:opacity-45"
          :disabled="props.busy"
          :title="`跳到 ${nextOtherSliceDraft.sliceTitle} 的未应用草稿`"
          @click="navigateToOtherDraft"
        >
          <span class="i-lucide-arrow-right-from-line h-3.5 w-3.5" />
          <span class="hidden xl:inline">跳到草稿</span>
        </button>
        <button
          v-if="allDirtyValueDraftCount"
          data-testid="mutation-editor-clear-all-drafts-toolbar"
          type="button"
          class="inline-flex h-7 items-center gap-1.5 rounded-md border border-[var(--we-border)] bg-[var(--we-bg-panel)] px-2 text-[11px] font-medium text-[var(--we-text-secondary)] transition-colors hover:bg-[var(--we-bg-hover)] hover:text-[var(--we-text-main)] disabled:opacity-45"
          :disabled="props.busy"
          title="放弃所有 slice 的未应用 value 草稿"
          @click="requestResetAllValueDrafts"
        >
        ...
        <button
          v-if="!props.collapsed"
          data-testid="mutation-editor-apply-all-toolbar"
          type="button"
          class="hidden h-7 items-center gap-1.5 rounded-md border border-[var(--we-border)] bg-[var(--we-bg-panel)] px-2 text-[11px] text-[var(--we-text-secondary)] transition-colors hover:bg-[var(--we-bg-hover)] hover:text-[var(--we-accent-strong)] disabled:opacity-45 lg:inline-flex"
          :disabled="props.busy || !dirtyValueDraftCount"
          title="应用当前切片所有未应用 value 草稿"
          @click="applyDirtyValueDrafts"
        >
          <span class="i-lucide-check-check h-3.5 w-3.5" />
          应用全部
        </button>
        <button
          v-if="!props.collapsed"
          data-testid="mutation-editor-reset-all-toolbar"
          type="button"
          class="hidden h-7 items-center gap-1.5 rounded-md border border-[var(--we-border)] bg-[var(--we-bg-panel)] px-2 text-[11px] text-[var(--we-text-secondary)] transition-colors hover:bg-[var(--we-bg-hover)] hover:text-[var(--we-text-main)] disabled:opacity-45 lg:inline-flex"
          :disabled="props.busy || !dirtyValueDraftCount"
          title="还原当前切片所有未应用 value 草稿"
          @click="resetDirtyValueDrafts"
        >
          <span class="i-lucide-undo-2 h-3.5 w-3.5" />
          还原全部
        </button>
        <SegmentedControl
          v-if="!props.collapsed"
          ...
          :wrap="false"
          @update:model-value="updateEditorView"
        />
        <button
          type="button"
          class="inline-flex h-7 w-7 items-center justify-center rounded-md text-[var(--we-text-muted)] transition-colors hover:bg-[var(--we-bg-hover)] hover:text-[var(--we-text-main)]"
          :title="props.collapsed ? '展开审查工作台' : '收起审查工作台'"
          @click="emit('toggleCollapsed')"
        >
          <span
            :class="props.collapsed ? 'i-lucide-panel-bottom-open' : 'i-lucide-panel-bottom-close'"
            class="h-4 w-4"
          />
        </button>
      </div>
    ...
    <Transition name="world-review-panel-body">
      <div
        v-if="!props.collapsed"
        class="flex min-h-0 flex-1 flex-col overflow-hidden px-3 py-3"
      >
        <div
          v-if="dirtyValueDraftCount || otherSliceDirtyDraftCount || valueDraftErrorCount"
          class="mb-3 shrink-0 rounded-md border border-[var(--we-warning-border)] bg-[var(--we-warning-soft)] px-3 py-2"
        >
          <div class="flex flex-wrap items-center justify-between gap-2">
            <div class="flex min-w-0 items-center gap-2">
              <span class="i-lucide-pencil-line h-4 w-4 shrink-0 text-[var(--we-warning)]" />
            ...
            <div class="flex shrink-0 items-center gap-1.5">
              <button
                v-if="nextOtherSliceDraft"
                data-testid="mutation-editor-next-draft-slice"
                type="button"
                class="inline-flex h-7 items-center gap-1.5 rounded-md border border-[var(--we-warning-border)] bg-[var(--we-bg-panel)] px-2 text-[11px] font-medium text-[var(--we-warning)] transition-colors hover:bg-[var(--we-bg-hover)] disabled:opacity-45"
                :disabled="props.busy"
                :title="`跳到 ${nextOtherSliceDraft.sliceTitle} 的未应用草稿`"
                @click="navigateToOtherDraft"
              >
              ...
              <button
                v-if="allDirtyValueDraftCount"
                data-testid="mutation-editor-clear-all-drafts"
                type="button"
                class="inline-flex h-7 items-center gap-1.5 rounded-md border border-[var(--we-warning-border)] bg-[var(--we-bg-panel)] px-2 text-[11px] font-medium text-[var(--we-text-secondary)] transition-colors hover:bg-[var(--we-bg-hover)] disabled:opacity-45"
                :disabled="props.busy"
                title="放弃所有 slice 的未应用 value 草稿"
                @click="requestResetAllValueDrafts"
              >
              ...
              <button
                data-testid="mutation-editor-apply-all-banner"
                type="button"
                class="inline-flex h-7 items-center gap-1.5 rounded-md border border-[var(--we-warning-border)] bg-[var(--we-bg-panel)] px-2 text-[11px] font-medium text-[var(--we-warning)] transition-colors hover:bg-[var(--we-bg-hover)] disabled:opacity-45"
                :disabled="props.busy || !dirtyValueDraftCount"
                @click="applyDirtyValueDrafts"
              ...
              <button
                data-testid="mutation-editor-reset-all-banner"
                type="button"
                class="inline-flex h-7 items-center gap-1.5 rounded-md border border-[var(--we-warning-border)] bg-[var(--we-bg-panel)] px-2 text-[11px] font-medium text-[var(--we-text-secondary)] transition-colors hover:bg-[var(--we-bg-hover)] disabled:opacity-45"
                :disabled="props.busy || !dirtyValueDraftCount"
                @click="resetDirtyValueDrafts"
        ...
        <div
          v-if="view === 'review'"
          class="min-h-0 flex-1 space-y-3 overflow-y-auto custom-scrollbar"
        >
          <div
            v-if="reviewFocusContext && reviewIssueExplanation"
            class="rounded-md border border-[var(--we-warning-border)] bg-[var(--we-warning-soft)] px-3 py-2"
          >
            <div class="flex flex-wrap items-start justify-between gap-2">
              <div class="min-w-0 flex-1">
                <div class="flex min-w-0 items-center gap-2">
                  <span class="i-lucide-crosshair h-4 w-4 shrink-0 text-[var(--we-warning)]" />
                  <span
                    class="shrink-0 rounded border px-1.5 py-0.5 font-mono text-[10px] font-semibold"
                    :class="issueLevelClass(reviewFocusContext.severity)"
                  >{{ reviewFocusContext.label }}</span>
                  ...
                  <span
                    ...
                    :class="issueStatusClass(reviewFocusContext.status)"
                  >{{ issueStatusLabel(reviewFocusContext.status) }}</span>
                </div>
                <div class="mt-1 text-[13px] font-semibold text-[var(--we-text-main)]">
                  {{ reviewIssueExplanation.title }}
                </div>
                ...
                <div class="mt-2 grid gap-1.5 text-[11px] leading-5 md:grid-cols-3">
                  <div class="rounded-md border border-[var(--we-warning-border)] bg-[var(--we-bg-panel)] px-2.5 py-1.5">
                    <div class="font-semibold text-[var(--we-warning)]">
                      发生了什么
                    </div>
                    <div class="mt-0.5 text-[var(--we-text-secondary)]">
                      {{ reviewIssueExplanation.detail }}
                    </div>
                  </div>
                  <div class="rounded-md border border-[var(--we-warning-border)] bg-[var(--we-bg-panel)] px-2.5 py-1.5">
                    <div class="font-semibold text-[var(--we-warning)]">
                  ...
                  <div class="rounded-md border border-[var(--we-warning-border)] bg-[var(--we-bg-panel)] px-2.5 py-1.5">
                    ...
                    <div class="mt-0.5 text-[var(--we-text-secondary)]">
                      {{ reviewIssueExplanation.action }}
                    </div>
                  </div>
                </div>
                <div
                  v-if="reviewFocusContext.status !== 'manual'"
                ...
                >
                  ...
                  <SegmentedControl
                    :model-value="reviewQueueMode"
                    :options="reviewQueueModeOptions"
                    size="xs"
                    @update:model-value="updateReviewQueueMode"
                  />
                  ...
                  <span
                    v-if="currentIssueOutsideVisibleQueue"
                    class="text-[11px] text-[var(--we-accent-strong)]"
                  >当前 issue 已处理</span>
              ...
              <div class="flex shrink-0 items-center gap-1.5">
                <button
                  v-if="reviewFocusContext.status !== 'manual'"
                  type="button"
                  class="inline-flex h-7 items-center gap-1 rounded-md border border-[var(--we-warning-border)] bg-[var(--we-bg-panel)] px-2 text-[11px] text-[var(--we-warning)] transition-colors hover:bg-[var(--we-bg-hover)] disabled:opacity-45"
                  :disabled="props.busy || !previousReviewQueueItem"
                  :title="previousReviewQueueItem ? `${previousReviewQueueItem.sliceTime} · ${previousReviewQueueItem.sliceTitle}` : '没有上一个可见 issue'"
                  @click="focusReviewIssue(previousReviewQueueItem)"
                >
                ...
                <button
                  ...
                  type="button"
                  class="inline-flex h-7 items-center gap-1 rounded-md border border-[var(--we-warning-border)] bg-[var(--we-bg-panel)] px-2 text-[11px] text-[var(--we-warning)] transition-colors hover:bg-[var(--we-bg-hover)] disabled:opacity-45"
                  :disabled="props.busy || !nextReviewQueueItem"
                  :title="nextReviewQueueItem ? `${nextReviewQueueItem.sliceTime} · ${nextReviewQueueItem.sliceTitle}` : '没有下一个可见 issue'"
                  @click="focusReviewIssue(nextReviewQueueItem)"
                >
                ...
                <button
                  type="button"
                  class="inline-flex h-7 items-center gap-1.5 rounded-md border border-[var(--we-warning-border)] bg-[var(--we-bg-panel)] px-2 text-[11px] font-medium text-[var(--we-warning)] transition-colors hover:bg-[var(--we-bg-hover)]"
                  @click="clearReviewFocus"
                >
          ...
          <div
            v-if="reviewFocusContext"
            data-testid="mutation-context"
            class="rounded-md border border-[var(--we-border)] bg-[var(--we-bg-subtle)] p-3"
          >
            <div class="mb-2 flex items-center justify-between gap-2">
              <div class="flex items-center gap-2">
                <span class="i-lucide-git-compare-arrows h-4 w-4 text-[var(--we-warning)]" />
            ...
            <div class="grid gap-2 xl:grid-cols-3">
              ...
              >
                <div class="mb-2 flex items-center justify-between gap-2">
                  ...
                  <span
                    v-if="contextItem.item"
                    class="rounded bg-[var(--we-bg-subtle)] px-1.5 py-0.5 font-mono text-[10px] text-[var(--we-text-muted)]"
                  >#{{ contextItem.item.index + 1 }}</span>
                </div>
                <div
                  v-if="contextItem.item"
                ...
                >
                  <div class="min-w-0">
                    <div
                      class="truncate font-semibold text-[var(--we-text-main)]"
                      :title="contextItem.item.slice.title"
                    >
                      {{ contextItem.item.slice.time }} · {{ contextItem.item.slice.title }}
                    </div>
                    <div class="mt-0.5 truncate font-mono text-[10px] text-[var(--we-text-muted)]">
                      {{ contextItem.item.slice.kind }} · {{ contextItem.item.slice.id }}
                    </div>
                  </div>
                  <div class="grid grid-cols-[minmax(0,1fr)_52px] gap-2 rounded-md border border-[var(--we-border)] bg-[var(--we-bg-data)] px-2 py-1.5">
                    <div class="min-w-0">
                      <div class="truncate font-mono font-semibold text-[var(--we-code-text)]">
                        {{ contextItem.item.mutation.path }}
                      </div>
                      <div class="mt-0.5 truncate text-[var(--we-text-secondary)]">
                        {{ subjectMap.get(contextItem.item.mutation.subjectId)?.name ?? contextItem.item.mutation.subjectId }}
                      </div>
                    </div>
                  ...
                  <div class="rounded-md border border-[var(--we-border)] bg-[var(--we-bg-data)] px-2 py-1.5">
                    <div class="text-[10px] font-semibold uppercase tracking-[0.12em] text-[var(--we-text-muted)]">
                      value
                    </div>
                    <div
                      class="mt-0.5 truncate text-[11px] text-[var(--we-text-main)]"
                      :title="buildMutationContextExplanation(contextItem.item, reviewFocusContext).valueLabel"
                    >
                      {{ buildMutationContextExplanation(contextItem.item, reviewFocusContext).valueLabel }}
                    </div>
                  </div>
                  <div class="grid gap-1.5 text-[11px]">
                    <div class="rounded-md border border-[var(--we-border)] bg-[var(--we-bg-data)] px-2 py-1.5">
                      <div class="font-semibold text-[var(--we-text-main)]">
                        动作
                      </div>
                      <div class="mt-0.5 text-[var(--we-text-secondary)]">
                        {{ buildMutationContextExplanation(contextItem.item, reviewFocusContext).action }}
                      </div>
                    </div>
                    <div class="rounded-md border border-[var(--we-border)] bg-[var(--we-bg-data)] px-2 py-1.5">
                      <div class="font-semibold text-[var(--we-text-main)]">
                      ...
                      <div class="mt-0.5 text-[var(--we-text-secondary)]">
                        {{ buildMutationContextExplanation(contextItem.item, reviewFocusContext).relation }}
                      </div>
                    </div>
                    <div class="rounded-md border border-[var(--we-border)] bg-[var(--we-bg-data)] px-2 py-1.5">
                      <div class="font-semibold text-[var(--we-text-main)]">
                      ...
                      <div class="mt-0.5 text-[var(--we-text-secondary)]">
                        {{ buildMutationContextExplanation(contextItem.item, reviewFocusContext).relevance }}
                      </div>
                    </div>
                    <div class="rounded-md border border-[var(--we-border)] bg-[var(--we-bg-data)] px-2 py-1.5">
                      <div class="font-semibold text-[var(--we-text-main)]">
                      ...
                      <div class="mt-0.5 text-[var(--we-text-secondary)]">
                        {{ buildMutationContextExplanation(contextItem.item, reviewFocusContext).confirmation }}
                      </div>
                    </div>
                  </div>
                </div>
                <div
                  v-else
                  class="rounded-md border border-dashed border-[var(--we-border)] bg-[var(--we-bg-muted)] px-3 py-6 text-center text-[12px] text-[var(--we-text-muted)]"
                >
                  {{ contextItem.empty }}
                </div>
              </div>
          ...
          <div
            v-else-if="currentSliceReviewItems.length"
            class="rounded-md border border-[var(--we-border)] bg-[var(--we-bg-subtle)] px-3 py-2"
          >
            <div class="mb-2 flex flex-wrap items-center justify-between gap-2">
              <div class="flex min-w-0 items-center gap-2">
                <span class="i-lucide-list-checks h-4 w-4 shrink-0 text-[var(--we-warning)]" />
            ...
            <div class="grid gap-1.5 xl:grid-cols-2">
              ...
              >
                <span
                  ...
                  :class="issueLevelClass(item.severity)"
                >{{ item.label }}</span>
                ...
                <span
                  ...
                  :class="issueStatusClass(item.status)"
                >{{ issueStatusLabel(item.status) }}</span>
              </button>
            </div>
          </div>
          <div
            v-else
            class="rounded-md border border-dashed border-[var(--we-border)] bg-[var(--we-bg-subtle)] px-3 py-8 text-center text-[12px] text-[var(--we-text-muted)]"
          >
            当前切片没有审查问题
          </div>
        </div>
        ...
        <div
          ...
          class="grid min-h-0 flex-1 gap-3 xl:grid-cols-[240px_minmax(0,1fr)]"
        >
          <aside class="flex min-h-0 flex-col overflow-hidden rounded-md border border-[var(--we-border)] bg-[var(--we-bg-subtle)] p-2.5">
            <div class="mb-2 shrink-0 text-[11px] font-semibold uppercase tracking-[0.14em] text-[var(--we-text-muted)]">
              {{ t("worldEngine.workbenchPreview.touchedSubjects") }}
            </div>
            <div class="min-h-0 flex-1 space-y-1 overflow-y-auto pr-1 custom-scrollbar">
              ...
              >
                <div class="truncate text-[12px] font-semibold text-[var(--we-text-main)]">
                  {{ subjectMap.get(subjectId)?.name ?? subjectId }}
                </div>
                <div class="mt-0.5 truncate font-mono text-[10px] text-[var(--we-text-muted)]">
                  {{ subjectId }} · {{ subjectMap.get(subjectId)?.type ?? "unknown" }}
                </div>
              </button>
            </div>
          </aside>
          ...
          <main class="flex min-w-0 min-h-0 flex-col overflow-hidden rounded-md border border-[var(--we-border)] bg-[var(--we-bg-subtle)] p-3">
            <div class="mb-3 flex shrink-0 flex-wrap items-center justify-between gap-2">
              <div class="min-w-0">
                <div class="truncate text-[13px] font-semibold text-[var(--we-text-main)]">
                  {{ activeSubject?.name ?? (activeSubjectId || "未选择 subject") }}
                </div>
                <div class="mt-0.5 font-mono text-[10px] text-[var(--we-text-muted)]">
                  {{ activeSubjectId }}
                </div>
              </div>
              <div class="flex items-center gap-1.5">
                <SegmentedControl
                  :model-value="subjectNavigationScope"
                  :options="subjectNavigationScopeOptions"
                  size="xs"
                  @update:model-value="updateSubjectNavigationScope"
                />
                <span
                  class="hidden rounded-md border border-[var(--we-border)] bg-[var(--we-bg-panel)] px-2 py-1 font-mono text-[10px] text-[var(--we-text-muted)] 2xl:inline"
                  :title="subjectNavigationScopeLabel"
                >{{ relatedSlicePosition }}</span>
                <button
                  type="button"
                  class="inline-flex h-7 items-center gap-1 rounded-md border border-[var(--we-border)] bg-[var(--we-bg-panel)] px-2 text-[11px] text-[var(--we-text-secondary)] transition-colors hover:bg-[var(--we-bg-hover)] disabled:opacity-50"
                  :disabled="props.busy || !previousRelatedSlice"
                  @click="navigateSubjectSlice('previous')"
                >
                ...
                <button
                  type="button"
                  class="inline-flex h-7 items-center gap-1 rounded-md border border-[var(--we-border)] bg-[var(--we-bg-panel)] px-2 text-[11px] text-[var(--we-text-secondary)] transition-colors hover:bg-[var(--we-bg-hover)] disabled:opacity-50"
                  :disabled="props.busy || !nextRelatedSlice"
                  @click="navigateSubjectSlice('next')"
                >
                  下一个
            ...
            <div class="grid min-h-0 flex-1 gap-3 2xl:grid-cols-2">
              <section class="flex min-h-0 flex-col">
                ...
                <div class="flex min-h-0 flex-1 overflow-hidden rounded-md border border-[var(--we-border)] bg-[var(--we-bg-panel)]">
                  <JsonViewer
                    v-if="activeSubjectState"
                    class="min-h-0 flex-1"
                    :value="activeSubjectState.attrs"
                    :main-menu-bar="false"
                    :max-height="0"
                  />
                  <div
                    v-else
                    class="flex min-h-0 flex-1 items-center justify-center px-3 py-6 text-center text-[12px] text-[var(--we-text-muted)]"
                  >
                    当前 snapshot 没有状态
                  </div>
                </div>
        ...
        <div
          v-else
          class="min-h-0 flex-1 overflow-auto rounded-md border border-[var(--we-border)] bg-[var(--we-bg-panel)] custom-scrollbar"
        >
          ...
          >
            <div class="grid grid-cols-[42px_150px_minmax(0,1fr)_92px_58px] items-center gap-2">
              ...
              <label class="grid min-w-0 grid-cols-[minmax(72px,0.5fr)_minmax(0,1fr)] items-center gap-2">
                <span class="min-w-0">
                  ...
                  <span
                    v-if="isValueDraftDirty(row.index, row.mutation)"
                    class="mt-1 inline-flex rounded border border-[var(--we-warning-border)] bg-[var(--we-warning-soft)] px-1.5 py-0.5 text-[10px] font-semibold text-[var(--we-warning)]"
                  >dirty</span>
                  <span
                    v-if="isHighlightedMutation(row.mutation)"
                    class="mt-1 inline-flex rounded border border-[var(--we-warning-border)] bg-[var(--we-bg-panel)] px-1.5 py-0.5 text-[10px] font-semibold text-[var(--we-warning)]"
                  >issue target</span>
                </span>
                <span class="min-w-0">
                  ...
                  <span class="mt-1 grid grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-1 font-mono text-[10px] text-[var(--we-text-muted)]">
                    <span
                      class="truncate"
                      :title="`切片前：${mutationBeforeValue(row.mutation)}`"
                    >{{ mutationBeforeValue(row.mutation) }}</span>
                    <span class="i-lucide-arrow-right h-3 w-3 text-[var(--we-accent)]" />
                    <span
                      class="truncate text-[var(--we-text-secondary)]"
                      :title="`切片后：${mutationAfterValue(row.mutation)}`"
                    >{{ mutationAfterValue(row.mutation) }}</span>
                  </span>
                </span>
              </label>
              ...
              <span class="flex justify-end gap-1">
                <button
                  type="button"
                  class="inline-flex h-6 w-6 items-center justify-center rounded-md text-[var(--we-text-muted)] transition-colors hover:bg-[var(--we-bg-hover)] hover:text-[var(--we-accent-strong)] disabled:opacity-40"
                  title="应用 value"
                  :disabled="props.busy || !isValueDraftDirty(row.index, row.mutation)"
                  @click="applyValueDraft(row.index)"
                >
                  <span class="i-lucide-check h-3.5 w-3.5" />
                </button>
                <button
                  type="button"
                  class="inline-flex h-6 w-6 items-center justify-center rounded-md text-[var(--we-text-muted)] transition-colors hover:bg-[var(--we-bg-hover)] hover:text-[var(--we-text-main)] disabled:opacity-40"
                  title="还原 value"
                  :disabled="props.busy || !isValueDraftDirty(row.index, row.mutation)"
                  @click="resetValueDraft(row.index)"
                >
                  <span class="i-lucide-undo-2 h-3.5 w-3.5" />
                </button>
              </span>
            ...
            <div
              v-if="valueDraftErrors[valueDraftKey(row.index)]"
              class="mt-1 rounded bg-[var(--we-danger-soft)] px-2 py-1 text-[11px] text-[var(--we-danger)]"
            >
              {{ valueDraftErrors[valueDraftKey(row.index)] }}
            </div>
          </div>
        </div>
      </div>
