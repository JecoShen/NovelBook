...
<template>
  ...
  >
    ...
    <button
      v-if="props.collapsed"
      type="button"
      class="absolute right-2 top-2 z-10 inline-flex h-7 w-7 items-center justify-center rounded-md text-[var(--we-text-muted)] transition-colors hover:bg-[var(--we-bg-hover)] hover:text-[var(--we-text-main)]"
      title="展开左侧栏"
      @click="emit('toggleCollapsed')"
    >
      <span
        :class="props.collapsed ? 'i-lucide-panel-left-open' : 'i-lucide-panel-left-close'"
        class="h-4 w-4"
      />
    </button>
    ...
    <div
      v-if="props.collapsed"
      class="flex min-h-0 flex-1 flex-col items-center gap-3 px-2 py-12 text-[var(--we-text-muted)]"
    >
      <span class="i-lucide-table-properties h-4 w-4" />
      <span class="i-lucide-users-round h-4 w-4" />
      <span class="[writing-mode:vertical-rl] text-[11px] tracking-[0.18em]">{{ t("worldEngine.workbenchPreview.subjects") }}</span>
      <div
        data-testid="world-sidebar-collapsed-summary"
        class="mt-1 flex flex-col items-center gap-1.5"
      >
        <span
          class="inline-flex h-7 w-8 items-center justify-center gap-0.5 rounded-md border border-[var(--we-border)] bg-[var(--we-bg-subtle)] font-mono text-[10px] text-[var(--we-text-secondary)]"
          :title="t('worldEngine.workbenchPreview.activeSubjects')"
        >
          <span class="i-lucide-activity h-3 w-3" />
          {{ activeSubjectCount }}
        </span>
        <span
          v-if="props.selectedSubjectIds.length"
          class="inline-flex h-7 w-8 items-center justify-center gap-0.5 rounded-md border border-[var(--we-accent-border)] bg-[var(--we-accent-soft)] font-mono text-[10px] text-[var(--we-accent-strong)]"
          :title="t('worldEngine.workbenchPreview.selectedSubjects')"
        >
          <span class="i-lucide-filter h-3 w-3" />
          {{ props.selectedSubjectIds.length }}
        </span>
        <span
          v-if="openReviewSubjectCount"
          class="inline-flex h-7 w-8 items-center justify-center gap-0.5 rounded-md border border-[var(--we-warning-border)] bg-[var(--we-warning-soft)] font-mono text-[10px] text-[var(--we-warning)]"
          :title="t('worldEngine.workbenchPreview.subjectsWithOpenReviewIssues')"
        >
          <span class="i-lucide-alert-triangle h-3 w-3" />
          {{ openReviewSubjectCount }}
        </span>
        <span
          v-if="draftSubjectCount"
          class="inline-flex h-7 w-8 items-center justify-center gap-0.5 rounded-md border border-[var(--we-warning-border)] bg-[var(--we-warning-soft)] font-mono text-[10px] text-[var(--we-warning)]"
          :title="t('worldEngine.workbenchPreview.subjectsWithValueDrafts')"
        >
          <span class="i-lucide-pencil-line h-3 w-3" />
          {{ draftSubjectCount }}
        </span>
      </div>
    </div>
      ...
      <div class="shrink-0 space-y-3 border-b border-[var(--we-border)] px-3 py-3">
        <div>
          <div class="flex items-center justify-between gap-2">
            <div class="text-[11px] font-semibold uppercase tracking-[0.18em] text-[var(--we-text-muted)]">
              {{ t("worldEngine.workbenchPreview.schema") }}
            </div>
          ...
          <div
            ...
            title="Project Workspace 内的 World Engine 配置文件"
          >
            <button
              type="button"
              class="inline-flex min-w-0 items-center gap-1 rounded border border-[var(--we-border)] bg-[var(--we-bg-subtle)] px-1.5 py-0.5 font-mono text-[10px] text-[var(--we-text-secondary)] transition-colors hover:bg-[var(--we-bg-hover)] hover:text-[var(--we-text-main)] disabled:opacity-45"
              :disabled="props.busy"
              title="打开 schema 配置文件"
              @click="emit('openWorkspacePath', schemaSourcePath)"
            >
              <span class="i-lucide-table-properties h-3 w-3 shrink-0" />
            ...
            <button
              ...
              class="inline-flex min-w-0 items-center gap-1 rounded border border-[var(--we-border)] bg-[var(--we-bg-subtle)] px-1.5 py-0.5 font-mono text-[10px] text-[var(--we-text-secondary)] transition-colors hover:bg-[var(--we-bg-hover)] hover:text-[var(--we-text-main)] disabled:opacity-45"
              :disabled="props.busy"
              title="打开 calendar.ts 配置文件"
              @click="emit('openWorkspacePath', calendarSourcePath)"
            >
              <span class="i-lucide-calendar-clock h-3 w-3 shrink-0" />
        ...
        <div class="flex items-center justify-between gap-2 pt-1">
          <div class="text-[11px] font-semibold uppercase tracking-[0.18em] text-[var(--we-text-muted)]">
            {{ t("worldEngine.workbenchPreview.subjects") }}
          </div>
          <div class="flex shrink-0 items-center gap-2">
            <button
              v-if="activeSubjectContextId"
              type="button"
              class="text-[11px] font-medium text-[var(--we-accent-strong)] transition-colors hover:text-[var(--we-accent)] disabled:opacity-45"
              :disabled="props.busy"
              title="清空主体文件建议语境，不改变 timeline 过滤"
              @click="clearSubjectContext"
            >
              清语境
            </button>
            <button
              type="button"
              class="text-[11px] text-[var(--we-text-secondary)] transition-colors hover:text-[var(--we-accent)] disabled:opacity-45"
              :disabled="props.busy"
              @click="clearSubjects"
            >
              整体世界
        ...
        <div class="grid grid-cols-[minmax(0,1fr)_100px] gap-2">
          <FormInput
            v-model="search"
            type="search"
            :placeholder="t('worldEngine.workbenchPreview.searchSubject')"
          >
            <template #prefix>
              <span class="i-lucide-search h-3.5 w-3.5 shrink-0 text-[var(--we-text-muted)]" />
            </template>
          </FormInput>
          <FormSelect
        ...
        <SegmentedControl
          :model-value="subjectReviewFilter"
          :options="subjectReviewFilterOptions"
          size="xs"
          @update:model-value="updateSubjectReviewFilter"
        />
        <div
          v-if="subjectReviewFilter !== 'all'"
          class="flex items-center justify-between gap-2 rounded-md border border-[var(--we-accent-border)] bg-[var(--we-accent-soft)] px-2 py-1 text-[11px] text-[var(--we-accent-strong)]"
        >
          <span class="min-w-0 truncate">左栏筛选：{{ subjectReviewFilterLabel }}</span>
          <button
            type="button"
            class="inline-flex h-5 w-5 shrink-0 items-center justify-center rounded text-[var(--we-accent-strong)] transition-colors hover:bg-[var(--we-bg-panel)]"
            title="清空左栏状态过滤"
            @click="clearSubjectReviewFilter"
          >
      ...
      <div class="min-h-0 flex-1 overflow-y-auto px-3 py-3 custom-scrollbar">
        <div class="space-y-1">
          ...
          >
            <button
              type="button"
              class="w-full text-left disabled:opacity-45"
            ...
            >
              ...
              <div class="mt-0.5 truncate font-mono text-[11px] text-[var(--we-text-muted)]">
                {{ subject.id }}
              </div>
              <div
                v-if="subjectSystemSummary(subject.id)"
                class="mt-2 flex min-w-0 flex-wrap items-center gap-1"
                :title="subjectSystemSyncTitle(subjectSystemSummary(subject.id))"
              >
                <span
                  class="inline-flex items-center gap-1 rounded border px-1.5 py-0.5 text-[10px] font-medium"
                  :class="subjectSystemSummary(subject.id)?.syncStatus === 'linked' ? 'border-[var(--we-accent-border)] bg-[var(--we-accent-soft)] text-[var(--we-accent-strong)]' : subjectSystemSummary(subject.id)?.syncStatus === 'pending-world-subject' ? 'border-[var(--we-warning-border)] bg-[var(--we-warning-soft)] text-[var(--we-warning)]' : 'border-[var(--we-border)] bg-[var(--we-bg-muted)] text-[var(--we-text-muted)]'"
                >
                  <span class="i-lucide-folder-tree h-3 w-3" />
                  {{ subjectSystemSyncLabel(subjectSystemSummary(subject.id)) }}
                </span>
                <span
                  v-if="subjectSystemSummary(subject.id)?.eventCount !== null"
                  class="rounded border border-[var(--we-border)] bg-[var(--we-bg-subtle)] px-1.5 py-0.5 font-mono text-[10px] text-[var(--we-text-muted)]"
                >{{ subjectSystemSummary(subject.id)?.eventCount }} events</span>
                <span
                  v-if="subjectSystemSummary(subject.id)?.memoryCount !== null"
                  class="rounded border border-[var(--we-border)] bg-[var(--we-bg-subtle)] px-1.5 py-0.5 font-mono text-[10px] text-[var(--we-text-muted)]"
                >{{ subjectSystemSummary(subject.id)?.memoryCount }} memory</span>
                <span
                  v-if="subjectSystemSummary(subject.id)?.legacyKind"
                  class="rounded border border-[var(--we-border)] bg-[var(--we-bg-subtle)] px-1.5 py-0.5 text-[10px] text-[var(--we-text-muted)]"
                >{{ subjectSystemSummary(subject.id)?.legacyKind }}</span>
                <span
                  v-if="subjectSystemSummary(subject.id)?.controlledBy"
                  class="rounded border border-[var(--we-border)] bg-[var(--we-bg-subtle)] px-1.5 py-0.5 text-[10px] text-[var(--we-text-muted)]"
                >{{ subjectSystemSummary(subject.id)?.controlledBy }}</span>
              </div>
              <div class="mt-2 grid grid-cols-[minmax(0,1fr)_auto] items-center gap-2 text-[10px]">
                <div class="min-w-0 truncate text-[var(--we-text-muted)]">
                  ...
                  <span
                    v-if="subjectStat(subject.id)?.latestKind"
                    class="ml-1 rounded border border-[var(--we-border)] bg-[var(--we-bg-subtle)] px-1 font-mono text-[var(--we-text-secondary)]"
                  >{{ subjectStat(subject.id)?.latestKind }}</span>
                </div>
                <div class="flex min-w-0 flex-wrap items-center justify-end gap-1">
                  ...
                  <span
                    v-if="subjectDraftCount(subject.id)"
                    class="rounded border border-[var(--we-warning-border)] bg-[var(--we-warning-soft)] px-1.5 py-0.5 font-mono font-semibold text-[var(--we-warning)]"
                    :title="t('worldEngine.workbenchPreview.valueDraftCountTitle', { count: subjectDraftCount(subject.id) })"
                  >{{ t("worldEngine.workbenchPreview.valueCountShort", { count: subjectDraftCount(subject.id) }) }}</span>
                  <span
                    v-if="subjectStat(subject.id)?.openIssueCount"
                    class="rounded border border-[var(--we-warning-border)] bg-[var(--we-warning-soft)] px-1.5 py-0.5 font-mono font-semibold text-[var(--we-warning)]"
                    :title="subjectReviewTitle(subjectStat(subject.id))"
                  >{{ t("worldEngine.workbenchPreview.openCountShort", { count: subjectStat(subject.id)?.openIssueCount }) }}</span>
                  <template v-if="subjectStat(subject.id)?.doneIssueCount">
                    <span
                      class="rounded border border-[var(--we-accent-border)] bg-[var(--we-accent-soft)] px-1.5 py-0.5 font-mono font-semibold text-[var(--we-accent-strong)]"
                      :title="subjectReviewTitle(subjectStat(subject.id))"
                    >{{ t("worldEngine.workbenchPreview.doneCountShort", { count: subjectStat(subject.id)?.doneIssueCount }) }}</span>
                    <span
                      v-if="subjectStat(subject.id)?.confirmedIssueCount"
                      class="rounded border border-[var(--we-accent-border)] bg-[var(--we-bg-panel)] px-1.5 py-0.5 font-mono text-[var(--we-accent-strong)]"
                      :title="subjectReviewTitle(subjectStat(subject.id))"
                    >{{ t("worldEngine.workbenchPreview.confirmedCountShort", { count: subjectStat(subject.id)?.confirmedIssueCount }) }}</span>
                    <span
                      v-if="subjectStat(subject.id)?.ignoredIssueCount"
                      class="rounded border border-[var(--we-border)] bg-[var(--we-bg-muted)] px-1.5 py-0.5 font-mono text-[var(--we-text-muted)]"
                      :title="subjectReviewTitle(subjectStat(subject.id))"
                    >{{ t("worldEngine.workbenchPreview.ignoredCountShort", { count: subjectStat(subject.id)?.ignoredIssueCount }) }}</span>
                  </template>
                </div>
              </div>
            </button>
            <div
              v-if="subjectSystemSummary(subject.id)"
              class="mt-2 flex min-w-0 flex-wrap gap-1 border-t border-[var(--we-border)] pt-2"
            >
              <button
                type="button"
                class="inline-flex h-6 items-center gap-1 rounded border px-1.5 font-mono text-[10px] transition-colors hover:bg-[var(--we-bg-hover)] disabled:opacity-45"
                :class="activeSubjectContextId === subject.id ? 'border-[var(--we-accent-border)] bg-[var(--we-accent-soft)] text-[var(--we-accent-strong)]' : 'border-[var(--we-border)] bg-[var(--we-bg-subtle)] text-[var(--we-text-secondary)] hover:text-[var(--we-text-main)]'"
                :aria-pressed="activeSubjectContextId === subject.id"
                :disabled="props.busy"
                :title="activeSubjectContextId === subject.id ? '当前主体文件建议语境，不改变 timeline 过滤' : '设为主体语境，不改变 timeline 过滤'"
                @click="focusSubjectContext(subject.id)"
              >
                <span class="i-lucide-target h-3 w-3" />
                {{ activeSubjectContextId === subject.id ? "语境中" : "语境" }}
              </button>
              <button
                type="button"
                class="inline-flex h-6 items-center gap-1 rounded border border-[var(--we-border)] bg-[var(--we-bg-subtle)] px-1.5 font-mono text-[10px] text-[var(--we-text-secondary)] transition-colors hover:bg-[var(--we-bg-hover)] hover:text-[var(--we-text-main)] disabled:opacity-45"
                :disabled="props.busy"
                title="打开 subject.md"
                @click="openSubjectSystemFile(subjectSystemSummary(subject.id), 'subject')"
              >
              ...
              <button
                ...
                @click="openSubjectSystemFile(subjectSystemSummary(subject.id), 'events')"
              >
                <span class="i-lucide-list-plus h-3 w-3" />
                events
              </button>
              <button
                type="button"
                ...
                @click="openSubjectSystemFile(subjectSystemSummary(subject.id), 'memory')"
              >
                <span class="i-lucide-brain h-3 w-3" />
                memory
              </button>
              <button
                type="button"
                ...
                :disabled="props.busy"
                title="打开 state.md"
                @click="openSubjectSystemFile(subjectSystemSummary(subject.id), 'state')"
              >
                <span class="i-lucide-eye h-3 w-3" />
                state
              </button>
            </div>
          </div>
        ...
        <div
          ...
          class="rounded-md border border-dashed border-[var(--we-border)] bg-[var(--we-bg-subtle)] px-3 py-8 text-center"
        >
          <div class="text-[12px] font-semibold text-[var(--we-text-secondary)]">
            没有匹配的 subject
          </div>
          <div class="mt-1 text-[11px] text-[var(--we-text-muted)]">
            当前搜索、type 或状态过滤没有命中
          </div>
          <button
            v-if="hasLocalFilters"
            type="button"
            class="mt-3 inline-flex h-7 items-center gap-1.5 rounded-md border border-[var(--we-border)] bg-[var(--we-bg-panel)] px-2.5 text-[11px] text-[var(--we-text-secondary)] transition-colors hover:bg-[var(--we-bg-hover)] hover:text-[var(--we-text-main)]"
