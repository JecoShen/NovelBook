  ...
  >
    <div class="grid gap-3 p-3 xl:grid-cols-[184px_minmax(0,1fr)]">
      <div class="flex flex-col gap-1.5 border-b border-[var(--we-border)] pb-3 xl:border-b-0 xl:border-r xl:pb-0 xl:pr-3">
        <div class="font-mono text-[12px] font-semibold leading-5 text-[var(--we-text-main)]">
          {{ displayTime }}
        </div>
        <div class="flex items-center gap-1.5">
          ...
          <span
            class="font-mono text-[10px] text-[var(--we-text-muted)]"
            :title="props.slice.id"
          >#{{ props.slice.id.slice(0, 8) }}</span>
        </div>
        <div
          v-if="displaySummary"
          class="mt-2 line-clamp-4 text-[11px] leading-relaxed text-[var(--we-text-secondary)]"
      ...
      <div class="min-w-0">
        <div class="flex items-start justify-between gap-3">
          <div class="min-w-0 flex-1">
            <div class="flex flex-wrap items-center gap-2">
              <span
                v-if="hasMetadataDraft"
                class="rounded-md border border-[var(--we-warning-border)] bg-[var(--we-warning-soft)] px-2 py-1 text-[11px] font-semibold text-[var(--we-warning)]"
                :title="`未应用 metadata 草稿：${metadataDraftDiffLabel}`"
              >meta draft</span>
              <span
                v-if="hasValueDraft"
                class="rounded-md border border-[var(--we-warning-border)] bg-[var(--we-warning-soft)] px-2 py-1 text-[11px] font-semibold text-[var(--we-warning)]"
                :title="`${props.valueDraftCount} 个未应用 value 草稿`"
              >value draft {{ props.valueDraftCount }}</span>
              <button
                v-if="subjectFileProposalCount"
                data-testid="slice-card-subject-file-proposal-count"
                type="button"
                class="inline-flex items-center gap-1 rounded-md border border-[var(--we-accent-border)] bg-[var(--we-accent-soft)] px-2 py-1 text-[11px] font-semibold text-[var(--we-accent-strong)] transition-colors hover:bg-[var(--we-bg-active)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--we-accent-border)]"
                title="按当前主体语境，当前切片有主体文件建议；打开右侧 Inspector 查看"
                @click.stop="openSubjectFileProposals"
              >
                <span class="i-lucide-files h-3 w-3" />
                files {{ subjectFileProposalCount }}
              </button>
              <span
                v-if="hasIssues"
                class="rounded-md border px-2 py-1 text-[11px] font-semibold"
                :class="hasOpenIssues ? 'border-[var(--we-warning-border)] bg-[var(--we-warning-soft)] text-[var(--we-warning)]' : 'border-[var(--we-accent-border)] bg-[var(--we-accent-soft)] text-[var(--we-accent-strong)]'"
              >{{ reviewBadgeLabel }}</span>
            </div>
            <div
              class="mt-2 truncate text-[14px] font-semibold text-[var(--we-text-main)]"
              :title="hasMetadataDraft ? `草稿：${displayTitle}；已应用：${props.slice.title || props.slice.id}` : displayTitle"
            >
              {{ displayTitle }}
            </div>
            <div
              v-if="hasMetadataDraft"
              class="mt-1 truncate text-[10px] text-[var(--we-text-muted)]"
            >
              已应用：{{ props.slice.title || props.slice.id }}
            </div>
            <div
              v-if="props.reviewItems.length"
            ...
            >
              ...
              <div
                v-if="hiddenReviewItemCount"
                class="rounded-md border border-[var(--we-border)] bg-[var(--we-bg-subtle)] px-2 py-1 text-[11px] text-[var(--we-text-muted)]"
              >
          ...
          <div class="flex shrink-0 items-center gap-0.5 rounded-md border border-[var(--we-border)] bg-[var(--we-bg-subtle)] p-0.5">
            <button
              type="button"
              class="inline-flex h-6 w-6 items-center justify-center rounded text-[var(--we-text-muted)] transition-colors hover:bg-[var(--we-bg-hover)] hover:text-[var(--we-text-main)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--we-accent-border)]"
              title="在此 Slice 之前插入新 Slice"
              aria-label="在此 Slice 之前插入新 Slice"
              @click.stop="insertSliceBefore"
            >
              <span class="i-lucide-arrow-up-to-line h-3 w-3" />
            </button>
            <button
              type="button"
              class="inline-flex h-6 w-6 items-center justify-center rounded text-[var(--we-text-muted)] transition-colors hover:bg-[var(--we-bg-hover)] hover:text-[var(--we-text-main)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--we-accent-border)]"
              title="在此 Slice 之后插入新 Slice"
              aria-label="在此 Slice 之后插入新 Slice"
              @click.stop="insertSliceAfter"
            >
              <span class="i-lucide-arrow-down-to-line h-3 w-3" />
            </button>
          </div>
        </div>
        ...
        <div
          ...
          :class="props.layoutCols === 'double' ? '2xl:grid-cols-2' : 'grid-cols-1'"
        >
          <div
          ...
          >
            <div class="flex items-center justify-between gap-2 border-b border-[var(--we-border)] bg-[var(--we-bg-subtle)] px-2.5 py-1.5">
              <div class="flex min-w-0 items-center gap-2">
                ...
                <span
                  class="font-mono text-[10px] text-[var(--we-text-muted)]"
                  :title="group.subjectId"
                >({{ group.subjectId.slice(0, 8) }})</span>
              </div>
            </div>
            <div class="divide-y divide-[var(--we-border)]">
              ...
              >
                ...
                <span
                  class="min-w-0 truncate font-mono text-[var(--we-code-text)]"
                  :title="patchValueLabel(mutation)"
                >{{ patchValueLabel(mutation) }}</span>
                <span
                  class="min-w-0 truncate text-[var(--we-text-secondary)]"
