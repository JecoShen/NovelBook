...
<template>
  ...
  <section class="flex min-h-0 flex-1 flex-col overflow-hidden bg-[var(--we-bg-canvas)]">
    <div class="shrink-0 border-b border-[var(--we-border)] bg-[var(--we-bg-panel)] px-3 py-3">
      <div class="mb-2 flex flex-wrap items-center justify-between gap-2">
        <div class="flex min-w-0 items-center gap-2">
          <span class="i-lucide-clock-3 h-4 w-4 text-[var(--we-text-secondary)]" />
          <div class="min-w-0">
            <h2 class="m-0 text-[15px] font-semibold leading-5 text-[var(--we-text-main)]">
              世界切片列表
            </h2>
          </div>
        </div>
        <div class="flex items-center gap-2">
          ...
          <span
            v-if="props.selectedSubjectIds.length <= 1"
            class="inline-flex h-7 items-center gap-1.5 rounded-md border border-[var(--we-border)] bg-[var(--we-bg-subtle)] px-2 text-[11px] text-[var(--we-text-secondary)]"
          >
            <span
              :class="props.selectedSubjectIds.length ? 'i-lucide-user-round' : 'i-lucide-globe-2'"
              class="h-3.5 w-3.5 text-[var(--we-text-muted)]"
            />
            {{ scopeLabel }}
          </span>
          <SegmentedControl
            v-else
            :model-value="props.subjectFilterMode"
            :options="subjectFilterModeOptions"
            tone="accent"
            @update:model-value="updateSubjectFilterMode"
          />
          <button
          ...
          >
            <span class="i-lucide-filter-x h-3.5 w-3.5" />
            清空过滤
          </button>
          <button
      ...
      <FormInput
        :model-value="props.sliceSearch"
        type="search"
        :placeholder="t('worldEngine.workbenchPreview.searchSlicePlaceholder')"
        :disabled="props.busy"
        @update:model-value="emit('updateSliceSearch', $event)"
      >
        <template #prefix>
      ...
      <div
        v-if="draftQueueItems.length"
        data-testid="slice-list-draft-queue"
        class="mt-2 rounded-md border border-[var(--we-warning-border)] bg-[var(--we-warning-soft)] px-2.5 py-2"
      >
        <div class="flex flex-wrap items-center justify-between gap-2">
          <div class="flex min-w-0 items-center gap-1.5 text-[11px] font-medium text-[var(--we-warning)]">
            <span class="i-lucide-list-todo h-3.5 w-3.5 shrink-0" />
          ...
          <button
            type="button"
            class="inline-flex h-6 items-center gap-1 rounded-md border border-[var(--we-warning-border)] bg-[var(--we-bg-panel)] px-2 text-[10px] font-medium text-[var(--we-warning)] transition-colors hover:bg-[var(--we-bg-hover)] disabled:opacity-45"
            :disabled="props.busy"
            title="清空 search / kind / subject 过滤，只看有未应用草稿的切片"
            @click="showDraftSlices"
          >
        ...
        <div class="mt-2 flex gap-1.5 overflow-x-auto pb-0.5 custom-scrollbar">
          ...
          >
            ...
            <span
              v-if="item.metadataDraftCount"
              class="shrink-0 rounded border border-[var(--we-warning-border)] bg-[var(--we-warning-soft)] px-1 font-mono text-[10px] text-[var(--we-warning)]"
            >meta</span>
            <span
              v-if="item.valueDraftCount"
              class="shrink-0 rounded border border-[var(--we-warning-border)] bg-[var(--we-warning-soft)] px-1 font-mono text-[10px] text-[var(--we-warning)]"
            >value {{ item.valueDraftCount }}</span>
            <span
              v-if="item.draftSummary"
              class="shrink-0 rounded bg-[var(--we-bg-subtle)] px-1 text-[10px] text-[var(--we-text-muted)]"
            >preview</span>
          </button>
        </div>
      </div>
      <div
        class="mt-2 flex flex-wrap items-center gap-2 rounded-md border border-[var(--we-border)] bg-[var(--we-bg-subtle)] px-2 py-1.5 text-[11px]"
        data-testid="slice-list-filter-toolbar"
      >
        <div class="flex min-w-0 flex-wrap items-center gap-1">
          <span class="px-1 font-mono text-[10px] uppercase tracking-[0.12em] text-[var(--we-text-muted)]">kind</span>
          <SegmentedControl
            :model-value="props.sliceKindFilter"
            :options="kindFilterOptions"
            tone="accent"
            @update:model-value="updateSliceKindFilter"
          />
        </div>
        <div class="h-5 w-px bg-[var(--we-border)]" />
        <div class="flex min-w-0 flex-wrap items-center gap-1">
          <span class="px-1 font-mono text-[10px] uppercase tracking-[0.12em] text-[var(--we-text-muted)]">status</span>
          <SegmentedControl
            :model-value="props.sliceHealthFilter"
            :options="statusFilterOptions"
            @update:model-value="updateSliceHealthFilter"
          />
        </div>
        <div class="h-5 w-px bg-[var(--we-border)]" />
        <div class="flex min-w-0 flex-wrap items-center gap-1">
          ...
          <SegmentedControl
            :model-value="layoutCols"
            :options="layoutOptions"
            tone="accent"
            @update:model-value="layoutCols = $event as 'single' | 'double'"
          />
        </div>
        <button
          v-if="props.sliceKindFilter !== 'all' || props.sliceHealthFilter !== 'all'"
          type="button"
          class="ml-auto inline-flex h-7 items-center gap-1 rounded-md border border-[var(--we-border)] bg-[var(--we-bg-panel)] px-2 text-[11px] text-[var(--we-text-secondary)] transition-colors hover:bg-[var(--we-bg-hover)] hover:text-[var(--we-text-main)] disabled:opacity-45"
          :disabled="props.busy"
          title="清空 kind / status 过滤"
      ...
      <div
        v-if="activeFilterChips.length || selectedSubjectFilters.length"
      ...
      >
        ...
        <button
          type="button"
          class="inline-flex h-7 items-center gap-1 rounded-md border border-[var(--we-border)] bg-[var(--we-bg-panel)] px-2 text-[11px] text-[var(--we-text-secondary)] transition-colors hover:bg-[var(--we-bg-hover)] hover:text-[var(--we-text-main)] disabled:opacity-45"
          :disabled="props.busy"
          @click="emit('clearSubjectFilter')"
        >
          <span class="i-lucide-rotate-ccw h-3.5 w-3.5" />
    ...
    <div
      ref="scrollerRef"
      class="min-h-0 flex-1 overflow-y-auto px-3 py-3 custom-scrollbar"
    >
      <div class="space-y-3">
        <div
      ...
      <div
        v-if="!filteredSlices.length"
        class="rounded-md border border-dashed border-[var(--we-border)] bg-[var(--we-bg-panel)] px-4 py-12 text-center"
      >
        <div class="text-[13px] font-semibold text-[var(--we-text-secondary)]">
          没有匹配当前条件的切片
        </div>
        <div class="mx-auto mt-1 max-w-md text-[12px] leading-5 text-[var(--we-text-muted)]">
          <template v-if="hasActiveFilters">
            当前筛选组合过窄<template v-if="selectedSubjectLabel">
        ...
        >
          <button
            v-if="props.sliceSearch.trim()"
            type="button"
            class="inline-flex h-8 items-center gap-1.5 rounded-md border border-[var(--we-border)] bg-[var(--we-bg-subtle)] px-3 text-[12px] text-[var(--we-text-secondary)] transition-colors hover:bg-[var(--we-bg-hover)] hover:text-[var(--we-text-main)] disabled:opacity-45"
            :disabled="props.busy"
            @click="clearSearch"
          >
            <span class="i-lucide-search-x h-3.5 w-3.5" />
            清空搜索
          </button>
          <button
            v-if="props.sliceKindFilter !== 'all' || props.sliceHealthFilter !== 'all'"
          ...
          <button
            v-if="props.selectedSubjectIds.length"
            type="button"
            class="inline-flex h-8 items-center gap-1.5 rounded-md border border-[var(--we-accent-border)] bg-[var(--we-accent-soft)] px-3 text-[12px] font-medium text-[var(--we-accent-strong)] transition-colors hover:bg-[var(--we-bg-active)] disabled:opacity-45"
            :disabled="props.busy"
            @click="emit('clearSubjectFilter')"
          >
            <span class="i-lucide-users-round h-3.5 w-3.5" />
            取消 subject 过滤
          </button>
          <button
            type="button"
            class="inline-flex h-8 items-center gap-1.5 rounded-md border border-[var(--we-border)] bg-[var(--we-bg-panel)] px-3 text-[12px] text-[var(--we-text-secondary)] transition-colors hover:bg-[var(--we-bg-hover)] hover:text-[var(--we-text-main)] disabled:opacity-45"
            :disabled="props.busy"
