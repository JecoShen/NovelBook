  ...
  <article class="profile-prompt-message-card">
    <header class="profile-prompt-message-header">
      <button
        ...
        :title="props.collapsed ? '展开消息' : '收起消息'"
        @click="emit('toggle')"
      >
        <span
          :class="props.collapsed ? 'i-lucide-chevron-right' : 'i-lucide-chevron-down'"
          class="h-3.5 w-3.5"
        />
      </button>
    ...
    >
      <div
        ...
        class="profile-prompt-message-markdown custom-scrollbar"
      >
      ...
      <div
        v-if="props.message.toolCalls?.length"
        class="space-y-2"
      >
        <div class="text-[10px] font-semibold uppercase tracking-[0.18em] text-[var(--text-muted)]">
          Tool Calls
        </div>
        <div
          v-for="toolCall in props.message.toolCalls"
          :key="toolCall.id"
          class="rounded-md border border-[var(--border-color)] bg-[var(--bg-input)]/55"
        >
          <div class="flex min-w-0 items-center gap-2 border-b border-[var(--border-color)]/60 px-2 py-1.5">
            <span class="i-lucide-wrench h-3.5 w-3.5 text-[var(--accent-text)]" />
