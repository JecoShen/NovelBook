...
<template>
  ...
  <section class="panel flex min-h-0 flex-col">
    <div class="mb-3 flex shrink-0 items-center justify-between gap-3">
      <div class="min-w-0">
        <div class="flex items-center gap-2">
          <span class="i-lucide-chevron-right h-4 w-4 text-[var(--accent-main)]" />
        ...
        <div class="mt-1 text-[11px] text-[var(--text-muted)]">
          System、HistorySet、ModelContext、AppendingSet 分别映射到 ProfileTurnPlan 分区。
        </div>
      </div>
    ...
    <div class="min-h-0 flex-1 overflow-auto pr-1 custom-scrollbar">
      ...
      <button
        class="mt-3 flex h-11 w-full items-center justify-center gap-2 rounded-md border border-dashed border-[var(--border-color)] bg-[var(--bg-input)]/35 text-xs font-medium text-[var(--accent-text)] transition-colors hover:bg-[var(--accent-bg)]"
