  ...
  <section class="panel flex min-w-0 min-h-0 flex-1 flex-col overflow-hidden">
    ...
    <div class="min-h-0 flex-1 overflow-auto pr-1 custom-scrollbar">
      ...
      <div v-else-if="props.activeTab === 'props'">
        ...
        >
          <div
            v-if="props.selectedPropEntries.length > 0"
            class="space-y-3"
          >
            <div
              v-for="[key, value] in props.selectedPropEntries"
              :key="key"
              class="space-y-1"
            >
              <div class="flex items-center justify-between gap-2">
                ...
                <span
                  v-if="props.isExpressionValue(value)"
                  class="rounded border border-[var(--border-color)] bg-[var(--bg-input)] px-1.5 py-0.5 text-[10px] font-semibold text-[var(--accent-text)]"
                >表达式</span>
              </div>
              <FormSelect
                v-if="key === 'role'"
                :model-value="String(value ?? 'user')"
                :options="props.roleOptions"
                @update:model-value="emit('update-prop', key, $event)"
              />
              <FormSelect
                v-else-if="key === 'status'"
                :model-value="String(value ?? 'drafting')"
                :options="props.toolStatusOptions"
                @update:model-value="emit('update-prop', key, $event)"
              />
              <FormSelect
                v-else-if="key === 'source'"
                :model-value="String(value ?? 'context')"
                :options="props.sourceOptions"
                @update:model-value="emit('update-prop', key, $event)"
              />
              <FormSelect
                v-else-if="props.selectedNode.type === 'FileChangeNotice' && key === 'mode' && !props.isExpressionValue(value)"
                :model-value="String(value ?? 'minimal')"
                :options="fileChangeNoticeModeOptions"
                @update:model-value="emit('update-prop', key, $event)"
              />
              <FormTextarea
              ...
              <FormInput
                ...
                type="number"
                @update:model-value="emit('update-prop', key, Number($event || 0))"
              />
              <FormInput
                v-else
                :model-value="props.propInputValue(value)"
          ...
          <div
            v-else
            class="rounded-md border border-[var(--border-color)] bg-[var(--bg-input)]/45 px-3 py-2 text-xs text-[var(--text-muted)]"
          >
            此节点暂无属性。
          </div>
          ...
          <template v-if="props.selectedNode.type === 'System' || props.selectedNode.type === 'Message' || props.selectedNode.type === 'AIMessage' || props.selectedNode.type === 'ToolCall' || props.selectedNode.type === 'ToolResult' || props.selectedNode.type === 'Text' || props.selectedNode.type === 'CompactionPrompt' || props.selectedNode.type === 'CompactionSummaryPrefix'">
            <div class="field-label">
              {{ props.selectedNode.textKind === "source" ? "内容（TSX 表达式内容）" : props.selectedNode.type === "Text" ? "文本片段" : "内容（支持变量引用）" }}
            </div>
            ...
            />
            <div class="text-right text-[11px] text-[var(--text-muted)]">
              字数：{{ props.selectedTextLength }} / 20000
            </div>
          </template>
        </div>
        <div
        ...
        <div class="mt-4 border-t border-[var(--border-color)] pt-3">
          ...
          <div
            ...
            class="text-xs text-[var(--status-success)]"
          >
            暂无问题
          </div>
          <div
            v-for="issue in props.issues"
            :key="`${issue.message}-${issue.nodeId ?? ''}`"
            class="mb-1 rounded-md border px-2 py-1 text-xs"
            :class="issue.severity === 'error' ? 'border-[var(--status-danger-border)] bg-[var(--status-danger-bg)] text-[var(--status-danger)]' : 'border-[var(--status-warning-border)] bg-[var(--status-warning-bg)] text-[var(--status-warning)]'"
          >
            <div class="font-medium">
              {{ issue.message }}
            </div>
            <div
              v-if="props.issueDetail(issue)"
              class="mt-1 text-[10px] leading-4 opacity-80"
            >
              {{ props.issueDetail(issue) }}
            </div>
          </div>
        </div>
      </div>
      ...
      <div
        v-else-if="props.activeTab === 'variables'"
        class="space-y-3"
      >
        <div class="text-[11px] leading-5 text-[var(--text-muted)]">
          变量信息仅用于检查 Profile schema 与运行时上下文，不会生成 TSX helper。
        </div>
        <FormInput
          :model-value="props.variableSearch"
          placeholder="搜索变量、路径或当前值"
          @update:model-value="emit('update:variableSearch', $event)"
        />
      ...
      <div
        v-else-if="props.activeTab === 'runtime'"
        class="space-y-3"
      >
        <FormInput
          :model-value="props.variableSearch"
      ...
      >
        <div class="rounded-md border border-[var(--border-color)] bg-[var(--bg-input)]/45 p-3 text-xs leading-5 text-[var(--text-secondary)]">
          <div class="mb-2 font-semibold text-[var(--text-main)]">
            Agent Profile
          </div>
          <template v-if="props.profileDetail">
            <div class="profile-row">
              <span>key</span><code>{{ props.profileDetail.catalogItem.profileKey }}</code>
            </div>
            <div class="profile-row">
              <span>kind</span><code>{{ props.profileDetail.catalogItem.kind ?? "unknown" }}</code>
            </div>
            <div class="profile-row">
              <span>来源</span><code>{{ props.profileDetail.catalogItem.source }} / {{ props.profileDetail.catalogItem.overrideState }}</code>
            </div>
            <div class="profile-row">
              <span>加载</span><code>{{ props.profileDetail.catalogItem.loadStatus }}</code>
            </div>
            <div class="profile-row">
              <span>schema</span><code>{{ props.profileDetail.catalogItem.schemaLocked ? "builtin locked" : props.profileDetail.initialSchema.editMode }}</code>
            </div>
          </template>
          <div v-else>
            当前模板不是动态 Agent Profile。
          </div>
        </div>
        <div
          v-if="props.profileDetail"
          class="rounded-md border border-[var(--border-color)] bg-[var(--bg-input)]/45 p-3 text-xs leading-5 text-[var(--text-secondary)]"
        >
          <div class="mb-2 font-semibold text-[var(--text-main)]">
            工具权限
          </div>
          <div
            v-if="props.profileDetail.toolKeys.length"
            class="flex flex-wrap gap-1.5"
          >
            <code
              v-for="toolKey in props.profileDetail.toolKeys"
              :key="toolKey"
              class="rounded border border-[var(--border-color)] bg-[var(--bg-panel)] px-1.5 py-0.5"
            >{{ toolKey }}</code>
          </div>
          <div v-else>
            未声明工具。
        ...
        >
          ...
          <pre
            v-if="props.profileDetail.initialSchema.jsonSchema"
            class="schema-preview"
          >{{ JSON.stringify(props.profileDetail.initialSchema.jsonSchema, null, 2) }}</pre>
        </div>
        <div
          v-if="props.profileDetail"
          class="rounded-md border border-[var(--border-color)] bg-[var(--bg-input)]/45 p-3 text-xs leading-5 text-[var(--text-secondary)]"
        >
          ...
          <pre
            v-if="props.profileDetail.payloadSchema.jsonSchema"
            class="schema-preview"
          >{{ JSON.stringify(props.profileDetail.payloadSchema.jsonSchema, null, 2) }}</pre>
        </div>
        <div
          v-if="props.profileDetail"
          class="rounded-md border border-[var(--border-color)] bg-[var(--bg-input)]/45 p-3 text-xs leading-5 text-[var(--text-secondary)]"
        >
          <div class="mb-2 flex items-center justify-between gap-2">
            ...
            <FormSelect
              ...
              class="w-40"
              :options="[{ value: 'InitialSchema', label: 'InitialSchema' }, { value: 'PayloadSchema', label: 'PayloadSchema' }, { value: 'OutputSchema', label: 'OutputSchema' }]"
            />
          </div>
          ...
          <pre
            v-if="currentSchemaDetail()?.jsonSchema"
            class="schema-preview"
          >{{ JSON.stringify(currentSchemaDetail()?.jsonSchema, null, 2) }}</pre>
