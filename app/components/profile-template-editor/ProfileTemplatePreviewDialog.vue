  ...
  >
    <template #header-extra>
      <div class="flex shrink-0 items-center gap-2">
        ...
        <button
          class="small-btn"
          :disabled="props.previewing || !props.hasRoot"
          @click="emit('refresh-preview')"
        >
          <span
            class="i-lucide-refresh-cw h-3.5 w-3.5"
            :class="props.previewing ? 'animate-spin' : ''"
          />
          <span>刷新</span>
        </button>
      </div>
      ...
      <aside class="preview-variable-pane">
        ...
        <section class="preview-section">
          ...
          <div class="mt-2 text-[11px] leading-5 text-[var(--text-muted)]">
            变量当前值来自所选线程 scope；切换线程后会重新生成预览。
          </div>
        </section>
        ...
        <section class="preview-section">
          ...
          <div class="preview-variable-list custom-scrollbar">
            <section
              v-for="group in props.filteredRuntimeVariableGroups"
              :key="`dialog-variable-${group.group}`"
              class="variable-group-section"
            >
              <button
                class="variable-group-header"
                @click="emit('toggle-variable-group', group.group)"
              >
                <span
                  :class="props.isVariableGroupCollapsed(group.group) ? 'i-lucide-chevron-right' : 'i-lucide-chevron-down'"
                  class="h-3.5 w-3.5"
                />
              ...
              <div
                v-if="!props.isVariableGroupCollapsed(group.group)"
              ...
              >
                ...
                >
                  <div class="flex min-w-0 items-start justify-between gap-2">
                    ...
                    <span
                      v-if="item.editable"
                      class="rounded border border-[var(--border-color)] bg-[var(--bg-panel)] px-1.5 py-0.5 text-[10px] font-semibold text-[var(--text-muted)]"
                    >只读预览</span>
                  </div>
                  <pre
                    v-if="props.shouldShowVariableValue(item)"
                    class="preview-variable-value"
                  >{{ props.formatVariableValue(item.currentValue) }}</pre>
                </div>
              </div>
            </section>
          </div>
        ...
        >
          ...
          <div class="space-y-2">
            <div
              ...
              :key="`dialog-${issue.message}-${issue.nodeId ?? issue.path ?? ''}`"
            ...
            >
              ...
              <div
                ...
                class="mt-1 leading-5 opacity-80"
              >
