...
<template>
  <div
    ref="themeHostRef"
    class="tsx-profile-editor-page flex h-full min-h-0 flex-col overflow-hidden bg-[var(--bg-main)] text-[var(--text-main)] transition-colors duration-300"
    :class="props.mode === 'system-template' ? 'h-screen' : ''"
  >
      ...
      >
        ...
        >
          ...
          <div class="flex min-h-0 flex-1 flex-col gap-1 overflow-y-auto overflow-x-hidden pr-0.5 custom-scrollbar">
            <template
              v-for="group in compactComponentGroups"
              :key="group.group"
            >
              <div
                class="rail-group-divider"
                :title="group.label"
              />
              <button
    ...
    >
      <div class="space-y-3">
        ...
        <label class="form-row">
          ...
          <FormInput
            v-model="newProfileForm.profileKey"
        ...
        <label class="form-row">
          ...
          <FormInput
            v-model="newProfileForm.description"
            placeholder="可选，用于 catalog 展示"
          />
        </label>
        <label class="form-row">
          <span class="form-label">初始提示词</span>
          <FormTextarea
            v-model="newProfileForm.prompt"
            :rows="7"
            placeholder="写入这个 profile 的系统提示词"
          />
        </label>
