...
<template>
  ...
  <div class="rounded-xl border border-[var(--border-color)] bg-[var(--bg-panel)] p-4">
    ...
    <div class="mb-3 flex flex-wrap items-center gap-3">
      ...
      >{{ jobStatusText }}</span>
      <button
        v-if="props.jobId && (canCancelJob || jobCancelRequested)"
        type="button"
        ...
        :disabled="jobCancelling || jobCancelRequested"
        @click="cancelJob"
      >
      ...
      <span
        v-if="error"
        class="text-xs text-[var(--status-danger)]"
      >{{ error }}</span>
    </div>

    <!-- 正式后台 job 的有界预览与观测错误。 -->
    <div
      v-if="props.jobId && (job?.preview || jobError)"
    ...
    <!-- run 失败信息 -->
    <div
      v-if="state?.view.error"
      class="mb-3 rounded border border-[var(--status-danger-border)] bg-[var(--status-danger-bg)] px-3 py-2 text-xs text-[var(--status-danger)]"
    >
      {{ state.view.error }}
    </div>
    ...
    <!-- ask 应答卡片：用户参与点 -->
    <div
      v-for="ask in state?.view.pendingAsks ?? []"
      :key="ask.key"
      class="mb-3 rounded-lg border border-[var(--status-warning-border)] bg-[var(--status-warning-bg)] p-3"
    >
      <div class="mb-2 text-sm font-semibold text-[var(--status-warning)]">
        🙋 轮到你了：{{ ask.spec.title }}
      </div>
      ...
      >
        <button
          ...
          @click="toggleSelect(ask.key, option.id, ask.spec.multi)"
        >
          {{ option.label }}
        </button>
      </div>
      <input
        ...
        class="w-full rounded border border-[var(--border-color)] bg-[var(--bg-main)] px-2 py-1 text-sm text-[var(--text-main)]"
        placeholder="输入应答…"
        :disabled="askSubmitState !== 'idle'"
        @input="askDrafts[ask.key] = ($event.target as HTMLInputElement).value"
      >
      <div
        v-else
        class="flex gap-2"
      ...
      <button
        :disabled="askSubmitState !== 'idle'"
        class="mt-2 rounded border border-[var(--accent-main)] bg-[var(--accent-bg)] px-4 py-1 text-xs text-[var(--accent-text)] disabled:cursor-wait disabled:opacity-50"
        @click="submitAsks"
      >
        {{ askSubmitState === "submitting" ? "提交中…" : askSubmitState === "submitted" ? "已提交" : "应答并继续" }}
      </button>
    </div>
      ...
      <div class="min-w-0 flex-1 basis-[320px]">
        ...
        <ul class="max-h-[360px] overflow-y-auto rounded border border-[var(--border-color)] bg-[var(--bg-main)] p-2 text-xs">
          <li
            ...
            :key="logs.length - i"
            class="border-b border-dashed border-[var(--border-color)] py-0.5 last:border-0"
            :class="{
    ...
    <!-- 完成后的动作区 -->
    <div
      v-if="props.mode === 'demo' && state && (state.view.status === 'completed' || state.view.status === 'failed')"
      class="mt-3 flex flex-wrap items-center gap-2"
    >
      <button
        class="rounded border border-[var(--border-color)] bg-[var(--bg-main)] px-3 py-1 text-xs text-[var(--text-secondary)] hover:bg-[var(--bg-hover)]"
        @click="rerun(false)"
      >
        ↺ 重放恢复（应全部命中缓存）
      </button>
      <template v-if="scenarioKey === 'split-book' && state.view.status === 'completed'">
        <input
          v-model="styleModeInput"
          class="w-32 rounded border border-[var(--border-color)] bg-[var(--bg-main)] px-2 py-1 text-xs text-[var(--text-main)]"
        >
        <button
          class="rounded border border-[var(--status-warning-border)] bg-[var(--status-warning-bg)] px-3 py-1 text-xs text-[var(--status-warning)]"
          @click="rerun(true)"
        >
          改文风参数并重跑（看局部失效）
        </button>
    ...
    <!-- 结果 -->
    <details
      v-if="state?.view.result !== undefined"
      class="mt-3"
    >
    ...
    >
      <div class="mb-1 text-xs text-[var(--text-muted)]">
        参与者（每个都是真实 JSONL session，点开看树：蓝=workflow 写入，黄=用户直聊，绿=真实 harness）
      </div>
      <div class="mb-2 flex flex-wrap gap-2">
        <button
          ...
          @click="toggleSession(p.sessionId)"
        >
          {{ busySessionIds.has(p.sessionId) ? "💭 " : "" }}{{ p.name }}<span
            v-if="p.tag"
            class="opacity-70"
          >·{{ p.tag }}</span>
        </button>
      ...
      <div class="flex flex-col gap-3">
        <WorkflowSessionTree
          v-for="sessionId in expandedSessions"
          :key="sessionId"
