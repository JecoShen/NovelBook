  ...
  <div>
    ...
    >
      ...
      >
        ...
        >
          ...
          <span
            v-if="phase.status === 'active' && progressText"
            class="opacity-80"
          >{{ progressText }}</span>
          <span
            v-if="phase.askTitles.length"
            title="本阶段有用户参与点"
          >🙋</span>
        </div>
        <span
    ...
    <template v-if="activeView === 'machine' && showMachineTab">
      <div class="mb-1 text-xs text-[var(--text-muted)]">
        状态图（零预置只增不删，随代码执行长出来；边上 ①②③=执行顺序（终图即流程记录），〔名字〕=在此干活的 agent，橙=有执行线停留，绿=走过）
      </div>
      <WorkflowMermaid
        v-if="machineMermaid"
        :code="machineMermaid"
        :max-height="mermaidMaxHeight"
      />
      <div
        v-else
        class="rounded border border-[var(--status-info-border)] bg-[var(--status-info-bg)] px-3 py-2 text-xs text-[var(--status-info)]"
      >
    ...
    <template v-else-if="activeView === 'flow'">
      <div class="mb-1 text-xs text-[var(--text-muted)]">
        参与者对话流（编排器 ⇄ 各 agent session ⇄ 用户；writer↔critic 这类循环在这里一目了然）
      </div>
      <WorkflowMermaid
        v-if="flowMermaid"
        :code="flowMermaid"
    ...
    <template v-else-if="activeView === 'timeline'">
      <div class="mb-1 text-xs text-[var(--text-muted)]">
        泳道时间线（每个 agent 一条泳道，条形=一次调用；并发交错、耗时长短直观可见）
      </div>
      <WorkflowTimeline :lanes="timeline" />
    </template>
    <template v-else-if="activeView === 'cards'">
      <div class="mb-1 text-xs text-[var(--text-muted)]">
        直播卡片（每个 agent 的当前状态与最近一问一答，像聊天室成员列表）
      </div>
      <WorkflowAgentCards :cards="live" />
    </template>
    <template v-else-if="activeView === 'relation'">
      <div class="mb-1 text-xs text-[var(--text-muted)]">
        实时生长关系图（创建 agent 长节点、每次调用长一条边；橙色虚线=正在进行）
      </div>
      <WorkflowMermaid
        v-if="relationMermaid"
        :code="relationMermaid"
        :max-height="mermaidMaxHeight"
      />
    </template>
    <template v-else>
      <div class="mb-1 text-xs text-[var(--text-muted)]">
        执行图（Activity 级工程视图：虚线橙=进行中，绿=缓存命中，体育场=用户参与点）
      </div>
      <WorkflowMermaid
        v-if="traceMermaid"
        :code="traceMermaid"
