...
<template>
  ...
  >
    ...
    <div class="mx-auto flex min-h-screen w-full max-w-7xl flex-col gap-6 px-6 py-6">
      <header class="flex items-center justify-between gap-4 border-b border-[var(--border-color)] pb-4">
        ...
        <div class="flex items-center gap-3">
          ...
          <button
            class="rounded-lg border border-[var(--border-color)] bg-[var(--bg-input)] px-3 py-2 text-sm hover:bg-[var(--bg-hover)]"
            @click="void logout()"
          >
            {{ t("common.logout") }}
          </button>
      ...
      <div class="flex items-center justify-between">
        ...
        <button
          class="rounded-lg bg-[var(--accent-main)] px-4 py-2 text-sm font-medium text-[var(--text-inverse)] hover:opacity-90"
      ...
      <div
        v-if="errorMessage"
        class="rounded-lg border border-[var(--status-danger-border)] bg-[var(--status-danger-bg)] px-4 py-3 text-sm text-[var(--status-danger)]"
      >
        {{ errorMessage }}
      </div>
        ...
        <table class="w-full border-collapse text-sm">
          ...
          <tbody>
            <tr
              v-for="user in users"
              :key="user.id"
              class="border-t border-[var(--border-color)] bg-[var(--bg-panel)]"
            >
              <td class="px-4 py-3">
                {{ user.username }}
              </td>
              ...
              <td class="px-4 py-3">
                <div class="flex flex-wrap gap-2">
                  <button
                    class="rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-3 py-1.5 text-xs hover:bg-[var(--bg-hover)]"
                    @click="openEdit(user)"
                  >
                    {{ t("admin.edit") }}
                  </button>
    ...
    <Dialog
      ...
      @confirm="void createUser()"
    >
      <div class="space-y-4 text-sm">
        <div
          v-if="dialogErrorMessage"
          class="rounded-lg border border-[var(--status-danger-border)] bg-[var(--status-danger-bg)] px-3 py-2 text-[var(--status-danger)]"
        >
          {{ dialogErrorMessage }}
        </div>
        <label class="block">
        ...
        <label class="block">
          <div class="mb-2 flex items-center justify-between gap-2">
            ...
            <button
              type="button"
              class="inline-flex items-center gap-1 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-2 py-1 text-[11px] text-[var(--text-secondary)] transition-colors hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)]"
    ...
    >
      <div class="space-y-4 text-sm">
        ...
        <div class="text-[var(--text-secondary)]">
          {{ t("admin.setNewPasswordFor", { username: resetTarget?.username ?? "" }) }}
        </div>
        <label class="block">
          <div class="mb-2 flex items-center justify-between gap-2">
