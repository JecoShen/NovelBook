      ...
      <div class="rounded-[36px] border border-white/80 bg-white/78 px-6 py-6 shadow-[0_24px_80px_rgba(15,23,42,0.08)] backdrop-blur">
        <div class="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div class="max-w-[760px]">
            <div class="text-[11px] font-semibold uppercase tracking-[0.36em] text-sky-700">
              dnd preview playground
            </div>
            <h1 class="mt-3 font-serif text-3xl font-bold tracking-[0.04em] text-slate-950">
              篇章管理拖拽预览页
            </h1>
          ...
          <div class="flex flex-wrap items-center gap-3">
            <button
              class="rounded-full border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 transition-colors hover:border-slate-300 hover:bg-slate-50"
              @click="clearSelection"
            >
              清空选区
            </button>
            <button
              class="rounded-full bg-slate-950 px-4 py-2 text-sm font-semibold text-white transition-opacity hover:opacity-88"
              @click="resetData"
            >
              重置预览数据
            </button>
        ...
        <div class="mt-5 grid gap-3 text-sm text-slate-600 md:grid-cols-4">
          <div class="rounded-[22px] border border-slate-200/80 bg-white/90 px-4 py-3">
            <div class="text-[11px] uppercase tracking-[0.24em] text-slate-400">
              模块规模
            </div>
            <div class="mt-2 text-lg font-semibold text-slate-950">
              {{ volumes.length }} 篇 / {{ totalChapters }} 章
            </div>
          </div>
          <div class="rounded-[22px] border border-slate-200/80 bg-white/90 px-4 py-3">
            <div class="text-[11px] uppercase tracking-[0.24em] text-slate-400">
            ...
            <div class="mt-2 text-lg font-semibold text-slate-950">
              {{ expandedVolumeCount }} / {{ volumes.length }} 已展开
            </div>
          </div>
          <div class="rounded-[22px] border border-slate-200/80 bg-slate-50/80 px-4 py-3">
            ...
            <div class="mt-2 leading-6">
              点击单选，`Ctrl/Cmd` 增减选中，`Shift` 按当前可见顺序连选。
            </div>
          </div>
          <div class="rounded-[22px] border border-slate-200/80 bg-slate-50/80 px-4 py-3">
            <div class="text-[11px] uppercase tracking-[0.24em] text-slate-400">
            ...
            <div class="mt-2 leading-6">
              若拖拽的章节已在选区内，则整组选中章节一起移动；否则仅拖动当前章节。
            </div>
          </div>
        </div>
      </div>
    ...
    <main class="mx-auto grid max-w-[1440px] gap-6 px-6 pb-8 lg:grid-cols-[minmax(0,1fr)_360px] lg:px-8">
        ...
        <div class="flex items-center justify-between gap-3 border-b border-slate-200/80 px-2 pb-4">
          <div>
            ...
            <div class="mt-1 text-sm text-slate-500">
              {{ volumes.length }} 篇 / {{ totalChapters }} 章 / 已选 {{ selectedChapterIds.length }} 章
            </div>
          </div>
      ...
      <aside class="space-y-4">
        <section class="rounded-[32px] border border-white/80 bg-white/72 p-5 shadow-[0_24px_80px_rgba(15,23,42,0.08)] backdrop-blur">
          <div class="text-[11px] font-semibold uppercase tracking-[0.3em] text-slate-400">
            预览目标
          </div>
        ...
        <section class="rounded-[32px] border border-white/80 bg-slate-950 p-5 text-slate-50 shadow-[0_24px_80px_rgba(15,23,42,0.16)]">
          <div class="text-[11px] font-semibold uppercase tracking-[0.3em] text-sky-300">
            selection
          </div>
        ...
        <section class="rounded-[32px] border border-white/80 bg-white/72 p-5 shadow-[0_24px_80px_rgba(15,23,42,0.08)] backdrop-blur">
          ...
          <div
            v-else
            class="mt-4 rounded-[20px] border border-dashed border-slate-300 px-4 py-6 text-sm leading-6 text-slate-500"
          >
