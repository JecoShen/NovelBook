...
import AgentTraceViewerDialog from 'nbook/app/components/novel-ide/agent/trace-viewer/AgentTraceViewerDialog.vue'
import WorkspaceHistoryInboxDialog from 'nbook/app/components/novel-ide/history/WorkspaceHistoryInboxDialog.vue'
...
<template>
  ...
  >
    ...
    <div
      ...
      class="fixed inset-0 z-[120] grid place-items-center bg-[var(--overlay-bg)] px-4"
      role="status"
      aria-live="polite"
      aria-atomic="true"
    >
      ...
      <section class="w-full max-w-[360px] rounded-md border border-[var(--border-color)] bg-[var(--bg-panel)] px-5 py-4 text-[var(--text-main)] shadow-lg">
        <div class="flex items-start gap-3">
          <span
            class="project-loading-spinner i-lucide-loader-circle mt-0.5 h-4 w-4 shrink-0 animate-spin text-[var(--status-info)]"
            aria-hidden="true"
          />
          <div class="min-w-0 flex-1">
            <h2 class="text-sm font-medium">
              {{ projectTransitionView.title }}
            </h2>
            <div class="mt-1 flex min-h-5 items-center justify-between gap-3 text-xs text-[var(--text-muted)]">
              ...
              <span
                v-if="projectTransitionView.mode === 'determinate'"
                class="shrink-0 tabular-nums"
              >{{ projectTransitionView.stepLabel }}</span>
            </div>
          </div>
        </div>
        <div
        ...
        >
          <div
            v-if="projectTransitionView.mode === 'determinate'"
            class="project-loading-fill h-full rounded-full bg-[var(--status-info)]"
            :style="{ width: projectTransitionView.width }"
          />
          <div
            v-else
            class="project-loading-indeterminate h-full w-1/3 rounded-full bg-[var(--status-info)]"
          />
        </div>
      </section>
    </div>
    ...
    <div class="relative flex min-w-0 flex-1 flex-col overflow-hidden">
      ...
      <ProjectPickerScreen
        v-if="projectPickerActive"
        @open="void openProjectFromPicker($event)"
        @open-user-assets="openUserAssets"
      />
      <WorldEngineWorkbenchDialog
        v-if="projectSurfaceActive && !isUserAssetsWorkspace"
        v-model="worldEngineWorkbenchOpen"
        :project-root="currentProjectRoot"
        :project-title="displayNovelTitle"
        @has-unsaved-drafts-change="worldEngineWorkbenchHasUnsavedDrafts = $event"
        @saving-change="worldEngineWorkbenchSaving = $event"
        @open-workspace-path="void openWelcomeWorkspacePath($event)"
      />
      ...
      >
        ...
        >
          <template v-if="isAgentMode && agentStudioPanelOpen">
            <div
              ref="agentStudioResizeHandleRef"
              class="group absolute -left-1 top-0 z-30 h-full w-2 cursor-col-resize"
            >
              <div
                class="ml-1 h-full w-[2px] bg-[var(--accent-main)] opacity-0 transition-all duration-150 group-hover:opacity-100"
                :class="resizingAgentStudioPanel ? 'opacity-100 shadow-[0_0_0_1px_color-mix(in_srgb,var(--accent-main)_28%,transparent)]' : ''"
              />
            </div>
          </template>
          <div
            v-if="isAgentMode"
            class="flex h-10 shrink-0 items-center justify-between border-b border-[var(--border-color)] bg-[var(--bg-panel)] px-3"
          >
            <div class="min-w-0">
              <div class="text-[12px] font-semibold uppercase tracking-[0.18em] text-[var(--text-secondary)]">
                Studio
              </div>
            </div>
            <button
              type="button"
              class="flex h-7 w-7 items-center justify-center rounded-md text-[var(--text-muted)] hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)]"
              :title="agentStudioFileTreeOpen ? t('ide.shell.collapseFileTree') : t('ide.shell.expandFileTree')"
              @click="agentStudioFileTreeOpen = !agentStudioFileTreeOpen"
            >
              <span
                :class="agentStudioFileTreeOpen ? 'i-lucide-panel-right-close' : 'i-lucide-folder-tree'"
                class="h-4 w-4"
              />
            </button>
          </div>
          ...
          <div class="flex min-h-0 flex-1 overflow-hidden">
            <div
              class="contain-layout-paint flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden"
              :class="resizingAgentStudioPanel || resizingAgentStudioFileTree ? 'pointer-events-none select-none' : ''"
            >
            ...
            >
              <div
                ref="agentStudioFileTreeResizeHandleRef"
                class="group absolute -left-1 top-0 z-30 h-full w-2 cursor-col-resize"
              >
                <div
                  class="ml-1 h-full w-[2px] bg-[var(--accent-main)] opacity-0 transition-all duration-150 group-hover:opacity-100"
                  :class="resizingAgentStudioFileTree ? 'opacity-100 shadow-[0_0_0_1px_color-mix(in_srgb,var(--accent-main)_28%,transparent)]' : ''"
                />
              </div>
        ...
        >
          <template v-if="!isAgentMode">
            <div
              ...
              data-agent-panel-resize-handle
              class="group absolute -left-1 top-0 z-30 h-full w-2 cursor-col-resize"
            >
              <div
                class="ml-1 h-full w-[2px] bg-[var(--accent-main)] opacity-0 transition-all duration-150 group-hover:opacity-100"
                :class="resizingAgentPanel ? 'opacity-100 shadow-[0_0_0_1px_color-mix(in_srgb,var(--accent-main)_28%,transparent)]' : ''"
              />
            </div>
          </template>
          <AgentChatSurface
      ...
      <AgentTraceViewerDialog
        v-if="projectSurfaceActive"
        v-model="traceViewerOpen"
        @open-session="void openTraceSession($event)"
      />
      <WorkspaceHistoryInboxDialog
        v-if="projectSurfaceActive"
        v-model="historyInboxOpen"
