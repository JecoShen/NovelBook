  ...
  <div class="min-h-screen bg-[var(--bg-main)] text-[var(--text-main)]">
    ...
    <header class="border-b border-[var(--border-color)] bg-[var(--toolbar-bg)]">
      <div class="mx-auto flex max-w-[1760px] flex-col gap-4 px-5 py-5 lg:flex-row lg:items-end lg:justify-between">
        <div class="min-w-0">
          <div class="text-[11px] uppercase tracking-[0.24em] text-[var(--text-muted)]">
            World Engine
          </div>
        ...
        <div class="grid w-full gap-2 sm:w-auto sm:min-w-[520px] sm:grid-cols-[minmax(180px,220px)_minmax(260px,1fr)_auto]">
          <input
            ...
            class="h-9 min-w-0 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-3 text-sm outline-none focus:border-[var(--accent-main)]"
            placeholder="搜索 Project"
          >
          <select
            v-model="selectedProjectRoot"
            class="h-9 min-w-0 rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-3 text-sm outline-none focus:border-[var(--accent-main)] disabled:opacity-50"
          ...
          >
            ...
            >
              {{ project.title }} · {{ project.projectRoot }}
            </option>
          </select>
          <button
            type="button"
            class="inline-flex h-9 items-center gap-2 rounded-md border border-[var(--border-color)] px-3 text-sm hover:bg-[var(--bg-hover)] disabled:opacity-50"
            :disabled="loadingProjects || loadingWorld || actionBusy"
            @click="void refreshProjects()"
          >
            <span
              :class="loadingProjects ? 'i-lucide-loader-2 animate-spin' : 'i-lucide-refresh-cw'"
              class="h-4 w-4"
            />
            刷新
          </button>
          <div
            v-if="projectSearch.trim() && projectOptions.length === 0"
            class="text-xs text-[var(--text-muted)] sm:col-span-3"
          >
            没有匹配的 Project
          </div>
        </div>
