...
1. 澄清基础定位与读者承诺。
2. 和用户讨论并确定创作宪法（核心价值观、质量标准、内容红线、读者契约）。
3. 和用户讨论并确定故事概念。
4. 从故事概念中提炼小说简介。
5. 确定初步成功标准（SMART 质量目标）。
6. 构思初步剧情，构思开局剧情。
7. 填写最小 lorebook 骨架。
8. 更新 PROJECT-STATUS 待办和待定问题。
9. 汇报初始化结果与下一步建议。
...
- 故事概念：接近长简介式作品介绍，连续说明开局、世界背景、主角处境、核心设定、主要矛盾、后续方向、故事气质和待定问题。**需包含成功标准段和规格标记（`[核心需求]` / `[需要澄清]` / `[可选特性]`）。**
- 简介：可同步到 novel summary 的短简介。
- 主角定位：身份、初始困境、核心看点。
- 初步剧情大纲：只写粗略方向，用于辅助后续流程，不写完整章节级大纲。
- 开局剧情构思：说明初始处境、第一股压力和可能行动方向。
- 创作边界：禁忌项、不采用方向、容易误写的套路。
- **成功标准：至少 2-3 条 SMART 质量目标（如读者体验目标、节奏标准、设定自洽性）。**
- 待定问题：重要但尚不能定死的决策。
...
- `lorebook/instruction/constitution/`：**创作宪法（最高法）**——核心价值观、质量标准、风格原则概要、内容红线、读者契约。本文件是所有 Agent 的第一参考源，在与其他设定冲突时优先。
- `lorebook/note/project-positioning/`：类型与基调、核心 hook、目标读者和创作边界。
- `lorebook/note/theme/`：主题、情绪底色、价值张力。
- `lorebook/note/story-concept/`：故事概念，篇幅可明显长于简介；用连续叙述介绍作品，覆盖开局、世界背景、主角处境、核心设定、主要矛盾、后续方向、故事气质、成功标准和待定问题。成功标准和剧情方向中应使用 `[核心需求]` / `[需要澄清]` / `[可选特性]` 标记区分优先级。
...
- `constitution`：创作宪法，包含核心价值观、质量标准、风格原则摘要、内容红线、读者契约。**推荐 `inject.always: true`**，注入 leader 和 writer。
...
- 已确定的小说基础定位。
- 已建立的创作宪法（`lorebook/instruction/constitution/`），包含核心价值观、质量标准、内容红线和读者契约。
- 已建立或更新的 lorebook 骨架。
- 已保存的故事概念文件路径。
...
- 用户已经能看懂”这本小说大概是什么”。
- 创作宪法已建立，包含核心价值观、质量标准、内容红线和读者契约。
- 最小 lorebook 骨架能支撑下一阶段继续工作。
- `lorebook/note/story-concept/` 已经写成接近成品介绍的故事概念，包含成功标准段和规格标记。
- 简介已经足够放进 novel summary。
- 初步剧情大纲和开局剧情只服务后续流程，没有提前扩成完整章节级规划。
- inject 只用于稳定长期约束（constitution 推荐 always: true），没有把临时剧情或待定问题直接注入。
