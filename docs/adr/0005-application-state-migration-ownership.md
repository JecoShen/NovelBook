...
Release 构建拒绝缺失声明。`automatic` 的 step 必须存在于 Product catalog；`manual` 必须提供 `docs/migrations/` 下的说明，并在 Manager 修改数据或切换 Product 前停止。实际执行依据数据 sentinel，不依据应用版本号，因此可以跨多个版本直接升级。
