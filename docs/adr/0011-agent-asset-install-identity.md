...
[Task 135](../../packages/neuro-book/.agents/tasks/135-agent-asset-install-protocol/README.md) 决定兼容 [Agent Skills 开放标准](https://agentskills.io/specification)，因此 Skill 的身份读取位置发生变化。本 ADR 的核心原则不变——**仍然只有一个安装身份，不新增 `assetKey`，不用 slug 代替**——变的只是 Skill 从哪里读它：
