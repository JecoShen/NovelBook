...
- 当前 workspace 包包括 `neuro-book`、`neuro-book-manager`、`owned-process`、`file-snapshot-cache`、`neuro-book-contracts`、`neuro-book-test-support` 以及五个自治收编包 `nb-history`、`nb-workflow`、`nb-memory`、`neuro-agent-harness`、`llmlint`；主应用已位于 `packages/neuro-book`。`nb-ui` 已按 ADR 0021 废弃归档：目录原地保留作 provenance，不在 workspaces、install 与 CI 面内。
