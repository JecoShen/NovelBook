                        ...
                        <li v-for="beat in sortedBeats" :key="beat.id" class="group flex items-start gap-2.5 rounded-lg border border-[var(--border-color)] bg-[var(--bg-input)]/50 px-3 py-2">
                            ...
                            <div class="min-w-0 flex-1">
                                <div class="flex min-w-0 items-center gap-2">
                                    ...
                                    <button type="button" class="min-w-0 truncate text-left text-[12px] font-medium text-[var(--text-main)] underline-offset-2 transition-colors hover:text-[var(--accent-text)] hover:underline" :title="`跳到场景:${beat.scene.title || '未命名场景'}`" @click="emit('selectScene', beat.sceneId)">{{ beat.scene.title || "未命名场景" }}</button>
