...
缓存只降低 Source 类型检查的生成成本；删除缓存不会删除作者源码、发布 Profile artifact、Variable 定义或 Project 数据。实际单核单 Worker Profile CLI 测量中，冷缓存 `MAX_SINGLE_RSS_KIB=761120`、热缓存 `MAX_SINGLE_RSS_KIB=459036`，两次退出码均为 `0` 且无目标进程组残留；完整验证数字和未完成的 typecheck/full 边界见 [Task 125](../../../.agents/tasks/125-runtime-artifact-storage-lifecycle/README.md)。
