...
import { runNonDesktopTypecheck, type TypecheckRunReport } from '#scripts/typecheck/non-desktop-runner'
 ...
 * 主应用包根。
 *
 * `matchRepoPaths` 按仓库根锚定前缀匹配，拆包后应用源码整体下沉一层，needle 必须同步带上
 * 这段前缀——否则 `not.toContainPath` 断言会因匹配不到而恒过，守卫被静默关掉。
 */
const APP_PACKAGE_ROOT = 'packages/neuro-book'

/** 把包内相对路径转成仓库根相对路径，供 needle 与 legacy 入口共用同一个改动点。 */
function appPath(packageRelative: string): string {
  return `${APP_PACKAGE_ROOT}/${packageRelative}`
}
...
const FORBIDDEN_CONTRACT_PATHS: readonly string[] = [
  appPath('server/workspace-files/project-session.ts'),
  appPath('server/workspace-history/project-history.ts'),
  appPath('server/agent/harness'),
  appPath('server/plot/index.ts'),
]
...
const LEGACY_TYPECHECK_ENTRIES: readonly string[] = [
  appPath('server/runtime/tsconfig.json'),
...
describe('layered typecheck project graph', () => {
  ...
  it('keeps contract closure free of composition roots', async () => {
    ...
    // 闭包为空会让下面的 not.toContainPath 全部空转通过。
    expect(graph.files).not.toHaveLength(0)

    for (const forbidden of FORBIDDEN_CONTRACT_PATHS) {
      expect(graph).not.toContainPath(forbidden)
    }
...
/**
 * 层配置声明过文件的 workspace 包，也就是受层边界约束的那些。
 *
 * 层配置的 `files` 是所有权的唯一事实源，这里按它反推，新增层或迁移文件后自动跟随。
 */
let layerManagedPackagesCache: ReadonlySet<string> | undefined

function layerManagedPackages(): ReadonlySet<string> {
  if (layerManagedPackagesCache) return layerManagedPackagesCache

  const names = new Set<string>()
  for (const configPath of ts.sys.readDirectory(resolveRepoPath(LAYER_CONFIG_ROOT), ['.json'])) {
    for (const [, name] of (ts.sys.readFile(configPath) ?? '').matchAll(/"(?:\.\.\/)+packages\/([^/"]+)\//gu)) {
      names.add(name)
    }
  }

  layerManagedPackagesCache = names
  return names
}
 ...
 * 不受层边界约束的解析结果。
 *
 * 除 node_modules 外还要排除层从未声明过的 workspace 包：`@notnotype/*` 经
 * `node_modules` 里的符号链接解析，TS 默认 realpath 掉链接，结果落回
 * `packages/<pkg>/src/**` 且路径里不再有 node_modules 段。只按路径段判断会把这类
 * 真依赖误判成层边界泄漏——它与 typebox 同类，从来不该受边界约束。
 */
function isDependencyFile(file: string): boolean {
  const segments = file.split(sep)
  if (segments.includes('node_modules')) return true

  const packagesAt = segments.indexOf('packages')
  if (packagesAt < 0) return false

  const packageName = segments[packagesAt + 1]
  return packageName !== undefined && !layerManagedPackages().has(packageName)
}
