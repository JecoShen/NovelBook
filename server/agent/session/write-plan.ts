...
type SessionWriteAuthority
  = | { kind: 'unowned' }
    | { kind: 'invocation', invocationId: string }
    | { kind: 'forced-abort', invocationId: string }
    | { kind: 'forced-abort-recovery', invocationId: string }
...
export class SessionWriteExecutor {
  ...
  private assertForcedAbortPlan(plan: SessionWritePlan, invocationId: string): void {
    ...
    const [firstOp] = plan.ops
    if (plan.cause !== 'lifecycle.aborted.force'
      || plan.ops.length !== 1
      || firstOp?.kind !== 'append'
      || firstOp.projection !== undefined
      || firstOp.entry.type !== 'invocation_lifecycle'
      || firstOp.entry.status !== 'aborted'
      || firstOp.entry.invocationId !== invocationId) {
      throw invalid()
    }
  }
